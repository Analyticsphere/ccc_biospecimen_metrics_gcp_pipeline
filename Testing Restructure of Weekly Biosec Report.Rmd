...
## all vars for collection times need to be converted from character vars to proper time stamps

## These tend to stall the code when being ran repeatedly as new requests come up

biocol.tb <- biocol.tb %>%
  mutate(across(ends_with("d_926457119"), ~ as.POSIXct(lubridate::ymd_hms(.))),
         d_678166505 = as.POSIXct(ymd_hms(d_678166505)),
         d_822499427 = as.POSIXct(ymd_hms(d_822499427)),
         d_331584571_d_266600170_d_840048338 = as.POSIXct(ymd_hms(d_331584571_d_266600170_d_840048338)),
         d_331584571_d_266600170_d_343048998 = as.POSIXct(ymd_hms(d_331584571_d_266600170_d_343048998)),
         d_173836415_d_266600170_d_139245758 = as.POSIXct(ymd_hms(d_173836415_d_266600170_d_139245758)),
         d_173836415_d_266600170_d_982213346 = as.POSIXct(ymd_hms(d_173836415_d_266600170_d_982213346)),
         d_173836415_d_266600170_d_740582332 = as.POSIXct(ymd_hms(d_173836415_d_266600170_d_740582332)),
         d_764863765 = as.POSIXct(ymd_hms(d_764863765)),
         d_534669573 = as.POSIXct(ymd_hms(d_534669573)),
         d_915838974 = as.POSIXct(ymd_hms(d_915838974)),
         d_556788178 = as.POSIXct(ymd_hms(d_556788178)),
         d_822499427 = as.POSIXct(ymd_hms(d_822499427)),
         d_222161762 = as.POSIXct(ymd_hms(d_222161762)))
...
table1 <- biocol.tb %>%
  mutate(biocol.appoint = case_when(BiospeCollection == "Any biospecimen Collections" ~ "Baseline Collection", # & d_410912345== 353358909
...
settings_breakdown <- knitr::kable(table1_allsett, 
  ...
  footnote(general = "Note: Any collections indicated under ’Home’ setting reflect collections in which home mouthwash
was the only sample collected; these collections are not counted in Table 1.1.", 
           general_title = "",
...
col_num_1.2 <- col_num_1.2 %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` %in% c("Elk River", "Chanhassen", "Park Nicollet Minneapolis Clinic", "Brooklyn Center", "Clinic Stillwater", "New Richmond Clinic",
                                 "Burnsville Clinic and Specialty Center", "St. Paul Clinic- Wabasha", "Andover Clinic",
...
coll_bptl_diff2 <- biospe__2  %>% group_by(as.character(Connect_ID),Site, Clin_Site_location) %>% 
  mutate(any_collection = min(d_232343615_d_926457119, d_299553921_d_926457119, d_376960806_d_926457119, d_454453939_d_926457119, d_505347689_d_926457119,
         ...
         clin_coll_bu = as.Date(min(d_173836415_d_266600170_d_982213346, d_173836415_d_266600170_d_139245758, na.rm=T)),
         day_past = difftime(as.Date(any_collection), clin_coll_bu, units="days"),
         coll_days_diff = ifelse(as.numeric(day_past)<2,"1 Day",
                                 ifelse(as.numeric(day_past)<3,"2 Days",
                                        ifelse(as.numeric(day_past)<4,"3 Days", 
                                               ifelse(as.numeric(day_past)<5,"4 Days", 
...
clin_coll_days <- clin_coll_days %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` %in% c("Elk River", "Chanhassen", "Park Nicollet Minneapolis Clinic", "Brooklyn Center", "Clinic Stillwater", "New Richmond Clinic", 
                                 "Burnsville Clinic and Specialty Center", "St. Paul Clinic- Wabasha", "Andover Clinic", "NSC", "Missing HP location") ~ "HealthPartners", 
...
coll_bptl_diff <- coll_bptl_diff %>% 
  mutate(hrs_past = as.numeric(difftime(any_collection, d_678166505, units="hours")),
...
coll_bptl_diff2 <- coll_bptl_diff2 %>% 
  mutate(hrs_past = as.numeric(difftime(any_collection, clin_coll_bu, units="hours")),
         Weekdays = factor(weekdays(clin_coll_bu),
...
research_plot <- function(df, site_name, start_date, plot_title){
  
  data <- df %>% filter(Site==site_name)
...
clinical_plot <- function(df, site_name, start_date, plot_title){
  
  data <- coll_bptl_diff2 %>% filter(Site==site_name)
  
  ggplot(df, aes(x = clin_coll_bu, y = hrs_past, color = factor(weekdays(clin_coll_bu)))) + 
  geom_point() + 
    scale_x_date(name="Collection Date",
                 #Bio Team wants first letter of the month - year
                 ## b for abbreviate, 1,1 for first letter of the month
...
research_plot(coll_bptl_diff, "Marshfield Clinic Health System", "2022-06-20", 
              "Figure 5.1: Baseline Collection-To-Receipt Time (in Hours), \n Marshfield")

research_plot(coll_bptl_diff, "Sanford Health", "2022-06-20", 
              "Figure 5.2: Baseline Collection-To-Receipt Time (in Hours), \n Sanford Health Research Setting")
clinical_plot(coll_bptl_diff2, "Sanford Health", "2024-03-25", 
              "Figure 5.3: Baseline- Collection-To-Receipt Time (in Hours), \n Sanford Health Clinical Setting")

research_plot(coll_bptl_diff, "University of Chicago Medicine", "2022-06-20", 
              "Figure 5.4: Baseline Collection-To-Receipt Time (in Hours),  \n University of Chicago Medicine Research Setting")
clinical_plot(coll_bptl_diff2, "University of Chicago Medicine", "2024-03-25", 
              "Figure 5.5: Baseline- Collection-To-Receipt Time (in Hours), \n University of Chicago Medicine Clinical Setting")

clinical_plot(coll_bptl_diff2, "Kaiser Permanente Colorado", "2022-10-10", 
              "Figure 5.6: Baseline Collection-To-Receipt Time (in Hours),  \n Kaiser Permanente Colorado")
clinical_plot(coll_bptl_diff2, "Kaiser Permanente Georgia", "2022-10-10", 
              "Figure 5.7: Baseline Collection-To-Receipt Time (in Hours),  \n Kaiser Permanente Georgia")
clinical_plot(coll_bptl_diff2, "Kaiser Permanente Hawaii", "2022-10-10", 
              "Figure 5.8: Baseline Collection-To-Receipt Time (in Hours),  \n Kaiser Permanente Hawaii")
clinical_plot(coll_bptl_diff2, "Kaiser Permanente Northwest", "2022-10-10", 
              "Figure 5.9: Baseline Collection-To-Receipt Time (in Hours),  \n Kaiser Permanente Northwest")

research_plot(coll_bptl_diff, "HealthPartners", "2022-06-20", 
              "Figure 5.10: Baseline- Collection-To-Receipt Time (in Hours), \n HealthPartners Research Setting")
clinical_plot(coll_bptl_diff2, "HealthPartners", "2023-10-23", 
              "Figure 5.11: Baseline- Collection-To-Receipt Time (in Hours), \n HealthPartners Clinical Setting")

research_plot(coll_bptl_diff, "Henry Ford Health", "2022-06-20", "Figure 5.12: Baseline- Collection-To-Receipt Time (in Hours), \n Henry Ford Research Setting")
clinical_plot(coll_bptl_diff2, "Henry Ford Health", "2023-04-24", 
              "Figure 5.13: Baseline- Collection-To-Receipt Time (in Hours), \n Henry Ford Clinical Setting")
  ...
  theme(
    ...
    plt <- ggplot(weekly_counts, aes(x = as.Date(col.Monday.date), y = n, color = .data[[color_var]])) +
      scale_color_manual(values=c("NSC"="#99C0C4", "Elk River"="#565C65", "Chanhassen"="#CC7D15", "Park Nicollet Minneapolis Clinic"="#FDBE19",
                                ...
                                "NSC"="#309EBD", "Andover Clinic"="#B1C7D9"),
...
```{r Section6_Figures3, fig.height=9, eval=TRUE,echo=FALSE, warning = FALSE, message = FALSE}
...
END AS Card_Status,
d_633640710_d_950521660, d_633640710_d_545319575, d_633640710_d_938338155, d_633640710_d_205954477, d_633640710_d_289239334,d_633640710_d_992420392, d_633640710_d_541085383,d_633640710_d_427719697, d_633640710_d_100618603, d_426588510, d_221592017, d_633640710_d_309189173
...
```{r RequestKits, include=FALSE, echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

# ----- Not yet in prod -------------------------

# add d_173836415_d_266600170_d_319972665_d_638781731, d_173836415_d_266600170_d_319972665_d_759651991
### Request a kit table 1

# req_kit <- kit_table %>%
#   filter(d_173836415_d_266600170_d_319972665_d_638781731 == "353358909") %>%
#   mutate(req = if_else(!is.na(d_173836415_d_266600170_d_319972665_d_759651991),
#                        "Requested", "Not Requested"),
#          elg = "Eligible")  
# 
# req_kit_table1 <- req_kit %>% 
#   count(Site, req, name = "Requested") %>%             
#   mutate(Eligible = sum(Requested), .by = Site) %>%     # All are eligible
#   filter(req == "Requested") %>%               
#   select(Site, Eligible, Requested) %>% 
#   #summary row
#   bind_rows(summarise(.data = ., 
#                       Site = "Total",
#                       Eligible = sum(Eligible),
#                       Requested = sum(Requested))) %>% 
#   #percentage
#   mutate(Requested = paste0(Requested, " (",
#                             round((Requested/Eligible)*100,2), "%)"))
# 
# 
# knitr::kable(req_kit_table1,
#              caption="Table tbd: Request a Kit",
#              row.names=FALSE, align=c("l", "c"),booktabs = TRUE) %>%
#   add_indent(seq(1, nrow(req_kit_table1) - 1)) %>%
#   kable_styling(latex_options = "scale_down")
# 
# 
# 
# 
# ### Request a kit table 2
...
#              caption="Table tbd2: Request a Kit",
#              row.names=FALSE, align=c("l", rep("c", 7)),booktabs = TRUE) %>%
#   add_indent(seq(1, nrow(req_kit_table1) - 1)) %>%
#   kable_styling(latex_options = "scale_down")

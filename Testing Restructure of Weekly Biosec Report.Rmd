...
kit_table <- kit_table %>%  mutate(Site = case_when(d_827220437==125001209 ~ "Kaiser Permanente Colorado",
                                   ...
                                   Kit_Status_int = factor(Kit_Status_int,levels=c("No Status","Initialized", "Address Printed", "Assigned", "Shipped", "Received")),
                                   Kit_Status_r1 = factor(Kit_Status_r1,levels=c("No Status","Initialized", "Address Printed", "Assigned", "Shipped", "Received")),
                                   Kit_Status_r2 = factor(Kit_Status_r2,levels=c("No Status","Initialized", "Address Printed", "Assigned", "Shipped", "Received")))
...
location_stat <- kit_table %>%  select(Connect_ID, Kit_Status_int,Kit_Status_r1,Kit_Status_r2, d_173836415_d_266600170_d_319972665_d_661940160, d_173836415_d_266600170_d_982213346,
                                       d_173836415_d_266600170_d_139245758, d_547363263, Site, d_195145666, d_173836415_d_266600170_d_448660695)
...
## Because kit assembly has duplicate rows (somehow multiple initial with different kit statuses), only the most recent kit status should remain. 
## Somehow we're getting rows with the same values for Connect_ID, Kit_Status, kit_level, and kit ID (d_687158491). Need to remove the duplicate rows
kit_status_table_7.1 <- table27_elg_distinct %>%
  mutate(kit_status_level = factor(Kit_Status, levels = c("No Status","Initialized", "Address Printed", "Assigned", "Shipped", "Received"), ordered = TRUE)) %>%
  group_by(Connect_ID, kit_level, d_687158491, Site) %>%
  slice_max(order_by = kit_status_level, n = 1, with_ties = FALSE) %>%
  ungroup()
...
filtered_data <- filter(kit_status_table_7.1, Kit_Status %in% c("Received", "Shipped"))

summary_table2_all<- filtered_data %>%
  filter(kit_level=="Initial Kit") %>% 
  group_by(Site) %>%
  summarise(Shipped = sum(Kit_Status %in% c("Received", "Shipped")),
            Shipped_TwoWeeks = sum(Kit_Status %in% c("Received", "Shipped") & 
...
Coll_Card_Table <- kit_status_table_7.1 %>% 
  filter(Kit_Status=="Received" & kit_level=="Initial Kit") %>% 
...
kit_status_table_7.1 <- kit_status_table_7.1 %>%
  mutate(
    multi_cond = rowSums(across(all_of(condition_cols), ~ .x == 353358909, .names = NULL), na.rm = TRUE)
  )

table_32 <- kit_status_table_7.1 %>% filter(Kit_Status=="Received" & kit_level=="Initial Kit") %>%  
...
## Initial only, need to merge in the kit_assembly table
int_kit <- kitassembly_data %>% 
  mutate(kit_level= case_when(d_426588510 == 772116457 ~ "Replacement Kit 2",
                              d_426588510 == 389478821 ~ "Replacement Kit 1",
                              TRUE ~ "Initial Kit")) %>% 
  filter(kit_level=="Initial Kit")

kit_set_clean2 <- left_join(int_kit, kit_set_clean, by="Connect_ID")
kit_table2 <- left_join(int_kit, kit_table, by="Connect_ID")

##Received 
kit_set_col_2 <- kit_set_clean2 %>% select(Connect_ID, d_143615646_d_826941471)
...
kit_fig_2 <- inner_join(kit_set_col_2,kit_table2,   by="Connect_ID")
...
fig_7.3_totals[,3] <- fig5_week_total_2[,2]
...
table7_8 <- kit_table %>%  filter(Kit_Status=="Received") %>%
...
table7_9 <- kit_status_table_7.1 %>% filter(Kit_Status=="Received" & d_137401245==104430631 &  as.Date(d_173836415_d_266600170_d_319972665_d_661940160)<"2024-05-22") %>%
  mutate(MWSrvy_status = case_when(d_547363263==972455046 ~ "Not Started",
                                   d_547363263==615768760 ~ "Started",
                                   d_547363263==231311385 ~ "Submitted"),
...
table7_10 <- kit_status_table_7.1 %>% filter(Kit_Status=="Received" & d_137401245==104430631 &  as.Date(d_173836415_d_266600170_d_319972665_d_661940160)>"2024-05-22") %>%
  mutate(MWSrvy_status = case_when(d_547363263==972455046 ~ "Not Started",
                                   d_547363263==615768760 ~ "Started",
                                   d_547363263==231311385 ~ "Submitted"),
         MW_completion = difftime(as.Date(d_173836415_d_266600170_d_448660695),as.Date(d_195145666),units="days"),
...
Table7_10a <- knitr::kable(Table7.10a,
                           caption='Table 7.10a Baseline- Mouthwash Biospecimen Survey Completion Time Relative to Collection Time Reported on Collection Card for Initial Kit (Post update to  survey introduction)',
...
survey_cc_r1 <- kit_status_table_7.1 %>% 
  filter(!is.na(d_195145666) & !is.na(d_173836415_d_266600170_d_448660695) &
           Kit_Status=="Received" & kit_level=="Replacement Kit 1" & 
           d_137401245==353358909  & as.Date(d_195145666) > "2025-05-01") 
...
Table7_10b <- knitr::kable(Table7.10b,
                         caption='Table 7.10b Baseline- Mouthwash Biospecimen Survey Completion Time Relative to Collection Time Reported on Collection Card for Replacement Kit 1 ',
                         row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE) %>% 
  kable_styling(latex_options = "HOLD_position")

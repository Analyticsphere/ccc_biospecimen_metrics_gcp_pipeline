...
---
title: "Testing Restructure of Weekly Biosec Report"
...
col_num_1.2 <- col_num_1.2 %>%
  mutate(Site_Group = case_when(
    `Collection Location` %in% c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", "K1 HFH Main","Sterling Heights","Columbus", "Livonia","Taylor") ~ "Henry Ford Health",
...
biospe.bld <- knitr::kable(table2_totals ,col.names=c("Site", "Any Baseline Blood Collected","No Baseline Blood Collected", "Total Baseline Collections"), caption='Table 1.4: Blood Collection Status Among Baseline Biospecimen Collections', row.names=FALSE,align=c("l","c","c","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")
...
biosettings5_2[4,c(1:4)] <- c("0", "Home",  "0", "0")
...
d5 <- round(as.numeric(as.character(bioset5_2[2,2]))/ as.numeric(as.character(bioset5_2[4,2])), digits=2) *100
e5<- round(as.numeric(as.character(bioset5_2[2,3]))/ as.numeric(as.character(bioset5_2[4,3])), digits=2)*100
f5 <- round(as.numeric(as.character(bioset5_2[2,4]))/ as.numeric(as.character(bioset5_2[4,4])), digits=2)*100
...
#Test this change!! HF is already a hybird site to be counted, and now HP too (12/7/23)
...
ClinSrvy_Lgth_Before = test22cb %>% group_by(Site) %>% 
  dplyr::summarize('Number of Submitted Clinical Surveys'=n(),
                   Min = min(ctime),
                   Q1 = quantile(ctime, 0.25),
                   Median = median(ctime),
                   Mean = mean(ctime),
...
research.vt.length.UC <- research.vt.length.comb %>% filter(Site=="University of Chicago Medicine") %>% 
  mutate('Collection Location' = case_when(d_951355211==777644826 ~ "UC-DCAM",
                                       d_951355211==145191545 ~ "Ingalls Harvey",
                                       d_951355211==489380324 ~ "River East",
                                       d_951355211==120264574 ~ "South Loop",
                                       d_951355211==319518299 ~ "UCM Pop-Up",
...
research.vt.length.UC.table = research.vt.length.UC %>% group_by(research.vt.length.UC$'Collection Location') %>% #biochk_outlier,
  dplyr::summarize('Number of Research Collection Visits'=n(),
                   Min = min(time,na.rm = TRUE),
                   Q1 = quantile(time, 0.25,na.rm = TRUE),
                   Median = median(time,na.rm = TRUE),
...
current_colnames <- colnames(research.vt.length.UC.table)
current_colnames[1] <- "Collection Location"
colnames(research.vt.length.UC.table) <- current_colnames
...
ver_draw <- tab22 %>%  filter(biocol_Setting== "Clinical" & !is.na(d_914594314) & !is.na(d_556788178) & d_914594314>=as.Date("2023-02-01")  & Site!='HealthPartners') ##won't receive a draw order from HP--Github issue 72
...
#clc.both.wk_site.CO <- clc.both.vt.CO %>% arrange(Site,BU.Sunday.week,BU.Sunday.date) %>% group_by(Site,BU.Sunday.week,BU.Sunday.date) %>% tally()
 ...
 clc_both_vt.site.CO<- clc.both.wk_site.CO  %>% ggplot( aes(x=as_date(clc.both.wk_site.CO$Monday.date.start), y=n)) + 
  ...
  ggtitle(label="Figure 6.13: Weekly Baseline Biospecimen Collections,  \n at Kaiser Permanente Colorado") +
...
clc_both_vt.site.GA<- clc.both.wk_site.GA  %>% ggplot( aes(x=as_date(clc.both.wk_site.GA$Monday.date.start), y=n)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2023-01-16"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") + 
  ggtitle(label="Figure 6.14: Weekly Baseline Biospecimen Collections,  \n at Kaiser Permanente Georgia") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
clc_both_vt.site.HI<- clc.both.wk_site.HI %>% ggplot( aes(x=as_date(clc.both.wk_site.HI$Monday.date.start), y=n)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2023-01-16"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") + 
  ggtitle(label="Figure 6.15: Weekly Baseline Biospecimen Collections,  \n at Kaiser Permanente Hawaii") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
clc_both_vt.site.NW<- clc.both.wk_site.NW  %>% ggplot( aes(x=as_date(clc.both.wk_site.NW$Monday.date.start), y=n)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2023-01-16"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") + 
  ggtitle(label="Figure 6.16: Weekly Baseline Biospecimen Collections,  \n at Kaiser Permanente Northwest") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
avg_coll <- clinic.survey %>%  filter(d_173836415_d_266600170_d_982213346>=as.Date("2023-02-01") & Site!='HealthPartners') ##won't receive a draw order from HP--Github issue 72
...
bldorder_present <- recrver1 %>%  filter(d_878865966==104430631 & !is.na(d_173836415_d_266600170_d_769615780) & Site!='HealthPartners') %>%  
...
```{r TempTable, include=FALSE, echo=FALSE, warning=FALSE}
...
compTS <- bq_table_download(timestamps_cc, bigint="integer64",n_max = Inf, page_size = 10000)
collTS <- bq_table_download(TSofcollection_cc, bigint="integer64",n_max = Inf, page_size = 1000)
...
research.4_1 <- ggplot(visit_wk_site4__1, aes(x=as.Date(col.Monday.date), y=100*(hours3/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3.4: Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Baseline Research Collection Visit Check−In, Henry Ford Health  ") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
...
research.4_2 <- ggplot(visit_wk_site4__2, aes(x=as.Date(col.Monday.date), y=100*(hours3/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3.5: Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Baseline Research Collection Visit Check−In, Sanford Health") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
research.4_3 <- ggplot(visit_wk_site4__3, aes(x=as.Date(col.Monday.date), y=100*(hours3/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3.3: Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Baseline Research Collection Visit Check−In, University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
tab5=knitr::kable(bioset, col.names=c("Collection Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 1.5: Collection Setting Among Baseline Biospecimen Collections by Specimen Type', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)   
...
tab5_1=knitr::kable(bioset5_1, col.names=c("Collection Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 1.6: Collection Setting Among Baseline Biospecimen Collections by Specimen Type, Henry Ford Health', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)   
...
tab5_2=knitr::kable(bioset5_2, col.names=c("Collection Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 1.7: Collection Setting Among Baseline Biospecimen Collections by Specimen Type, HealthPartners', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)   
...
Bld_urn_cli = knitr::kable(newtable_4, caption='Table 1.8: Data System Status Among Baseline Clinical Biospecimen Collections by Specimen Type', row.names=FALSE,align=c("l","c","c"), booktabs = TRUE) %>%  kable_styling(latex_options = "hold_position") 
...
knitr::kable(bldcol_research1, col.names=c("Site","All Blood Collected","Partial Blood Collected","No Blood Collected","Total Research Collections"), caption='Table 2.3: Blood Collection Completeness Among Baseline Research Biospecimen Collections', row.names=FALSE, align=c("l","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")


knitr::kable(bldcol_research_UC1, col.names=c("Collection Location","All Blood Collected","Partial Blood Collected","No Blood Collected","Total Research Collections"), caption='Table 2.4: Blood Collection Completeness Among Baseline Research Collections by Collection Location, University of Chicago Medicine', row.names=FALSE, align=c("l","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")
```
...
final_fig1_table <- help2_1_fig1_inc %>%  group_by(col.Monday.date) %>%  dplyr::summarize( incomplete=sum(incomp_blood),
  partial=sum(partial_blood), none_blood=sum(no_blood), all_collections=sum(all_blood),  All_Incomplete=100*(incomplete/all_collections),
  Partial__Blood=100*(partial/all_collections),  No__Blood=100*(none_blood/all_collections))



research.1_1 <- ggplot() + #, color=UC_Sites) + #,fill=Site, color=Site)) +
  geom_line(data=final_fig1_table, aes(x=as.Date(col.Monday.date), y=All_Incomplete, colour="All_Incomplete"),na.rm=T) +
  geom_line(data=final_fig1_table, aes(x=as.Date(col.Monday.date), y=Partial__Blood, colour="Partial__Blood"), na.rm=T) +
  geom_line(data=final_fig1_table, aes(x=as.Date(col.Monday.date), y=No__Blood, colour="No__Blood"), na.rm=T) +
  geom_point(width = 0.2) +
  scale_y_continuous(name="Percent %", limits=c(0,100),expand = c(0, 0), breaks=seq(0,100,25)) +  
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  scale_color_manual(name = "Incompletion Level", values = c("All_Incomplete"="darkred","Partial__Blood" = "blue", "No__Blood" = "darkgreen"), labels = c("Total","Partial Blood", "No Blood")) +
  #scale_fill_discrete(name = "Incompletion Level", labels = c("Partial Blood", "No Blood")) +
  ggtitle(label="Figure 2.1: Weekly Percent of Incomplete Blood Collections, University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
...
knitr::kable(bldcol_research_HF1, col.names=c("Collection Location","All Blood Collected","Partial Blood Collected","No Blood Collected","Total Research Collections"), caption='Table 2.5: Blood Collection Completeness Among Baseline Research Collections \n by Collection Location, Henry Ford Health', row.names=FALSE, align=c("l","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")
```

\FloatBarrier
```{r Figure2, eval=TRUE,echo=FALSE, warning=FALSE}
#NEW FIGURE 2, NOW MOVED UP
...
research.1_2 <- ggplot() + #, color=UC_Sites) + #,fill=Site, color=Site)) +
  ...
  scale_color_manual(name = "Incompletion Level", values = c("All_Incomplete"="darkred","Partial__Blood" = "blue", "No__Blood" = "darkgreen"), labels = c("Total","Partial Blood", "No Blood")) +
  #scale_fill_discrete(name = "Incompletion Level", labels = c("Partial Blood", "No Blood")) +
  ggtitle(label="Figure 2.2: Weekly Percent of Incomplete Blood Collections, Henry Ford Health") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position="bottom",
...
knitr::kable(bldcol_research_HF2, col.names=c("Clinic Location","All Blood Collected","Partial Blood Collected","Total Clinical Collections"), caption='Table 2.6: Blood Collection Completeness Among Baseline Clinical Collections \n by Clinic Location, Henry Ford Health', row.names=FALSE, align=c("l","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") #"No Blood Tubes Collected",
...
tab2_7= knitr::kable(research.coltube_totals, caption='Table 2.7: Total Blood Tubes Collected Among Baseline Research Baseline Collections', row.names=FALSE,align=c("l","c","c","c","c","c","c","c","c","c", "c"), booktabs = TRUE) %>%  add_indent(c(1:6))  %>% kable_styling(latex_options = "scale_down")
add_footnote(tab2_7, "Note: Research collections are expected to have a total of 5 blood tubes collected.", notation = "none") 

tab2_8= knitr::kable(Research_blood_totals, caption='Table 2.8: Blood Tubes Collected Among Baseline Research Collections by Tube Type', row.names=FALSE,align=c("l","c","c","c","c","c","c","c","c","c", "c"), booktabs = TRUE) %>%  add_indent(c(1:6))  %>% kable_styling(latex_options = "scale_down")
...
tab10_2= knitr::kable(tube_completeness_totals, caption='Table 2.9: Total Blood Tubes Collected Among Baseline Clinical Biospecimen Collections', row.names=FALSE,align=c("l","c","c","c","c","c","c","c","c","c", "c"), booktabs = TRUE) %>%  add_indent(c(1:6))  %>% kable_styling(latex_options = "scale_down")
...
tab8=knitr::kable(notcol_combo2_totals,caption='Table 2.10: Blood Tubes Collected Among Baseline Clinical Biospecimen Collections by Tube Type', row.names=FALSE,align=c("l","c","c","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>%  kable_styling(latex_options = "scale_down", full_width = FALSE)   %>% column_spec(column = 2:ncol(notcol_combo2_totals),width = "4cm")  %>% column_spec(column = 1:ncol(notcol_combo2_totals),width = "5cm") %>%  landscape()
add_footnote(tab8, "Note: The expected collected tube type varies by site and is as follows: (1)KPCO: 4 SST, 2 Heparin, 2 EDTA. (2)KPGA: 4 SST, 1 Heparin, 2 EDTA. (3)KPHI: 4 SST, 2 Heparin, 3 EDTA. (4)KPNW: 5 SST, 2 Heparin, 2 EDTA. (5)HFH: 2 SST, 1 Heparin, 1 EDTA, 1 ACD. (6) HP: 2 SST, 1 Heparin, 1 EDTA, 1 STRECK.", notation = "none") 


knitr::kable(Deviation_Freq[,c(7,5,6,4)],col.names=c("Site","Any Deviations","No Deviations", "Total Tubes Collected"), caption='Table 2.11: Deviation Status Among Baseline Biospecimen Tubes Collected',row.names=FALSE, align=c("l","c","c","c"), booktabs = TRUE )
```
...
knitr::kable(research.notcolnotes_site_totals,  caption='Table 2.12: Reason Not Collected Among Baseline Research Collection Tubes Not Collected', row.names=FALSE,align=c("l","c","c","c","c","c","c", "c"), booktabs = TRUE) %>%  add_indent(c(1:5)) %>% kable_styling(latex_options = "scale_down")

tubes2_12 <- knitr::kable(research.notcolnotes_tube_totals,  caption='Table 2.13: Reason Not Collected Among Baseline Research Collection Tubes Not Collected by Tube Type', row.names=FALSE,align=c("l","c","c","c","c","c","c", "c"), booktabs = TRUE)  %>%  add_indent(c(1:8))  %>% kable_styling(latex_options = "scale_down")
add_footnote(tubes2_12, "Note: 'Short Draw' reason applicable to blood collection tubes only.", notation="none")


knitr::kable(discard.site, caption='Table 2.14: Discarded Tubes Among Baseline Biospecimen Collections by Tube Type', row.names=FALSE,  col.names=c("Site",  "SST 1", "SST 2", "SST 5", "Heparin 1", "Heparin 2", "EDTA 1, 2, or 3", "ACD", "Urine", "Total Discarded Tubes"), align=c("l","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")
...
knitr::kable(Table15_combined_totals,  caption='Table 3.1: Biospecimen Survey Status Among Baseline Research Collections', row.names=FALSE,align=c("l","c","c","c","c","c","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")
...
research.3 <- ggplot(figsub3_5, aes(x=as.Date(col.Monday.date), y=100*(cum_sub/cumtotal))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Table 3.1: Biospecimen Survey Status Among Baseline Research Collections") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
research.3.1 <- ggplot(figsub3_5, aes(x=as.Date(col.Monday.date), y=100*(weekly_sub/per_week_parts))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3.2: Weekly Percent of Baseline Biospecimen Surveys Submitted, \n University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
knitr::kable(Table16_combined_totals, caption='Table 3.2: Biospecimen Survey Status Among Baseline Clinical Collections', row.names=FALSE,align=c("l","c","c","c","c","c","c","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")


tbl19a <-knitr::kable(Table17_combinedB_totals, caption='Table 3.3: Biospecimen Survey Completion Time from Baseline Research Collection Visit Check-In (Pre-COVID- 19 Questions Removal)', row.names=FALSE,align=c("l","c","c","c","c","c","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")
...
tbl19b <- knitr::kable(Table17_combinedA_totals, caption='Table 3.4: Biospecimen Survey Completion Time from Baseline Research Collection Visit Check-In (Post- COVID-19 Questions Removal)', row.names=FALSE,align=c("l","c","c","c","c","c","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")
...
knitr::kable(Table18_combined_totals, caption='Table 3.5: Biospecimen Survey Completion Time Among Submitted Baseline Clinical Surveys', row.names=FALSE,align=c("l","c","c","c","c","c","c","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")



tab21_rb=knitr::kable(Table21R_Before,caption='Table 3.6: Biospecimen Survey Completion Length (in Minutes) Among Submitted Baseline Research Surveys (Pre-COVID-19 Questions Removal)',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
tab21_ra=knitr::kable(Table21R_After,caption='Table 3.7: Biospecimen Survey Completion Length (in Minutes) Among Submitted Baseline Research Surveys (Post-COVID-19 Questions Removal)',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
tab21_cb=knitr::kable(ClinSrvy_Lgth_Before,caption='Table 3.8: Biospecimen Survey Completion Length (in Minutes) Among Submitted Baseline Clinical Surveys (Pre-COVID-19 Questions Removal)',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
tab21_ca=knitr::kable(ClinSrvy_Lgth_After,caption='Table 3.9: Biospecimen Survey Completion Length (in Minutes) Among Submitted Baseline Clinical Surveys (Post-COVID-19 Questions Removal)',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
uc_col_time <- knitr::kable(research.vt.length.UC.table,caption='Table 4.2: Baseline Research Collection Visit Length (in Minutes) by Collection Location, University of Chicago Medicine',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down") #%>% landscape()
...
#research.vt.length.UC$time


UC_Tm_plot <- research.vt.length.UC 
UC_Tm_plot$Site_location <- UC_Tm_plot$`Collection Location`

UC_Time_plot <- UC_Tm_plot %>%  group_by(col.Monday.date, Site_location) %>% tally(mean(time))
...
UC_Time <- ggplot(UC_Time_plot, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0)) +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 4.1: Weekly Average Baseline Research Collection Visit Length (in Minutes) \n by Collection Location, University of Chicago Medicine") +
...
tabbA <- knitr::kable(verif_BSL_colctC,caption='Table 4.3: Time (in Days) from Verification to Baseline Research Collection Among Participants Verified Before 8/01/2022',row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")
...
tabbB <- knitr::kable(verif_BSL_colctD,caption='Table 4.4: Time (in Days) from Verification to Baseline Research Collection Among Participants Verified on or After 8/01/2022',row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")
...
BSL_A <- knitr::kable(verif_BSL_colctA,caption='Table 4.5: Time (in Days) from Verification to Baseline Clinical Collection Among Participants Verified Before 2/01/2023',row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)  %>%  kable_styling(latex_options = "scale_down")
add_footnote(BSL_A,"Note: Baseline Clinical Collections were launched in February 2023.", notation = "none")

BSL_B <-knitr::kable(verif_BSL_colctB,caption='Table 4.6: Time (in Days) from Verification to Baseline Clinical Collection Among Participants Verified on or After 2/01/2023',row.names=FALSE,  align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)  %>%  kable_styling(latex_options = "scale_down")
add_footnote(BSL_B,"Note: Baseline Clinical Collections were launched in February 2023.", notation = "none")




AF26 <- knitr::kable(verif_baseline_Af[c(1,2,7:9,3:6), ],caption = 'Table 4.7: Time (in Days) from Verification to Baseline Clinical Collection Among Participants Verified on or After 6/01/2022 by Campaign Type',row.names=FALSE, booktabs = T, linesep = "", format.args = list(big.mark = ","), align = c("l", "c", "c","c", "c", "c", "c", "c", "c", "c", "c","c", "c","c"), col.names=c("Site",  'Number of Clinnical Collections',"Min", "Median", "Mean", "SD", "Max", 'Number of Research Collections', "Min", "Median", "Mean", "SD", "Max","Total Baseline Collections") ) %>% kable_styling(latex_options = "scale_down") %>% add_indent(c(1:9)) %>% add_header_above(c(" " = 1, "Upcoming Appointment" = 6, "All Other Campaigns" = 6,  " "=1)) %>% landscape() 
...
tabl27 <- knitr::kable(verif_draw_order,caption='Table 4.8: Time (in Days) from Verification to Baseline Draw Order Placement',row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")
...
tab4_9 <- knitr::kable(avg_draw_order, caption='Table 4.9: Time (in Days) from Draw Order Placement to Baseline Clinical Collection', row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down") #%>% landscape()
...
tab4_10 <- knitr::kable(avg_bldorder_present, caption='Table 4.10: Time (in Days) from Draw Order Placement to Present Date by Site', row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down") #%>% landscape()
add_footnote(tab4_10, "Note: HealthPartners is exluded due to the limited availability of draw order placement data.", notation="none")

add_footnote(Table_bld_draw , "Note: HealthPartners is exluded due to the limited availability of draw order placement data.", notation="none")

coll_bplt_knitr <- knitr::kable(crosstable.coll_bptl1, col.names=c("Site", "< 30 Days", "30 to <60 Days","60 to <90 Days", "90 to <180 Days", "180 to <270 Days", "270 Days or More", "Total Participants with No Basline Collections"), caption='Table 4.12: Time (in Days) Since Verification Among Participants with No Baseline Biospecimen Collection ', row.names=FALSE, align=c("l","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
coll_bplt_knitr
...
\newpage
\FloatBarrier
# Needle-To-Receipt Times
\vspace{-0.5em}
\FloatBarrier
```{r KnitrTable28_30,echo=FALSE,error=FALSE,message=FALSE,results='asis'}

knitr::kable(crosstable.coll_bptl28a, col.names=c("Site", "1 Day", "2 Days", "3 Days", "4 Day", ">4 Days", "Total Received Research Collections"), caption='Table 5.1: Needle-to-Receipt Time (in Days) Among Baseline Research Collections', row.names=FALSE, align=c("l","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")

needle5_2 <- knitr::kable(crosstable.coll_bptl28, col.names=c("Collection Location", "1 Day", "2 Days", "3 Days", "4 Day", ">4 Days", "Total Received Research Collections"), caption='Table 5.2: Needle-to-Receipt Time (in Days) Among Baseline Research Collections by Collection Location', row.names=FALSE, align=c("l","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
knitr::kable(crosstable.coll_bptl29a, col.names=c("Site", "1 Day", "2 Days", "3 Days", "4 Days", ">4 Days", "Total Received Clinical Collections"), caption='Table 5.3: Needle-to-Receipt Time (in Days) Among Baseline Clinical Collections', row.names=FALSE, align=c("l","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 


coll_bplt_knitr2 <- knitr::kable(crosstable.coll_bptl_2, col.names=c("Collection Location", "1 Day", "2 Days", "3 Days", "4 Days", ">4 Days", "Total Recived Clinical Collections"), caption='Table 5.4: Needle-to-Receipt Time (in Days) Among Baseline Clinical Collections by Clinic Location', row.names=FALSE, align=c("l","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
coll_bplt_knitr3 <- knitr::kable(Hrs_bptl_col, col.names=c("Collection Location", 'Min', 'Q1', 'Median', 'Mean', 'Q3','Perc.90','Perc.98'), caption='Table 5.5: Time (in Hours) from Baseline Research Collection to Receipt by Collection Location', row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
coll_bplt_knitr4 <- knitr::kable(Hrs_bptl_col4, col.names=c("Collection Location", 'Min', 'Q1', 'Median', 'Mean', 'Q3','Perc.90','Perc.98'), caption='Table 5.6: Time (in Hours) from Baseline Clinical Collection to Receipt by Clinic Location', row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
ggplot(scatMF, aes(x=as.Date(d_678166505), y=hrs_past, color = Weekdays)) + 
  ...
  theme(panel.background = element_blank(),         
        ...
        axis.title.y = element_text(size = 14, face = "bold",color="black"),         
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +     
  theme(plot.caption = element_text(hjust=0))  + ggtitle(label="Figure 5.1: Baseline Needle-to-Receipt Time (in Hours), \n Marshfield")
...
ggplot(scatSF, aes(x=as.Date(d_678166505), y=hrs_past, color = Weekdays)) + 
  ...
  scale_color_manual(values=c("Sunday"="red", "Monday"="orange", "Tuesday"="pink", "Wednesday"="lightgreen", 
    ...
    theme(panel.background = element_blank(),         
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +     
  theme(plot.caption = element_text(hjust=0))  +
  ggtitle(label="Figure 5.2: Baseline Needle-to-Receipt Time (in Hours), \n Sanford Health")
...
ggplot(scatUC, aes(x=as.Date(d_678166505), y=hrs_past, color = Weekdays)) + 
  ...
  scale_color_manual(values=c("Sunday"="red", "Monday"="orange", "Tuesday"="pink", "Wednesday"="lightgreen", 
    ...
    theme(panel.background = element_blank(),         
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +     
  theme(plot.caption = element_text(hjust=0))  +
  ggtitle(label="Figure 5.3: Baseline Needle-to-Receipt Time (in Hours),  \n University of Chicago Medicine")
...
ggplot(scatKPCO, aes(x=as.Date(clin_coll_bu), y=hrs_past, color = Weekdays)) + 
  ...
  scale_color_manual(values=c("Sunday"="red", "Monday"="orange", "Tuesday"="pink", "Wednesday"="lightgreen", 
    ...
    theme(panel.background = element_blank(),         
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +     
  theme(plot.caption = element_text(hjust=0))  +
  ggtitle(label="Figure 5.4: Baseline Needle-to-Receipt Time (in Hours),  \n Kaiser Permanente Colorado")
...
ggplot(scatKPGA, aes(x=as.Date(clin_coll_bu), y=hrs_past, color = Weekdays)) + 
  ...
  scale_color_manual(values=c("Sunday"="red", "Monday"="orange", "Tuesday"="pink", "Wednesday"="lightgreen", 
    ...
    theme(panel.background = element_blank(),         
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +     
  theme(plot.caption = element_text(hjust=0))  +
  ggtitle(label="Figure 5.5: Baseline Needle-to-Receipt Time (in Hours), \n Kaiser Permanente Georgia")
...
ggplot(scatKPHI, aes(x=as.Date(clin_coll_bu), y=hrs_past, color = Weekdays)) + 
  ...
  scale_color_manual(values=c("Sunday"="red", "Monday"="orange", "Tuesday"="pink", "Wednesday"="lightgreen", 
    ...
    theme(panel.background = element_blank(),         
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +     
  theme(plot.caption = element_text(hjust=0))  +
  ggtitle(label="Figure 5.6: Baseline Needle-to-Receipt Time (in Hours), \n Kaiser Permanente Hawaii")
...
ggplot(scatKPNW, aes(x=as.Date(clin_coll_bu), y=hrs_past, color = Weekdays)) + 
  ...
  scale_color_manual(values=c("Sunday"="red", "Monday"="orange", "Tuesday"="pink", "Wednesday"="lightgreen", 
    ...
    theme(panel.background = element_blank(),         
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +     
  theme(plot.caption = element_text(hjust=0))  +
  ggtitle(label="Figure 5.7: Baseline Needle-to-Receipt Time (in Hours), \n Kaiser Permanente Northwest")
...
combined_plotHP <- ggplot(combined_dataHP, aes(x = as.Date(coldate), y = hrs_past, shape = Group, color = factor(weekdays(as.Date(coldate))))) +
  ...
  scale_y_continuous(name="Number of Hours",  breaks=seq(0,168,24), limits = c(0, 168), expand = c(0, 0)) +
  labs(color = "Weekdays", shape="Collection Setting") +
  scale_color_manual(values=c("Sunday"="red", "Monday"="orange", "Tuesday"="pink", "Wednesday"="lightgreen", 
  ...
  ggtitle(label="Figure 5.8: Baseline Needle-to-Receipt Time (in Hours) by Collection Setting, \n HealthPartners")
combined_plotHP
...
combined_plotHF <- ggplot(combined_dataHF, aes(x = as.Date(coldate), y = hrs_past, shape = Group, color = factor(weekdays(as.Date(coldate))))) +
  ...
  scale_shape_manual(values = c("Research" = 0, "Clinical" = 17)) + 
    theme(panel.background = element_blank(),         
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +     
  theme(plot.caption = element_text(hjust=0), legend.title = element_text(size = 8.5))  +
  ggtitle(label="Figure 5.9: Baseline Needle-to-Receipt Time (in Hours) by Collection Setting, \n Henry Ford")
combined_plotHF
```
...
research.vt1 <- ggplot(visit_wk_site1, aes(x=as.Date(col.Monday.date), y=n)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0)) +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 6.1: Weekly Total Baseline Biospecimen Collections") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        legend.position="bottom",
...
visit_wk_site.hybrid <-research.survey.hybrid %>% arrange(Site,Site_location, Setting,col.Sunday.week,col.Monday.date) %>% group_by(Site,Site_location, Setting,col.Sunday.week,col.Monday.date) %>% tally()
 ...
 research.vt_plotH <- ggplot(visit_wk_site.hybridHF, aes(x=as.Date(col.Monday.date), y=n, color=Setting )) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 6.2: Weekly Baseline Biospecimen Collections by Collection Setting, \n at Henry Ford") +
 ...
 edited_fig3_1 <-  visit_wk_site.hybridHF %>%  filter(Site_location!="Hybrid")
  research.vt_plotHF <- ggplot(edited_fig3_1, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   geom_point(width = 0.2) + geom_line() + 
   scale_y_continuous(name="Number of Collections") +
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 6.3: Weekly Baseline Research Biospecimen Collections \n by Collection Location, Henry Ford Health") +
   labs(color = "Collection Location") +
   theme(panel.background = element_blank(),
         ...
         axis.title.y = element_text(size = 14,color="black"),   #, face = "bold"
         plot.title = element_text(hjust = 0.5,size = 12, face = "bold"))  
research.vt_plotHF + guides(color = guide_legend(title = "Collection Location")) 
...
edited_fig3_1_Clin_tally <- edited_fig3_1_Clin %>% arrange(HF_Sites, col.Sunday.week,col.Monday.date) %>% group_by(HF_Sites, col.Sunday.week,col.Monday.date) %>% tally()
  ...
  research.vt_plotH_Clin <- ggplot(edited_fig3_1_Clin_tally, aes(x=as.Date(col.Monday.date), y=n, color=HF_Sites )) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 6.4: Weekly Baseline Clinical Biospecimen Collections \n by Clinic Location, Henry Ford Health") +
...
#- SANFORD = 589224449, 467088902
 ...
 research.vt_plotUC <- ggplot(research.vt_plUC, aes(x=as.Date(col.Monday.date), y=total_collections)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 6.5: Weekly Baseline Biospecimen Collections, \n University of Chicago Medicine") +
   #theme_ipsum() +
   theme(panel.background = element_blank(),
         #legend.title = element_text(size=7),
         axis.line = element_line(size=0.2),
 ...
 research.vt_plotUC <- ggplot(research.vt_plUC, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 6.6: Weekly Research Biospecimen Collections \n by Collection Location, University of Chicago Medicine ") +
   labs(color = "Collection Location") +
   theme(panel.background = element_blank(),
         #legend.title = element_text(text="Site Location"),
         legend.position = "bottom",
...
research.vt_plotUC + guides(color = guide_legend(title = "Clinic Location")) 
 ...
 research.vt_plotMF <- ggplot(research.vt_plMF, aes(x=as.Date(col.Monday.date), y=total_collections)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 6.7: Weekly Baseline Biospecimen Collections, \n Marshfield Clinic Health System") +
   #theme_ipsum() +
   theme(panel.background = element_blank(),
         #legend.title = element_text(size=7),
         axis.line = element_line(size=0.2),
 ...
 print(research.vt_plotMF)
  ...
  research.vt_plMF= visit_wk_site_edited %>%  filter( Site=="Marshfield Clinic Health System")
 research.vt_plotMF <- ggplot(research.vt_plMF, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   geom_line() + geom_point(width = 0.2) +
   scale_y_continuous(name="Number of Collections", limits=c(0,30), breaks=seq(0,30,5)) +
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 6.8: Weekly Baseline Research Biospecimen Collections \n by Collection Location, Marshfield Clinic Health System") +
   labs(color = "Collection Location")+
   theme(panel.background = element_blank(),
         #legend.title = element_text(text="Site Location"),
         legend.position = "bottom",
         ...
         axis.title.y = element_text(size = 14,color="black"),   #, face = "bold"
         plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) #guides(fill=guide_legend(nrow=2,byrow=TRUE))
research.vt_plotMF + guides(color = guide_legend(title = "Collection Location")) 
...
research.vt_plotSH <- ggplot(research.vt_plSH, aes(x=as.Date(col.Monday.date), y=n)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 6.9: Weekly Baseline Biospecimen Collections, \n Sanford Health") +
  labs(color = "Collection Location") +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
research.vt_plotSH <- ggplot(research.vt_plSH, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 6.10: Weekly Baseline Research Biospecimen Collections \n by Collection Location, Sanford Health") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
research.vt_plotHP <- ggplot(research.vt_plHP, aes(x=as.Date(col.Monday.date), y=n, color=Setting)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 6.11: Weekly Baseline Biospecimen Collections \n by Collection Setting, HealthPartners") +
  labs(color = "Collection Setting") +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
research.vt_plotHP_locs <- ggplot(edited_fig11_1_Clin_tally, aes(x=as.Date(col.Monday.date), y=n, color=HP_cl_locs)) +  
  ...
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 6.12: Weekly Baseline Clinical Biospecimen Collections \n  by Clinic Location, HealthPartners ") +
  labs(color = "Clinic Location") +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",

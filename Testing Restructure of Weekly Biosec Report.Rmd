...
tb_bq <- eval(parse(text=paste("bq_project_query(project, query=\"SELECT token,Connect_ID,d_512820379,d_821247024,d_914594314,d_827220437,d_699625233, d_265193023, d_822499427, 
                               ...
                               state_d_706256705, d_173836415_d_266600170_d_319972665_d_661940160, d_173836415_d_266600170_d_319972665_d_687158491, d_949302066,
                               d_173836415_d_266600170_d_319972665_d_221592017, d_173836415_d_266600170_d_541483796_d_221592017, d_173836415_d_266600170_d_641006239_d_221592017,",
                               select,
...
biocol.tb <- biocol.tb %>%
    mutate(across(all_of(tube_time_cols), ymd_hms),
           ...
           d_914594314 = ymd_hms(d_914594314),
           d_173836415_d_266600170_d_769615780 = ymd_hms(d_173836415_d_266600170_d_769615780),
           d_173836415_d_266600170_d_822274939 = ymd_hms(d_173836415_d_266600170_d_822274939),
           d_915838974 = ymd_hms(d_915838974),
           d_173836415_d_266600170_d_184451682 = ymd_hms(d_173836415_d_266600170_d_184451682))
...
col_num_1.1 <- col_num_1.1 %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` %in% c("Sioux Falls Imagenetics", "Fargo South University","Edith Sanford Breast Center (coded as Sanford Center)", 
                                  "Edith Sanford Breast Center","Fargo Amber Valley","Bemidji Clinic","Missing SF location") ~ "Sanford Health",
...
research.survey <- coll_sett_all %>% filter(all_settings == "Research") %>%
  mutate(Site=droplevels(as.factor(Site)),
         ctime = round(as.numeric(difftime(d_222161762,d_822499427,units="mins")), digits=1),
         tm_biocolsuv = difftime(d_222161762,d_331584571_d_266600170_d_840048338,units="hours"), #time b/w survey completion to bio collection visit check in
         ...
         time_biochk = difftime(d_331584571_d_266600170_d_343048998,d_331584571_d_266600170_d_840048338,units="mins"),
...
# 
# 
# UC_Time <- ggplot(research.vt.length.UC, aes(x=col.Monday.date, y=n, color=Site_location)) + 
...
#                #date_breaks = "4 weeks", 
#                date_labels = "%y-%m-%d", date_breaks = "4 weeks",
#                limits=c( as.Date("2022-06-20"),(currentDate-1)),expand=c(0,0)) +
...
Section3_Figures <- function(site, start_date, title) {
  ...
  plots3 <- ggplot(weekly_summary, aes(x = col.Monday.date, y = pct3)) +
    geom_line() +
    ...
    scale_x_date(name = "Date",
                 ...
                 date_labels = "%y-%m-%d", 
                 date_breaks = "4 weeks",
                 limits = c(as.Date(start_date), currentDate - 1), expand = c(0, 0)) +
...
coll_bptl_diff <- biospe__all %>%  
  ...
  mutate(any_collection = min(d_143615646_d_926457119, d_232343615_d_926457119, d_299553921_d_926457119, d_376960806_d_926457119, d_454453939_d_926457119, 
         ...
         day_past = difftime(any_collection, d_678166505, units="days"),
...
ctable_rsch_loc_bptl <- rsch_loc_coll_bptl %>%
  mutate(Site_Group = case_when(
    ...
    Collection_Location %in% c("Sioux Falls Imagenetics", "Fargo South University", "Edith Sanford Breast Center (coded as Sanford Center)", 
                                  "Edith Sanford Breast Center","Fargo Amber Valley","Bemidji Clinic","Missing SF location") ~ "Sanford Health",
    Collection_Location %in% c("HFH K-13 Research Clinic", "HFH Livonia Research Clinic", "HFH Cancer Pavilion Research Clinic", "HFH Pop-Up", 
                                 "HFH Detroit Northwest", "In-home Collection","HFH Jackson","Missing HFH location") ~ "Henry Ford Health",
    Collection_Location %in% c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving","Irving","NTX Biorepository","BCC- Fort Worth", 
                                 "North Garland", "Temple CDM","Temple Roney", "Waco - MacArthur", "Temple Westfield",
                               "Waco Fishpond","Killeen Main",
                               "Missing BSWH location") ~ "Baylor Scott and White Health",
...
clinical.survey <- coll_sett_all %>% filter(all_settings == "Clinical") %>%
  mutate(clin.sv = case_when(d_253883960 == 231311385 ~ "Submitted",
         ...
         d_173836415_d_266600170_d_139245758_clc = d_173836415_d_266600170_d_139245758, ##urine
         d_173836415_d_266600170_d_982213346_clc = d_173836415_d_266600170_d_982213346, ##blood collected
         d_173836415_d_266600170_d_740582332_clc = d_173836415_d_266600170_d_740582332, ##Autogenerated date/time stamp for Any specimen collected at RRL
         clinic.svtime =  difftime(d_764863765,d_534669573,units="hours"),
         ctime = round(as.numeric(difftime(d_764863765,d_534669573,units="mins")), digits=1)) ##survey time from start to complete

clinical.survey <- clinical.survey %>% 
  mutate(clc.coltm  = do.call(pmin, c(select(.,ends_with('clc')),na.rm=TRUE)),
         tm_biocolsuv =  difftime(d_764863765,clc.coltm,units="hours")) #time between survey completion to biospecimen collection visit check in
...
clinical.survey <- clinical.survey %>%  
  mutate(coll_to_DB_BU = as.numeric(difftime(d_915838974,
                                             pmin(d_173836415_d_266600170_d_982213346, 
                                                  d_173836415_d_266600170_d_139245758, na.rm=TRUE),units="hours")))
...
clinical.survey <- clinical.survey %>%  
  mutate(trig_to_comp_BU = as.numeric(difftime(d_764863765,d_915838974,units="hours")))
...
avg_draw_order <- clinical.survey %>%  filter(Site!='HealthPartners' & Site!="Sanford Health" & Site!='University of Chicago Medicine') %>% 
  mutate(time = as.numeric(difftime(
    ...
    pmin(d_173836415_d_266600170_d_982213346, d_173836415_d_266600170_d_139245758), 
    #and BldUrnPlcdTmBL
...
coll_bptl_diff2 <- biospe__2  %>% group_by(as.character(Connect_ID),Site, Clin_Site_location) %>% 
  mutate(any_collection = min(d_232343615_d_926457119, d_299553921_d_926457119, d_376960806_d_926457119, d_454453939_d_926457119, d_505347689_d_926457119,
         ...
         day_past = difftime(any_collection, clin_coll_bu, units="days"),
         coll_days_diff = ifelse(as.numeric(day_past)<2,"1 Day",
                                 ifelse(as.numeric(day_past)<3,"2 Days",
                                        ifelse(as.numeric(day_past)<4,"3 Days", 
                                               ifelse(as.numeric(day_past)<5,"4 Days", 
...
BSL_colctD <- baseline_colctD %>%  mutate(time = as.numeric(difftime(d_556788178,d_914594314,units="days")))


verif_BSL_colctD = BSL_colctD %>% group_by(Site) %>% 
  dplyr::summarize('Number of Research Collection Visits'= scales::comma(n()),
                   Min = round(min(time,na.rm = TRUE), digits=1),
                   Q1 = round(quantile(time, 0.25,na.rm = TRUE), digits=1),
                   Median = round(median(time,na.rm = TRUE), digits=1),
                   Mean = round(mean(time,na.rm = TRUE), digits=1),
...
verif_BSL_colctD_total <- BSL_colctD %>% 
...
verif_BSL_colctD <- rbind(verif_BSL_colctD, verif_BSL_colctD_total)
...
BSL_colctA <- baseline_colctA %>%  mutate(time = as.numeric(difftime(d_556788178,d_914594314,units="days")),
                                          "Number of Participants" = n(),
                                          'Number of Baseline Collections' = case_when(!is.na(d_556788178) ~ n()))


verif_BSL_colctA = BSL_colctA %>% group_by(Site) %>%
  dplyr::summarize('Number of Clinical Collections' = scales::comma(n()),
                   Min = round(min(time,na.rm = TRUE), digits=1),
                   Q1 = round(quantile(time, 0.25,na.rm = TRUE), digits=1),
                   Median = round(median(time,na.rm = TRUE), digits=1),
                   Mean = round(mean(time,na.rm = TRUE), digits=1),
...
BSL_colctB <- baseline_colctB %>%  mutate(time = as.numeric(difftime(d_556788178,d_914594314,units="days")))
...
verif_BL_Af <- coll_sett_all %>%  filter(d_331584571== 266600170 & !is.na(d_914594314) & !is.na(d_556788178) & d_914594314>=as.Date("2022-06-01")) %>%  
  mutate(time = as.numeric(difftime(d_556788178,d_914594314,units="days")),
...
all_coll_to_sub <- coll_sett_all %>%  filter(d_265193023 == 'Submitted' | d_253883960=='231311385') %>% 
  mutate(collection = ifelse(d_265193023 == 'Submitted', d_678166505, d_556788178),
         surv_sub = ifelse(d_265193023 == 'Submitted', d_222161762, d_764863765),
...
avg_bldorder_present <- table1 %>%  filter(d_878865966=="No blood collected" & !is.na(d_173836415_d_266600170_d_769615780) & 
  ...
  mutate(time = as.numeric(difftime(currentDate,d_173836415_d_266600170_d_769615780,units="days"))) %>% 
  group_by(Site) %>% 
...
ver_date_diff <-table1  %>% filter(biocol.appoint=="No Baseline Collection")  %>% 
  mutate(day_past = difftime(currentDate, as.Date(d_914594314), units="days"),
...
knitr::kable(spec_by_site_C, 
  ...
  column_spec(1, width = "4cm") %>% #make column 1's long title wrap into 2 lines
  kable_styling(latex_options = "scale_down") 
...
tab4_10 <- knitr::kable(avg_bldorder_present, 
                        ...
                        row.names=FALSE, align=c("l", rep("c", 11)),digits=1, booktabs = TRUE) %>% 
  column_spec(2, width = "4cm") %>% #make column 2's long title wrap into 2 lines
  kable_styling(latex_options = "scale_down") 
...
# --- Pre-process once, outside the plot function ---

prep_coll_data <- function(df, date_col, setting_label) {
  df %>%
    mutate(
      hrs_past = as.numeric(difftime(any_collection,
                                     .data[[date_col]],
                                     units = "hours")),
      Weekdays = factor(weekdays(.data[[date_col]]),
...
research_prepped <- prep_coll_data(coll_bptl_diff,  "d_678166505",  "Research")
clinical_prepped <- prep_coll_data(coll_bptl_diff2, "clin_coll_bu", "Clinical")

# Pre-split by site so filter() isn't called inside every plot call
research_by_site <- split(research_prepped, research_prepped$Site)
clinical_by_site <- split(clinical_prepped, clinical_prepped$Site)


# --- Single unified plot function ---

weekday_colors <- c("Sunday"="#B1C7D9", "Monday"="#164C71", "Tuesday"="#FDBE19",
                    "Wednesday"="#FBE5B5", "Thursday"="#565C65",
                    "Friday"="#3C989E",   "Saturday"="#CC7D15")

make_coll_plot <- function(site_data, date_col, start_date, title) {
  ggplot(site_data, aes(x = .data[[date_col]], y = hrs_past, color = Weekdays)) +
    geom_point() +
    scale_x_date(name = "Collection Date",
                 ...
                 date_labels = "%y-%m-%d",
                 date_breaks = "4 weeks",
                 limits = c(as.Date(start_date), currentDate - 7),
                 expand = c(0, 0)) +
    scale_y_continuous(name = "Number of Hours",
                       breaks = seq(0, 168, 24),
                       limits = c(0, 168),
                       expand = c(0, 0)) +
    scale_color_manual(values = weekday_colors) +
    ggtitle(label = title) +
    theme(panel.background  = element_blank(),
          legend.position   = "bottom",
          axis.line         = element_line(size = 0.2),
...
# --- Plot calls (now just passing pre-filtered data) ---

make_coll_plot(research_by_site[["Marshfield Clinic Health System"]],
               "d_678166505", "2022-06-20",
               "Figure 5.1: Baseline Collection-To-Receipt Time (in Hours), \n Marshfield")

make_coll_plot(research_by_site[["Sanford Health"]],
               "d_678166505", "2022-06-20",
               "Figure 5.2: Baseline Collection-To-Receipt Time (in Hours), \n Sanford Health Research Setting")

make_coll_plot(clinical_by_site[["Sanford Health"]],
               "clin_coll_bu", "2024-03-25",
               "Figure 5.3: Baseline Collection-To-Receipt Time (in Hours), \n Sanford Health Clinical Setting")

make_coll_plot(clinical_by_site[["Kaiser Permanente Colorado"]],
               "clin_coll_bu", "2022-10-10",
               "Figure 5.4: Baseline Collection-To-Receipt Time (in Hours), \n Kaiser Permanente Colorado")

make_coll_plot(clinical_by_site[["Kaiser Permanente Georgia"]],
               "clin_coll_bu", "2022-10-10",
               "Figure 5.5: Baseline Collection-To-Receipt Time (in Hours), \n Kaiser Permanente Georgia")

make_coll_plot(clinical_by_site[["Kaiser Permanente Hawaii"]],
               "clin_coll_bu", "2022-10-10",
               "Figure 5.6: Baseline Collection-To-Receipt Time (in Hours), \n Kaiser Permanente Hawaii")

make_coll_plot(clinical_by_site[["Kaiser Permanente Northwest"]],
               "clin_coll_bu", "2022-10-10",
               "Figure 5.7: Baseline Collection-To-Receipt Time (in Hours), \n Kaiser Permanente Northwest")

make_coll_plot(research_by_site[["HealthPartners"]],
               "d_678166505", "2022-06-20",
               "Figure 5.8: Baseline Collection-To-Receipt Time (in Hours), \n HealthPartners Research Setting")

make_coll_plot(clinical_by_site[["HealthPartners"]],
               "clin_coll_bu", "2023-10-23",
               "Figure 5.9: Baseline Collection-To-Receipt Time (in Hours), \n HealthPartners Clinical Setting")

make_coll_plot(research_by_site[["Henry Ford Health"]],
               "d_678166505", "2022-06-20",
               "Figure 5.10: Baseline Collection-To-Receipt Time (in Hours), \n Henry Ford Research Setting")

make_coll_plot(clinical_by_site[["Henry Ford Health"]],
               "clin_coll_bu", "2023-04-24",
               "Figure 5.11: Baseline Collection-To-Receipt Time (in Hours), \n Henry Ford Clinical Setting")
...
Section6_plot_function <- function(data, start_date, title, site = "all", color_by_setting = FALSE, color_var = NULL) {
  ...
  if (color_by_setting && !is.null(color_var)) {
    ...
    plt <- ggplot(weekly_counts, aes(x = col.Monday.date, y = n, color = .data[[color_var]])) +
    ...
    plt <- ggplot(weekly_counts, aes(x = col.Monday.date, y = n))
  }
  ...
  plot6 <- plt +
    ...
    scale_x_date(name = "Date", 
                 ...
                 date_labels = "%y-%m-%d",
                 date_breaks = "4 weeks",
                 limits = c(as.Date(start_date), currentDate - 7), expand = c(0, 0)) +
    ggtitle(label = title) +
    theme(panel.background = element_blank(),
...
Section6_plot_function2 <- function(data, start_date, title, site, settings, color_var) {
  ...
  theme(
    ...
    plt <- ggplot(weekly_countsUC_Clin, aes(x = col.Monday.date, y = n, color = .data[[color_var]])) +
...
test30 <-  test30 %>%
  mutate(ctime = as.numeric(difftime(d_173836415_d_266600170_d_319972665_d_826941471,
                                     d_173836415_d_266600170_d_319972665_d_661940160,
                                     units="days"))) %>% 
  filter(ctime>0)
...
received_long <- kit_table %>%
  ...
  mutate(
    col.Monday.date = floor_date(as.Date(received_date), unit = "week", week_start = 1)
  ) %>%
  filter(
...
shipped_long <- kit_table %>%
  ...
  mutate(
    col.Monday.date = floor_date(as.Date(shipped_date), unit = "week", week_start = 1)
  ) %>%
  filter(
    d_173836415_d_266600170_d_319972665_d_379252329 == 976461859,
    col.Monday.date >= as.Date("2024-03-29"),
...
MW_Coll_Kits <- ggplot(fig5_week_final, aes(x = col.Monday.date)) +
  ...
  scale_x_date(name = "Date", 
               ...
               date_labels = "%y-%m-%d",
                 date_breaks = "4 weeks",
               limits = c(as.Date("2024-03-25"), currentDate - 7), expand = c(0,0)) +
...
kit_set_col_2 <- kit_table %>% filter(Kit_Status_int=="Received") %>% 
  mutate(col.Monday.date = floor_date(d_173836415_d_266600170_d_319972665_d_826941471, unit = "week", week_start = 1))

 
fig5_new_2 <- kit_set_col_2 %>%
  filter(d_173836415_d_266600170_d_319972665_d_379252329 == 976461859 & col.Monday.date >= as.Date("2024-03-29") & col.Monday.date < currentDate)
...
MW_overtime <- ggplot(fig5_week_total2, aes(x = col.Monday.date)) +
  ...
  labs(caption = paste("Note: Shipped data are current through",currentDate-7, ".")) +
  theme(panel.background = element_blank(),
        legend.position = "bottom",
        axis.line = element_line(size = 0.2),
        axis.text.x = element_text(angle = 60, hjust = 1),
...
generate_mw_table <- function(comparison) {
  kit_table %>% 
    filter(d_547363263 == '231311385' & !is.na(d_195145666) & 
             ...
             eval(parse(text = paste0("d_173836415_d_266600170_d_319972665_d_661940160 ", comparison, " '2024-05-22'")))) %>%
    mutate(MW_completion = difftime(d_173836415_d_266600170_d_448660695, d_195145666, units = "days"),
...
## Cut off at 5/1/2025
table7_10 <- table7_10 %>%  filter(d_173836415_d_266600170_d_319972665_d_661940160 <= '2025-05-01')
...
survey_cc_int <- kit_table %>%  
  filter(!is.na(d_195145666) & !is.na(d_173836415_d_266600170_d_448660695) & 
           d_195145666 > as.Date("2025-05-01") & d_547363263 == '231311385') %>% 
...
kit_tm_don_inv <- kit_request_table %>% 
  ...
  mutate(tm_don_inv_diff = as.numeric(difftime(d_173836415_d_266600170_d_319972665_d_759651991,
                                              d_173836415_d_266600170_d_319972665_d_278539982,units="days")),

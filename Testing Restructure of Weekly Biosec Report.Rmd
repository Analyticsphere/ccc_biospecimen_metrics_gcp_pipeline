...
col_num_1.1 <- col_num_1.1 %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` %in% c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving","Irving","NTX Biorepository","BCC- Fort Worth","North Garland", "Missing BSWH location") ~ "Baylor Scott and White",
    TRUE ~ "Other"
  ))
...
bldcol_research$n_pct_BloodCompletion <- ifelse(!grepl("Total",rownames(bldcol_research)),  paste(format(bldcol_research[,1],big.mark=",")," (", round(100*bldcol_research[,5],1),"%)"), format(bldcol_research[,1],big.mark="," ))
bldcol_research$n_pct_BloodInompletion = ifelse(!grepl("Total",rownames(bldcol_research)),  paste(format(bldcol_research[,2],big.mark=",")," (", round(100*bldcol_research[,6],1),"%)"), format(bldcol_research[,2],big.mark="," ))
bldcol_research$n_pct_NoBlood = ifelse(!grepl("Total",rownames(bldcol_research)),  paste(format(bldcol_research[,3],big.mark=",")," (", round(100*bldcol_research[,7],1),"%)"), format(bldcol_research[,3],big.mark="," ))
...
for (i in 1:length(tube.ls)){
  ...
  dev_long <- reshape2::melt(tmp, #data.table::melt(tmp,
                               id.vars=c(id,tube), 
...
coll_bptl_diff <- biospe_adhoc_all %>%  
  ...
  mutate(Site_location = factor(Site_location,
                                levels=c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving",
                                         "Irving", "NTX Biorepository","BCC- Fort Worth","North Garland",
...
## Need to remove duplicates the same way as in the beginning of the code
...
## Keep only the HMW collections OR the first blood/urine collection to properly remove duplicates
# 1. Keep all CHA rows-- those are the HMW and we have no duplicates so far
cha_rows <- kit_set %>%
  filter(grepl("^CHA", d_820476880))

# 2. Among CXA rows, keep earliest timestamp per Connect_ID-- this is what we did with the original biospecimen table duplicates
cxa_rows <- kit_set %>%
...
kit_sett <- left_join(location_stat, kit_set_clean,   by="Connect_ID") #%>% left_join(recrver1,  by="Connect_ID")
...
#Updates: need kit assembly merged in to detmine kit replacement status
table27_elg_distinct <- left_join(table27_elg, kitassembly_data, by="Connect_ID")
...
# a case when works here because we only want the top value for the participant
table27_elg_distinct <- table27_elg_distinct %>%
  filter(d_650516960=='664882224' | is.na(d_650516960)) %>%
  mutate(kit_level= case_when(d_426588510 == 772116457 ~ "Replacement Kit 2",
                              d_426588510 == 389478821 ~ "Replacement Kit 1",
                              TRUE ~ "Initial Kit"))
 
 
## Participants are getting Kit_Status and kit_level values for both their HMW and blood/urine collection, causing dupicate rows
...
## This will ONLY work for the first table to get the kit status counts as the CHA columns don't have values for d_678166505 and d_143615646_d_826941471
 
kit_status_table_7.1 <- table27_elg_distinct %>% filter(d_650516960=='664882224')
 
## this looks correct, only 77 duplicates which is the number of replacement 1 and 2 kits, but I don't know why there's still more rows then kit assembly rows here
## I think 'undeliverable', 'no status', 'inititalized', 'address printed', and 'assigned' don't show up in that table
...
status_elg_knB <- status_elgB %>%
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 7.1b:  Baseline- Home Mouthwash Replacement Kit 1 \n Status Among Eligible Participants") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) %>%  kable_styling(latex_options = "scale_down")
...
status_elg_knC <- status_elgC %>%
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 7.1c:  Baseline- Home Mouthwash Replacement Kit 2 \n Status Among Eligible Participants") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) %>%  kable_styling(latex_options = "scale_down")
...
summary_table2_all<- filtered_data %>%
  filter(kit_level=="Initial Kit") %>%
  group_by(Site) %>%
  summarise(Shipped = sum(Kit_Status_int %in% c("Received", "Shipped")),
...
all_shipped_2_filtered <- all_shipped_2 %>%
  ...
  mutate(Site = gsub("Kaiser Permanente", "KP", Site),
         Site = gsub("University of ", "U", Site))
...
test30 <- Coll_Card_Table %>%  filter(!is.na(d_143615646_d_826941471) & !is.na(d_173836415_d_266600170_d_319972665_d_661940160))
 
test30 <-  test30 %>%
  mutate(ctime = as.numeric(difftime(as.POSIXct(ymd_hms(d_143615646_d_826941471)),as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_661940160)),units="days"))) %>% 
  filter(ctime>0)
...
## Here we want ALL kits shipped, so using the CXA collections
...
##Received
kit_set_col_1 <- kit_set_clean %>% select(Connect_ID, d_143615646_d_826941471)
...
#renaming so I can select a subgroup in the next chunk
...
##Received
kit_set_col_2 <- kit_set_clean %>% select(Connect_ID, d_143615646_d_826941471)
 
kit_set_col_2$col.Sunday.week <- ceiling(as.numeric(difftime(kit_set_col_2$d_143615646_d_826941471,as.Date(veristart.date-days(2)),units="days"))/7)
...
kit_table_int <- kit_table %>%  filter(Kit_Status_int=="Received")
 
kit_fig_2 <- inner_join(kit_set_col_2,kit_table_int,   by="Connect_ID")
 
 
fig5_new_2 <- kit_fig_2 %>%
  filter(d_173836415_d_266600170_d_319972665_d_379252329 == 976461859 & as.Date(col.Monday.date) >= "2024-03-29" & as.Date(col.Monday.date) < currentDate)
...
add_footnote(status_elg_knB, "Note: “Eligible participants” are those who had a clinical collection with blood or urine collected on or after March 29, 2024, and did not refuse, withdraw, or pass away prior to April 2024. Home mouthwash collections were launched for eligible participants in April 2024. Home mouthwash collections for backlog collections (before April 2024) will be launched at a later date.", notation = "none")
```
...
survey_cc_int <-  survey_cc_int %>%
  mutate(ctime = as.numeric(difftime(as.POSIXct(ymd_hms(d_195145666)),as.POSIXct(ymd_hms(d_173836415_d_266600170_d_448660695)),units="days"))) %>% 
  filter(ctime>0)
...
Table7.10a = survey_cc_int %>% group_by(Site) %>% 
>>>>>>> fa3f357027ec066b91f66bc9516e14c63b58d38c
  dplyr::summarize(N=n(),
                   Min = min(ctime),
                   Q1 = quantile(ctime, 0.25),
                   Median = median(ctime),
...
Table7_10a <- knitr::kable(Table7.10a,
                           caption='Table 7.10a Baseline- Mouthwash Biospecimen Survey Completion Time (in Days) Relative to Collection Time \n Reported on Collection Card for Initial Kit (Post update to  survey introduction)',
...
survey_cc_r1 <- kits_hmw %>%
  filter(!is.na(d_195145666) & !is.na(d_173836415_d_266600170_d_448660695) &
           Kit_Status_r1=="Received"  & kit_level=="Replacement Kit 1"  &
           as.Date(d_195145666) > "2025-05-01") #& d_137401245==353358909
 
survey_cc_r1 <-  survey_cc_r1 %>%
  mutate(ctime = as.numeric(difftime(as.POSIXct(ymd_hms(d_195145666)),as.POSIXct(ymd_hms(d_173836415_d_266600170_d_448660695)),units="days"))) %>% 
  filter(ctime>0)
 
Table7.10b = survey_cc_r1 %>% group_by(Site) %>%
  dplyr::summarize(N=n(),
                   Min = min(ctime),
                   Q1 = quantile(ctime, 0.25),
                   Median = median(ctime),
...
Table7_10b <- knitr::kable(Table7.10b,
                         caption='Table 7.10b Baseline- Mouthwash Biospecimen Survey Completion Time (in Days) Relative to Collection Time \n Reported on Collection Card for Replacement Kit 1 ',
                         row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE) %>%
  kable_styling(latex_options = "HOLD_position")
add_footnote(Table7_10b, "Note: Only includes data from mouthwash kits returned with completed Collection Card.", notation = "none")

...
tb_bq <- eval(parse(text=paste("bq_project_query(project, query=\"SELECT token,Connect_ID,d_512820379,d_821247024,d_914594314,d_827220437,d_699625233, d_265193023, d_822499427, 
                               ...
                               d_949302066, d_173836415_d_266600170_d_319972665_d_221592017, d_173836415_d_266600170_d_541483796_d_221592017, d_173836415_d_266600170_d_641006239_d_221592017,
...
coll_sett_all <- coll_sett_all %>%  
  mutate(col.Monday.date = floor_date(ymd_hms(d_556788178), unit = "week", week_start = 1)) %>% # week of collection
  ungroup()
...
col_num_1.1 <- col_num_1.1 %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` %in% c("Lake Hallie", "Marshfield", "MF Pop-Up", "Minocqua", "Weston", "Wisconsin Rapids", "Rice Lake", 
                                 "Colby Abbotsford", "Merrill", "Stevens Point", "Neillsville", 
                                 "Missing MF location") ~ "Marshfield Clinic Health System",
    ...
    `Collection Location` %in% c("HFH K-13 Research Clinic", "HFH Livonia Research Clinic", "HFH Cancer Pavilion Research Clinic", 
                                 "HFH Pop-Up", "HFH Detroit Northwest","In-home Collection", "HFH Jackson",
                                 "Missing HFH location") ~ "Henry Ford Health",
    `Collection Location` %in% c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving","Irving","NTX Biorepository",
                                 "BCC- Fort Worth", "North Garland", "Temple CDM", "Temple Roney", "Waco - MacArthur","Temple Westfield",
...
#---- Combine stacked crosstabulations for Race, Sex, and Ethnicity -------
...
coll_sett_all_m1 <- coll_sett_all_m1 %>% 
  # Need 'yes' values to be the same for the pivot longer
  mutate(rsch_coll = case_when(all_settings=="Research" ~ 1),
         clin_coll = case_when(all_settings=="Clinical" ~ 1),
         hmw_coll = case_when(d_173836415_d_266600170_d_319972665_d_221592017 == '375535639' |
  ...
  pivot_longer(
    cols = c(
      rsch_coll, # initial received
      clin_coll, # repl1 received
      hmw_coll  # repl2 received
    ),
    names_to = "sett",
    values_to = "received"
  ) %>%
  filter(received==1) 
  #mutate(sett = factor(sett, levels=c("rsch_coll", "clin_coll", "hmw_coll")))
                          # First collection in reearch setting
  # mutate(sett = case_when(all_settings=="Research" ~ "Rsch_m1",
  #                         # First collection in clinical setting
  #                         all_settings=="Clinical" ~ "Cl_m1",
  #                         #HMW Kit Received
  #                           # Initial, Replacement Kit1, or Replacement Kit2 Rec'vd
  #                         d_173836415_d_266600170_d_319972665_d_221592017 == '375535639' |
  #                           d_173836415_d_266600170_d_541483796_d_221592017 == '375535639' |
  #                         d_173836415_d_266600170_d_641006239_d_221592017 == '375535639'  ~ "H_m1"),
  #        sett = factor(sett, levels=c("Rsch_m1", "Cl_m1", "H_m1")))
...
research.survey <- coll_sett_all %>% filter(all_settings == "Research") %>%
  mutate(Site=droplevels(as.factor(Site)),
         ctime = round(as.numeric(difftime(ymd_hms(d_222161762),ymd_hms(d_822499427),units="mins")), digits=1),
         tm_biocolsuv = difftime(ymd_hms(d_222161762),ymd_hms(d_331584571_d_266600170_d_840048338),units="hours"), #time b/w survey completion to bio collection visit check in
...
coll_bptl_diff <- biospe__all %>%  
  ...
  mutate(Site_location = factor(Site_location,
                                levels=c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving",
                                         ...
                                         "Southwest Family Medicine","Grapevine","Plano Heart","Missing BSWH location",
                                         "HP Research Clinic", "HP Park Nicollet", "Missing HP location",
                                         "HFH K-13 Research Clinic","HFH Cancer Pavilion Research Clinic",
                                         "HFH Livonia Research Clinic", "HFH Pop-Up", "HFH Detroit Northwest","In-home Collection", 
                                         "HFH Jackson","Missing HFH location",
...
ctable_rsch_loc_bptl <- rsch_loc_coll_bptl %>%
  mutate(Site_Group = case_when(
    ...
    Collection_Location %in% c("Sioux Falls Imagenetics", "Fargo South University", "Edith Sanford Breast Center (coded as Sanford Center)",
                                  "Edith Sanford Breast Center","Fargo Amber Valley","Bemidji Clinic",
                               "Missing SF location") ~ "Sanford Health",
    Collection_Location %in% c("HFH K-13 Research Clinic", "HFH Livonia Research Clinic", "HFH Cancer Pavilion Research Clinic", 
                               "HFH Pop-Up", "HFH Detroit Northwest", "In-home Collection","HFH Jackson",
                               "Missing HFH location") ~ "Henry Ford Health",
    Collection_Location %in% c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving","Irving","NTX Biorepository",
                               "BCC- Fort Worth", "North Garland", "Temple CDM","Temple Roney", "Waco - MacArthur", "Temple Westfield",
                               "Waco Fishpond","Killeen Main","Southwest Family Medicine","Grapevine","Plano Heart",
                               "Missing BSWH location") ~ "Baylor Scott and White Health",
...
BU_coll_DB_comp = clinical.survey %>% group_by(Site) %>% 
  filter(coll_to_DB_BU >= 0) %>% #request to exclude negative values
...
avg_draw_order <- clinical.survey %>%  filter(Site!='HealthPartners' & Site!="Sanford Health" & Site!='University of Chicago Medicine') %>% 
  mutate(time = as.numeric(difftime(
    #Blood or urine, whichever occured first
    pmin(ymd_hms(d_173836415_d_266600170_d_982213346), ymd_hms(d_173836415_d_266600170_d_139245758)), 
    #and BldUrnPlcdTmBL
    ymd_hms(d_173836415_d_266600170_d_184451682),
    units="days"))) %>%
  group_by(Site) %>% 
...
baseline_colct <- coll_sett_all %>%
  ...
  mutate(time = as.numeric(difftime(as.Date(d_556788178),as.Date(d_914594314),units="days")))
...
all_coll_to_sub <- coll_sett_all %>%  
  ...
  mutate(collection = ifelse(d_265193023 == 'Submitted', ymd_hms(d_678166505), ymd_hms(d_556788178)),
         surv_sub = ifelse(d_265193023 == 'Submitted', ymd_hms(d_222161762), ymd_hms(d_764863765)),
         coll_to_sub = as.numeric(round(difftime(surv_sub, collection, units="days"), digits=0)),
...
avg_bldorder_present <- table1 %>%  filter(d_878865966=="No blood collected" & !is.na(d_173836415_d_266600170_d_769615780) & 
  ...
  mutate(time = as.numeric(difftime(currentDate,ymd_hms(d_173836415_d_266600170_d_769615780),units="days"))) %>% 
  group_by(Site) %>% 
...
coll_bptl_diff <- coll_bptl_diff %>% 
  mutate(hrs_past = difftime(ymd_hms(any_collection), ymd_hms(d_678166505), units="hours"),
  ...
  theme(
    ...
    plt <- ggplot(weekly_counts, aes(x = as.Date(col.Monday.date), y = n, color = .data[[color_var]])) +
      scale_color_manual(values=c("NSC"="#99C0C4", "Elk River"="#565C65", "Chanhassen"="#CC7D15", 
                                  "Park Nicollet Minneapolis Clinic"="#FDBE19",
...
d_784810139, d_389890053, d_843688458, d_543379310, d_100767870, 
d_517311251, d_832139544, d_770257102, d_264644252, d_199471989,
...
d_173836415_d_266600170_d_319972665_d_221592017, d_173836415_d_266600170_d_319972665_d_278539982,
...
test30 <-  test30 %>%
  mutate(ctime = as.numeric(difftime(ymd_hms(d_173836415_d_266600170_d_319972665_d_826941471),ymd_hms(d_173836415_d_266600170_d_319972665_d_661940160),units="days"))) %>% 
  filter(ctime>0)
...
test31 <-  test31 %>%
  mutate(ctime = as.numeric(difftime(ymd_hms(d_173836415_d_266600170_d_319972665_d_661940160),
                                     ymd_hms(d_556788178),units="days")))
...
received_long <- kit_table %>%
  ...
  mutate(
    col.Monday.date = floor_date(ymd_hms(received_date), unit = "week", week_start = 1)
  ) %>%
  filter(
...
shipped_long <- kit_table %>%
  ...
  mutate(
    col.Monday.date = floor_date(ymd_hms(shipped_date), unit = "week", week_start = 1)
  ) %>%
  filter(
    d_173836415_d_266600170_d_319972665_d_379252329 == 976461859,
    col.Monday.date >= as.Date("2024-03-29"),
...
kit_set_col_2 <- kit_table %>% filter(Kit_Status_int=="Received") %>% 
  mutate(col.Monday.date = floor_date(ymd_hms(d_173836415_d_266600170_d_319972665_d_826941471), unit = "week", week_start = 1))
...
kit_table2 <- kit_table %>%  filter(!is.na(d_173836415_d_266600170_d_319972665_d_661940160)) %>% 
  mutate(col.Monday.date = floor_date(ymd_hms(d_173836415_d_266600170_d_319972665_d_661940160), unit = "week", week_start = 1))
...
generate_mw_table <- function(comparison) {
  kit_table %>% 
    ...
    mutate(MW_completion = difftime(as.Date(d_173836415_d_266600170_d_448660695), as.Date(d_195145666), units = "days"),
...
survey_cc_int <- kit_table %>%  
  ...
  mutate(ctime = round(as.numeric(difftime(ymd_hms(d_195145666),
                                           ymd_hms(d_173836415_d_266600170_d_448660695),units="days")), 
                       digits=0),
...
kit_request_table <- kit_request_table %>% 
  mutate(tm_don_inv_years = as.numeric(difftime(ymd_hms(d_173836415_d_266600170_d_319972665_d_278539982),
                                        ymd_hms(d_173836415_d_266600170_d_740582332),units="days")),
...
kit_tm_don_inv <- kit_request_table %>% 
  filter(!is.na(d_173836415_d_266600170_d_319972665_d_759651991)) %>% 
  mutate(tm_don_inv_diff = as.numeric(difftime(ymd_hms(d_173836415_d_266600170_d_319972665_d_759651991),
                                              ymd_hms(d_173836415_d_266600170_d_319972665_d_278539982),units="days")),
...
kit_tm_inv_recvd <- kit_request_table %>% 
  ...
  mutate(tm_inv_recvd_weeks = as.numeric(difftime(ymd_hms(d_173836415_d_266600170_d_319972665_d_826941471),
                                              ymd_hms(d_173836415_d_266600170_d_319972665_d_278539982),units="weeks")),
...
kit_request_table <- kit_request_table %>% 
  mutate(last_survey = pmax(ymd_hms(d_784810139), #COVID completion time
                            ymd_hms(d_389890053), #CSH completion time
                            ymd_hms(d_843688458), #PROMIS completion time
                            ymd_hms(d_543379310), #ROI completion time
                            ymd_hms(d_517311251), #BOH completion time
                            ymd_hms(d_832139544), #MRE completion time
                            ymd_hms(d_770257102), #SAS completion time
                            ymd_hms(d_264644252), #LAW completion time
                            ymd_hms(d_199471989), na.rm=T), #CES completion time
         tm_surv_daymonths = as.numeric(difftime(ymd_hms(d_173836415_d_266600170_d_319972665_d_278539982),

...
where Connect_ID IS NOT NULL and d_906417725='104430631'  and d_747006172='104430631' and d_987563196='104430631' and 
...
kit_table$d_173836415_d_266600170_d_319972665_d_687158491 <- as.numeric(kit_table$d_173836415_d_266600170_d_319972665_d_687158491)
...
### Table that look at these look at initial kits only
#### Participants table KitAssembledIDs and ConnectID match those in the KitAssembly Table
kitA_bq <- "SELECT Connect_ID,
...
d_633640710_d_950521660, d_633640710_d_545319575, d_633640710_d_938338155, d_633640710_d_205954477, d_633640710_d_289239334,d_633640710_d_992420392, d_633640710_d_541085383,d_633640710_d_427719697, d_633640710_d_100618603,
 FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.kitAssembly_JP` 
 where Connect_ID is not null and d_426588510='663273321'"
...
joined_kit_table <- left_join(kit_table, kit_assembly, by="Connect_ID")
...
Coll_Card_Table <- joined_kit_table %>%
...
joined_kit_table <- joined_kit_table %>%
  mutate(
    multi_cond = rowSums(across(all_of(condition_cols), ~ .x == 353358909, .names = NULL), na.rm = TRUE)
  )
 
table_32 <-joined_kit_table %>% filter(Kit_Status_int=="Received" ) %>% 

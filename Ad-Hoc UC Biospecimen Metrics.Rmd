...
# 
# 
# biospeUC<- expss::apply_labels(biospe,d_951355211 = "Site_location",#RcrtES_Site_v1r0
#                      d_827220437 = c("UC-DCAM"= 777644826, "Ingalls Harvey"=548392715,"River East" = 303349821, "South Loop" = 657167265, "UCM Pop-Up" = 809703864))
# 
# biospeUC$Site_location <- factor(biospeUC$d_951355211, levels= c("UC-DCAM", "Ingalls Harvey","River East", "South Loop", "UCM Pop-Up"))
# 
# biospeUC$Site_location<- droplevels(biospeUC$Site_location)

biospe_UC <- biospe %>%  filter(Site=="University of Chicago Medicine")

biospeUC<-biospe_UC %>% mutate(Site_location = case_when(d_951355211==777644826 ~ "UC-DCAM",
...
biospeUC <- biospeUC %>%  filter(Site_location =="UC-DCAM" | Site_location == "Ingalls Harvey"| Site_location == "River East"| Site_location == "South Loop"| Site_location == "UCM Pop-Up")

b6 <- biospeUC %>%  select(Connect_ID, Site_location, d_650516960, d_410912345)
...
#table4_redone__R$Site_location <- droplevels(table4_redone__R$Site_location)
table4_redone__R$'Specimens Collected' <- droplevels(table4_redone__R$'Specimens Collected')
coll_completenessR <- table4_redone__R %>% dplyr::select('Specimens Collected', Site_location )  %>% 
  tbl_cross(
    row =  Site_location,
    col = 'Specimens Collected',
    digits=c(0,1),
    percent = "row",
    label=list(Site_location=" "),
    missing="ifany",
    margin_text="Total")
...
T8UC <- biospeUC %>%  select(Site_location, Site) ##MAY NOT WORK ADDING CONNECTID 
...
# tube_col_UC3$Site_location <- tube_col_UC3$UC_Sites 
# df3 <- left_join(tube_col_UC3, T8__UC, by='Site_location')
# tube_col_UC3 <- tube_col_UC %>% select(UC_Sites)
# 
...
All_UC3 <- biospeUC %>% select(Site_location, Connect_ID,  d_820476880,  d_299553921_d_825582494,  d_454453939_d_825582494, d_652357376_d_825582494, d_703954371_d_825582494,  d_838567176_d_825582494) #d_505347689_d_825582494,
tube_col_UC3$Site_location <- tube_col_UC3$UC_Sites 
df3 <- left_join(tube_col_UC3, All_UC3, by=c("Connect_ID", 'Site_location'))
...
#df4 <- left_join(df3, All_UC3, by=c("Connect_ID", 'Site_location'))    


tbl_3 <- df3 %>%  filter(bldcol_max!=5 & bldcol_max!=0)
tbl_3_ncol <- tbl_3 %>% dplyr::select(Site_location, tube_type )  %>% tbl_cross(
  row = Site_location,
  col = tube_type, 
  digits=c(0,1),
  percent = "row",
  label=list(Site_location ~ " ", tube_type ~ " "),
  # label=list(Site_location,="",
  #            tube_type = ""),
  missing="ifany") %>%
  #bold_labels() %>%
...
# ctable <-  CrossTable(T8__UC$Site_location, T8__UC$tube_type)
# 
# 
...
#   
#   notcol_combo$Site_location <- rownames(count2)
# 
#   #notcol_combo[6,c(1:8)] <- as.character(apply(count,2,sum))
# notcol_combo[6,c(1:6)] <- as.character(apply(count2,2,sum))
#   notcol_combo$Site_location[is.na(notcol_combo$Site_location)] <- replace_na("Total") 
...
#research.notcol$Site_location <- droplevels(research.notcol$Site_location)
...
T12UC <- T12UC %>%  filter(!is.na(Site_location))

research.notcolnotes_site <- T12UC %>% select(notcol.notes,Site_location) %>% 
  tbl_cross(  col = notcol.notes,
                  row = Site_location,
                   percent = "row",
                   digits= c(0,1),
                   label= c(notcol.notes~ " ", Site_location ~ " "),
                   missing="ifany",
                   missing_text="0 ( 0 %)")%>%
     modify_header() %>%
...
discard.tb <- T12__16  %>% filter(tube_discard == 353358909) %>% 
  mutate(#Site_location=droplevels(Site_location),
...
All_UC5 <- biospeUC %>% select(Site, Site_location, Connect_ID,  d_820476880,  d_299553921_d_825582494,  d_454453939_d_825582494, d_652357376_d_825582494, d_703954371_d_825582494,  d_838567176_d_825582494, d_820476880, d_589588440_d_825582494, d_958646668_d_825582494, d_973670172_d_825582494) #d_505347689_d_825582494,
...
#17- by site
discard.site5 <- df5_2 %>% dplyr::select(Site_location, tube_type )  %>% tbl_cross(
    row = Site_location,
    col = tube_type, 
    digits=c(0,1),
    percent = "row",
...
T8UC <- biospeUC %>%  select(Site_location, Site, Connect_ID)  
...
Res_Survey_status <- T14_UC %>% dplyr::select(Site_location, research.sv )  %>% tbl_cross(
  row = Site_location,
  col = research.sv, 
  digits=c(0,1),
  percent = "row",
  label=list(Site_location ~ " ", research.sv ~ " "),
  # label=list(Site_location,="",
  #            research.sv = ""),
  missing="ifany") %>%
  #bold_labels() %>%
...
Table7A_combinedB <- testing7b %>% dplyr::select(Site_location, checktmvisit )  %>% tbl_cross(
    row = Site_location,
    col = checktmvisit,
    digits=c(0,2),
    percent = "row",
    label=list(Site_location ~ " ", checktmvisit ~ " "),
    missing="ifany") %>%
    #bold_labels() %>%
    #italicize_levels() %>% 
    modify_header() %>%
...
Table7B_combinedA <- testing7a %>% dplyr::select(Site_location, checktmvisit )  %>% tbl_cross(
    row = Site_location,
    col = checktmvisit,
    digits=c(0,2),
    percent = "row",
...
# T8UC <- biospeUC %>%  select(Site_location, Site)  
...
# 
# test21rb$Site_location <- droplevels(test21rb$Site_location)
# test21ra$Site_location <- droplevels(test21ra$Site_location)
...
Table8_Before = test21r__b %>% group_by(Site_location) %>% 
...
Table8_After = test21r__a %>% group_by(Site_location) %>% 
  dplyr::summarize(#'Number of Collection Visits'=n(),
                   Min = min(ctime),
                   Q1 = quantile(ctime, 0.25),
                   Median = median(ctime),
...
research.vt.length9 = research.vt.length %>%group_by(Site_location) %>% #biochk_outlier,
  dplyr::summarize(#'Number of Collection Visits'=n(),
...
discard.site5 %>% kable_styling(latex_options = "scale_down")

# knitr::kable(discard.site5, caption='Table 5. Research Tube Types Discarded by Collection Location', row.names=FALSE,  
#              #col.names=c("Site",  "SS Tube1", "SS Tube2", "SS Tube5", "Heparin Tube1", "Heparin Tube2", "EDTA Tubes", "ACD Tubes", "Urine Tubes", "Total Tubes"),
#              align=c("l","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")

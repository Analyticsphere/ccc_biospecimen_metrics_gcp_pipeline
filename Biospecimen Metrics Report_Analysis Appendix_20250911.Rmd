...
col_num_1.2 <- col_num_1.2 %>%
  mutate(Site_Group = case_when(
    `Collection Location` %in% c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", "K1 HFH Main",
                                 ...
                                 "Woodhaven Pathology", "CANPAV Pathology OP Lab", 
...
setting_ctabs <- coll_sett_all %>%  
  mutate(settings = factor(all_settings, levels=c("Clinical", "Research", "Home")),
         BL_BioCol_BloodSetting_v1r0 = coalesce(BL_BioCol_BloodSetting_v1r0, settings),
         BL_BioCol_UrineSetting_v1r0 = coalesce(BL_BioCol_UrineSetting_v1r0, settings),
         BL_BioSpm_MWSetting_v1r0 = coalesce(BL_BioSpm_MWSetting_v1r0, settings))


all_blood_settings <- as.data.frame(table(setting_ctabs$BL_BioCol_BloodSetting_v1r0[setting_ctabs$d_878865966 == "Any blood collected"], exclude = NULL))
allurine_settings <- as.data.frame(table(setting_ctabs$BL_BioCol_UrineSetting_v1r0[setting_ctabs$d_167958071 == "Any urine collected"], exclude = NULL))
...
all_blood_settings_Site <- as.data.frame(table(setting_ctabs_Site$BL_BioCol_BloodSetting_v1r0[setting_ctabs_Site$d_878865966 == "Any blood collected"], exclude = NULL))
allurine_settings_Site <- as.data.frame(table(setting_ctabs_Site$BL_BioCol_UrineSetting_v1r0[setting_ctabs_Site$d_167958071 == "Any urine collected"], exclude = NULL))
...
tube_dev <- tube_dev %>% 
  dplyr::rename(Site = Group.1, Any_Deviation = colnames(tube_dev)[2])
...
tube_nodev <- tube_nodev %>% 
  dplyr::rename(Site = Group.1, No_Deviation = colnames(tube_nodev)[2]) #sometimes this switches from d_299553921_d_593843561 to d_958646668_d_593843561
...
coll_bptl_diff <- biospe_adhoc_all %>%  
  ...
  mutate(Site_location = factor(Site_location,
                                levels=c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving",
                                         ...
                                         "Temple Roney", "Waco - MacArthur","Temple Westfield","Missing BSWH location",
...
# 2) Subtotal the N’s within each Site_Group
...
subtotals_fmt <- subtotals_count %>%
    ...
    dplyr::mutate(
        ...
        Collection_Location = "Total"              # label for the within-site total row
    ) %>%
    dplyr::select(Collection_Location, dplyr::all_of(value_cols), Site_Group)
...
rsch_coll_times <- dplyr::bind_rows(rsch_coll_times, subtotals_fmt) %>%
    dplyr::arrange(Site_Group, (Collection_Location == "Total"), Collection_Location)
...
Section6_plot_function2 <- function(data, start_date, title, site, settings, color_var) {
  ...
  if (site == "Henry Ford Health" && settings=="Research"){
    ...
    scale_color_manual(values=c("Wyandotte"="#B1C7D9", "Macomb"="#164C71", "West Bloomfield"="#FDBE19", "Royal Oak"="#F8D991", 
                              ...
                              "Missing Both Location IDs" = "black")) + 
...
generate_mw_table <- function(comparison) {
  kit_table %>% 
    filter(d_547363263 == '231311385' &  (Kit_Status_r2 == "Received" | Kit_Status_r1 == "Received" | Kit_Status_int == "Received") &
...
coll_sett_all_m1 <- coll_sett_all_m1 %>%  
  #filter(BiospeCollection == "Any biospecimen Collections" & (d_410912345== 353358909 | BL_BioSpm_MWSetting_v1r0=="Home")) %>% 
    mutate(all_settings = case_when(BiospeCollection == "Any biospecimen Collections" & d_410912345== 353358909 & 
                                      (d_650516960=="664882224" | BL_BioCol_BloodSetting_v1r0=="Clinical" | 
                                         BL_BioCol_UrineSetting_v1r0=="Clinical") ~ "Clinical",
...
collection_bysex <- knitr::kable(BLCol_by_Sex_df, 
  ...
  kable_styling(latex_options = "scale_down") %>%  
  add_indent(seq(1, nrow(BLCol_by_Sex_df) - 1)) %>% 
  kableExtra::footnote(general="Note: This table was taken directly from the Biospecimen Weekly Metrics Report: Table 1.13. Sex defined as the de-identified sex data reported by sites at time of invitation.",
           general_title = "",
           footnote_as_chunk = TRUE,
           escape = FALSE,
           threeparttable = TRUE)
...
coll_sett_all_NR <- table1 %>%  
  filter(BiospeCollection == "Any biospecimen Collections" & (d_410912345== 353358909 | BL_BioSpm_MWSetting_v1r0=="Home")) %>% 
  mutate(all_settings = case_when((d_650516960=="534621077" | BL_BioCol_BloodSetting_v1r0=="Research Only" | 
                                    BL_BioCol_UrineSetting_v1r0=="Research" | BL_BioSpm_MWSetting_v1r0=="Research")~ "Research",
                                  
                                  (d_650516960=="664882224" | BL_BioCol_BloodSetting_v1r0=="Clinical" | 
                                    BL_BioCol_UrineSetting_v1r0=="Clinical") & d_684635302=="No mouthwash collected" ~ "Clinical Only", #exclude HMW
                                  
                                  (d_650516960=="103209024" | BL_BioCol_BloodSetting_v1r0=="Home" | 
                                    BL_BioCol_UrineSetting_v1r0=="Home" | BL_BioSpm_MWSetting_v1r0=="Home") &
                                    d_878865966=="No blood collected" & d_167958071=="No urine collected" ~ "Home MW Only", #exclude blood or urine
                                  
                                  (d_650516960=="103209024" | BL_BioCol_BloodSetting_v1r0=="Home" | 
                                    BL_BioCol_UrineSetting_v1r0=="Home" | BL_BioSpm_MWSetting_v1r0=="Home") ~ "Clinical + Home")) #else clinical & home
...
kit_table_merged <- kit_table %>%
  ...
  left_join(
    clinical.survey.m1 %>%
      ...
      dplyr::rename(multi_race = m1_race),
    by = "Connect_ID"
  )

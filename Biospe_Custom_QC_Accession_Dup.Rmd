...
# d_928693120 FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.biospecimen_JP`  where Connect_ID IS NOT NULL"
...
#Blood
qc1_blood <- qc1 %>%  filter(str_sub(d_173836415_d_266600170_d_341570479,1,11)!=str_sub(d_646899796,1,11)) 

qc1_blood <- qc1_blood %>% select(Site, Connect_ID, d_173836415_d_266600170_d_341570479, d_646899796 ) %>%  sort_by(Site)
...
#Urine
qc1_urine <- qc1 %>%  filter(str_sub(qc1$d_173836415_d_266600170_d_198261154,1,11)!=str_sub(qc1$d_928693120,1,11)) 

qc1_urine$AID<- strtoi(qc1_urine$d_928693120, base = 0L)

qc1_urine <- qc1_urine %>% select(Site, Connect_ID, d_173836415_d_266600170_d_198261154, d_928693120 ) %>%  sort_by(Site)
...
qcKP_blood <- qcKP %>%  filter(d_173836415_d_266600170_d_341570479!=d_646899796)  %>% select(Site, Connect_ID, d_173836415_d_266600170_d_341570479, d_646899796 ) %>%  sort_by(Site)
...
qcKP_urine <- qcKP %>%  filter(d_173836415_d_266600170_d_198261154!=d_928693120) %>% select(Site, Connect_ID, d_173836415_d_266600170_d_198261154, d_928693120 ) %>%  sort_by(Site)
...
qc12 <- bioqc %>%  filter(Connect_ID!=5667758818 & d_973670172_d_593843561==353358909  & d_650516960==664882224 & as.numeric(round(difftime(currentDate, d_915838974, units="days"), digits=0)) >7 & is.na(d_173836415_d_266600170_d_786930107)) %>% 
  select(Site, Connect_ID, d_173836415_d_266600170_d_786930107) %>%  sort_by(Site)
...
### 18. IF BioClin_BldOrderPlaced_v1r0 = 1, THEN BioClin_BldOrderPlacdDt_v1r0 must be populated. HP and SF Excluded
...
### 19. IF BioClin_UrnOrdPlaced = 1, THEN BioClin_BldOrderPlacdDt_v1r0 must be populated. HP and SF Excluded
...
bioqc1 <- bioqc %>%  filter(d_173836415_d_266600170_d_543608829!="[]" & !is.na(d_646899796) &
...
bioqc1 <- bioqc1 %>%  #filter(d_173836415_d_266600170_d_543608829!="[]" & !is.na(d_646899796)) %>%  
mutate(BioClin_DBBloodID_v1r0=paste0("L", d_646899796))
...
bioqc2 <- bioqc %>%  filter(d_173836415_d_266600170_d_110349197!="[]" & !is.na(d_928693120) &
...
bioqc2 <- bioqc2 %>%  
mutate(BioClin_DBUrineID_v1r0=paste0("L", d_928693120))
...
```{r pending, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r all_populate, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r HC_Rules1, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r HC_Rules2, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r assigned, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r shipped, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r recvd, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r collected, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r mwtubeid, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r HC_Rules3, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r HC_Rules4, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r homsetting, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r bl_mw, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}
...
```{r mw_dt, echo=FALSE, warning=FALSE, message=FALSE, include=FALSE}

...
qc8 <- bioqc %>%  filter(d_878865966=="Yes" & d_167958071=="Yes" & d_684635302=="Yes" & 
  ...
  select(Site, Connect_ID, d_254109640) %>%  safe_arrange(Site)
...
qc13_tube5 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_703954371_d_593843561=="Yes" & 
                                  d_703954371_d_762124027=="No" & is.na(d_703954371_d_926457119)
                                & Connect_ID!="7609852429") %>% 
...
qc13_tube_sst3 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_376960806_d_593843561=="Yes" & 
                                    d_376960806_d_762124027=="No" & is.na(d_376960806_d_926457119) & 
...
qc13_tube_sst4 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_232343615_d_593843561=="Yes" & 
                                      d_232343615_d_762124027=="No" & is.na(d_232343615_d_926457119) & 
                                      Connect_ID!="9729007313" & Connect_ID!="7609852429" & Connect_ID!="6380609394") %>% 
...
qc13_tube_sst5 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_589588440_d_593843561=="Yes" & 
                                      d_589588440_d_762124027=="No" & is.na(d_589588440_d_926457119) & 
...
qc13_tube3 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_454453939_d_593843561=="Yes" & 
                                  d_454453939_d_762124027=="No" & is.na(d_454453939_d_926457119) & 
...
qc13_tube_edat2 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_677469051_d_593843561=="Yes" & 
                                       d_677469051_d_762124027=="No" & is.na(d_677469051_d_926457119) & 
                                       Connect_ID!="9729007313" & Connect_ID!="7609852429") %>% 
...
qc13_tube_edat3 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_683613884_d_593843561=="Yes" & 
                                       d_683613884_d_762124027=="No" & is.na(d_683613884_d_926457119) & 
                                       Connect_ID!="9729007313" & Connect_ID!="7609852429") %>% 
...
qc13_tube4 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_652357376_d_593843561=="Yes" & 
                                  d_652357376_d_762124027=="No" & is.na(d_652357376_d_926457119) & d_827220437==548392715 & d_650516960=="Clinical") %>% # | d_827220437==657167265 once Sanford is hybrid
...
qc13_tube6 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_838567176_d_593843561=="Yes" & d_838567176_d_762124027=="No" &  is.na(d_838567176_d_926457119) & Connect_ID!="7609852429") %>% 
...
qc13_tube8 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_958646668_d_593843561=="Yes" & d_958646668_d_762124027=="No" &  is.na(d_958646668_d_926457119) & Connect_ID!="7609852429") %>% 
...
qc13_Rtube7 <- bioqcR %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_561681068, units="days"), digits=0))>4 &  d_973670172_d_593843561=="Yes" & d_973670172_d_762124027=="No" & is.na(d_973670172_d_926457119) & !(Connect_ID %in% c(9145733933,9618974099,8527264977,6473532641,4174960021, 4435276749, 1834747483))) %>% #Sanford exclusion
...
qc13_Rtube8 <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0))>4 & d_505347689_d_593843561=="Yes" & d_505347689_d_762124027=="No" & is.na(d_505347689_d_926457119) & d_827220437=="548392715" & d_650516960=="Clinical") %>% # | d_827220437==657167265 once Sanford is hybrid
  select(Site, Connect_ID, d_505347689_d_926457119) %>%  safe_arrange(Site)
...
c_urine3.4 <- bioqc %>%  filter(!is.na(d_173836415_d_266600170_d_139245758) & 
...
bioqc1 <- bioqc %>%
  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0)) > 7 &
           ...
           !(Connect_ID %in% c("2824989966","3759939890" ,"4688823720" ,"8589481620" ,"6336258662","5461650381","4900116130",
             ...
             "4782195164", "6229346420", '1271036774','6288600449',# UC exclusions
...
Bio_HMW_merged <- Bio_HMW_merged %>% filter(d_650516960=='664882224') %>%  #remove research collections 
  mutate(kit_level= case_when(d_426588510 == 772116457 ~ "Replacement Kit 2",
                              ...
                              d_426588510 == 663273321 ~ "Initial Kit"))
...
pend <- HMW %>%  filter((d_173836415_d_266600170_d_319972665_d_221592017==517216441 | d_173836415_d_266600170_d_541483796_d_221592017==517216441 |
                           d_173836415_d_266600170_d_641006239_d_221592017==517216441) & is.na(d_341636034)) %>% 
  select(Site, Connect_ID, d_341636034) %>%  safe_arrange(Site)
...
asgn <- HMW %>%  filter((d_173836415_d_266600170_d_319972665_d_221592017==241974920 | d_173836415_d_266600170_d_541483796_d_221592017==241974920 | d_173836415_d_266600170_d_641006239_d_221592017==241974920) & 
...
##and kit level (BioKit_KitLevel_v1r0)
...
recvd <- Bio_HMW %>%  filter((d_173836415_d_266600170_d_319972665_d_221592017==375535639 & kit_level=="Initial Kit" & 
                               ...
                               (d_173836415_d_266600170_d_641006239_d_221592017==375535639 & kit_level=="Replacement Kit 2" & 
                                  (is.na(d_531858099) | is.na(d_661940160) | is.na(d_826941471) |  is.na(d_137401245) | is.na(d_426588510)))) %>%  
  select(Site, Connect_ID, kit_level, d_531858099, d_661940160, d_826941471, d_137401245) %>%  safe_arrange(Site)
...
hmw3 <- Bio_HMW %>%  filter(str_sub(d_820476880,1,3)=="CHA" & ((d_173836415_d_266600170_d_319972665_d_221592017==375535639 & d_173836415_d_266600170_d_319972665_d_379252329==976461859) | (d_173836415_d_266600170_d_541483796_d_221592017==375535639 & d_173836415_d_266600170_d_541483796_d_379252329==976461859) | (d_173836415_d_266600170_d_641006239_d_221592017==375535639 & d_173836415_d_266600170_d_641006239_d_379252329==976461859)) & d_143615646_d_825582494!=d_259846815) %>% 
  select(Site, Connect_ID, d_143615646_d_825582494) %>%  safe_arrange(Site)
...
homsetting <- HMW %>%  filter(((d_173836415_d_266600170_d_319972665_d_221592017==375535639 & d_173836415_d_266600170_d_319972665_d_379252329==976461859) | (d_173836415_d_266600170_d_541483796_d_221592017==375535639 & d_173836415_d_266600170_d_541483796_d_379252329==976461859) | (d_173836415_d_266600170_d_641006239_d_221592017==375535639 & d_173836415_d_266600170_d_641006239_d_379252329==976461859)) & d_173836415_d_266600170_d_915179629!=103209024) %>% 
...
different_values <- bio_dup_with_status %>%
  filter(d_410912345=="353358909" & 
           !(Connect_ID %in% c("2589415525", "4545815202", "4545815202", "2589415525", '9055246833', '4987045154'))) %>% #Biospecimen team has handled these ones
  group_by(Connect_ID) %>% 
...
r1_r2_undeliverable <- HMW %>%  filter(d_173836415_d_266600170_d_641006239_d_221592017=="332067457" & (d_426588510=="389478821" | d_426588510=="772116457")) %>% 
  select(Site, Connect_ID, d_173836415_d_266600170_d_641006239_d_221592017, d_426588510) %>%  safe_arrange(Site)


colnames(r1_r2_undeliverable) <- c("Site", "Connect ID", "R2_BioKit_KitStatus_v1r0", "BioKit_KitLevel_v1r0")
knitr::kable(r1_r2_undeliverable)
...
knitr::kable(issue212) %>% add_footnote("Excludes those where BioFin_BMTimeBL_v1r0 is in the same month OR the month on either side of BioKit_KitShipTm_v1r0 or BioKit_KitRecdTm_v1r0", notation= "none")
...
knitr::kable(Github262_spec_tubeU)

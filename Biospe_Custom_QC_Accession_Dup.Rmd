...
### Tube Date Received Clinical - If BioClin_DBBloodRRLDt_v1r0 occured more than 4 days ago, (tube type) was collected, and (tube type) was not discarded, then BioBPTL_DateRec_v1r0 for that tube type should be populated 
...
qc13_Rtube1 <- bioqcR %>%  filter( d_143615646_d_593843561==353358909 & d_143615646_d_762124027==104430631 & is.na(d_143615646_d_926457119)) %>%
...
qc13_Rtube2 <- bioqcR %>%  filter(d_299553921_d_593843561==353358909 & d_299553921_d_762124027==104430631 & is.na(d_299553921_d_926457119)) %>% 
  select(Site, Connect_ID, d_299553921_d_926457119) %>%  sort_by(Site)
...
qc13_Rtube3 <- bioqcR %>%  filter(d_454453939_d_593843561==353358909 & d_454453939_d_762124027==104430631 & is.na(d_454453939_d_926457119)) %>% 
  select(Site, Connect_ID, d_454453939_d_926457119) %>%  sort_by(Site)
...
qc13_Rtube4 <- bioqcR %>%  filter(d_652357376_d_593843561==353358909 & d_652357376_d_762124027==104430631 & is.na(d_652357376_d_926457119)) %>% # | d_827220437==657167265 once Sanford is hybrid
  select(Site, Connect_ID, d_652357376_d_926457119) %>%  sort_by(Site)
...
qc13_Rtube5 <- bioqcR %>%  filter(d_703954371_d_593843561==353358909 & d_703954371_d_762124027==104430631 & is.na(d_703954371_d_926457119)) %>% 
  select(Site, Connect_ID, d_703954371_d_926457119) %>%  sort_by(Site)
...
qc13_Rtube6 <- bioqcR %>%  filter(d_838567176_d_593843561==353358909 & d_838567176_d_762124027==104430631 &  is.na(d_838567176_d_926457119)) %>% 
  select(Site, Connect_ID, d_838567176_d_926457119) %>%  sort_by(Site)
...
qc13_Rtube8 <- bioqcR %>%  filter(d_958646668_d_593843561==353358909 & d_958646668_d_762124027==104430631 &  is.na(d_958646668_d_926457119)) %>% 
  select(Site, Connect_ID, d_958646668_d_926457119) %>%  sort_by(Site)
...
qc13_Rtube7 <- bioqcR %>%  filter( d_973670172_d_593843561==353358909 & d_973670172_d_762124027==104430631 & is.na(d_973670172_d_926457119)) %>% 
  select(Site, Connect_ID, d_973670172_d_926457119) %>%  sort_by(Site)
...
# remove and update from Jake's rule list
  #NA?

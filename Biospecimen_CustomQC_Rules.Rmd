...
## Home MW collections are separate, their collection IDs start with CHA and will never be finalized.
...
d_914594314, d_685002411_d_194410742, d_685002411_d_949501163, d_331584571_d_266600170_d_343048998
...
### 1. (Derived) Clinical DB Blood RRL Received (BioCol_DBBloodRRL_v1r0): If all blood tubes are not collected, this should be no
```{r BioCol_DBBloodRRL_v1r0, echo=FALSE, warning=FALSE, message=FALSE}
...
qc2_1a <- bioqc %>%  filter( d_973670172_d_593843561=="No"  & d_173836415_d_266600170_d_786930107=="Yes" &
                               !(Connect_ID %in% c("1140047316", "1215003341", "1250934825", "2300063524", "2039357566", "2755205973", 
                                                   ...
                                                   '4958431450', '9973370517', "9358513094", "7414720886", "4054300519", "9906761921", 
                                                   "7945071279", "8783197795", "1289559030", "6250053683", "3720410076", "8288718444", 
                                                   "5967357815", "3968179760", "2452097537", "8926083140", "1376492836", "8288718444",
                                                   "6554752349", "1291437931", "3508580541", "8130614446", "6966066624", "7304905537",
                                                   "2321589610", "2911866308", "9272279347", "4892006412","4753650657", "1474401708",
                                                   "7200773109", "6157056255", '5978022819', '7620372153', '4897113924', '7754715436',
                                                   '8062144301', '8003993386', '1020636422',
                                                   '4769462840','3526598284', '7176361975',
                                                   '5489793909', '6157114236')))

qc2_1a <- qc2_1a  %>% select(Site, Connect_ID, d_973670172_d_593843561, d_173836415_d_266600170_d_786930107)  %>%  safe_arrange(Site)


colnames(qc2_1a) <- c("Site", "Connect ID", "Urine Collected", "BioCol_SiteUrineCollBL_v1r0")
...
### 5. If BioCol_BldOrderPlacdDt_v1r0 occurred before BioCol_UrnOrderPlacdDt_v1r0, then BioCol_BldUrnPlacedTm_v1r0 should = BioCol_BldOrderPlacdDt_v1r0

```{r BioCol_BldUrnPlacedTm_1, echo=FALSE, warning=FALSE, message=FALSE}
...
### 6. If BioCol_BldOrderPlacdDt_v1r0 occurred after BioCol_UrnOrderPlacdDt_v1r0, then BioCol_BldUrnPlacedTm_v1r0 should = BioCol_UrnOrderPlacdDt_v1r0
```{r BioCol_BldUrnPlacedTm_2, echo=FALSE, warning=FALSE, message=FALSE}
...
###	7. If BioFin_BaseUrineCol_v1r0 was collected, BioCol_BloodSettingBL_v1r0 is clinical, and BioCol_DBUrineRRLDt_v1r0 occurred more than seven days ago, BioCol_SiteUrineColl_v1r0 must be yes
```{r BioCol_SiteUrineColl_v1r0, echo=FALSE, warning=FALSE, message=FALSE}
...
###	8. If BioFin_BaseBloodCol_v1r0 = yes, collection setting is clinical and BioCol_DBBloodRRLDtBL_v1r0 occurred more than 7 days ago, BioCol_SiteBloodColl_v1r0 must be yes
```{r BioCol_SiteBloodColl_v1r0, echo=FALSE, warning=FALSE, message=FALSE}
...
### 10. If BioFin_BaseBloodCol_v1r0 is yes and the collection setting is Research, and BioCol_ColTime_v1r0 occurred on or after 11/21/2022 then (Autogenerated) Date/time Baseline Research Blood Collected must be populated  
...
### 11. If BioFin_BaseUrineCol_v1r0 is yes and the collection setting is Research, and BioCol_ColTime_v1r0 occurred on or after 11/21/2022 then (Autogenerated) Date/time Baseline Research Urine Collected must be populated
...
### 13. If BioCol_ObjCollected_v1r0 (mouthwash), and BioCol_ColTime_v1r0 occurred on or after 11/21/2022, then  BioFin_BaseMouthCol_v1r0 should be no 
...
```{r BioCol_SiteUrineColl, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r BldOrder_NoDate, echo=FALSE, warning=FALSE, message=FALSE}
...
colnames(BloodDtTm_noHP) <- c("Site", "Connect ID", "BioCol_BldOrderPlacdDt")
...
```{r BldOrderDt_NoOrder, echo=FALSE, warning=FALSE, message=FALSE}

##Rule excluded for now

### IF  BioCol_BldOrderPlacdDt_v1r0 is populated, THEN BioCol_BldOrderPlaced_v1r0 = 1. HP Excluded
...
colnames(BloodDtTmVs_noHP) <- c("Site", "Connect ID", "BioCol_BldOrderPlaced")
...
```{r UrnOrder_NoDate, echo=FALSE, warning=FALSE, message=FALSE}
...
colnames(UrineDtTm_noHP) <- c("Site", "Connect ID", "BioCol_BldOrderPlacdDt")
...
```{r UrnOrderDt_NoOrder, echo=FALSE, warning=FALSE, message=FALSE}

##Rule excluded for now

### IF  BioCol_BldOrderPlacdDt_v1r0 is populated, THEN BioCol_UrnOrdPlaced must = 1. HP Excluded
...
colnames(UrineDtTmVs_noHP) <- c("Site", "Connect ID", "BioCol_UrnOrdPlaced")
...
### 17. Tube Date Received Clinical - If BioCol_DBBloodRRLDt_v1r0 occurred more than 4 days ago, (tube type) was collected, and (tube type) was not discarded, then BioBPTL_DateRec_v1r0 for that tube type should be populated 
...
qc_sst1 <- rule17 %>%  filter(d_299553921_d_593843561=="Yes" & d_299553921_d_762124027=="No" & is.na(d_299553921_d_926457119)
                                & !(Connect_ID %in% c("7609852429", "5487473343"))) %>% 
  select(Site, Connect_ID, d_299553921_d_926457119) %>%  safe_arrange(Site)

colnames(qc_sst1) <- c("Site", "Connect ID", "Serum Separator Tube 1- Date Received")
knitr::kable(qc_sst1, longtable = TRUE)


qc_sst2 <- rule17 %>%  filter(d_703954371_d_593843561=="Yes" & d_703954371_d_762124027=="No" & is.na(d_703954371_d_926457119)
                                & !(Connect_ID %in% c("7609852429", "5487473343"))) %>% 
  select(Site, Connect_ID, d_703954371_d_926457119) %>%  safe_arrange(Site)

colnames(qc_sst2) <- c("Site", "Connect ID", "Serum Separator Tube 2- Date Received")
knitr::kable(qc_sst2, longtable = TRUE)

qc_sst3 <- rule17 %>%  filter(d_376960806_d_593843561=="Yes" & d_376960806_d_762124027=="No" & is.na(d_376960806_d_926457119) & 
                                !(Connect_ID %in% c("7609852429", "9729007313", "6380609394", "5487473343"))) %>% 
  select(Site, Connect_ID, d_376960806_d_926457119) %>%  safe_arrange(Site)

colnames(qc_sst3) <- c("Site", "Connect ID", "Serum Separator Tube 3- Date Received")
knitr::kable(qc_sst3, longtable = TRUE)

qc_sst4 <- rule17 %>%  filter(d_232343615_d_593843561=="Yes" & d_232343615_d_762124027=="No" & is.na(d_232343615_d_926457119) & 
                                      !(Connect_ID %in% c("7609852429", "9729007313", "6380609394", "5487473343"))) %>% 
  select(Site, Connect_ID, d_232343615_d_926457119) %>%  safe_arrange(Site)

colnames(qc_sst4) <- c("Site", "Connect ID", "Serum Separator Tube 4- Date Received")
knitr::kable(qc_sst4, longtable = TRUE)

qc_sst5 <- rule17 %>%  filter(d_589588440_d_593843561=="Yes" & d_589588440_d_762124027=="No" & is.na(d_589588440_d_926457119) & 
                                      !(Connect_ID %in% c("6877720616", "1158253659"))) %>% 
  select(Site, Connect_ID, d_589588440_d_926457119) %>%  safe_arrange(Site)

colnames(qc_sst5) <- c("Site", "Connect ID", "Serum Separator Tube 5- Date Received")
knitr::kable(qc_sst5, longtable = TRUE)

qc_edta1 <- rule17 %>%  filter(d_454453939_d_593843561=="Yes" & d_454453939_d_762124027=="No" & is.na(d_454453939_d_926457119) & 
                                  !(Connect_ID %in% c("7609852429", "9729007313", "5487473343"))) %>% 
  select(Site, Connect_ID, d_454453939_d_926457119) %>%  safe_arrange(Site)

colnames(qc_edta1) <- c("Site", "Connect ID", "EDTA Tube 1- Date Received")
knitr::kable(qc_edta1, longtable = TRUE)


qc_edta2 <- rule17 %>%  filter(d_677469051_d_593843561=="Yes" & d_677469051_d_762124027=="No" & is.na(d_677469051_d_926457119) & 
                                 !(Connect_ID %in% c("7609852429", "9729007313", "2917006220", "5487473343"))) %>% 
  select(Site, Connect_ID, d_677469051_d_926457119) %>%  safe_arrange(Site)

colnames(qc_edta2) <- c("Site", "Connect ID", "EDTA Tube 2- Date Received")
knitr::kable(qc_edta2, longtable = TRUE)


qc_edta3 <- rule17 %>%  filter(d_683613884_d_593843561=="Yes" & d_683613884_d_762124027=="No" & is.na(d_683613884_d_926457119) & 
                                       !(Connect_ID %in% c("7609852429", "9729007313", "5487473343"))) %>% 
  select(Site, Connect_ID, d_683613884_d_926457119) %>%  safe_arrange(Site)

colnames(qc_edta3) <- c("Site", "Connect ID", "EDTA Tube 3- Date Received")
knitr::kable(qc_edta3, longtable = TRUE)



qc_acd1 <- rule17 %>%  filter(d_652357376_d_593843561=="Yes" & d_652357376_d_762124027=="No" & is.na(d_652357376_d_926457119) & 
...
colnames(qc_acd1) <- c("Site", "Connect ID", "ACD Tube 1- Date Received")
knitr::kable(qc_acd1, longtable = TRUE)



qc_hep1 <- rule17 %>%  filter(d_838567176_d_593843561=="Yes" & d_838567176_d_762124027=="No" &  is.na(d_838567176_d_926457119) & 
                                   Connect_ID!="7609852429") %>% 
  select(Site, Connect_ID, d_838567176_d_926457119) %>%  safe_arrange(Site)

colnames(qc_hep1) <- c("Site", "Connect ID", "Heparin Tube 1- Date Received")
knitr::kable(qc_hep1, longtable = TRUE)



qc_hep2 <- rule17 %>%  filter(d_958646668_d_593843561=="Yes" & d_958646668_d_762124027=="No" &  is.na(d_958646668_d_926457119) & 
                                   Connect_ID!="7609852429") %>% 
  select(Site, Connect_ID, d_958646668_d_926457119) %>%  safe_arrange(Site)

colnames(qc_hep2) <- c("Site", "Connect ID", "Heparin Tube 2- Date Received")
knitr::kable(qc_hep2, longtable = TRUE)




qc_urn <- bioqc %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_541311218, units="days"), digits=0))>4 & 
                                  d_973670172_d_593843561=="Yes" & d_973670172_d_762124027=="No" & is.na(d_973670172_d_926457119) & 
                                  !(Connect_ID %in% c("2215062576","4002548016","7609852429", '5400707754', "5487473343"))) %>% 
  select(Site, Connect_ID, d_973670172_d_926457119) %>%  safe_arrange(Site)

colnames(qc_urn) <- c("Site", "Connect ID", "Urine Tube 1- Date Received")
knitr::kable(qc_urn, longtable = TRUE)



qc_streck <- rule17 %>%  filter(d_505347689_d_593843561=="Yes" & d_505347689_d_762124027=="No" & is.na(d_505347689_d_926457119) & 
                                   Connect_ID!="8824215834") %>%
  select(Site, Connect_ID, d_505347689_d_926457119) %>%  safe_arrange(Site)


colnames(qc_streck) <- c("Site", "Connect ID", "STRECK Tube- Date Received")
knitr::kable(qc_streck, longtable = TRUE)
...
### 18. Tube Date Received Research - If BioFin_ResearchBldTmBL_v1r0 occurred more then 4 days ago, (tube type) was collected, and (tube type) was not discarded, then BioBPTL_DateRec_v1r0 for that tube type should be populated 
```{r BioBPTL_DateRecResearch, echo=FALSE, warning=FALSE, message=FALSE}


 ###########change to tube collected =yes and BioBPTL_DateRec_v1r0 is na for all & deviation!= discard deviation (one flag triggered)
...
qc13_mw <- bioqcR %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_448660695, units="days"), digits=0))>4 & d_143615646_d_593843561=="Yes" & d_143615646_d_762124027=="No" & is.na(d_143615646_d_926457119) & 
                                    !(Connect_ID %in% c("2330722643", "3080768933", "9375710580", 
                                                        "3409243008", '2262855648', '2697629782', '2697629782', '2262855648'))) %>% 

  select(Site, Connect_ID, d_143615646_d_926457119)%>%  safe_arrange(Site)

colnames(qc13_mw) <- c("Site", "Connect ID", "Mouthwash Tube- Date Received")
knitr::kable(qc13_mw, longtable = TRUE)




qc13_sst1 <- rule18 %>%  filter(d_299553921_d_593843561=="Yes" & d_299553921_d_762124027=="No" & is.na(d_299553921_d_926457119) &
                                  !(Connect_ID %in% c("3409243008","8598143342", "1618743232"))) %>% 
  select(Site, Connect_ID, d_299553921_d_926457119) %>%  safe_arrange(Site)

colnames(qc13_sst1) <- c("Site", "Connect ID", "Serum Separator Tube 1- Date Received")
knitr::kable(qc13_sst1, longtable = TRUE)




qc13_edta1 <- rule18 %>%  filter(d_454453939_d_593843561=="Yes" & d_454453939_d_762124027=="No" & is.na(d_454453939_d_926457119) &
                                    !(Connect_ID %in% c("3409243008", "8193439990", "5743151720", 
                                                        "5861017989", "8598143342"))) %>% 
  select(Site, Connect_ID, d_454453939_d_926457119) %>%  safe_arrange(Site)

colnames(qc13_edta1) <- c("Site", "Connect ID", "EDTA Tube 1- Date Received")
knitr::kable(qc13_edta1, longtable = TRUE)




qc13_acd1 <- rule18 %>%  filter(d_652357376_d_593843561=="Yes" & d_652357376_d_762124027=="No" & is.na(d_652357376_d_926457119) & #| d_827220437==657167265 once Sanford is hybrid
                                    !(Connect_ID %in% c("1735874266", "3409243008"))) %>% # HF exclusion

  select(Site, Connect_ID, d_652357376_d_926457119) %>%  safe_arrange(Site)

colnames(qc13_acd1) <- c("Site", "Connect ID", "ACD Tube 1- Date Received")
knitr::kable(qc13_acd1, longtable = TRUE)




qc13_sst2 <- rule18 %>%  filter(d_703954371_d_593843561=="Yes" & d_703954371_d_762124027=="No" & is.na(d_703954371_d_926457119) &
                                    Connect_ID!="3409243008") %>% 
  select(Site, Connect_ID, d_703954371_d_926457119) %>%  safe_arrange(Site)

colnames(qc13_sst2) <- c("Site", "Connect ID", "Serum Separator Tube 2- Date Received")
knitr::kable(qc13_sst2, longtable = TRUE)




qc13_hep1 <- rule18 %>%  filter(d_838567176_d_593843561=="Yes" & d_838567176_d_762124027=="No" &  is.na(d_838567176_d_926457119) & 
                                    !(Connect_ID %in% c("3409243008", "8193439990", "5842989867"))) %>% 
  select(Site, Connect_ID, d_838567176_d_926457119) %>%  safe_arrange(Site)

colnames(qc13_hep1) <- c("Site", "Connect ID", "Heparin Tube 1- Date Received")
knitr::kable(qc13_hep1, longtable = TRUE)


qc13_urn <- rule18 %>%  filter( d_973670172_d_593843561=="Yes" & d_973670172_d_762124027=="No" & is.na(d_973670172_d_926457119) & !(Connect_ID %in% c(9145733933,9618974099,8527264977,6473532641,4174960021, 4435276749, 1834747483))) %>% #Sanford exclusion

  select(Site, Connect_ID, d_973670172_d_926457119) %>%  safe_arrange(Site)

colnames(qc13_urn) <- c("Site", "Connect ID", "Urine Tube 1- Date Received")
knitr::kable(qc13_urn, longtable = TRUE)



qc13_streck <- rule18 %>%  filter(d_505347689_d_593843561=="Yes" & d_505347689_d_762124027=="No" & is.na(d_505347689_d_926457119) & !(Connect_ID %in% c("3978016781"))) %>% 
  select(Site, Connect_ID, d_505347689_d_926457119)  %>%  safe_arrange(Site)


colnames(qc13_streck) <- c("Site", "Connect ID", "STRECK Tube 1- Date Received")
knitr::kable(qc13_streck, longtable = TRUE)
...
```{r BioCol_BldUrnPlcdTmBL_v1r0, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r BioCol_BloodSettingBL_res, echo=FALSE, warning=FALSE, message=FALSE}
...
colnames(bio_bl_setting_res) <- c("Site", "Connect ID", "BioCol_BloodSettingBL_v1r0")
...
```{r BioCol_BloodSettingBL_clin, echo=FALSE, warning=FALSE, message=FALSE}
...
colnames(bio_bl_setting_clin) <- c("Site", "Connect ID", "BioCol_BloodSettingBL_v1r0")
...
```{r BioCol_UrineSettingBL_clin, echo=FALSE, warning=FALSE, message=FALSE}
...
colnames(bio_ur_setting_clin) <- c("Site", "Connect ID", "BioCol_UrineSettingBL_v1r0")
...
```{r BioCol_UrineSettingBL_res, echo=FALSE, warning=FALSE, message=FALSE}
...
colnames(bio_ur_setting_res) <- c("Site", "Connect ID", "BioCol_UrineSettingBL_v1r0")
...
### 27.a. If BioFin_BaseUrineCol_v1r0= yes, and BioCol_UrineSettingBL_v1r0= Research, then BioFin_ResearchUrnTmBL_v1r0 must be populated.
...
```
### 27.b. If BioCol_UrineSettingBL_v1r0 is populated, then BioFin_BaseUrineCol_v1r0= yes.
...
```
### 27.c. If BioFin_BaseUrineCol_v1r0= yes, then BioCol_UrineSettingBL_v1r0 must be populated.
...
```
### 27.d. If BioFin_ResearchUrnTmBL_v1r0 is populated, then BioCol_UrineSettingBL_v1r0 must be Research and BioFin_BaseUrineCol_v1r0 must be yes.
...
### 30.a. If BioFin_BaseUrineCol_v1r0= yes, and BioCol_UrineSettingBL_v1r0= Clinical, and BioCol_DBUrineRRLDt_v1r0 occurred more than seven days ago, then BioCol_ClinicalUrnTmBL_v1r0 must be populated.
...
c_urine3.4 <- parts_data %>%  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_139245758, units="days"), digits=0)) > 7 &
                                  ...
                                  !(Connect_ID %in% c('6862754687', '1371560328', '1176687465', '5885436394', '1850586900', '3319331872', 
                                                      '6575901705', '6862754687','9258958214', '1250934825', '7882825421', '3287102562', 
                                                      '1215003341', '7352978604', '1140047316', '4479873637','8633594373', '2755205973',
                                                      '3862626013', '8184138489', '4980503471', '2039357566', '6321294709',
                                                      '2300063524','5899565591', '8625295305', '9167792140', '9143436002', '7276829171',
                                                      '8625295305', '1274744512', '9262617255', '4838682809', '2887898061', '2871811858',
                                                      '4207529981', '3456526723', '9366425281', '7106367407', '6523900663',
                                                      '3992444928', '6538980272', '6467484794', '1375421153', '1102086935', '9672292896',
                                                      '9652100378', '3362078899','6437389686', '5972658064', '8553891957', '9575612025',
                                                      '2769291903', '1731138933', '3589595480','1349953410', '3722445358', '9694753790',
                                                      '1703960468', '2512896461', '8924667241','2431356225', '4057490101', '5213644501',
                                                      '3137297902', '8370075725', '7789082855', '8148304740', "7536254831", "8685711133",
                                                      "1573897668", "4332317182", '2643537513', "2145164291", "4246383289", "6568449334",
                                                      '3882691411', '6248007969','9772303289', '6422794867','7338222763', '9973370517',
                                                      '6351945356', '5919403814', '7143228040','6144633182', '4958431450', '7414720886',
                                                      '5367655537', "4054300519", "9906761921", "9358513094", "7945071279", "8783197795",
                                                      "1289559030", "6250053683", "3720410076", "8288718444", "5967357815", "3968179760",
                                                      "2452097537", "8926083140", "1376492836", "8288718444", "6554752349","1291437931",
                                                      "3508580541", "8130614446", '3108798408', '6966066624',"7304905537", "2321589610",
                                                      "2911866308","9272279347", "4892006412", "4753650657", "1474401708","7200773109",
                                                      "6157056255", "5978022819", '7620372153', '4897113924', '7754715436', '8062144301', 
                                                      '4810024579', '8003993386', '1020636422',
                                                      '4769462840','3526598284', '7176361975',
                                                      '5489793909', '6157114236')))   %>%  
...
\newpage
\FloatBarrier
### 31. BioClin_DBBloodID_v1r0 must be in BioCol_PolyBloodID_v1r0 if and BioCol_DBBloodRRLDt_v1r0 occurred more than 7 days ago


```{r BioCol_PolyBloodID_v1r0, echo=FALSE, warning=FALSE, message=FALSE}
...
bioqc1 <- bioqc %>%
  filter(as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_398645039, units="days"), digits=0)) > 7 &
           ...
           !(Connect_ID %in% c("2824989966","3759939890" ,"4688823720" ,"8589481620"
                               ,"6336258662","5461650381","4900116130",
                               "4462616750","9111871102","8476952861","2974054638"
                               ,"3544623873","3293040395","7571439842","3165368475",
                               "6954174713","6361337944","1629086681","3544623873","3293040395","7571439842",
                               "3165368475","6954174713", "6361337944", "1629086681", "9768745029", "3770024701",
                               "8836251245", "3312148528", "6806962114", 
                               "4782195164", "6229346420", '1271036774','6288600449', '9692694571', '4327492344',
                               '8372596629', '9399609746','1608206802', 
                               "8336144924", "7515171846", "9963945282" , "9554775290" ,"8570090186",
                               "4727512872", "5506703775", "8123989364", "9843228847", "7152112646",
                               "1671653943", "6446294667", '1208904227', '9889955644', '6412332391',
                               '5574963823', '9059927400', '4752949625', '7585628344', '6862843114')) & 
...
colnames(polyblood) <- c("Site", "Connect ID", "BioClin_DBBloodID_v1r0", "BioCol_PolyBloodID_v1r0")
...
bioqc2 <- bioqc %>%  filter( as.numeric(round(difftime(currentDate, d_173836415_d_266600170_d_541311218, units="days"), digits=0)) > 7 &
                               ...
                               !(Connect_ID %in% c("1629086681", "3165368475", "3293040395", "3544623873",
                                                   "6361337944", "6954174713", "7571439842",
                                                   "3770024701", "8836251245", "9768745029", "5461650381",
                                                   "4900116130", "4435276749",  '6288600449',
                                                   "6473532641", "6336258662", "8589481620", "4688823720",
                                                   "3759939890", "2824989966", "1608206802",
                                                   "1671653943", "6806962114", "6446294667",
                                                   '1208904227', '9889955644', '6412332391', '5574963823',
                                                   '9059927400', '4752949625', '7585628344', '6862843114')) & 
...
colnames(polyurine) <- c("Site", "Connect ID", "BioClin_DBUrineID_v1r0", "BioCol_PolyUrineID_v1r0")
...
## Because we need to determine the kit level to asses each sceario, we do not need separate code to find whether or not kit level is populated (like we do for the date/time and tracking variables)
recvd <- Bio_HMW %>%  filter((d_173836415_d_266600170_d_319972665_d_221592017=="Received" & kit_level=="Initial Kit" & 
                               ...
                               (d_173836415_d_266600170_d_541483796_d_221592017=="Received" & kit_level=="Replacement Kit 1" & 
                                  (is.na(d_531858099) | is.na(d_173836415_d_266600170_d_541483796_d_661940160) | 
                                     is.na(d_173836415_d_266600170_d_541483796_d_826941471) )) | 
                               (d_173836415_d_266600170_d_641006239_d_221592017=="Received" & kit_level=="Replacement Kit 2" & 
                                  (is.na(d_531858099) | is.na(d_173836415_d_266600170_d_641006239_d_661940160) | 
                                     is.na(d_173836415_d_266600170_d_641006239_d_826941471) ))) %>%  
  select(Site, Connect_ID, kit_level, d_531858099) %>%  safe_arrange(Site)

colnames(recvd) <- c("Site", "Connect ID", "Kit Level", "SupplyKitTrack")
...
```{r BioCol_SiteBldLocBL_v1r0, echo=FALSE, warning=FALSE, message=FALSE}
...
colnames(blood_loc) <- c("Site", "Connect ID", "BioCol_DBBloodRRLDtBL", "BioCol_SiteBldLocBL")
...
colnames(blood_locPL) <- c("Site", "Connect ID", "BioCol_SntBloodAccIDBL", "BioCol_PolyBloodIDBL")
...
```{r BioCol_SiteUrLocatBL_v1r0, echo=FALSE, warning=FALSE, message=FALSE}
...
colnames(urine_loc) <- c("Site", "Connect ID", "BioCol_DBUrineRRLBL", "BioCol_SiteUrLocatBL")
...
colnames(urine_locPL) <- c("Site", "Connect ID", "BioCol_SntUrineAccIDBL", "BioCol_PolyUrineIDBL")
...
###	69. BioFin_BMTimeBL_v1r0 must occur after BioKit_KitShipTm_v1r0 or before BioKit_KitRecdTm_v1r0. (If BioFin_BMTimeBL_v1r0 is in the same month OR the month on either side of BioKit_KitShipTm_v1r0 or BioKit_KitRecdTm_v1r0, then they are excluded.)
        ...
        !(abs(interval(floor_date(BMTimeBL, "month"), floor_date(KitShipTm, "month")) %/% months(1)) <= 1 |
          abs(interval(floor_date(BMTimeBL, "month"), floor_date(KitRecdTm, "month")) %/% months(1)) <= 1) & 
            !(Connect_ID %in% c("2356168653", '1129239381', '1235516301', '1315346907', '1417042322', '1490795015', '1554435136', 
                                ...
                                '8811209117', '5019127359','3486230846', '8954870489', '2254978243', '1050654902', '9863548767', '6288600449', '3805509458', "2401165994", "4967873956", "7192141098", "1242942518", "4545815202", "6244974048"))) %>%
...
knitr::kable(issue212, longtable = TRUE, font_size=7) %>% add_footnote("Excludes those where BioFin_BMTimeBL_v1r0 is in the same month OR the month on either side of BioKit_KitShipTm_v1r0 or BioKit_KitRecdTm_v1r0", notation= "none")
...
colnames(GH262_tube2) <- c("Site", "Connect ID", "SST1 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube5) <- c("Site", "Connect ID", "SSST2 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube_sst3) <- c("Site", "Connect ID", "SST3 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube_sst4) <- c("Site", "Connect ID", "SST4 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube_sst5) <- c("Site", "Connect ID", "SST5 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube3) <- c("Site", "Connect ID", "EDTA1 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube_edat2) <- c("Site", "Connect ID", "EDTA2 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube_edat3) <- c("Site", "Connect ID", "EDTA3 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube4) <- c("Site", "Connect ID", "ACD Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube6) <- c("Site", "Connect ID", "Hep1 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube8) <- c("Site", "Connect ID", "Hep2 Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(GH262_tube9) <- c("Site", "Connect ID", "STRECK Collected", "BioCol_DBBloodRRLB", "BioCol_DBBloodRRLDtBL")
...
colnames(Github262_2) <- c("Site", "Connect ID", "BioCol_DBUrineRRLBL", "BioCol_DBUrineRRLDtBL")
...
colnames(GH262_tube2) <- c("Site", "Connect ID", "SST1 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube2, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube5) <- c("Site", "Connect ID", "SSST2 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube5, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube_sst3) <- c("Site", "Connect ID", "SST3 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube_sst3, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube_sst4) <- c("Site", "Connect ID", "SST4 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube_sst4, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube_sst5) <- c("Site", "Connect ID", "SST5 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube_sst5, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube3) <- c("Site", "Connect ID", "EDTA1 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube3, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube_edat2) <- c("Site", "Connect ID", "EDTA2 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube_edat2, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube_edat3) <- c("Site", "Connect ID", "EDTA3 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube_edat3, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube4) <- c("Site", "Connect ID", "ACD Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube4, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube6) <- c("Site", "Connect ID", "Hep1 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube6, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube8) <- c("Site", "Connect ID", "Hep2 Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube8, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(GH262_tube9) <- c("Site", "Connect ID", "STRECK Collected", "BioCol_AnySpecRRLBL", "BioCol_AnySpecRRLBTmL")
knitr::kable(GH262_tube9, longtable = TRUE) %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7)
...
colnames(Github262_spec_tubeU) <- c("Site", "Connect ID", "BioCol_DBUrineRRLBL", "BioCol_DBUrineRRLDtBL")
...
BioChk_TimeBL_v1r0_RschTmBL <- parts_data %>%  
  filter((as.POSIXct(ymd_hms(d_331584571_d_266600170_d_840048338)) > 
           pmin(as.POSIXct(ymd_hms(d_173836415_d_266600170_d_561681068)), as.POSIXct(ymd_hms(d_173836415_d_266600170_d_847159717)), na.rm=T)) & 
...
DrawOrder_Verif4 <- bioqc %>%  filter(str_sub(d_820476880, start=1, end=3)=="CXA" & #blood/urine only
                                        ...
                                        !(Connect_ID %in% c('3259382811', '9402029018'))) %>%
  select(Site, Connect_ID, d_914594314, d_173836415_d_266600170_d_184451682) %>%  safe_arrange(Site)


colnames(DrawOrder_Verif4) <- c("Site", "Connect ID", "RcrtV_VerificationTm", "BioCol_BldUrnPlcdTmBL")
...
```{r DrawOrder_Verif30, echo=FALSE, warning=FALSE, message=FALSE}
 ...
 DrawOrder_verif <- bioqc %>% filter(d_914594314<as.Date(currentDate -14) & (Site %in% 
                                       c("Kaiser Permanente Colorado","Kaiser Permanente Georgia",
                                       ...
                                       d_878865966=="No" & d_167958071=="No" & d_684635302 == "No") %>% 
   select(Site, Connect_ID, d_914594314, d_173836415_d_266600170_d_184451682) %>%  safe_arrange(Site)
 
 
 colnames(DrawOrder_verif) <- c("Site", "Connect ID", "RcrtV_VerificationTm", "BioCol_BldUrnPlcdTmBL")
 knitr::kable(DrawOrder_verif, longtable = TRUE)  %>% kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=8)

```

###	80. Clinical collection date (BioCol_ClinBloodTmBL_v1r0 or BioCol_ClinicalUrnTmBL_v1r0) should not precede draw order date (BioCol_BldUrnPlcdTmBL_v1r0) by more than 2 days.
```{r DrawOrder_Colct2, echo=FALSE, warning=FALSE, message=FALSE}


DrawOrder_Colct2 <- bioqc %>%  filter(as.numeric(difftime(d_173836415_d_266600170_d_184451682,
                                        ...
                                        !(Connect_ID %in% c('1094193968', '8393015300', '4621132537')))  %>%
  select(Site, Connect_ID, d_173836415_d_266600170_d_184451682, d_173836415_d_266600170_d_982213346,
         d_173836415_d_266600170_d_139245758) %>%  safe_arrange(Site)

colnames(DrawOrder_Colct2) <- c("Site", "Connect ID", "BioCol_BldUrnPlcdTmBL", "BioCol_ClinBloodTmBL", "BioCol_ClinicalUrnTmBL")
...
### 81. The research collection visit check-in time (BioChk_TimeBL_v1r0) must be before the check-out time (BioFin_CheckOutTmBL_v1r0).
```{r Checkin_Checkout, echo=FALSE, warning=FALSE, message=FALSE}

Checkin_Checkoout <- bioqc %>%  
  filter(as.POSIXct(ymd_hms(d_331584571_d_266600170_d_840048338)) > as.POSIXct(ymd_hms(d_331584571_d_266600170_d_343048998)) &
           Connect_ID!="9005924628") %>%
  select(Site, Connect_ID, d_331584571_d_266600170_d_840048338,
         d_331584571_d_266600170_d_343048998) %>%  safe_arrange(Site)

colnames(Checkin_Checkoout) <- c("Site", "Connect ID", "BioChk_TimeBL_v1r0", "BioFin_CheckOutTmBL_v1r0")
knitr::kable(Checkin_Checkoout)  %>% kable_styling(latex_options = "scale_down")

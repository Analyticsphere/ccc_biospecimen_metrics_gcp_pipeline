...
verif_BSL_colct_rsch_8_total <- BL_coll_rsch_8 %>% 
  dplyr::summarize(Site = "All Sites",
                   'Number of Research Collection Visits'= scales::comma(n()),
...
verif_BSL_colctC_2Before_total <- BL_coll_clin_2Before %>% 
  dplyr::summarize(Site = "All Sites",
                   'Number of Clinical Collections'= scales::comma(n()),
                   Min = round(min(time,na.rm = TRUE), digits=1),
                   Q1 = round(quantile(time, 0.25,na.rm = TRUE), digits=1),
                   Median = round(median(time,na.rm = TRUE), digits=1),
                   Mean = round(mean(time,na.rm = TRUE), digits=1),
...
verif_BSL_colctC_2After_total <- BL_coll_clin_2After %>% 
  dplyr::summarize(Site = "All Sites",
                   'Number of Clinical Collections'= scales::comma(n()),
                   Min = round(min(time,na.rm = TRUE), digits=1),
                   Q1 = round(quantile(time, 0.25,na.rm = TRUE), digits=1),
...
verif_BL_RC <- baseline_colct %>%  
  ...
  filter( (all_settings=="Clinical" & d_914594314>=as.Date("2023-02-01")) | 
    ...
    dplyr::summarize('Number of Collections'= scales::comma(n()),
                     Min = round(min(time,na.rm = TRUE), digits=1),
                     Q1 = round(quantile(time, 0.25,na.rm = TRUE), digits=1),
                     Median = round(median(time,na.rm = TRUE), digits=1),
                     Mean = round(mean(time,na.rm = TRUE), digits=1),
...
req_rec_inv_kable1 <- function(data, group_var, group_var_name, title) { 
  ...
  mutate(
    ...
    `MW Kit Requested` = paste0(
      Requested, " (",
      round(Requested / Invited * 100, 2), "%)"
    ),
    # Of those that were invited, how many have we received
    `MW Sample Received (of total invited)` = paste0(
      Received, " (",
      ifelse(Requested == 0, 0,
             round(Received / Invited * 100, 2)), "%)"
    )
  ) %>%
...
kit_tm_don_inv <- kit_request_table %>% 
  ...
  mutate(Percentage = round((`MW Kit Requested` / sum(`MW Kit Requested`) * 100), digits=1)) %>% 
  ...
  ## Update to N (%) style 
  mutate(`MW Kit Requested` = paste0(`MW Kit Requested`, " (", Percentage, "%)")) %>% 
...
kit_tm_inv_recvd <- kit_request_table %>% 
  ...
  mutate(Percentage = round((`MW Sample Received` / sum(`MW Sample Received`) * 100), digits=1)) %>% 
  ...
  ## Update to N (%) style 
  mutate(`MW Sample Received` = paste0(`MW Sample Received`, " (", Percentage, "%)")) %>% 
...
up_hist <- up_hist %>% 
  ...
  bind_rows(
    summarise(.,
              Address_Update_Status = "Total",
              `N` = sum(`N`),
              Percentage = 100)
    ) %>% 
  dplyr::rename("Number of Participants" = "N") %>% 
  
  ## Update to N (%) style 
  mutate(`Number of Participants` = paste0(`Number of Participants`, " (", Percentage, "%)")) %>% 
  ...
  knitr::kable(up_hist, 
               ...
               align=c("l", "c"),
               digits=1, booktabs = TRUE) %>% 
    add_indent(seq(1, nrow(up_hist) - 1)) %>%
    kable_styling(latex_options ="scale_down") 

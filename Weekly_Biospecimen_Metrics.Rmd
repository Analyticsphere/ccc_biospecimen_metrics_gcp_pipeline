...
biocol.tb <- base::merge(recrver1[,c("Connect_ID","Site","BiospeCollection","d_878865966","d_684635302","d_167958071", "d_173836415_d_266600170_d_185243482", 
                                     "d_173836415_d_266600170_d_452847912", "d_173836415_d_266600170_d_822274939", "d_173836415_d_266600170_d_139245758",
                                     "BL_BioSpm_BloodSetting_v1r0", "BL_BioSpm_UrineSetting_v1r0", "BL_BioSpm_MWSetting_v1r0", "BioFin_BaseBloodCol_v1r0", 
                                     "BioFin_BaseUrineCol_v1r0", "BioFin_BaseMWCol_v1r0")],
                         biospe[,c("Connect_ID", "d_410912345", "Site_location", "d_650516960", "d_951355211")],
                         by.x="Connect_ID",by.y="Connect_ID",all.x=TRUE )
...
biocounts <- biospe %>%  select(Connect_ID, bldcol, d_556788178, d_650516960)
HFSITES <- table1_2__locations %>%  filter(Site=="Henry Ford Health") %>% select(Connect_ID, Site_location )
left7 <- left_join(HFSITES, biocounts, by="Connect_ID")
biospe7$HF_Sites <- biospe7$Site_location                             

tube_col_HF2 <- biospe7  %>% group_by(Connect_ID,HF_Sites,d_650516960) %>%
  dplyr::summarise(bldcol_max = max(bldcol,na.rm=TRUE),
                   collection.tm =max(as.POSIXct(ymd_hms(d_556788178)),na.rm=TRUE))
...
tube_research_HF2 <- tube_col_HF2 %>% filter(d_650516960==664882224) %>%
  mutate(bld.complt = ifelse(ifelse(collection.tm < as.Date("2024-03-01"), bldcol_max==5, bldcol_max==4),"All Blood Collected",
         ...
         bld.complt = factor(bld.complt, levels = c("All Blood Collected","Partial Blood Collected",  "No Blood Collected" ))) 
...
## Need to run chunks: Table6, Table14, Figures16_18, 

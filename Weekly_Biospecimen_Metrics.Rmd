...
req_rec_inv_kable1 <- function(data, group_var, group_var_name, title) { 
  ...
  mutate(
    ...
    # Of those who requested a kit, how many have we received
...
#Specifically requested to have labels as years with these exact counts of days
         tm_don_inv = case_when(as.numeric(tm_don_inv_years) < 730 ~ "Less than 2 years",
                                  as.numeric(tm_don_inv_years) < 911 ~"2- <2.5 years",
                                  as.numeric(tm_don_inv_years) < 1094 ~ "2.5- <3 years",
                                  as.numeric(tm_don_inv_years) < 1276 ~ "3- <3.5 years",
                                  as.numeric(tm_don_inv_years) < 1459 ~ "3.5- <4 years",
...
kit_request_table <- kit_request_table %>% 
  mutate(last_survey = pmax(as.Date(d_784810139), #COVID completion time
                            as.Date(d_389890053), #CSH completion time
                            as.Date(d_843688458), #PROMIS completion time
                            as.Date(d_543379310), #ROI completion time
                            as.Date(d_517311251), #BOH completion time
                            as.Date(d_832139544), #MRE completion time
                            as.Date(d_770257102), #SAS completion time
                            as.Date(d_264644252), #LAW completion time
                            as.Date(d_199471989), na.rm=T), #CES completion time
...
query= "WITH address_update_flags AS (
  ...
  SELECT
    ...
    MAX(
      CASE
        WHEN DATE(up.d_371303487) > DATE(p.d_173836415_d_266600170_d_319972665_d_278539982)
         AND DATE(up.d_371303487) < DATE(p.d_173836415_d_266600170_d_319972665_d_759651991)
        THEN 1 ELSE 0
      END

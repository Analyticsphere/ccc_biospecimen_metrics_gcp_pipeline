...
avg_draw_order <- clinical.survey %>%  filter(Site!='HealthPartners' & Site!="Sanford Health" & Site!='University of Chicago Medicine') %>% 
  mutate(time = as.numeric(difftime(
    ...
    pmin(as.POSIXct(ymd_hms(d_173836415_d_266600170_d_982213346)), as.POSIXct(ymd_hms(d_173836415_d_266600170_d_139245758))), 
    #and BldUrnPlcdTmBL
    as.POSIXct(ymd_hms(d_173836415_d_266600170_d_184451682)),
    units="days"))) %>%
  group_by(Site) %>% 
...
all_coll_to_sub <- coll_sett_all %>%  filter(d_265193023 == 'Submitted' | d_253883960=='231311385') %>% 
  mutate(collection = ifelse(d_265193023 == 'Submitted', as.POSIXct(ymd_hms(d_678166505)), as.POSIXct(ymd_hms(d_556788178))),
         surv_sub = ifelse(d_265193023 == 'Submitted', as.POSIXct(ymd_hms(d_222161762)), as.POSIXct(ymd_hms(d_764863765))),
...
survey_cc_int <- kit_table %>%  
  ...
  mutate(ctime = round(as.numeric(difftime(as.POSIXct(ymd_hms(d_195145666)),
                                           as.POSIXct(ymd_hms(d_173836415_d_266600170_d_448660695)),units="days")), 
                       digits=0),
...
kit_request_table <- kit_request_table %>% 
  mutate(tm_don_inv_years = as.numeric(difftime(as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_278539982)),
                                        as.POSIXct(ymd_hms(d_173836415_d_266600170_d_740582332)),units="days")),
...
kit_tm_don_inv <- kit_request_table %>% 
  filter(!is.na(d_173836415_d_266600170_d_319972665_d_759651991)) %>% 
  mutate(tm_don_inv_diff = as.numeric(difftime(as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_759651991)),
                                              as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_278539982)),units="days")),
...
kit_tm_inv_recvd <- kit_request_table %>% 
  ...
  mutate(tm_inv_recvd_weeks = as.numeric(difftime(as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_826941471)),
                                              as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_278539982)),units="weeks")),
...
kit_request_table <- kit_request_table %>% 
  mutate(last_survey = pmax(as.POSIXct(ymd_hms(d_784810139)), #COVID completion time
                            as.POSIXct(ymd_hms(d_389890053)), #CSH completion time
                            as.POSIXct(ymd_hms(d_843688458)), #PROMIS completion time
                            as.POSIXct(ymd_hms(d_543379310)), #ROI completion time
                            as.POSIXct(ymd_hms(d_517311251)), #BOH completion time
                            as.POSIXct(ymd_hms(d_832139544)), #MRE completion time
                            as.POSIXct(ymd_hms(d_770257102)), #SAS completion time
                            as.POSIXct(ymd_hms(d_264644252)), #LAW completion time
                            as.POSIXct(ymd_hms(d_199471989)), na.rm=T), #CES completion time
         tm_surv_daymonths = as.numeric(difftime(as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_278539982)),

...
# bldcol_research_HF2$n_pct_NoBlood = ifelse(!grepl("Total",rownames(bldcol_research_HF2)),  paste(format(bldcol_research_HF2[,3],big.mark=","),"(", round(100*bldcol_research_HF2[,6],1),"%)",sep=" "), format(bldcol_research_HF2[,3],big.mark="," ))
...
bldcol_research_HF2 <- bldcol_research_HF2[,c(7:10,3)]
...
fig3_sub_all <- research.survey %>%  filter(Site=="University of Chicago Medicine")
fig3_sub_all <- fig3_sub_all %>%  filter(!is.na(research.sv)) #d_684635302 == 353358909 | d_878865966 == 353358909| d_167958071 == 353358909 & d_222161762<as.Date("2023-07-05") & 

fig3_sub_all$Site<- droplevels(fig3_sub_all$Site)




fig3_sub_1<- fig3_sub_all %>% mutate(not_submitted = case_when(research.sv!="Submitted"~1,
                                                TRUE~0),
                                    start.date = min(schedulecol.tm),
                                    current.date=max(as.POSIXct(ymd_hms(schedulecol.tm))),
                                    BioRec_CollectScheduled = ifelse(as.Date(ymd_hms(schedulecol.tm)) >= as.Date(as.POSIXct(ymd_hms(d_914594314))),1,0))
                                    
fig3_sub_1$visit.wk <- ceiling(as.numeric(difftime(ymd_hms(fig3_sub_1$collection.tm),fig3_sub_1$start.date, units="weeks")))
fig3_sub_1$visit.wkdate <- fig3_sub_1$start.date + dweeks(fig3_sub_1$visit.wk)
visit_wk_site3 <- fig3_sub_1 %>% arrange(Site,not_submitted,visit.wk,visit.wkdate) %>% group_by(Site,not_submitted,visit.wk,visit.wkdate) %>% tally()
visit_wk_site3 <- visit_wk_site3 %>%  filter(not_submitted>0)

##UC
research.3 <- ggplot(visit_wk_site3, aes(x=as.Date(visit.wkdate), y=100*(not_submitted/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
  geom_line(na.rm=T) + geom_point(width = 0.2) +
  scale_y_continuous(name="Percent") + #, limits=c(0,30), breaks=seq(0,30,5)) +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks")  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3: Weekly Percent of Biospecimen Surveys Not Submitted, \n University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
...
research.4_1 <- ggplot(visit_wk_site4_1, aes(x=as.Date(visit.wkdate), y=100*(hours3/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
...
##Sanford
visit_wk_site4_2 <- visit_wk_site4__1 %>%  filter(Site=="Sanford Health")
research.4_2 <- ggplot(visit_wk_site4_2, aes(x=as.Date(visit.wkdate), y=100*(hours3/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Percent",breaks=seq(0,100,25)) +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks")  +
  scale_fill_brewer(palette="Dark2") +
...
##UC
visit_wk_site4_3 <- visit_wk_site4__1 %>%  filter(Site=="University of Chicago Medicine")
research.4_3 <- ggplot(visit_wk_site4_3, aes(x=as.Date(visit.wkdate), y=100*(hours3/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Percent",breaks=seq(0,100,25)) +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks")  +
  scale_fill_brewer(palette="Dark2") +
...
help_fig1 <- fig1_UC_inc %>%  select(visit.wkdate, bld.complt) %>%  mutate(incomp_blood = case_when(bld.complt=="BloodIncomplet" | bld.complt=="NoBlood" ~1,
                                                            TRUE ~ 0))

help_fig1 <- help_fig1 %>% mutate(all_blood=case_when(incomp_blood==1 | incomp_blood==0 ~1,
                                                            TRUE ~ 0))


help2_1_fig1 <- help_fig1 %>%  group_by(visit.wkdate, incomp_blood, all_blood) #%>%  tally() 

help2_1_fig1_inc <- help2_1_fig1 %>%  select(visit.wkdate, incomp_blood, all_blood) %>%  group_by(visit.wkdate)


final_fig1_table <- help2_1_fig1_inc %>%  group_by(visit.wkdate) %>%  summarize(incomplete=sum(incomp_blood), all_collections=sum(all_blood), percent=(incomplete/all_collections))


research.1_1 <- ggplot(final_fig1_table, aes(x=as.Date(visit.wkdate), y=(100*round(percent, digits=1)))) + #, color=UC_Sites) + #,fill=Site, color=Site)) +
  geom_line(na.rm=T) + geom_point(width = 0.2) +
  scale_y_continuous(name="Percent", limits=c(0,100), breaks=seq(0,100,25)) +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "1 weeks")  +
  scale_fill_brewer(palette="Dark2") +
...
help_fig2 <- fig2_HF_inc %>%  select(visit.wkdate,bld.complt) %>%  mutate(incomp_blood = case_when(bld.complt=="BloodIncomplet" | bld.complt=="NoBlood" ~1,
                                                            TRUE ~ 0))


help_fig2 <- help_fig2 %>% mutate(all_blood=case_when(incomp_blood==1 | incomp_blood==0 ~1,
                                                            TRUE ~ 0))


help2_1_fig2 <- help_fig2 %>%  group_by(visit.wkdate, incomp_blood, all_blood) #%>%  tally() 

help2_1_fig2_inc <- help2_1_fig2 %>%  select(visit.wkdate, incomp_blood, all_blood) %>%  group_by(visit.wkdate)


final_fig2_table <- help2_1_fig2_inc %>%  group_by(visit.wkdate) %>%  summarize(incomplete=sum(incomp_blood), all_collections=sum(all_blood), percent=(incomplete/all_collections))

#colnames(final_fig2_table) <- c("Location","Week", "Incomplete Collections", "Total Collections", "Percent")

#final_fig2_table %>%  gt() #<- just needed to see how it looked

#write.csv(final_fig2_table,"C:/Users/dowlingk2/Desktop/Figure2 Table.csv",row.names = F,na="") #<- just needed to send to domonique to verify





research.1_2 <- ggplot(final_fig2_table, aes(x=as.Date(visit.wkdate), y=(100*round(percent, digits=1)))) + #, color=HF_Sites) + #,fill=Site, color=Site)) +
  geom_line(na.rm=T) + geom_point(width = 0.2) +
  scale_y_continuous(name="Percent", limits=c(0,100), breaks=seq(0,100,25)) +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "1 weeks")  +
  scale_fill_brewer(palette="Dark2") +
...
tab8=knitr::kable(notcol_combo2[,c(1:12)],caption='Table 10. Blood Tubes Collected by Tube Type at Clinical Collections', row.names=FALSE,align=c("l","c","c","c","c","c","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>%  kable_styling(latex_options = "scale_down") %>% landscape()
...
tabl27 <- knitr::kable(verif_draw_order,caption='Table 24. Time (in Days) from Verification to Draw Order Placement',row.names=FALSE, align=c("l","c","c","c","c","c","c"),digits=1, booktabs = TRUE)  %>% 

...
recrvar <- bigrquery::bq_table_download(recr_var, bigint="integer64",n_max = Inf, page_size = 8000)
...
recr.bio <- bigrquery::bq_table_download(tb_bq,bigint="integer64",n_max = Inf, page_size = 5000)
...
bld.ls <- c("d_299553921","d_703954371","d_454453939","d_838567176","d_652357376","d_376960806","d_232343615","d_589588440","d_677469051", "d_683613884","d_958646668") 
...
biospe7= left7  %>%  
  #something odd going odd with the values
  filter(as.numeric(d_173836415_d_266600170_d_185243482)>72  & !is.na(d_173836415_d_266600170_d_185243482) ) %>% # & d_827220437==548392715
  mutate(HF_Sites= case_when(d_173836415_d_266600170_d_185243482==1050010095 ~ "Wyandotte",
                                       ...
                                       #d_173836415_d_266600170_d_185243482==1011220006 ~ "West Bloomfield Hospital (Bloom TWP)", 
...
#biospe7$HF_Sites<- droplevels(biospe7$HF_Sites)    #Error in UseMethod("droplevels") :  no applicable method for 'droplevels' applied to an object of class "character"
...
bldurcl <- bldurcl %>% filter(!is.na(Data_System)) #will comment out once we find out whats going on
...
#   notcol_combo8_1 = array(pct_n(ctable$t,ctable$prop.col),dim=c(5,6))
# 
...
## Number of tube tubes not collected at clinical collections
  ...
  clinic.coltube<- collection_long %>% filter(biocol_Setting=="Clinical" & tubecol == 353358909) %>%
    mutate(Site=droplevels(Site),
           tube_type = factor(tube_type,levels=c("SS Tube1", "SS Tube2","SS Tube3", "SS Tube4", "SS Tube5", "Heparin Tube1","Heparin Tube2","EDTA Tube1","EDTA Tube2","EDTA Tube3","ACD Tube1", "STRECK Tube1", "Urine Tube1" ))) #, "STRECK Tube1" NEEDS TO BE ADDED WHEN APPLICABLE
...
notcol_combo2 <- array(pct_n(clinic.ctable$t,clinic.ctable$prop.col), dim=c(6,12)) #changed back to 6 with HP now hybrid
  ...
  notcol_combo2 <- notcol_combo2[, c(13, 1:11)] #removing Urine
  #notcol_combo2[5,1] <- "Total"
  notcol_combo2 <- notcol_combo2[,c(1:12)]
...
research.notcol <- notcol_notes %>% filter(biocol_Setting=="Research" & tubecol==104430631) %>% 
   mutate(Site=droplevels(Site),
           tube_type = factor(tube_type,levels=c("SS Tube1", "SS Tube2","Heparin Tube1","EDTA Tube1","ACD Tube1", "Urine Tube1","MW Tube1" ))) #"STRECK Tube1", when applicable, right now is causing error in line reseachnotes.coltype
...
Table17_combinedB <- test17_b %>% dplyr::select(Site, checktmvisit )  %>% tbl_cross(
    ...
    #italicize_levels() %>% 
    modify_header() %>%
    modify_caption("16a. Baseline Research Biospecimen Survey Completion Time from Visit Check-In Time (Pre-COVID-19 Questions Removal)") %>%
    as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
Table17_combinedA <- test17_a %>% dplyr::select(Site, checktmvisit )  %>% tbl_cross(
    ...
    #italicize_levels() %>% 
    modify_header() %>%
    modify_caption("16b. Baseline Research Biospecimen Survey Completion Time from Visit Check-In Time (Post-COVID-19 Questions Removal)") %>%
    as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
Table18_combined <- test18a_B %>% dplyr::select(Site, checktmvisit )  %>% tbl_cross(
  ...
  #italicize_levels() %>% 
  modify_header() %>%
  modify_caption("17. Baseline Clinical Biospecimen Survey Complete Time from Collection Time") %>% #on hold: check that HF shows up
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
```{r Figures12_15, eval=TRUE,echo=FALSE,include=FALSE,results='hold'}
#Code for figures
...
#clc.both.wk_site.CO <- clc.both.vt.CO %>% arrange(Site,BU.Sunday.week,BU.Sunday.date) %>% group_by(Site,BU.Sunday.week,BU.Sunday.date) %>% tally()
 ...
 clc_both_vt.site.CO<- clc.both.wk_site.CO  %>% ggplot( aes(x=as_date(clc.both.wk_site.CO$Monday.date.start), y=n)) + 
  ...
  scale_fill_brewer(palette="Dark2") + 
  ggtitle(label="Figure 12. Weekly Biospecimen Collections \n at Kaiser Permanente Colorado") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
clc_both_vt.site.GA<- clc.both.wk_site.GA  %>% ggplot( aes(x=as_date(clc.both.wk_site.GA$Monday.date.start), y=n)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") + 
  ggtitle(label="Figure 13. Weekly Biospecimen Collections \n at Kaiser Permanente Georgia") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
clc_both_vt.site.HI<- clc.both.wk_site.HI %>% ggplot( aes(x=as_date(clc.both.wk_site.HI$Monday.date.start), y=n)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") + 
  ggtitle(label="Figure 14. Weekly Biospecimen Collections \n at Kaiser Permanente Hawaii") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
```{r Table26, eval=TRUE,echo=FALSE,include=FALSE,results='hold'}
...
#Update to clinical sites only, so hybrid sites are included instead of specifying sites
blood_placed= blood_fig4 %>%  filter(d_650516960==664882224 & d_827220437!=531629870) ##won't receive a draw order from HP--Github issue 72
...
#   ggtitle("Fig 5. Clinical Blood Draw Order Status \n of Verified Participants by Site") + 
...
Table_bld_draw <- blood_placed %>% dplyr::select(Site, bld_placed)  %>% tbl_cross(
  ...
  #italicize_levels() %>% 
  modify_header() %>%
  modify_caption("Table 26. Clinical Blood Draw Order Status Among Verified Participants") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
```{r KnitrTables1_5, eval=TRUE,echo=FALSE, ,results='asis'}
options(knitr.table.format = "latex")
...
add_footnote(Bld_urn_cli, "Note: Data sent by sites experience an availablility delay of up to 48 hours behind data from the Biospecimen Dashboard. Data sent by sites experience an availability delay of up to 48 hours behind data from the Biospecimen Dashboard. 'Site Data Only' is assumed to have a value of '0(0%)' when not listed in this table.", notation = "none")
...
research.3 <- ggplot(figsub3_5, aes(x=as.Date(col.Monday.date), y=100*(cum_sub/cumtotal))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3. Cumulative Percent of Biospecimen Surveys \n Submitted, University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
research.3.1 <- ggplot(figsub3_5, aes(x=as.Date(col.Monday.date), y=100*(weekly_sub/per_week_parts))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3.1 Weekly Percent of Biospecimen Surveys Submitted, University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
\newpage
\FloatBarrier
```{r KnitrTables17_26,echo=FALSE,error=FALSE,message=FALSE,results='asis'}
...
tab23=knitr::kable(research.vt.length,caption='Table 20. Research Biospecimen Collection Visit Length (in Minutes) by Site',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down") #%>% landscape()
...
research.vt1 <- ggplot(visit_wk_site1, aes(x=as.Date(col.Monday.date), y=n)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0)) +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 6. Weekly Biospecimen Collections, \n All Sites") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        legend.position="bottom",
...
```{r Figures7_9, eval=TRUE,echo=FALSE}
...
visit_wk_site.hybrid <-research.survey.hybrid %>% arrange(Site,Site_location, Setting,col.Sunday.week,col.Monday.date) %>% group_by(Site,Site_location, Setting,col.Sunday.week,col.Monday.date) %>% tally()
 ...
 research.vt_plotH <- ggplot(visit_wk_site.hybridHF, aes(x=as.Date(col.Monday.date), y=n, color=Setting )) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 7. Weekly Biospecimen Collections \n at Henry Ford") +
   #theme_ipsum() +
   theme(panel.background = element_blank(),
         #legend.title = element_text(size=7),
         legend.position = "bottom",
 ...
 edited_fig3_1 <-  visit_wk_site.hybridHF %>%  filter(Site_location!="Hybrid")
  research.vt_plotHF <- ggplot(edited_fig3_1, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 7.1 Weekly Research Biospecimen Collections \n at Henry Ford by Collection Location") +
   #theme_ipsum() +
   theme(panel.background = element_blank(),
...
edited_fig3_1_Clin_tally <- edited_fig3_1_Clin %>% arrange(HF_Sites, col.Sunday.week,col.Monday.date) %>% group_by(HF_Sites, col.Sunday.week,col.Monday.date) %>% tally()
  ...
  research.vt_plotH_Clin <- ggplot(edited_fig3_1_Clin_tally, aes(x=as.Date(col.Monday.date), y=n, color=HF_Sites )) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 7.2 Weekly Clinical Biospecimen Collections \n at Henry Ford by Clinic Location") +
   #theme_ipsum() +
   theme(panel.background = element_blank(),
...
#- SANFORD = 589224449, 467088902
 ...
 research.vt_plotUC <- ggplot(research.vt_plUC, aes(x=as.Date(col.Monday.date), y=total_collections)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 8. Weekly Biospecimen Collections at \n University of Chicago Medicine") +
   #theme_ipsum() +
   theme(panel.background = element_blank(),
         #legend.title = element_text(size=7),
         axis.line = element_line(size=0.2),
 ...
 research.vt_plotUC <- ggplot(research.vt_plUC, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 8.1 Weekly Biospecimen Collections at \n University of Chicago Medicine Locations") +
   #theme_ipsum() +
   theme(panel.background = element_blank(),
         #legend.title = element_text(text="Site Location"),
         legend.position = "bottom",
...
research.vt_plotUC + guides(color = guide_legend(title = "Site Locations")) 
 ...
 research.vt_plotMF <- ggplot(research.vt_plMF, aes(x=as.Date(col.Monday.date), y=total_collections)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 9. Weekly Biospecimen Collections at \n Marshfield Clinic Health System") +
   #theme_ipsum() +
   theme(panel.background = element_blank(),
         #legend.title = element_text(size=7),
         axis.line = element_line(size=0.2),
 ...
 research.vt_plotMF <- ggplot(research.vt_plMF, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
   ...
   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
   scale_fill_brewer(palette="Dark2") +
   ggtitle(label="Figure 9.1 Weekly Biospecimen Collections at \n Marshfield Clinic Health System Locations") +
   #theme_ipsum() +
   theme(panel.background = element_blank(),
         #legend.title = element_text(text="Site Location"),
         legend.position = "bottom",
...
```{r Figures10_11, eval=TRUE,echo=FALSE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
...
research.vt_plotSH <- ggplot(research.vt_plSH, aes(x=as.Date(col.Monday.date), y=n)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 10. Weekly Biospecimen Collections \n at Sanford Health") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
research.vt_plotSH <- ggplot(research.vt_plSH, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 10.1 Weekly Biospecimen Collections \n at Sanford Health Locations") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
research.vt_plotHP <- ggplot(research.vt_plHP, aes(x=as.Date(col.Monday.date), y=n, color=Setting)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 11. Weekly Biospecimen Collections \n at HealthPartners") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
research.vt_plotHP_locs <- ggplot(edited_fig11_1_Clin_tally, aes(x=as.Date(col.Monday.date), y=n, color=HP_cl_locs)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 11.1 Weekly Clinical Biospecimen Collections at HealthPartners by Clinic Location") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
```{r Fig12_16, eval=TRUE,echo=FALSE}
...
```{r DeviationTable,echo=FALSE,error=FALSE,message=FALSE,results='asis'}
...
for (i in 1:length(dev_count.site)){
  ...
  n <- i
  print(knitr::kable(dev_count.site[[site]], col.names=c("Deviation", "Number"), caption=paste( "Biospecimen Deviations Reported by ",site, sep=""), row.names=FALSE,

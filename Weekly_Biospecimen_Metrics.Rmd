...
visit_wk_site4__1 <- visit_wk_site4 %>%  filter(hours3>0 & col.Monday.date < currentDate)
...
visit_wk_site4__2 <- visit_wk_site4 %>%  filter(hours3>0 & col.Monday.date < currentDate)
...
visit_wk_site4__3 <- visit_wk_site4 %>%  filter(hours3>0 & col.Monday.date < currentDate)
...
crosstable.coll_bptl29a$Site <- list("HealthPartners", "Henry Ford Health", "Kaiser Permanente Colorado", "Kaiser Permanente Georgia",
                                     "Kaiser Permanente Hawaii", "Kaiser Permanente Northwest", "Sanford Health")
...
crosstable.coll_bptl29a[8,] <- list('Total', sum(as.numeric(gsub(",", "", str_extract(crosstable.coll_bptl29a[,2], "\\b\\d{1,4}(,\\d{3})*(?=\\s*\\()")))),
...
crosstable.coll_bptl29a[8,2] <- paste(crosstable.coll_bptl29a[8,2], "(",round(100*as.numeric(crosstable.coll_bptl29a[8,2])/as.numeric(crosstable.coll_bptl29a[8,7]), digits=1), "%)")
crosstable.coll_bptl29a[8,3] <- paste(crosstable.coll_bptl29a[8,3], "(",round(100*as.numeric(crosstable.coll_bptl29a[8,3])/as.numeric(crosstable.coll_bptl29a[8,7]), digits=1), "%)")
crosstable.coll_bptl29a[8,4] <- paste(crosstable.coll_bptl29a[8,4], "(",round(100*as.numeric(crosstable.coll_bptl29a[8,4])/as.numeric(crosstable.coll_bptl29a[8,7]), digits=1), "%)")
crosstable.coll_bptl29a[8,5] <- paste(crosstable.coll_bptl29a[8,5], "(",round(100*as.numeric(crosstable.coll_bptl29a[8,5])/as.numeric(crosstable.coll_bptl29a[8,7]), digits=1), "%)")
crosstable.coll_bptl29a[8,6] <- paste(crosstable.coll_bptl29a[8,6], "(",round(100*as.numeric(crosstable.coll_bptl29a[8,6])/as.numeric(crosstable.coll_bptl29a[8,7]), digits=1), "%)")
crosstable.coll_bptl29a[8,7] <- paste(crosstable.coll_bptl29a[8,7], " (100%)")
...
Bld_urn_cli = knitr::kable(newtable_4, caption='Table 1.9: Baseline- Data System Status Among Clinical Biospecimen Collections by Specimen Type', row.names=FALSE,align=c("l","c","c"), booktabs = TRUE) %>%  kable_styling(latex_options = "hold_position") 
...
add_footnote(tab10_2, "Note: The expected total blood collection tubes vary by site and are as follows: KPHI-9, KPGA-7, KPNW-9, KPCO-8, HFH-5, HP-5, SFH-9", notation = "none") 
...
UC_Time <- ggplot(UC_Time_plot, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=Site, color=Site)) +
  ...
  #scale_x_date(name="Date", date_labels = function(date) ifelse(as.numeric(format(date, "%U")) %% 2 == 0, "", format(date, "%y-%m-%d")), #date_labels = "%y-%m-%d",  date_breaks = "1 weeks", limits=c( as.Date("2022-06-20"),(as.Date(currentDate)-1)),expand=c(0,0))  +
...
AF26 <- knitr::kable(verif_baseline_Af[c(1,2,7:9,3:6), ],caption = 'Table 4.7: Baseline- Time (in Days) from Verification to Collection Among Participants Verified on or After 6/01/2022 by Campaign Type',row.names=FALSE, booktabs = T, linesep = "", format.args = list(big.mark = ","), align = c("l", "c", "c","c", "c", "c", "c", "c", "c", "c", "c","c", "c","c"), col.names=c("Site",  'Number of Collections',"Min", "Median", "Mean", "SD", "Max", 'Number of Collections', "Min", "Median", "Mean", "SD", "Max","Total Baseline Collections") ) %>% kable_styling(latex_options = "scale_down") %>% add_indent(c(1:9)) %>% add_header_above(c(" " = 1, "Upcoming Appointment" = 6, "All Other Campaigns" = 6,  " "=1)) %>% landscape() 
...
research.vt1 <- ggplot(visit_wk_site1, aes(x=as.Date(col.Monday.date), y=n)) + #,fill=Site, color=Site)) +
  ...
  #scale_x_date(name="Date", date_labels = function(date) ifelse(as.numeric(format(date, "%U")) %% 2 == 0, "", format(date, "%y-%m-%d")), #date_labels = "%y-%m-%d",  date_breaks = "1 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)),expand=c(0,0))  +
...
VWS <- left_join(research.surveyVW, VW_biospe, by="Connect_ID")
 ...
 research.survey.hybrid <- Hybrid_VWS %>% filter(Site=="Henry Ford Health System" | Site=="HealthPartners"| Site=="Sanford Health") #<- change to site=HF, color=d_650516960
...
new_SF_plot <- ggplot(visit_wk_site.hybridSF, aes(x=as.Date(col.Monday.date), y=n, color=Setting )) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  ...
  scale_color_manual(values=c("Clinical"="#164C71", "Research"="#FDBE19")) +
  #scale_fill_brewer(palette="Dark2") +
  ggtitle(label=paste("Figure 6.10 Baseline- Weekly Biospecimen Collections by Collection Setting \n  Sanford Health")) +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
research.vt_plotSH <- ggplot(research.vt_plSH, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + 
  ...
  ggtitle(label="Figure 6.11: Baseline- Weekly Research Biospecimen Collections \n by Collection Location, Sanford Health") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
research.vt_plotHP <- ggplot(research.vt_plHP, aes(x=as.Date(col.Monday.date), y=n, color=Setting)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-7),expand=c(0,0))  +
  scale_color_manual(values=c("Clinical"="#164C71", "Research"="#FDBE19")) +
  ggtitle(label="Figure 6.12: Baseline- Weekly Biospecimen Collections \n by Collection Setting, HealthPartners") +
  labs(color = "Collection Setting") +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
research.vt_plotHP_locs <- ggplot(edited_fig11_1_Clin_tally, aes(x=as.Date(col.Monday.date), y=n, color=HP_cl_locs)) +  
  ...
  ggtitle(label="Figure 6.13: Baseline- Weekly Clinical Biospecimen Collections \n  by Clinic Location, HealthPartners ") +
  labs(color = "Clinic Location") +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
d_173836415_d_266600170_d_982213346,d_173836415_d_266600170_d_139245758, d_265193023, d_253883960 FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` where Connect_ID IS NOT NULL and d_906417725='104430631'  and d_747006172='104430631' and 
...
MW_overtime <- ggplot(fig5_week_total2, aes(x = as.Date(col.Monday.date))) +
  ...
  geom_ribbon(aes(ymin = 0, ymax = cumulative_received, fill = "Return Rate"), alpha = 0.3) +
    ...
    #             y = max(fig5_week_total2$cumulative_received),
    #             label = paste0(percentage_received_to_shipped, "%")),
    #        vjust = -0.5, hjust = 0, color = "black", size = 2.2) +
geom_text(data = fig5_week_total2 %>% filter(col.Monday.date == max(col.Monday.date)),
          aes(x = as.Date(col.Monday.date), 
  ...
  ggtitle(label = "Figure 7.3 Update title to: Baseline− Home Mouthwash Kit Return Rate \n for Kits Shipped Up To 1 Week Ago") +
...
knitr::kable(all_shipped2,caption='Table 7.2 Baseline- Home Mouthwash Kit Return Rate for Packages Shipped up to 1 Week Ago',row.names=FALSE, align=c("l","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "HOLD_position")
...
# Add the footnote using the footnote() function
tbl_7_9 <- tbl_7_9 %>%
  footnote("Only includes data from mouthwash kits returned with completed Collection Card")

# Use landscape() function to make the table landscape
landscape(tbl_7_9)
...
recrbio_1 <- recrbio_1[, c("Connect_ID","RcrtES_Site_v1r0","BioFin_BaseBloodCol_v1r0","BioFin_BaseUrineCol_v1r0","BioFin_BaseMouthCol_v1r0","BioSpm_BloodSettingBL_v1r0",
                           ...
                           "SrvMtW_TmComplete_v1r0", "SrvMtW_TmStart_v1r0", "SrvMtW_BaseComplete_v1r0", "token")]

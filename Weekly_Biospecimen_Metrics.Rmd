...
tb_bq <- eval(parse(text=paste("bq_project_query(project, query=\"SELECT token,Connect_ID,d_512820379,d_821247024,d_914594314,d_827220437,d_699625233, d_265193023, d_822499427, d_222161762,", select,"FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` Where d_821247024 = '197316935' and  (d_512820379='486306141' or d_512820379='854703046' or d_831041022 = '353358909')  and d_831041022 = '104430631'\")",sep=" "))) 
...
recrver1 <- recrver1 %>% mutate(BiospeCollection = ifelse((recrver1$d_878865966 == 353358909 | recrver1$d_167958071 == 353358909 | recrver1$d_684635302 == 353358909 ), "Any biospecimen Collections","No biospecimen collections")) 

recrver1$Site<- droplevels(recrver1$Site)



bio.appoint <- no_dups %>% arrange(Connect_ID, d_410912345, d_951355211) %>%
  group_by(Connect_ID) %>%
  summarise(
...
## New Table 1.2 for all collection setting counts

coll_sett_all <- biocol.tb %>%  filter(BiospeCollection == "Any biospecimen Collections" & (d_410912345== 353358909 | BL_BioSpm_MWSetting_v1r0=="Home")) %>% 
  mutate(all_settings = case_when((BL_BioSpm_BloodSetting_v1r0=="Clinical" | BL_BioSpm_UrineSetting_v1r0=="Clinical" |
                                    BL_BioSpm_MWSetting_v1r0=="Clinical" | d_650516960=="664882224") ~ "Clinical",
                                  (BL_BioSpm_BloodSetting_v1r0=="Research" | BL_BioSpm_UrineSetting_v1r0=="Research" |
                                    BL_BioSpm_MWSetting_v1r0=="Research" | d_650516960=="534621077" )~ "Research",
                                  (BL_BioSpm_BloodSetting_v1r0=="Home" | BL_BioSpm_UrineSetting_v1r0=="Home" |
                                    BL_BioSpm_MWSetting_v1r0=="Home" | d_650516960=="103209024") ~ "Home"))
...
settings_breakdown <- knitr::kable(table1_allsett, 
                                   caption='Table 1.2 Baseline Biospeicmen Collection Setting', 
...
col_num_1.1 <- col_num_1.1 %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` %in% c("Sioux Falls Imagenetics", "Fargo South University", "Missing SF location") ~ "Sanford Health",
    `Collection Location` %in% c("HFH K-13 Research Clinic", "HFH Livonia Research Clinic", "HFH Cancer Pavilion Research Clinic", "HFH Pop-Up", "Missing HFH location") ~ "Henry Ford Health",
    `Collection Location` %in% c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving","NTX Biorepository","BCC- Fort Worth", "Missing BSWH location") ~ "Baylor Scott and White",
    TRUE ~ "Other"
  ))
...
table1_2__locations <- table1_2_locations %>% mutate(Site_location=case_when(#d_173836415_d_266600170_d_185243482==1 | d_173836415_d_266600170_d_452847912==1 ~ "NSC",
  ...
  Site=="Henry Ford Health" & is.na(d_173836415_d_266600170_d_185243482) & is.na(d_173836415_d_266600170_d_452847912) ~ "Missing HF Location ID",
  Site=="HealthPartners" & is.na(d_173836415_d_266600170_d_185243482) & is.na(d_173836415_d_266600170_d_452847912) ~ "Missing HP Location ID",
  Site=="University of Chicago Medicine" & is.na(d_173836415_d_266600170_d_185243482) & is.na(d_173836415_d_266600170_d_452847912) ~ "Missing UC Location ID",
...
col_num_1.2 <- col_num_1.2 %>%
  mutate(Site_Group = case_when(
    `Collection Location` %in% c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", "K1 HFH Main","Sterling Heights","Columbus", 
                                 "Livonia","Taylor", "Ford Road", "NCO","Brownstown", "Missing HF Location ID") ~ "Henry Ford Health", 
    `Collection Location` %in% c("Elk River", "Chanhassen", "Missing HP Location ID") ~ "HealthPartners", #"NSC", 
    `Collection Location` %in% c("South Loop","Orland Park","Kenwood","River East","Dearborn Station", "Missing UC Location ID") ~ "University of Chicago Medicine",
    TRUE ~ "Other"
  ))
...
table1.5 <- create_tbl_cross(table2, 'Site', "BioFin_BaseBloodCol_v1r0", "row", 'Site', " ")
...
#table1_4_locations <- table1_2_locations %>%  filter(Site=="Sanford Health")
...
#Only those with at least 1 for 'In the Past 7 Days' or 'In the Past 30 Days'
#Arrange in decending count
SF_collections <- final_col_num_1.5 %>% filter(Count_7days>0 | Count_30days>0)
SF_collections <- SF_collections %>% arrange(Region, desc(Count_30days))
...
SF_collections_tbl <- SF_collections %>%
  ...
  kbl(caption = "Table 1.4: Baseline Clinical Biospecimen Collections in the Past 30 Days at Sanford Health by Clinic Location",
      booktabs = TRUE) %>%
...
SF_top10_collections <- top10 %>%
  ...
  kable_styling(latex_options = "scale_down", full_width = FALSE) %>%
  row_spec(0, bold = TRUE) %>%
  #row_spec(1:nrow(top10), extra_latex_after = "\\hline") %>%
...
table2_hybrid <- table2 %>%  filter(Site=="Henry Ford Health" | Site=="HealthPartners" | Site=="Sanford Health" | Site=="University of Chicago Medicine")
...
table4_redone__R <- table2 %>%  filter(d_650516960==534621077) %>% 
...
table4_redone__C <- table2 %>%  filter(d_650516960==664882224) %>% 
...
recrver1 <- recrver1 %>% mutate(BiospmCols_v1r0 = 
                                  case_when((d_684635302 == 353358909 & d_878865966 == 353358909 & d_167958071 == 353358909) ~ "All blood, urine and mouthwash",
                                            (d_684635302 == 353358909 & d_878865966 == 353358909 & d_167958071 == 104430631) ~ "Mouthwash and blood, no urine",
                                            (d_684635302 == 353358909 & d_878865966 == 104430631 & d_167958071 == 104430631) ~ "Only mouthwash,no blood or urine",
                                            (d_684635302 == 104430631 & d_878865966 == 353358909 & d_167958071 == 353358909) ~ "Blood and urine, no mouthwash",
                                            (d_684635302 == 104430631 & d_878865966 == 104430631 & d_167958071 == 353358909) ~ "Only urine, no blood or mouthwash",
                                            (d_684635302 == 353358909 & d_878865966 == 104430631 & d_167958071 == 353358909) ~ "Mouthwash and urine, no blood",
                                            (d_684635302 == 104430631 & d_878865966 == 353358909 & d_167958071 == 104430631) ~ "Only blood, no urine or mouthwash",
                                            ((d_684635302 == 104430631 | is.na(d_684635302)) & (d_878865966 == 104430631 | is.na(d_878865966)) & (is.na(d_167958071) |d_167958071 == 104430631)) ~ "No any biospecimen collections"))
...
table_colspm <- recrver1[which(recrver1$d_878865966 == 353358909 | recrver1$d_167958071 == 353358909 | recrver1$d_684635302 == 353358909 ),] %>%
...
no_dups <- expss::apply_labels(no_dups,d_827220437 = "Site",#RcrtES_Site_v1r0
                             d_827220437 = c("Baylor Scott and White Health"=472940358, "HealthPartners"= 531629870, "Henry Ford Health"=548392715,"Marshfield Clinic Health System" = 303349821,"Sanford Health" = 657167265, "University of Chicago Medicine" = 809703864, "Kaiser Permanente Colorado" = 125001209,"Kaiser Permanente Georgia" = 327912200,"Kaiser Permanente Hawaii" = 300267574,"Kaiser Permanente Northwest" = 452412599,"National Cancer Institute" = 517700004,"National Cancer Institute" = 13,"Other" = 181769837))

no_dups$Site <- factor(no_dups$d_827220437, levels= c("Baylor Scott and White Health", "HealthPartners", "Henry Ford Health","Marshfield Clinic Health System","Sanford Health", "University of Chicago Medicine","Kaiser Permanente Colorado","Kaiser Permanente Georgia","Kaiser Permanente Hawaii","Kaiser Permanente Northwest")) 


no_dups<- no_dups%>% arrange(Site)
...
tubeid <- grep("d_825582494", names(no_dups), value=TRUE) #tube ID
deviation <- grep("d_678857215",names(no_dups),value=TRUE) 
colid <- grep("d_593843561", names(no_dups), value=TRUE) #collected yes/no
discard <- grep("d_762124027",names(no_dups),value=TRUE) #discard yes/no
biodiscard <- no_dups[,(which(names(no_dups) %in% discard))]

no_dups$blddev <-0 #blood deviations
no_dups$bldcol <- 0 #blood collections
no_dups$blddid <- 0 #discarded
for(i in 1:length(bld.ls)){
  
  bldcol <- ifelse(no_dups[,paste(bld.ls[i],"d_593843561",sep="_")] == 104430631 | is.na(no_dups[,paste(bld.ls[i],"d_593843561",sep="_")]),0,1)  ##for tube collected
  
  blddev <- ifelse(no_dups[,paste(bld.ls[i],"d_678857215",sep="_")]==104430631 | is.na(no_dups[,paste(bld.ls[i],"d_678857215",sep="_")]),0,1) #for tube deviations yes/no
  
  blddid <- ifelse(no_dups[,paste(bld.ls[i],"d_762124027",sep="_")]==104430631 | is.na(no_dups[,paste(bld.ls[i],"d_762124027",sep="_")]),0,1)
  
  no_dups$blddev <- no_dups$blddev+blddev
  no_dups$bldcol <- bldcol + no_dups$bldcol ###to check collection
  no_dups$blddid <- no_dups$blddid+blddid  
  
}

table(no_dups$d_650516960,no_dups$bldcol)
no_dups<- no_dups%>% mutate(bloodcomplete = ifelse((d_650516960==534621077 & bldcol == 5) | (d_650516960==664882224 & bldcol == 11), "Blood.Completion" ,#blood collection completion
...
no_dups$Biospe_col <- no_dups$bldcol + no_dups$Urine_col + no_dups$MW_col    
no_dups$BaseBLDCol <-ifelse(no_dups$bldcol>0,1,ifelse(is.na(no_dups$bldcol),NA,0)) #blood collection Yes/No
no_dups[duplicated(no_dups$Connect_ID),]$Connect_ID
...
### Table 5
tube_col <- no_dups %>% group_by(as.character(Connect_ID),Site,d_650516960) %>% 
...
#### U-Chicago
no_dups= no_dups%>%  mutate(UC_Sites= case_when(d_951355211==777644826 ~ "UC-DCAM",
                                               d_951355211==145191545 ~ "Ingalls Harvey",
                                               d_951355211==489380324 ~ "River East",
                                               d_951355211==120264574 ~ "South Loop",
                                               d_951355211==319518299 ~ "UCM Pop-Up",
                                               d_951355211==940329442 ~ "Orland Park"))
tube_col_UC <- no_dups  %>% filter(d_827220437==809703864) %>% group_by(as.character(Connect_ID),UC_Sites,d_650516960) %>% 
...
### HF Research
no_dups= no_dups %>% mutate(HF_Sites= case_when(d_951355211==736183094 ~ "HFH K-13 Research Clinic",
                                               d_951355211==886364332 ~ "HFH Cancer Pavilion Research Clinic",
                                               d_951355211==706927479 ~ "HFH Livonia Research Clinic",
                                               d_951355211==322059622 ~ "HFH Pop-Up"))
tube_col_HF <- no_dups %>% group_by(as.character(Connect_ID),HF_Sites,d_650516960) %>% 
...
biocounts <- no_dups%>%  select(Connect_ID, bldcol, d_556788178, d_650516960)
HFSITES <- table1_2__locations %>%  filter(Site=="Henry Ford Health") %>% select(Connect_ID, Site_location )
biospe7 <- left_join(HFSITES, biocounts, by="Connect_ID")

tube_col_HF2 <- biospe7  %>% group_by(Connect_ID,Site_location,d_650516960) %>%
  dplyr::summarise(bldcol_max = max(bldcol,na.rm=TRUE),
                   collection.tm =max(as.POSIXct(ymd_hms(d_556788178)),na.rm=TRUE))
...
tube_research_HF2 <- tube_col_HF2 %>% 
  ...
  select(Site_location, bld_complt) %>%
  ungroup()  # Explicitly ungroup to avoid the Connect_ID issue



bloodcomp_clinloc <- create_tbl_cross(tube_research_HF2, "Site_location", "bld_complt", "row", "Clinical Location", " ")
...
tube_completeness <- tube_clinic %>% ungroup() %>% 
...
#bloodtubes_bysite <- create_tbl_cross(tube_clinic, "Site", "bldcol_tubes", "row", "Site", " ")
...
# ## New table 4 8/22/23
clin_col <- no_dups%>% select(Connect_ID, d_650516960, d_410912345)
...
no_dups$tubedev <-0 #biospecimen deviations
no_dups$tubecol <- 0 #tube collections
no_dups$tubedid <- 0 #tube discarded
for(i in 1:length(tube.ls)){
  
  tubecol <- ifelse(no_dups[,paste(tube.ls[i],"d_593843561",sep="_")] == 104430631 | is.na(no_dups[,paste(tube.ls[i],"d_593843561",sep="_")]),0,1)  ##for tube collected
  
  tubedev <- ifelse(no_dups[,paste(tube.ls[i],"d_678857215",sep="_")]==104430631 | is.na(no_dups[,paste(tube.ls[i],"d_678857215",sep="_")]),0,1) #for tube deviations yes/no
  
  tubedid <- ifelse(no_dups[,paste(tube.ls[i],"d_762124027",sep="_")]==104430631 | is.na(no_dups[,paste(tube.ls[i],"d_762124027",sep="_")]),0,1)
  
  no_dups$tubedev <- no_dups$tubedev+tubedev
  no_dups$tubecol <- tubecol + no_dups$tubecol ###to check collection
  no_dups$tubedid <- no_dups$tubedid+tubedid  
}
table(no_dups$tubecol)
table(no_dups$tubedev)
no_dups$d_827220437 <- as.numeric(no_dups$d_827220437)
no_dups<- apply_labels(no_dups,d_827220437 = "Site",#RcrtES_Site_v1r0
                       d_827220437 = c("Baylor Scott and White Health"=472940358, "HealthPartners"= 531629870, 
                                       "Henry Ford Health"=548392715, "Kaiser Permanente Colorado" = 125001209,
...
no_dups$Site <-factor(no_dups$d_827220437,levels=c("Baylor Scott and White Health","HealthPartners", "Henry Ford Health",
                                                 "Marshfield Clinic Health System",
                                                 "Sanford Health", "University of Chicago Medicine","Kaiser Permanente Colorado",
                                                 "Kaiser Permanente Georgia","Kaiser Permanente Hawaii","Kaiser Permanente Northwest",
                                                 "National Cancer Institute","Other"))
no_dups$tube_nodev <- no_dups$tubecol-no_dups$tubedev
total1 <- sum(no_dups$tubedev) 
total2 <- sum(no_dups$tube_nodev) 
tube_dev <- as.data.frame(aggregate( no_dups$tubedev,                # Specify data column[,c("tubedev","tube_nodev")]
                                     by = list(no_dups$Site),              # Specify group indicator
                                     FUN = sum))
tube_nodev <- as.data.frame(aggregate( no_dups$tube_nodev,                # Specify data column[,c("tubedev","tube_nodev")]
                                       by = list(no_dups$Site),              # Specify group indicator
                                       FUN = sum))
...
Deviation_Freq[10,c(1:6)] <- c("Total",total1, total2,format(sum(no_dups$tubecol),big.mark=","),
...
for (i in 1:length(tube.ls)){
  ...
  deviation <- grep(tube_dev,colnames(no_dups),value=TRUE)
  tmp <- as.data.frame(no_dups[,which(colnames(no_dups) %in% c(id,tube,deviation))] )
...
notcol_tubes <- colnames(no_dups)[grepl("883732523",colnames(no_dups))] #7
col_tubes <- colnames(no_dups)[grepl("d_593843561",colnames(no_dups))] #13
dev_tubes <- colnames(no_dups)[grepl("d_678857215",colnames(no_dups))] #13
...
for (i in 1:length(notcol.ls)){
  ...
  tmp <- no_dups[,(colnames(no_dups)%in% c(select,"d_820476880","Site","biocol_Setting"))] 
  notcol <- tmp 
...
#bloodtube_R <- create_tbl_cross(Tb.research.coltube, "Site", "bldcol_tubes", "row", "Site", " ")
...
research.survey$checktmvisit <- factor(research.survey$checktmvisit, levels=c( "Not Submitted", "Missing Time", "0 hrs to 3 hrs","> 3 hrs to 12 hrs",
...
research.surveyHF <- research.survey %>%  filter(Site=="Henry Ford Health")
...
#17- by site
test17 <- research.survey %>%  filter(Site=="HealthPartners" |  Site=="Henry Ford Health" | Site=="Marshfield Clinic Health System" | 
                                        Site=="Sanford Health" | Site=="University of Chicago Medicine" | Site=="Baylor Scott and White Health")
test17$Site<- droplevels(test17$Site)



BUM_status <- create_tbl_cross(test17, "research.sv", "Site", "column", " ", " ")
...
clinic.survey$checktmvisit <- factor(clinic.survey$checktmvisit, levels=c("Not Submitted", "Missing Time", "0 hrs to 3 hrs","> 3 hrs to 12 hrs",
                                                                          "> 12 hrs to 24 hrs","> 24 hrs to 48 hrs","> 48 hrs to 72 hrs","> 72hrs"))
...
BU_status <- create_tbl_cross(test18, "clinical.sv", "Site", "column", " ", " ")
...
precovid_BUMtime <- create_tbl_cross(test17_b, "checktmvisit", "Site", "column", " ", " ")
...
postcovid_BUMtime <- create_tbl_cross(test17_a, "checktmvisit", "Site", "column", " ", " ")
...
coll_comp_BU <- create_tbl_cross(test18a_B, "checktmvisit", "Site", "column", " ", " ")
...
sitelocation_20a <- no_dups%>%  select(Connect_ID, d_951355211)
...
tab22 <- left_join(recrver1, no_dups, by=c("Connect_ID", "Site"))
...
tab24 <- left_join(recrver1, no_dups, by=c("Connect_ID", "Site")) %>% left_join(parts24_data, by="Connect_ID")
...
verif_baseline_Af <- verif_baseline_Af %>%  select(Site,  'Number of Baseline Collections1', Min1, Median1, Mean1, SD1, Max1, 
...
bio.appoint <- no_dups%>% arrange(Connect_ID, d_410912345) %>% group_by(Connect_ID) %>% summarise(d_410912345=mean(d_410912345,na.rm=TRUE)) #to remove the multiple collections by Connect_ID
...
bldorder_present <- recrver1 %>%  filter(d_878865966==104430631 & !is.na(d_173836415_d_266600170_d_769615780) & Site!='HealthPartners' & Site!='Henry Ford Health'  & Site!='Sanford Health') %>%  
...
str_sub(no_dups$d_143615646_d_926457119,12,24)[!is.na(no_dups$d_143615646_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_232343615_d_926457119,12,24)[!is.na(no_dups$d_232343615_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_299553921_d_926457119,12,24)[!is.na(no_dups$d_299553921_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_376960806_d_926457119,12,24)[!is.na(no_dups$d_376960806_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_454453939_d_926457119,12,24)[!is.na(no_dups$d_454453939_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_505347689_d_926457119,12,24)[!is.na(no_dups$d_505347689_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_589588440_d_926457119,12,24)[!is.na(no_dups$d_589588440_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_652357376_d_926457119,12,24)[!is.na(no_dups$d_652357376_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_677469051_d_926457119,12,24)[!is.na(no_dups$d_677469051_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_683613884_d_926457119,12,24)[!is.na(no_dups$d_683613884_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_703954371_d_926457119,12,24)[!is.na(no_dups$d_703954371_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_838567176_d_926457119,12,24)[!is.na(no_dups$d_838567176_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_958646668_d_926457119,12,24)[!is.na(no_dups$d_958646668_d_926457119)] <- "11:30:00.000Z"
str_sub(no_dups$d_973670172_d_926457119,12,24)[!is.na(no_dups$d_973670172_d_926457119)] <- "11:30:00.000Z"

#RESEARCH 
biospe_adhoc= no_dups%>%  filter(d_650516960==534621077 & d_410912345==353358909 & !is.na(d_678166505) & 
                                   #BPTL not null
...
biospe_adhoc$Site_location <- factor(biospe_adhoc$Site_location,
                                     levels=c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving",
                                              ...
                                              "HP Research Clinic", "HP Park Nicollet",
...
table5.2 <- left_join(no_dups, table2_col_bplt, by="Connect_ID")

#table5.2 <- left_join(table1_2_locations, no_dups,  by=c("Connect_ID", "Site", "d_410912345", "d_650516960", "d_951355211"))
...
biospe_adhoc2$Site_location <- factor(biospe_adhoc2$Site_location,levels=c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", 
                                                                           "Royal Oak", "Troy", "K1 HFH Main","Sterling Heights",
                                                                           "Columbus",  "Livonia","Taylor", "Ford Road", "NCO", 
                                                                           "Brownstown", #"Missing Location ID",#"NSC", 
                                                                           "Chanhassen", "Elk River", "Kaiser Permanente Colorado",
                                                                           "Kaiser Permanente Georgia", "Oahu Clinics", "Non-Oahu Clinics", 
...
no_dups$Connect_ID <- as.numeric(no_dups$Connect_ID)
table54 <- left_join(table1_4_locations, no_dups,  by=c("Connect_ID", "Site")) 
...
#SF_coll_rcpt <- create_tbl_cross(coll_bptl_diff5_4, "Site_location", "coll_days_diff", "row", "Collection Location", " ")
...
#add_footnote(settings_breakdown, "Note: Any collections indicated under 'Home' setting reflect collections in which home mouthwash was the only sample collected.", notation = "none")
  
```
\FloatBarrier
```{r KnitrTables1.2_1.3, eval=TRUE,echo=FALSE,error=FALSE, message=FALSE,warning=FALSE,results='asis'}
options(knitr.table.format = "latex")
options(kableExtra.auto_format = FALSE)
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
...
add_footnote(SF_collections_tbl, 
             ...
             paste0("This table reflects Clincal Collections in the past 30 days; the total number of Clinical Collections at Sanford Health is ",
                    sum(count_all$Count_Total)),
             notation = "none")

add_footnote(SF_top10_collections, paste0("This table reflects Clincal Collections at the top collection locations in each region. The total number of Clinical Collections in each region are as follows: Bemidji: ", Be_count,
"; Bismarck: ", Bi_count, "; Fargo: ", F_count, "; Sioux Falls: ", S_count, "."), notation = "none")
...
\FloatBarrier
```{r KnitrTables1.6, eval=TRUE,echo=FALSE,warning=FALSE, message=FALSE, results='asis'}
...
\FloatBarrier
```{r KnitrTables1.7, eval=TRUE,echo=FALSE,warning=FALSE, message=FALSE, results='asis'}
...
Bld_urn_cli = knitr::kable(newtable_4, 
                           caption='Table 1.12: Baseline- Data System Status Among Clinical Biospecimen Collections by Specimen Type', 
...
tab2_7= knitr::kable(research.coltube_totals, 
  ...
  add_indent(seq(1, nrow(research.coltube_totals) - 1))  %>% kable_styling(latex_options = "scale_down")
...
knitr::kable(discard.site, 
             caption='Table 2.14: Baseline- Discarded Tubes Among Biospecimen Collections by Tube Type', row.names=FALSE,  col.names=c("Site",  "SST 1", "SST 2", "SST 5", "Heparin 1", "Heparin 2", "EDTA 1, 2, or 3", "ACD", "Urine", "Total Discarded Tubes"), align=c("l","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")
...
knitr::kable(crosstable.coll_bptl28a, 
...
needle5_2 <- knitr::kable(crosstable.coll_bptl28, 
...
knitr::kable(crosstable.coll_bptl29a, 
             ...
             row.names=FALSE, align=c("l","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 


coll_bplt_knitr2 <- knitr::kable(crosstable.coll_bptl_2, 
                                 col.names=c("Collection Location", "1 Day", "2 Days", "3 Days", "4 Days", ">4 Days", "Total Recived Clinical Collections"), 
                                 caption='Table 5.4: Baseline Collection-To-Receipt Time (in Days) Among Clinical Collections by Clinic Location', 
                                 row.names=FALSE, align=c("l","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
table_ct5_4 <- knitr::kable(ct5_4 , 
                            caption="Table 5.4a Baseline Collection-To-Receipt Time (in Days) Among Clinical Collections by Clinic Location \n Sanford Health",
                            row.names=FALSE,align=c("l","c","c","c","c"), booktabs = TRUE)
table_ct5_4 <- table_ct5_4 %>% row_spec(1:(nrow(ct5_4) - 1), extra_css = "text-indent: 20px;")
kable_styling(table_ct5_4, latex_options = "scale_down", font_size=8)



coll_bplt_knitr3 <- knitr::kable(Hrs_bptl_col, 
...
coll_bplt_knitr4 <- knitr::kable(Hrs_bptl_col4, 
                                 col.names=c("Collection Location", 'Min', 'Q1', 'Median', 'Mean', 'Q3','Perc.90','Perc.98'), 
                                 caption='Table 5.6: Baseline- Time (in Hours) from Clinical Collection to Receipt by Clinic Location', 
                                 row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
#Hybird version
VW_biospe <- no_dups%>%  select(d_951355211, Connect_ID)
...
research.survey.hybrid <- Hybrid_VWS %>% filter(Site=="Henry Ford Health" | Site=="HealthPartners"| Site=="Sanford Health" | Site=="University of Chicago Medicine") #<- change to site=HF, color=d_650516960
...
edited_fig3_1_Clin <- clinic.survey %>% filter(d_827220437==548392715) %>% 
  mutate(HF_Sites= case_when(d_173836415_d_266600170_d_185243482==1050010095 | d_173836415_d_266600170_d_452847912==1050010095 |
                               d_173836415_d_266600170_d_185243482 == 1050010034 | d_173836415_d_266600170_d_452847912 == 1050010034 ~ "Wyandotte",
                             d_173836415_d_266600170_d_185243482==1060010063 | d_173836415_d_266600170_d_185243482==1060170018 |
                               d_173836415_d_266600170_d_452847912==1060010063 | d_173836415_d_266600170_d_452847912==1060170018~ "Macomb",
                             d_173836415_d_266600170_d_185243482==1040010047 | d_173836415_d_266600170_d_185243482==1040010046 |
...
# Create a sequence of dates for the range you are interested in
date_range <- seq.Date(from = as.Date("2024-05-20"), to = as.Date(currentDate)-7, by = "week")
clinic_locations <- c("South Loop", "Orland Park", "Kenwood", "River East", "Dearborn Station")

clinical.vt_plUC_locations <- clinic.survey %>%
  filter(Site == "University of Chicago Medicine") %>%
  mutate(UC_cl_locs = case_when(
    d_173836415_d_266600170_d_185243482 == 1 | d_173836415_d_266600170_d_452847912 == 1 ~ "South Loop",
    d_173836415_d_266600170_d_185243482 == 2 | d_173836415_d_266600170_d_452847912 == 2 ~ "Orland Park",
    d_173836415_d_266600170_d_185243482 == 3 | d_173836415_d_266600170_d_452847912 == 3 ~ "Kenwood",
    d_173836415_d_266600170_d_185243482 == 4 | d_173836415_d_266600170_d_452847912 == 4 ~ "River East",
    d_173836415_d_266600170_d_185243482 == 5 | d_173836415_d_266600170_d_452847912 == 5 ~ "Dearborn Station"
...
edited_fig11_1_Clin_tally <- clinical.vt_plUC_locations %>%
  arrange(UC_cl_locs, col.Sunday.week, col.Monday.date) %>%
  group_by(UC_cl_locs, col.Sunday.week, col.Monday.date) %>%
  tally()

# Complete the data to include all combinations of UC_cl_locs and dates in the range
complete_data <- edited_fig11_1_Clin_tally %>%
  ungroup() %>%
  complete(UC_cl_locs = clinic_locations, col.Monday.date = date_range, fill = list(n = 0))

clinical.vt_plotUC_locs <- ggplot(complete_data, aes(x = as.Date(col.Monday.date), y = n, color = UC_cl_locs)) +  
  geom_line() + geom_point(width = 0.2) +
...
## Need to run chunks: Table6, Table14, Table15, Figures16_18, 
...
knitr::kable(OrlandAH, 
             caption='Ad Hoc UChicago Orland Park Visit Count Per Week', 
             row.names=FALSE,align=c("l","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "HOLD_position")
```
...
#### THIS LOOKS LIKE THE CORRECT TABLE 2!!!

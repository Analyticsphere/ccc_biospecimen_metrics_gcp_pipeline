...
#------- Convert tbl cross to a dataframe and remove blank first row
...
create_tbl_cross2 <- function(data, row_var, colm_var, row_or_col_pct, row_name, column_name,
                              margin_text = "Total") {
    cross__table <- data %>%
        tbl_cross(
            row = !!sym(row_var),
...
summarize_time <- function(data, time_var, n_label = "N",
                           group_var = NULL,
                           filter_column = NULL,
                           filter_value = NULL,
                           std_dv  = FALSE,
  ...
  group_sym <- if (!is.null(group_var)) rlang::ensym(group_var) else NULL

  # Optional filter step
  if (!is.null(filter_column) && !is.null(filter_value)) {
    filter_sym <- rlang::ensym(filter_column)
    data <- data %>% dplyr::filter(!!filter_sym == filter_value)
  }

  # Optional grouping step
  if (!is.null(group_sym)) {
    data <- data %>% dplyr::group_by(!!group_sym)
  }
...
reason_notcol <- create_tbl_cross(research.notcol, "Site", "notcol.notes", "row", "Site", " ")
...
discard.site <- create_tbl_cross(discard.tb, "Site", "tube_type", "row", "Site", " ")
...
Shipped_TD <- create_tbl_cross(all_shipped_to_date, "ship_timeframe", "Site", "column", " ", " ")

...
biospe_locations <- biospe_locations %>% filter(!is.na(Site_location)) %>%  select(Site, Site_location, Connect_ID)

table1_1_locs <- left_join(table1, biospe_locations, by=c("Site", "Connect_ID"))
...
col_num_1.1 <- col_num_1.1 %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` == "HP Research Clinic" ~ "HealthPartners",
...
biospe_site<-biospe %>% filter(d_650516960== 664882224) %>% 
                            mutate(Site=case_when(siteAcronym=="HFHS" ~ "Henry Ford Health System",
                                                   siteAcronym=="HP" ~ "HealthPartners",
                                                   siteAcronym=="KPCO" ~ "Kaiser Permanente Colorado",
                                                   siteAcronym=="KPGA" ~ "Kaiser Permanente Georgia",
                                                   siteAcronym=="KPHI" ~ "Kaiser Permanente Hawaii",
...
Clin_Site_Loc <- recrver1 %>% select(Site, Connect_ID, d_173836415_d_266600170_d_185243482, d_173836415_d_266600170_d_452847912)
biospe1_2 <- biospe_site %>% select(d_650516960, Connect_ID, Site)

table1_2_locs <- left_join(table1, Clin_Site_Loc, by=c("Site", "Connect_ID")) %>% left_join(biospe1_2, by=c("Site", "Connect_ID"))
...
table1_2__locations <- table1_2_locations %>% mutate(Site_location=case_when(d_173836415_d_266600170_d_185243482==1 | d_173836415_d_266600170_d_452847912==1 ~ "NSC",
                                                                             d_173836415_d_266600170_d_185243482 == 2 |
                                                                               d_173836415_d_266600170_d_452847912 == 2 ~ "Elk River",
                                                                             d_173836415_d_266600170_d_185243482 == 3 |
                                                                               d_173836415_d_266600170_d_452847912 == 3 ~ "Chanhassen",
                                                                             ...
                                                                             Site== "Sanford Health" & d_650516960== 664882224 ~ "Sanford Health"))
...
col_num_1.2 <- col_num_1.2 %>%
  mutate(Site_Group = case_when(
    `Collection Location` %in% c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", "K1 HFH Main","Sterling Heights","Columbus", "Livonia","Taylor", "Ford Road", "NCO") ~ "Henry Ford Health System",
...
biospe_adhoc2$Site_location <- factor(biospe_adhoc2$Site_location,levels=c("Macomb","West Bloomfield","Wyandotte","Fairlane (Dearborn)","Plymouth", "Royal Oak","Troy", "Taylor",
                                                  "K1 HFH Main","Sterling Heights","Columbus", "Livonia" ,"Ford Road", "NCO","Chanhassen", "Elk River", "Kaiser Permanente Colorado", 
...
# new_SF_plot <- ggplot(visit_wk_site.hybridSF, aes(x=as.Date(col.Monday.date), y=n, color=Setting )) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
#   geom_line() + geom_point(width = 0.2) +
#   scale_y_continuous(name="# Number Collections") +
#   scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
#   scale_color_manual(values=c("Clinical"="#164C71", "Research"="#FDBE19")) +
#   #scale_fill_brewer(palette="Dark2") +
#   ggtitle(label=paste("Figure 6.10 (New) Baseline- Weekly Biospecimen Collections by Collection Setting \n  Sanford Health")) +
#   #theme_ipsum() +
#   theme(panel.background = element_blank(),
#         #legend.title = element_text(size=7),
#         legend.position = "bottom",
...
d_265193023, d_253883960 FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` where Connect_ID IS NOT NULL and d_906417725='104430631'  and d_747006172='104430631' and 
...
kit_sett <- left_join(recrver1, kit_set,   by="Connect_ID") %>% left_join(location_stat,  by="Connect_ID")
...
table27_elg <- kit_sett %>% filter(!(d_173836415_d_266600170_d_982213346<as.Date("2024-04-01") |d_173836415_d_266600170_d_139245758<as.Date("2024-04-01")))
...
```{r Table_new_7.2, eval=TRUE,echo=FALSE,results='hold'}
#,results='hold'

all_shipped <- as.data.frame(table(table27_elg_distinct$Site[!is.na(table27_elg_distinct$d_173836415_d_266600170_d_319972665_d_661940160)]))
all_recvd <- as.data.frame(table(table27_elg_distinct$Site[table27_elg_distinct$Kit_Status=="Received"]))

all_shipped <- merge(all_shipped, all_recvd, by="Var1")
...
#all_shipped[(as.numeric(nrow(all_shipped))+1),] <- c("Total", sum(all_shipped$Shipped), paste(all_shipped$Received, " (100%)"))
total_shipped <- sum(all_shipped$Shipped)
total_received <- sum(all_shipped$Received)
total_received_percentage <- paste(total_received, "(", round(total_received/total_shipped*100, digits = 1), "%)")
total_row <- c("Total", total_shipped, total_received)
...
for(i in 1:nrow(all_shipped2)){
  all_shipped2[i,"Received"] <- paste0(all_shipped2[i,"Received"], " (", round(as.numeric(all_shipped2[i,"Received"])/as.numeric(all_shipped2[i,"Shipped"])*100, digits = 1), "%)")
}




colnames(all_shipped2) <- c("Site", "Total Shipped", "Received")
all_shipped2

```


```{r new_Figs7.3, eval=TRUE,echo=FALSE, results='hold'}


fig7_3 <- ggplot(all_shipped, aes(x = Site)) +
  geom_bar(aes(y = Shipped, fill = "Shipped"), stat = "identity", width=0.8) +
  ...
  ggtitle("Figure 7.3. Total Kits Received of Total Kits Shipped, by Site")
...
```{r Table7.3_7.6, eval=TRUE,echo=FALSE,include=FALSE,results='hold'}


table27_elg_distinct$d_547363263 <- as.character(table27_elg_distinct$d_547363263)
kit_assembly$d_547363263 <- as.character(kit_assembly$d_547363263)
all_completed <- left_join(table27_elg_distinct,kit_assembly, by=c("Connect_ID", "Site", "d_547363263", "Kit_Status", "d_173836415_d_266600170_d_319972665_d_661940160")) 
...
#all_completed2 <- left_join(table27_elg_distinct,kit_date, by=c("Connect_ID", "d_173836415_d_266600170_d_319972665_d_661940160", "Site", "Kit_Status", "d_143615646_d_826941471"))
...
Table7.4 = test30b %>% group_by(Site) %>% 
  dplyr::summarize(N=n(),
                   Min = min(ctime),
                   Q1 = quantile(ctime, 0.25),
                   Median = median(ctime),
...
```{r FigMW1, eval=TRUE,echo=FALSE, include=FALSE}
kit_set_col <- kit_set %>% select(Connect_ID, d_143615646_d_826941471)
tube_col_plots2 <- tube_col_plots %>%  select(Connect_ID, col.Sunday.date,col.Sunday.week, col.Monday.date)
kit_fig <- left_join(kit_table,  tube_col_plots2, by="Connect_ID") %>% left_join(kit_set_col, by="Connect_ID")
...
### THIS IS A BANDAID UNTIL I CAN CORRECTLY GET THE SHIP/REC'V DATES MAPPED TO THE RIGHT MONDAYS!!!


##Date of shipment aren't being mapped to the right Monday. First retruned kit on 4/05/24
fig5_new$col.Monday.date[as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) <"2024-04-08" & as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) >"2024-03-31"]="2024-04-01"
fig5_new$col.Monday.date[as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) <"2024-04-15" & as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) >"2024-04-7"]="2024-04-08"
fig5_new$col.Monday.date[as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) <"2024-04-22" & as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) >"2024-04-14"]="2024-04-15"
fig5_new$col.Monday.date[as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) <"2024-04-29" & as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) >"2024-04-21"]="2024-04-22"
fig5_new$col.Monday.date[as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) <"2024-05-6" & as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) >"2024-04-28"]="2024-04-29"
fig5_new$col.Monday.date[as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) <"2024-05-13" & as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) >"2024-05-05"]="2024-05-06"
fig5_new$col.Monday.date[as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) <"2024-05-20" & as.Date(fig5_new$d_173836415_d_266600170_d_319972665_d_661940160) >"2024-05-12"]="2024-05-13"


##None of the received dates aren't being mapped to the right Monday. First retruned kit on 4/12/24
fig5_new$col.Monday.date[as.Date(fig5_new$d_143615646_d_826941471) <"2024-04-15"]="2024-04-08"
fig5_new$col.Monday.date[as.Date(fig5_new$d_143615646_d_826941471) <"2024-04-22" & as.Date(fig5_new$d_143615646_d_826941471) >"2024-04-14"]="2024-04-15"
fig5_new$col.Monday.date[as.Date(fig5_new$d_143615646_d_826941471) <"2024-04-29" & as.Date(fig5_new$d_143615646_d_826941471) >"2024-04-21"]="2024-04-22"
fig5_new$col.Monday.date[as.Date(fig5_new$d_143615646_d_826941471) <"2024-05-6" & as.Date(fig5_new$d_143615646_d_826941471) >"2024-04-28"]="2024-04-29"
fig5_new$col.Monday.date[as.Date(fig5_new$d_143615646_d_826941471) <"2024-05-13" & as.Date(fig5_new$d_143615646_d_826941471) >"2024-05-05"]="2024-05-06"
fig5_new$col.Monday.date[as.Date(fig5_new$d_143615646_d_826941471) <"2024-05-20" & as.Date(fig5_new$d_143615646_d_826941471) >"2024-05-12"]="2024-05-13"
...
fig5_week_total <- fig5_new %>%
  ...
  summarise(total_shipped = sum(!is.na(d_173836415_d_266600170_d_319972665_d_661940160)),
            total_received = sum(!is.na(d_143615646_d_826941471))) # Sum only for "Shipped" and "Received" statuses


MW_Coll_Kits <- ggplot(fig5_week_total, aes(x = as.Date(col.Monday.date))) +
...
```{r FigMW2, eval=TRUE,echo=FALSE, include=FALSE}

# Group by col.Monday.date and calculate cumulative sums
fig5_week_total2 <- fig5_week_total %>%
  group_by(col.Monday.date) %>%
  mutate(cumulative_shipped = cumsum(total_shipped),
         cumulative_received = cumsum(total_received))

# Calculate weekly percentage of ever received to ever shipped
fig5_week_total2 <- fig5_week_total2 %>%
  mutate(percentage_received_to_shipped = cumulative_received / cumulative_shipped * 100)
 

# Calculate cumulative sums of total_shipped and total_received
fig5_week_total2 <- fig5_new %>%
  group_by(col.Monday.date) %>%
  summarise(total_shipped = sum(!is.na(d_173836415_d_266600170_d_319972665_d_661940160)),
            total_received = sum(!is.na(d_143615646_d_826941471))) %>%
  ungroup() %>%
  mutate(cumulative_shipped = cumsum(total_shipped),
         cumulative_received = cumsum(total_received))
fig5_week_total2 <- fig5_week_total2 %>% group_by(col.Monday.date) %>% mutate(cum_perc= ((cumulative_received / cumulative_shipped) * 100))


last_observation <- fig5_week_total2 %>%
  group_by(col.Monday.date) %>%
  slice(n()) %>%
  ungroup()
...
latest_date <- max(last_observation$col.Monday.date) - 7
...
MW_overtime <- ggplot(fig5_week_total2, aes(x = as.Date(col.Monday.date))) +
  ...
  geom_text(data = last_observation[last_observation$col.Monday.date == latest_date,], 
            aes(y = cum_perc, label = paste0(round(cum_perc), "%")), 
            vjust = -0.5, hjust = 0, color = "#0072B2", size = 3) +
  scale_x_date(name = "Date", date_labels = "%y-%m-%d", date_breaks = "2 weeks",
               limits = c(as.Date("2024-03-25"), as.Date(Sys.Date()-7)), expand = c(0,0)) +
  scale_y_continuous(name = "# Kits Shipped") +
  scale_fill_manual(name = "", values = c("Total Shipped" = "#CC7D15", "Return Rate" = "#0072B2")) +
  ggtitle(label = "Figure 7.2 Baseline- Home Mouthwash Kit Return Rate") +
  labs(caption = paste("Note: Shipped data are current through ",as.Date(Sys.Date()-7),".")) +
  theme(panel.background = element_blank(),
        legend.position = "bottom",
        axis.line = element_line(size = 0.2),
        axis.text.x = element_text(angle = 60, hjust = 1),
...
\FloatBarrier
```{r newTables7.1,echo=FALSE,error=FALSE,message=FALSE,warning=FALSE, results='asis'}
#results='asis'
...
\FloatBarrier
```{r KnitrTables7.2,echo=FALSE,error=FALSE,message=FALSE,results='asis',  warning=FALSE}

knitr::kable(all_shipped2,caption='Table 7.2 Baseline- Home Mouthwash Kit Return Rate',row.names=FALSE, align=c("l","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "HOLD_position")
...
\FloatBarrier
```{r KnitrTables7.3,echo=FALSE,error=FALSE,message=FALSE,results='asis',  warning=FALSE}
...
\FloatBarrier
```{r KnitrTables7.4_7.6,echo=FALSE,error=FALSE,message=FALSE,  warning=FALSE, results='asis'}



knitr::kable(Table7.4,caption='Table 7.4: Baseline- Time (in Days) from Home Mouthwash Kit Shipment to BPTL Receipt',row.names=FALSE, align=c("l","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "HOLD_position")
...
```{r Tables7.7_7.9, eval=TRUE,echo=FALSE,results='asis'}
...
table7_9 <- all_completed %>% filter(Kit_Status=="Received" & d_137401245==104430631) %>%
  mutate(MWSrvy_status = case_when(d_547363263==972455046 ~ "Not Started",
                                   d_547363263==615768760 ~ "Started",
                                   d_547363263==231311385 ~ "Submitted"),
         MW_completion = difftime(as.Date(d_173836415_d_266600170_d_448660695.x),as.Date(d_195145666.x),units="days"),

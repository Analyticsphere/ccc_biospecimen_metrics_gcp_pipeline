...
that are finalized. This report is limited to information from participants' first finalized specimen collection. In the event that a participant donated specimen(s) in a repeat collection, that information is not reflected in this report.
...
### Each week must start on Monday and end with the following Sunday to be consistent with all reports
...
#Ops doesn't want the total column to have percentages in these tables
...
clinic.coltube<- clinic.coltube %>% filter(biocol_Setting == "Clinical" & tubecol == 353358909 & tube_types!="Urine 1") %>% # & tube_type!="MW Tube1") %>% 
  mutate(Site=droplevels(Site),
...
#order SSTs, EDTAs, Heaparin, Streck
...
precovR <- research.survey_sec3 %>%  filter(research.sv == "Submitted" & d_222161762<as.Date("2023-07-05"))

postcovR <- research.survey_sec3 %>%  filter(research.sv == "Submitted" & d_222161762>=as.Date("2023-07-05"))
...
research.vt.length.UC <- research.vt.length %>% filter(Site=="University of Chicago Medicine") %>% 
  mutate(Site_location = case_when(d_951355211==777644826 ~ "UC-DCAM",
                                   ...
                                   d_951355211==319518299 ~ "UCM Pop-Up",
                                   d_951355211==940329442 ~ "Orland Park"))
research.vt.length.UC.table = research.vt.length.UC %>%group_by(Site_location) %>%
  dplyr::summarize('Number of Research Collection Visits'=n(),
                   Min = min(time,na.rm = TRUE),
                   Q1 = quantile(time, 0.25,na.rm = TRUE),
                   Median = median(time,na.rm = TRUE),
...
Hrs_bptl_rsch_summ <- Hrs_bptl_rsch_summ %>%
  mutate(Site_Group = case_when(
    `Collection Location` %in% c("UC-DCAM", "Ingalls Harvey", "River East", "South Loop", "UCM Pop-Up", 
                                 "Orland Park", "Missing UC location") ~ "University of Chicago Medicine",
    `Collection Location` %in% c("HP Research Clinic",  "HP Park Nicollet", "Missing HP location") ~ "HealthPartners",
    `Collection Location` %in% c("Lake Hallie", "Marshfield", "MF Pop-Up", "Minocqua", "Weston", "Wisconsin Rapids", "Rice Lake", "Colby Abbotsford", 
                                 "Merrill", "Stevens Point",
                                 "Neillsville", "Missing MF location") ~ "Marshfield Clinic Health System",
...
Hrs_bptl_clin <- Hrs_bptl_clin_summ  %>%
  ...
  gt::tab_style(style = list(gt::cell_text(weight = "bold")),
                locations = gt::cells_row_groups()) %>% 
  gt::tab_options(table.width = "90%", table.align = "left", table.font.size = "7pt") #table.font.size = "smaller")
...
research.survey<- research.survey %>% mutate(schedulecol.tm =min(as.POSIXct(ymd_hms(d_678166505))),
...
##HF
fig4_sub_all <- research.survey %>%  filter(Site=="Henry Ford Health") 
fig4_sub_all <- fig4_sub_all %>%  filter((d_684635302 == 353358909 | d_878865966 == 353358909| d_167958071 == 353358909) & !is.na(checktmvisit)) 
...
##Sanford
fig4_sub_all <- research.survey %>%  filter(Site=="Sanford Health") 
fig4_sub_all <- fig4_sub_all %>%  filter((d_684635302 == 353358909 | d_878865966 == 353358909| d_167958071 == 353358909) & !is.na(checktmvisit)) 
...
research.4_2 <- ggplot(visit_wk_site4__2, aes(x=as.Date(col.Monday.date), y=pct3)) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Percent %",breaks=seq(0,100,25)) +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-1),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
...
##UC
fig4_sub_all <- research.survey %>%  filter(Site=="University of Chicago Medicine") 
fig4_sub_all <- fig4_sub_all %>%  filter((d_684635302 == 353358909 | d_878865966 == 353358909| d_167958071 == 353358909) & !is.na(checktmvisit)) 
...
research.4_3 <- ggplot(visit_wk_site4__3, aes(x=as.Date(col.Monday.date), pct3)) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Percent %",breaks=seq(0,100,25)) +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-1),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
...
tab8=knitr::kable(bloodtubetype_C,
  ...
  kable_styling(latex_options = "scale_down", full_width = FALSE)   %>% 
  landscape()
add_footnote(tab8, "Note: Tube configuration/quantity varies by site based on clincal lab supply in order to achieve the closes to collection volume to 20mL SST, 10mL EDTA, 10mL Heparin, 6mL ACD and 10mL Streck. Collection of ACD tube was discontinued on 03/01/2024; Collection of Heparin tube was discontinued on 06/01/2025; Streck tube is collection was introduced at different time points and is only collected at certain sites.", notation = "none") 
...
knitr::kable(discard.site, 
             ...
             row.names=FALSE,  
             align=c("l","c","c","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>%
  kable_styling(latex_options = "scale_down")
...
\FloatBarrier
```{r Figure3, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE}


figsub3_final <- research.survey %>%
    filter(Site == "University of Chicago Medicine", !is.na(research.sv)) %>%
    droplevels() %>%
    arrange(col.Monday.date, research.sv) %>%
    group_by(col.Monday.date, research.sv) %>%
    tally() %>%
    group_by(col.Monday.date) %>%
    mutate(per_week_parts = sum(n)) %>%
    filter(research.sv == "Submitted") %>%
    mutate(weekly_sub = n) %>% 
    ungroup() %>% 
    mutate(cum_sub = cumsum(n),
        cumtotal = cumsum(per_week_parts),
        percent_3 = 100 * (cum_sub / cumtotal))

ggplot(figsub3_final, aes(x=as.Date(col.Monday.date), y=100*(cum_sub/cumtotal))) + 
...
\newpage
\FloatBarrier
```{r Figure3.1, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE}

#Comparative Figure 3 of weekly not cumulative

ggplot(figsub3_final, aes(x=as.Date(col.Monday.date), y=100*(weekly_sub/per_week_parts))) + 
  geom_line(na.rm=T) + geom_point(width = 0.2) +
  scale_y_continuous(name="Percent %",limits=c(0,100),expand=c(0,0)) + 
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-1),expand=c(0,0))  +
...
tab23=knitr::kable(research.vt.length.table,
  ...
  kable_styling(latex_options = "scale_down") 
add_footnote(tab23,"Note: Expected visit length for each site is as follows: (1) HealthPartners: 25-60 minutes, (2) Henry Ford: 25-60 minutes, (3) Sanford: 10-45 minutes, (4) Marshfield: 10-45 minutes, (5) UChicago: 15-180 minute, and (6) BSWH: 25-60 minutes", notation = "none") #and greater than 500 minutes 
...
uc_col_time <- knitr::kable(research.vt.length.UC.table,
                            ...
                            row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% 
  kable_styling(latex_options = "scale_down")
add_footnote(uc_col_time,"Note: Visit lengths at or above the 95th percentile and above are likely the result of delayed participant visit check-out and do not reflect accurate visit lengths", notation = "none") 
...
\vspace{-0.5em}
\FloatBarrier
```{r Fig15, eval=TRUE,echo=FALSE, warning = FALSE, message = FALSE}
...
```{r Figures16_18, eval=TRUE,echo=FALSE, warning = FALSE, message = FALSE}
...
visit_wk_site.hybrid <-research.survey.hybrid %>% arrange(Site,Site_location, all_settings,col.Sunday.week,col.Monday.date) %>% 
  group_by(Site,Site_location, all_settings,col.Sunday.week,col.Monday.date) %>% tally()



##Need an ungrouped version for TTAl count per week, not each Site_location count for each week
research.vt_plHF <- visit_wk_site.hybrid %>%
     filter(Site == "Henry Ford Health") %>%
     ungroup() %>%
     select(col.Monday.date, all_settings, n) %>%
     group_by(col.Monday.date, all_settings) %>%
     summarise(weekly_collections = sum(n), .groups = "drop")
 
ggplot(research.vt_plHF, aes(x=as.Date(col.Monday.date), y=weekly_collections, color=all_settings )) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number of Collections") +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "4 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-7),expand=c(0,0))  +
...
edited_fig3_1 <- visit_wk_site.hybrid %>%  filter(Site == "Henry Ford Health" & all_settings=="Research")

ggplot(edited_fig3_1, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + 
...
research.vt_plotH_Clin <- ggplot(edited_fig3_1_Clin_tally, aes(x=as.Date(col.Monday.date), y=n, color=Clin_Site_location )) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number of Collections") +
...
visit_wk_site_edited <- coll_sett_all %>% arrange(Site,Site_location,all_settings,col.Sunday.week,col.Monday.date ) %>% 
  group_by(Site_location, Site,all_settings, col.Sunday.week,col.Monday.date) %>% tally()





research.vt_plUC= visit_wk_site.hybrid %>%  filter( Site=="University of Chicago Medicine")
research.vt_plUC <- research.vt_plUC %>% group_by(col.Monday.date) %>% mutate(total_collections = sum(n))
ggplot(research.vt_plUC, aes(x=as.Date(col.Monday.date), y=total_collections)) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number of Collections") +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "4 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-7),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
...
research.vt_plUC <- visit_wk_site.hybrid %>%
...
ggplot(research.vt_plUC, aes(x=as.Date(col.Monday.date), y=weekly_collections, color=all_settings )) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number of Collections") +
...
research.vt_plUC_R= visit_wk_site.hybrid %>%  filter(Site == "University of Chicago Medicine" & all_settings=="Research")
research.vt_plotUC <- ggplot(research.vt_plUC_R, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number of Collections") +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "4 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-7),expand=c(0,0))  +
  scale_color_manual(values=c("Ingalls Harvey"="#B1C7D9", "Orland Park"="#2973A5", "River East"="#FDBE19", 
...
#Need the graph to show weeks where counts are 0
...
ggplot(research.vt_plMF, aes(x=as.Date(col.Monday.date), y=total_collections)) + 
  geom_line() + geom_point(width = 0.2) +
...
```{r Figures19_20, eval=TRUE,echo=FALSE, warning = FALSE, message = FALSE}

research.vt_plSH= visit_wk_site.hybrid %>%  filter( Site=="Sanford Health")
research.vt_plSH <- research.vt_plSH %>% group_by(col.Monday.date) %>% mutate(total_collections = sum(n))
ggplot(research.vt_plSH, aes(x=as.Date(col.Monday.date), y=total_collections)) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number of Collections") +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "4 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-7),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
...
research.vt_plSF <- visit_wk_site.hybrid %>%
     filter(Site == "Sanford Health") %>%
     ungroup() %>%
     select(col.Monday.date, all_settings, n) %>%
     group_by(col.Monday.date, all_settings) %>%
     summarise(weekly_collections = sum(n), .groups = "drop")

ggplot(research.vt_plSF, aes(x=as.Date(col.Monday.date), y=weekly_collections, color=all_settings )) +
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="# Number Collections") +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "4 weeks", limits=c( as.Date("2022-06-20"),(as.Date(currentDate)-7)),expand=c(0,0))  +
  scale_color_manual(values=c("Clinical"="#164C71", "Research"="#FDBE19")) +
...
research.vt_plSH= visit_wk_site.hybrid %>%  filter( Site=="Sanford Health" & all_settings=="Research")
ggplot(research.vt_plSH, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number Collections") +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "4 weeks", limits=c( as.Date("2022-06-20"),(as.Date(currentDate)-7)),expand=c(0,0))  +
...
research.vt_plHP <- visit_wk_site.hybrid %>%
...
ggplot(research.vt_plHP, aes(x=as.Date(col.Monday.date), y=weekly_collections, color=all_settings)) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number of Collections") + 
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "4 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-7),expand=c(0,0))  +
  scale_color_manual(values=c("Clinical"="#164C71", "Research"="#FDBE19")) +
...
edited_fig11_1_Clin_tally <- research.vt_plHP_locations %>% arrange(Clin_Site_location, col.Sunday.week,col.Monday.date) %>% group_by(Clin_Site_location, col.Sunday.week,col.Monday.date) %>% tally()

research.vt_plotHP_locs <- ggplot(edited_fig11_1_Clin_tally, aes(x=as.Date(col.Monday.date), y=n, color=Clin_Site_location)) +  
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number of Collections", breaks = seq(min(edited_fig11_1_Clin_tally$n), max(edited_fig11_1_Clin_tally$n), by = 1)) + 
...
ggplot(research.vt_plBSW, aes(x=as.Date(col.Monday.date), y=total_collections)) + 
  geom_line() + geom_point(width = 0.2) +
...
```{r Fig21_24, eval=TRUE,echo=FALSE, warning = FALSE, message = FALSE}
...
```{r HomeMW_DataPull, include=FALSE, echo=FALSE, message=FALSE,warning=FALSE}

kitstatus <- "SELECT Connect_ID, d_173836415_d_266600170_d_319972665_d_379252329, d_173836415_d_266600170_d_319972665_d_221592017, d_173836415_d_266600170_d_319972665_d_661940160, d_265193023, d_547363263, d_173836415_d_266600170_d_448660695, d_195145666, d_286191859, d_123868967, d_906417725, d_747006172, d_987563196, d_167958071, d_878865966, d_684635302, d_827220437, d_173836415_d_266600170_d_915179629, d_914594314, d_173836415_d_266600170_d_982213346,d_173836415_d_266600170_d_139245758, d_265193023, d_253883960, d_173836415_d_266600170_d_319972665_d_826941471,d_173836415_d_266600170_d_319972665_d_379252329, d_173836415_d_266600170_d_319972665_d_221592017, d_173836415_d_266600170_d_319972665_d_826941471,d_173836415_d_266600170_d_541483796_d_661940160, d_173836415_d_266600170_d_541483796_d_759651991, d_173836415_d_266600170_d_541483796_d_687158491, d_173836415_d_266600170_d_541483796_d_379252329, d_173836415_d_266600170_d_541483796_d_221592017,
d_173836415_d_266600170_d_641006239_d_661940160, d_173836415_d_266600170_d_641006239_d_759651991, d_173836415_d_266600170_d_641006239_d_687158491,
d_173836415_d_266600170_d_641006239_d_379252329, d_173836415_d_266600170_d_641006239_d_221592017,
d_173836415_d_266600170_d_448660695, d_173836415_d_266600170_d_319972665_d_826941471
FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` 
where Connect_ID IS NOT NULL and d_906417725='104430631'  and d_747006172='104430631' and d_987563196='104430631'" #not dead or wd or refused
...
kit_table <- kit_table %>%  mutate(Site = case_when(d_827220437==125001209 ~ "Kaiser Permanente Colorado",
                                   ...
                                   Kit_Status_int = case_when(#d_173836415_d_266600170_d_319972665_d_221592017==517216441             ~"Pending", -- no one will have this
                                     ...
                                     d_173836415_d_266600170_d_319972665_d_221592017==332067457    ~ "Undeliverable",
                                     is.na(d_173836415_d_266600170_d_319972665_d_221592017) ~ "No Status"),
                                   Kit_Status_r1 = case_when(#d_173836415_d_266600170_d_541483796_d_221592017==517216441             ~"Pending", -- no one will have this
                                     d_173836415_d_266600170_d_541483796_d_221592017==728267588    ~"Initialized",
                                     d_173836415_d_266600170_d_541483796_d_221592017==849527480    ~"Address Printed",
                                     d_173836415_d_266600170_d_541483796_d_221592017==241974920    ~ "Assigned",
                                     d_173836415_d_266600170_d_541483796_d_221592017==277438316    ~ "Shipped",
                                     d_173836415_d_266600170_d_541483796_d_221592017==375535639    ~ "Received",
                                     d_173836415_d_266600170_d_541483796_d_221592017==332067457    ~ "Undeliverable",
                                     is.na(d_173836415_d_266600170_d_541483796_d_221592017) ~ "No Status"),
                                   Kit_Status_r2 = case_when(#d_173836415_d_266600170_d_641006239_d_221592017==517216441             ~"Pending", -- no one will have this
                                     d_173836415_d_266600170_d_641006239_d_221592017==728267588    ~"Initialized",
                                     d_173836415_d_266600170_d_641006239_d_221592017==849527480    ~"Address Printed",
                                     d_173836415_d_266600170_d_641006239_d_221592017==241974920    ~ "Assigned",
                                     d_173836415_d_266600170_d_641006239_d_221592017==277438316    ~ "Shipped",
                                     d_173836415_d_266600170_d_641006239_d_221592017==375535639    ~ "Received",
                                     d_173836415_d_266600170_d_641006239_d_221592017==332067457    ~ "Undeliverable",
                                     is.na(d_173836415_d_266600170_d_641006239_d_221592017) ~ "No Status"),
                                   Kit_Status_int = factor(Kit_Status_int,levels=c("Undeliverable","No Status","Initialized", "Address Printed", "Assigned", "Shipped", "Received")),
...
kitsetting <- "SELECT Connect_ID, d_650516960, d_678166505,d_143615646_d_826941471,  d_556788178, d_820476880
...
kit_set_clean <- bind_rows(cha_rows, cxa_rows)
 
```
 
 
 
```{r HomeMW_Data_Merging, include=FALSE, echo=FALSE, message=FALSE,warning=FALSE}
 
location_stat <- kit_table %>%  select(Connect_ID, Kit_Status_int,Kit_Status_r1,Kit_Status_r2, d_173836415_d_266600170_d_319972665_d_661940160, d_173836415_d_266600170_d_982213346,
                                       d_173836415_d_266600170_d_139245758, d_547363263, Site, d_195145666, d_173836415_d_266600170_d_448660695, d_173836415_d_266600170_d_319972665_d_379252329,
                                       d_173836415_d_266600170_d_319972665_d_826941471)
kit_sett <- left_join(location_stat, kit_set_clean,   by="Connect_ID") 
...
table27_elg <- kit_sett %>% filter( (d_173836415_d_266600170_d_982213346>=as.Date("2024-03-29") | is.na(d_173836415_d_266600170_d_982213346)) &
                                    (d_173836415_d_266600170_d_139245758>=as.Date("2024-03-29") | is.na(d_173836415_d_266600170_d_139245758)) &
                                      !(is.na(d_173836415_d_266600170_d_982213346) & is.na(d_173836415_d_266600170_d_139245758))) # needs at least one collection
                                     #Kit_Status!="Unknown")
 
 
#Updates: need kit assembly merged in to determine kit replacement status
kitassembly_data2 <- kitassembly_data %>%  select(Connect_ID, d_426588510, d_194252513, d_137401245,
                                                  d_633640710_d_100618603, d_633640710_d_205954477, d_633640710_d_289239334, d_633640710_d_427719697,
                                                  d_633640710_d_541085383, d_633640710_d_545319575, d_633640710_d_938338155, d_633640710_d_950521660,
                                                  d_633640710_d_992420392)  %>%
  mutate(kit_level= case_when(d_426588510 == 772116457 ~ "Replacement Kit 2",
                              d_426588510 == 389478821 ~ "Replacement Kit 1",
                              TRUE ~ "Initial Kit"))

table27_elg_kit <- left_join(table27_elg, kitassembly_data2, by="Connect_ID")



# When participants get replacement kits, then get an additional row in the kit_assembly table
# those with initial kits will have one row with d_426588510==663273321
...
table27_elg_kit <- table27_elg_kit %>%
  filter(d_650516960=='664882224' | is.na(d_650516960)) #remove research collections 

## Somehow the NAs are being separated
table27_elg_kit$kit_level[is.na(table27_elg_kit$kit_level)]= "Initial Kit"
...
table27_elg_distinct <- bind_rows(
  processed,
  table27_elg_kit %>% filter(!Connect_ID %in% cha_ids)
)
...
## Participants are getting Kit_Status and kit_level values for both their HMW and blood/urine collection, causing duplicate rows
## Blood urine collections will have all kit statuses, HMW collections will only include the received collections and some shipped collections
## Using blood urine collections (Collection ID starts with CXA) to ensure all kits are counted.
## This will ONLY work for the first table to get the kit status counts as the CHA rows don't have values for d_678166505 and d_143615646_d_826941471
 
kit_status_table_7.1 <- table27_elg_distinct %>% filter(d_650516960=='664882224')
 
## Note: 'undeliverable', 'no status', 'initialized', 'address printed', and 'assigned' don't show up in the kit assembly table
...
### Initial Kit
status_elgA <- kit_status_table_7.1 %>%
  filter(kit_level=="Initial Kit") %>%
  tbl_cross(
    row = Site,
    col = Kit_Status_int,
    digits=c(0,1),
    ...
    label=list(Site="Site",
               Kit_Status_int = "Kit Status"),
    missing="ifany",
    margin_text="Total Eligible")
 
 
status_elg_knA <- status_elgA %>%
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 7.1a:  Baseline- Home Mouthwash Initial Kit Status Among Eligible Participants") %>%
...
### Replacement Kit 1
status_elgB <- kit_status_table_7.1 %>%
  filter(kit_level=="Replacement Kit 1") %>%
  tbl_cross(
    row = Site,
    col = Kit_Status_r1,
    digits=c(0,1),
    percent = "row",
    label=list(Site="Site",
               Kit_Status_r1 = "Kit Status"),
    missing="ifany",
    margin_text="Total Eligible")
 
 
status_elg_knB <- status_elgB %>%
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 7.1b:  Baseline- Home Mouthwash Replacement Kit 1 Status \n Among All R1 Kits Requested") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) %>%  kable_styling(latex_options = "scale_down")
...
### Replacement Kit 2
status_elgC <- kit_status_table_7.1 %>%
  filter(kit_level=="Replacement Kit 2") %>%
  tbl_cross(
    row = Site,
    col = Kit_Status_r2,
    digits=c(0,1),
    percent = "row",
    label=list(Site="Site",
               Kit_Status_r2 = "Kit Status"),
    missing="ifany",
    margin_text="Total Eligible")
 
 
status_elg_knC <- status_elgC %>%
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 7.1c:  Baseline- Home Mouthwash Replacement Kit 1 Status \n Among All R2 Kits Requested") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) %>%  kable_styling(latex_options = "scale_down")
...
filtered_data <- filter(kit_status_table_7.1, Kit_Status_int %in% c("Received", "Shipped"))
...
## Here we need the CXA collections only- received or shipped and has values for d_678166505 and d_143615646_d_826941471
 
kits_hmw <- table27_elg_distinct %>% filter(is.na(d_650516960))
 
 
####################

Coll_Card_Table <- kits_hmw %>%
  filter(Kit_Status_int=="Received" & kit_level=="Initial Kit") %>%
...
Coll_Card_StatusBF <- Coll_Card_Table %>% dplyr::select(Site, Card_Status )  %>% tbl_cross(
  ...
  missing_text="0 (0%") %>%
  modify_header() %>%
  modify_caption("Table 7.3: Baseline- Collection Card Status Among Home Mouthwash Initial Kits Received") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
test30 <- Coll_Card_Table %>%  filter(!is.na(d_173836415_d_266600170_d_319972665_d_826941471) & !is.na(d_173836415_d_266600170_d_319972665_d_661940160))
...
firstcoltime <- tube_col %>% filter(all_settings=="Clinical")  %>% select(Connect_ID, collection.tm)
firstcoltime$Connect_ID <- as.numeric(firstcoltime$Connect_ID)
kit_status_table_7.1$Connect_ID <- as.numeric(kit_status_table_7.1$Connect_ID)
first_col_time <- left_join(kit_status_table_7.1, firstcoltime, by=c("Connect_ID", "Site"))
 
test31 <- first_col_time %>% 
  filter(!is.na(collection.tm) & !is.na(d_173836415_d_266600170_d_319972665_d_661940160) & kit_level=="Initial Kit")
 
test31 <-  test31 %>%
  mutate(ctime = as.numeric(difftime(as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_661940160)),
                                     as.POSIXct(ymd_hms(collection.tm)),units="days")))
...
table_32 <- kits_hmw %>% filter(Kit_Status_int=="Received" & kit_level=="Initial Kit") %>% 
...
table__32$Percentage <- paste0(
  table__32$n,
  " (",
  sprintf("%.2f%%", (table__32$n / sum(table__32$n)) * 100),
  ")"
)
...
```{r FigMW1, include=FALSE, echo=FALSE, message=FALSE,warning=FALSE}
 
 
#NOTE: MUST change currentDate to Mondays date in order for this to run correctly when testing for fixes
    #set currentDate= recent Monday date
 
##Received-- need one set of data for the received line
kit_status_table_7.1$col.Sunday.week <- ceiling(as.numeric(difftime(kit_status_table_7.1$d_173836415_d_266600170_d_319972665_d_826941471,as.Date(veristart.date-days(2)),units="days"))/7)
kit_status_table_7.1$col.Sunday.date <- veristart.date + days(5) + dweeks(kit_status_table_7.1$col.Sunday.week-1)
kit_status_table_7.1$col.Monday.date <- kit_status_table_7.1$col.Sunday.date-days(6)
 

fig5_new_1 <- kit_status_table_7.1 %>%
...
kit_table1 <- kit_status_table_7.1 %>%  filter(!is.na(d_173836415_d_266600170_d_319972665_d_661940160))
 
kit_table1$col.Sunday.week <- ceiling(as.numeric(difftime(kit_table1$d_173836415_d_266600170_d_319972665_d_661940160,as.Date(veristart.date-days(2)),units="days"))/7)
kit_table1$col.Sunday.date <- veristart.date + days(5) + dweeks(kit_table1$col.Sunday.week-1)
kit_table1$col.Monday.date <- kit_table1$col.Sunday.date-days(6)
 
 
fig5_new_2 <- kit_table1 %>%
  filter(d_173836415_d_266600170_d_319972665_d_379252329 == 976461859 & as.Date(col.Monday.date) >= "2024-03-29" & as.Date(col.Monday.date) < currentDate)
 
fig5_week_totals_2 <- fig5_new_2 %>%
  group_by(col.Monday.date) %>%
  summarise(total_shipped = sum(!is.na(d_173836415_d_266600170_d_319972665_d_661940160)))
 
 
fig5_week_total_2 <- left_join(all_mondays, fig5_week_totals_2, by = "col.Monday.date") %>%
...
MW_Coll_Kits <- ggplot(fig5_week_final, aes(x = as.Date(col.Monday.date))) +
  ...
  theme(panel.background = element_blank(),
        ...
        plot.title = element_text(hjust = 0.5, size = 12, face = "bold")) +
  guides(fill = guide_legend(nrow = 2, byrow = TRUE)) +
  labs(caption = "This figure includes all home mouthwsh kit types: Initial, Replacement Kit 1 and Replacement Kit 2 ") +
  theme(plot.caption = element_text(hjust=0))
...
##Received
kit_set_col_2 <- kit_status_table_7.1 %>% filter(Kit_Status_int=="Received") 
 
kit_set_col_2$col.Sunday.week <- ceiling(as.numeric(difftime(kit_set_col_2$d_173836415_d_266600170_d_319972665_d_826941471,as.Date(veristart.date-days(2)),units="days"))/7)
kit_set_col_2$col.Sunday.date <- veristart.date + days(5) + dweeks(kit_set_col_2$col.Sunday.week-1)
kit_set_col_2$col.Monday.date <- kit_set_col_2$col.Sunday.date-days(6)
...
##Shipped
kit_table2 <- kit_status_table_7.1 %>%  filter(!is.na(d_173836415_d_266600170_d_319972665_d_661940160) & kit_level=="Initial Kit")
 
kit_table2$col.Sunday.week <- ceiling(as.numeric(difftime(kit_table2$d_173836415_d_266600170_d_319972665_d_661940160,as.Date(veristart.date-days(2)),units="days"))/7)
kit_table2$col.Sunday.date <- veristart.date + days(5) + dweeks(kit_table2$col.Sunday.week-1)
kit_table2$col.Monday.date <- kit_table2$col.Sunday.date-days(6)
...
tab7_8 <- knitr::kable(table7_8_totals ,
                       col.names=c("Site","Not Started","Started","Submitted","Total Participants with Received Kits"),
...
table7_9 <- kits_hmw %>%  filter((Kit_Status_r2=="Received" | Kit_Status_r1=="Received" | Kit_Status_int=="Received") & d_137401245==104430631 &
                                   as.Date(d_173836415_d_266600170_d_319972665_d_661940160)<"2024-05-22") %>%
  mutate(MWSrvy_status = case_when(d_547363263==972455046 ~ "Not Started",
                                   d_547363263==615768760 ~ "Started",
                                   d_547363263==231311385 ~ "Submitted"),
...
tbl_7_9 <- table7_9 %>%
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 7.8: Baseline- Mouthwash Biospecimen Survey Completion Time Relative to Collection Time Reported on Collection Card \n(Pre-update to survey description on PWA)") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) %>%
  kable_styling(latex_options = "scale_down")
...
table7_10 <- kits_hmw %>%  filter((Kit_Status_r2=="Received" | Kit_Status_r1=="Received" | Kit_Status_int=="Received") & d_137401245==104430631 &
                                    as.Date(d_173836415_d_266600170_d_319972665_d_661940160)>="2024-05-22") %>%
  mutate(MWSrvy_status = case_when(d_547363263==972455046 ~ "Not Started",
                                   d_547363263==615768760 ~ "Started",
                                   d_547363263==231311385 ~ "Submitted"),
...
```{r Tables10s, eval=TRUE,echo=FALSE,results='asis'}




###### Initial 
survey_cc_int <- Coll_Card_Table %>%  filter(!is.na(d_195145666) & !is.na(d_173836415_d_266600170_d_448660695) & as.Date(d_195145666) > "2025-05-01") 
...
Table7_10a <- knitr::kable(Table7.10a,
                           ...
                           row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE) %>%

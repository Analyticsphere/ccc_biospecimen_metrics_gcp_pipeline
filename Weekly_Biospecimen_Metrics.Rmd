...
kit_tm_don_inv <- kit_request_table %>% 
  ...
  knitr::kable(kit_tm_don_inv, 
    ...
    kable_styling(latex_options ="scale_down") %>% 
    footnote(general="Note: Reminders are sent on Days 5 and 14 following initial invitation.",
           general_title = "",
           footnote_as_chunk = TRUE, 
           escape = FALSE, 
           threeparttable = TRUE)
...
query= "WITH status_counts AS (
  ...
  --  Once we have people with address updated this will be all we need
  SELECT
    CASE
      WHEN DATE(up.ts_user_profile_updated) > DATE(p.d_173836415_d_266600170_d_319972665_d_278539982)
       AND DATE(up.ts_user_profile_updated) < DATE(p.d_173836415_d_266600170_d_319972665_d_278539982)
      THEN 'Address Update'
...
-- But because there are 0 and we want both rows included, we have to manually complete: 

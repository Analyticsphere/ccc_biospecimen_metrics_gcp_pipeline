...
\FloatBarrier
```{r RequestKits,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}



### Request a kit table 1

req_rec_kit_table1 <- kit_table %>%
  # The three columns
  mutate(
    invited   = d_173836415_d_266600170_d_319972665_d_638781731 == "353358909",
    requested = !is.na(d_173836415_d_266600170_d_319972665_d_759651991),
    received  = Kit_Status_int == "Received"
  ) %>%

  # Keep only those invited (denominator)
  filter(invited) %>%
  ...
  summarise(
    Invited   = n(),
    Requested = sum(requested, na.rm = TRUE),
  ...
  select(
    Site,
    `Invited to Request a Kit` = Invited,
    `MW Kit Requested`,
    `MW Sample Received`
  )
 ...
 knitr::kable(req_rec_kit_table1,
              caption="Table 8.1 : Baseline- Home Mouthwash Kits Requested Among Participants Invited to Request a Kit",
              row.names=FALSE, align=c("l", "c"),booktabs = TRUE) %>%
   add_indent(seq(1, nrow(req_rec_kit_table1) - 1)) %>%
   kable_styling(latex_options = "scale_down")
...
### Request a kit table 2
 ...
 ## Need to get subgroup of only those who were invited
 
 req_kit <- kit_table %>% 
   filter(
     #invited -- some requested weren't invited
     d_173836415_d_266600170_d_319972665_d_638781731 == "353358909" &
     #date requested
     !is.na(d_173836415_d_266600170_d_319972665_d_759651991))

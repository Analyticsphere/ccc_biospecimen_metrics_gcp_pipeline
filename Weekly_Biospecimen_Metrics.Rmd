...
table4_redoneR <- table4_redone__R %>% dplyr::select('Specimens Collected', Site )  %>% 
  tbl_cross(
...
table4_redoneR <- table4_redoneR%>%
  #bold_labels() %>%
  #italicize_levels() %>% 
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 4a. Baseline Research Collection Completeness by Site") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
table4_redoneC <- table4_redone__C %>%   dplyr::select('Specimens Collected', Site )  %>% 
  tbl_cross(
    row = 'Specimens Collected',
    col = Site,
    digits=c(0,1),
...
table4_redoneC <- table4_redoneC%>%
  #bold_labels() %>%
  #italicize_levels() %>% 
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 4b. Baseline Clinical Collection Completeness by Site") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
bldcol_research_HF2$BloodIncomplet[1] <- paste0(bldcol_research_HF2$BloodCompletion[1], " (100%)")
bldcol_research_HF2$BloodIncomplet[2] <- paste0(bldcol_research_HF2$BloodCompletion[2], " (100%)")
bldcol_research_HF2$BloodIncomplet[3] <- paste0(bldcol_research_HF2$BloodCompletion[3], " (100%)")
...
bldcol_research_HF2$Partial <- list(paste0(), )
bldcol_research_HF2$None <- list("0 (0%)", "0 (0%)","0 (0%)", 0)


bldcol_research_HF2 <- bldcol_research_HF2[, c(7,1,8,9,3)]
...
table7rd <- table7rd %>%
  ...
  #italicize_levels() %>% 
  modify_header() %>%
  modify_caption("Table 7. Blood Completeness (Number of Blood Tubes Collected) for Participants with Baseline Clinical Collections") %>% ## used to be Table 8
  as_kable_extra()
table7rd
...
##Table 9- renumbered--- named Table8_BldCli to Table_BldCli to match report
...
Table_BldCli <- recr_clin %>% 
  ...
  tbl_cross(
    ...
    digits=c(0,1),
    margin_text="Total") 
Table_BldCli[["table_body"]]$stat_0 <- sapply(strsplit(Table_BldCli[["table_body"]]$stat_0," "),"[",1) 
Table_BldCli <- Table_BldCli %>%
  #bold_labels() %>%
  #italicize_levels() %>% 
  modify_header() %>%
  #modify_caption("Table 11. Baseline Blood Collection Status in Data Sent from KP vs Connect Biospecimen Dashboard") %>%
  modify_caption("Table 9. Baseline Blood Collection Status in Data Sent from Site vs Connect Biospecimen Dashboard") %>% #on hold- HF should show up under clinical 
  as_kable_extra()
...
## Table 10- renumbered---- renamed Table9_UrineCli to Table_UrineCli
        # table(recr_clin$Site) -- confirmed HF hybird is included in this 
Table_UrineCli <- recr_clin %>% 
...
Table_UrineCli[["table_body"]]$stat_0 <- sapply(strsplit(Table_UrineCli[["table_body"]]$stat_0," "),"[",1) 
Table_UrineCli <- Table_UrineCli%>%
  #bold_labels() %>%
  #italicize_levels() %>% 
  modify_header() %>%
  modify_caption("Table 10. Baseline Urine Collection Status in Data Sent From Site vs Connect Biospecimen Dashborad") %>% #on hold- HF should show up under clinical 
  as_kable_extra()
...
table4_redoneR %>% kable_styling(latex_options = "scale_down")
table4_redoneC %>% kable_styling(latex_options = "scale_down")
...
knitr::kable(bldcol_research1, col.names=c("Site","All Blood Tubes Collected","Partial Blood Tubes Collected","No Blood Tubes Collected","Total"), caption='Table 5. Blood Completeness for Participants with Baseline Research Collections', row.names=FALSE, align=c("l","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")


knitr::kable(bldcol_research_UC1, col.names=c("Collection Location","All Blood Tubes Collected","Partial Blood Tubes Collected","No Blood Tubes Collected","Total"), caption='Table 5.1 Blood Completeness among University of Chicago Baseline Research Collections by Collection Location', row.names=FALSE, align=c("l","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")


knitr::kable(bldcol_research_HF1, col.names=c("Collection Location","All Blood Tubes Collected","Partial Blood Tubes Collected","No Blood Tubes Collected","Total"), caption='Table 5.2 Blood Completeness among Henry Ford Baseline Research Collections by Collection Location', row.names=FALSE, align=c("l","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")


knitr::kable(bldcol_research_HF2, col.names=c("Collection Location","All Blood Tubes Collected","Partial Blood Tubes Collected","No Blood Tubes Collected","Total"), caption='Table 5.3 Blood Completeness among Henry Ford Baseline Clinical Collections by Clinic Location', row.names=FALSE, align=c("l","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")


knitr::kable(notcol_combo[, c(7,1:6)] ,col.names=c("Site", "SS Tube1", "SS Tube 2", "Heparin Tube1", "EDTA Tube1", "ACD Tube1", "Total Not Collected"), caption='Table 6. Blood Tubes Not Collected Among Partial Blood Collections at Research Collection Visits', row.names=FALSE,align=c("l","c","c","c","c","c","c","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down") %>% landscape()  ## used to be Table 13



#Switched order of 9 and 10, so now 10 is first 
tab10_2= table7rd %>% kable_styling(latex_options = "scale_down")
...
tab8=knitr::kable(notcol_combo2[,c(1:12)],caption='Table 8. Blood Tubes Collected by Tube Type at Clinical Collections', row.names=FALSE,align=c("l","c","c","c","c","c","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>%  kable_styling(latex_options = "scale_down") %>% landscape()
...
tab9=Table_BldCli
add_footnote(tab9, "Note: Data sent by KP lag up to 48 hours behind data from dashboard", notation = "none") 
tab10=Table_UrineCli
add_footnote(tab10, "Note: Data sent by KP lag up to 48 hours behind data from dashboard", notation = "none") 

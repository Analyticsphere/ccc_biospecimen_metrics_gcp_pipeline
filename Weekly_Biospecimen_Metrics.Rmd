...
tb_bq <- eval(parse(text=paste("bq_project_query(project, query=\"SELECT token,Connect_ID,d_512820379,d_821247024,d_914594314,d_827220437,d_699625233, d_265193023, d_822499427, 
                               ...
                               select,
                               "FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants` Where Connect_ID is not null and d_821247024 = '197316935' and  (d_512820379='486306141' or d_512820379='854703046')  and 
...
veristart.date = as.Date(min(as.POSIXct(recrver1$d_914594314),na.rm=TRUE))
...
## REMOVING DUPLICATES: Method- take the first collection only for blood and urine
...
table4_redone__C <- coll_sett_all %>%  
  ...
  mutate(blood = BioFin_BaseBloodCol_v1r0 == "Any blood collected",
         ...
         `Specimens Collected` = case_when(blood & urine ~ "Blood & Urine (All)",
                                           ...
                                           !blood & !urine ~ "No Specimens Collected")) %>%
  dplyr::select(-blood, -urine, -mouthwash) %>%  # Remove helper columns
  mutate(`Specimens Collected` = factor(`Specimens Collected`,levels=c('Blood & Urine (All)', 'Blood Only', 'Urine Only', 'No Specimens Collected')))
...
Deviation_Freq <- Deviation_Freq %>% select("Group.1.x", "d_299553921_d_678857215", "d_958646668_d_593843561") %>% 
  mutate(Total = d_299553921_d_678857215 + d_958646668_d_593843561,
         'Any Devaitaions' = paste(d_299553921_d_678857215, " (", round(d_299553921_d_678857215/Total*100, digits=1),"%)"),
         'No Devaitaions' = paste(d_958646668_d_593843561, " (", round(d_958646668_d_593843561/Total*100, digits=1),"%)"),
         Site = factor(Group.1.x,levels = c("Baylor Scott and White Health",  "HealthPartners",  "Henry Ford Health",
                                       "Marshfield Clinic Health System", "Sanford Health",  "University of Chicago Medicine",
                                       "Kaiser Permanente Colorado", "Kaiser Permanente Georgia", "Kaiser Permanente Hawaii", "Kaiser Permanente Northwest"))) %>% 
  select("Site", "Any Devaitaions", "No Devaitaions", "Total") %>% 
   arrange(Site)
...
total_row <- Deviation_Freq %>%
  summarise(Site = "Total",
            ...
            Any_num = sum(as.numeric(gsub(" .*", "", `Any Devaitaions`)), na.rm = TRUE),
            No_num = sum(as.numeric(gsub(" .*", "", `No Devaitaions`)), na.rm = TRUE)) %>%
  mutate(`Any Devaitaions` = paste0(Any_num, " (", round(Any_num / Total * 100, 1), "%)"),
         `No Devaitaions` = paste0(No_num, " (", round(No_num / Total * 100, 1), "%)")) %>%
  select(Site, `Any Devaitaions`, `No Devaitaions`, Total)
...
avg_bldorder_present <- recrver1 %>%  filter(d_878865966=="No blood collected" & !is.na(d_173836415_d_266600170_d_769615780) & 
...
Table_bld_draw= recrver1 %>%  filter(grepl("^Kaiser", Site)) %>% ## excluding hybrid sites
  mutate(bld_placed= case_when(BioClin_BldOrderPlaced_v1r0=="Yes" ~ "Blood Draw Order Placed",
...
coll_bptl_diff5.1 <- coll_bptl_diff %>% ungroup() %>% 
  mutate(coll_days_diff= droplevels(coll_days_diff))

crosstable.coll_bptl28a <- create_tbl_cross(coll_bptl_diff5.1, "Site", "coll_days_diff", "row", "Site", " ")
...
crosstable.coll_bptl28 <- create_tbl_cross(coll_bptl_diff5.1, "Site_location", "coll_days_diff", "row", "Site", " ")
crosstable.coll_bptl28 <- as.data.frame(crosstable.coll_bptl28)
colnames(crosstable.coll_bptl28)[1] <- "Collection_Location"



ctable.coll_bptl28 <- crosstable.coll_bptl28 %>%
  mutate(Site_Group = case_when(
...
ctable.coll_bptl28 <- ctable.coll_bptl28 %>%  filter(str_sub(Total, 1, 1) != "0")


# Separate the Total row
total_row <- ctable.coll_bptl28 %>% filter(Collection_Location == "Total")
...
coll_bptl_diff5.2 <- coll_bptl_diff2 %>% ungroup() %>% 
  mutate(coll_days_diff= droplevels(coll_days_diff))

crosstable.clin_coll_days <- create_tbl_cross(coll_bptl_diff5.2, "Site", "coll_days_diff", "row", "Site", " ")
...
#### By Site location
coll_bptl_diff5.2$Clin_Site_location[coll_bptl_diff5.2$Site=="Kaiser Permanente Colorado"]="KPCO"
coll_bptl_diff5.2$Clin_Site_location[coll_bptl_diff5.2$Site=="Kaiser Permanente Georgia"]="KPGA"
coll_bptl_diff5.2$Clin_Site_location[coll_bptl_diff5.2$Site=="Kaiser Permanente Northwest"]="KPNW"
coll_bptl_diff5.2$Clin_Site_location[coll_bptl_diff5.2$Site=="Sanford Health"]="KPNW"

clin_coll_days <- create_tbl_cross(coll_bptl_diff5.2, "Clin_Site_location", "coll_days_diff", "row", "Collection Location", " ")
...
crosstable.coll_bptl_2 <- knitr::kable(clin_coll_times[, c(1:7)], 
...
clc.both.vt <- clc.both.vt %>%  mutate(both.startdate = min(ymd_hms(d_173836415_d_266600170_d_139245758), ymd_hms(d_173836415_d_266600170_d_982213346),na.rm=TRUE),
                        bldcol.time = case_when(bldcol_max >0 ~ pmin(as.POSIXct(d_173836415_d_266600170_d_982213346), as.POSIXct(d_173836415_d_266600170_d_398645039),
                                                                     as.POSIXct(d_173836415_d_266600170_d_822274939),na.rm=TRUE)),
                        urine.time = case_when(Urine_col_max >0 ~ pmin(as.POSIXct(d_173836415_d_266600170_d_139245758), as.POSIXct(d_173836415_d_266600170_d_541311218),
                                                                       as.POSIXct(d_173836415_d_266600170_d_224596428),na.rm=TRUE)),
                        BU.time =  pmin(as.POSIXct(bldcol.time),as.POSIXct(urine.time), na.rm = TRUE), ## matches up exactly with operations code
                        BU.Sunday.week = ceiling(as.numeric(difftime(BU.time,as.Date(veristart.date-days(2)),units="days"))/7),
                        BU.Sunday.date = veristart.date + days(5)+dweeks(BU.Sunday.week -1), #veristart is 7/27, but that's a tueaday. Add 5 days to make it Sunday, then subtract a week to make it the start of that previous week
                        Monday.date.start = BU.Sunday.date-days(6))
...
## Repeating for each of the 4 KP Sites
plot_collections_by_week <- function(df, site_label, fig_number) {
  
  df_summary <- df %>%
    arrange(Site, BU.Sunday.week, Monday.date.start) %>%
    group_by(Site, BU.Sunday.week, Monday.date.start) %>%
    tally()
  
  ggplot(df_summary, aes(x = as_date(Monday.date.start), y = n)) +
    geom_line() +
    ...
    scale_x_date(
      ...
      date_labels = "%y-%m-%d",
      date_breaks = "4 weeks",
      limits = c(as.Date("2023-01-16"), Sys.Date() - 7),
      expand = c(0, 0)
    ) +
    scale_fill_brewer(palette = "Dark2") +
    ggtitle(label = paste0("Figure ", fig_number, ": Baseline- Weekly Biospecimen Collections,\n", site_label)) +
    theme(
...
clc_both_vt.site.CO <- plot_collections_by_week(clc.both.vt.CO, "Kaiser Permanente Colorado", "6.18")
clc_both_vt.site.GA <- plot_collections_by_week(clc.both.vt.GA, "Kaiser Permanente Georgia", "6.19")
clc_both_vt.site.HI <- plot_collections_by_week(clc.both.vt.HI, "Kaiser Permanente Hawaii", "6.20")
clc_both_vt.site.NW <- plot_collections_by_week(clc.both.vt.NW, "Kaiser Permanente Northwest", "6.21")
...
fig4_sub_all <- fig4_sub_all %>%  filter((d_684635302 == "Any mouthwash collected" | d_878865966 == "Any blood collected"| d_167958071 == "Any urine collected") & !is.na(checktmvisit)) 
...
research.4_1 <- ggplot(visit_wk_site4, aes(x=as.Date(col.Monday.date), y=pct3)) +
  ...
  ggtitle(label="Figure 3.1: Baseline- Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Research Collection Visit Check In, Henry Ford Health  ") +
...
research.4_2 <- ggplot(visit_wk_site4__2, aes(x=as.Date(col.Monday.date), y=pct3)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(currentDate)-1),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3.2: Baseline- Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Research Collection Visit Check In, Sanford Health") +
  theme(panel.background = element_blank(),
        axis.line = element_line(size=0.2),
        axis.text.x=element_text(angle=60, hjust=1), axis.title.x = element_text( size = 14, face = "bold") ,
        axis.title.y = element_text(size = 14, face = "bold",color="black"),
...
Bld_urn_cli = knitr::kable(system_coll, 
                           ...
                           row.names=FALSE,align=c("l",rep("c", 2)), booktabs = TRUE) %>%  
  kable_styling(latex_options = "scale_down") #"HOLD_position"
...
# knitr::kable(BU_trig_comp,
#              caption='Table 3.5.b Baseline Clinical Biospecimen Survey Complete Time From Survey Becoming Available',
#              row.names=FALSE, align=c("l", rep("c", 9)),digits=1, booktabs = TRUE) %>%
#   kable_styling(latex_options = "scale_down")
...
UC_Time <- ggplot(UC_Time_plot, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + 
  geom_line() + geom_point(width = 0.2) +
...
add_footnote(tabl27, "Time from verification to draw order placement calculated for participants verified on or after February 1, 2023. HealthPartners, University of Chicago Medicine and Sanford Health are excluded due to the limited availability of draw order placement data", notation="none")

#add_footnote(tabl27, "Time from verification to draw order placement calculated for participants verified on or after February 1, 2023. HealthPartners, University of Chicago Medicine and Sanford Health are excluded due to the limited availability of draw order placement data. Henry Ford is excluded because clinical collection was not an option in 2/2023 and participants have collection type options, therefore, it may take longer for draw orders to be requested and placed.", notation="none")
...
tab4_10 <- knitr::kable(avg_bldorder_present, 
                        caption='Table 4.8: Baseline- Time (in Days) from Draw Order Placement to Present Date by Site', 
                        row.names=FALSE, align=c("l", rep("c", 11)),digits=1, booktabs = TRUE) %>% 
  kable_styling(latex_options = "scale_down") 
...
research.vt1 <- ggplot(visit_wk_site1, aes(x=as.Date(col.Monday.date), y=n)) + 
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="Number of Collections") + 
...
clinic_locations <- unique(edited_fig11_1_Clin_tally$Clin_Site_location) 
...
# Updated kit eligibility: Initil Kit Status is not null
...
where Connect_ID IS NOT NULL and d_173836415_d_266600170_d_319972665_d_221592017 IS NOT NULL"
...
status_elgA <- kit_table %>%
  ...
  filter(!is.na(d_173836415_d_266600170_d_319972665_d_759651991) | Kit_Status_int!="No Status") %>% 
...
first_col_time <- left_join(kit_table, firstcoltime, by=c("Connect_ID", "Site"))
 
test31 <- first_col_time %>% 
  filter(!is.na(collection.tm) & !is.na(d_173836415_d_266600170_d_319972665_d_661940160))
...
kit_table$col.Sunday.week <- ceiling(as.numeric(difftime(kit_table$d_173836415_d_266600170_d_319972665_d_826941471,as.Date(veristart.date-days(2)),units="days"))/7)
...
kit_table1 <- kit_table %>%  filter(!is.na(d_173836415_d_266600170_d_319972665_d_661940160))
...
table7_8 <- kit_table %>%  filter(Kit_Status_r2=="Received" | Kit_Status_r1=="Received" | Kit_Status_int=="Received") %>%
  mutate(MWSrvy_status = case_when(d_547363263==972455046 ~ "Not Started",
         ...
         MWSrvy_status = factor(MWSrvy_status, levels=c("Not Started","Started", "Submitted"))) #%>% dplyr::select(Site, MWSrvy_status ) 

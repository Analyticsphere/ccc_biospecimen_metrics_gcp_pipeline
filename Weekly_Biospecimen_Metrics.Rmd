...
biospe7= left7  %>%  
  ...
  filter(as.numeric(d_173836415_d_266600170_d_185243482)>72  & !is.na(d_173836415_d_266600170_d_185243482)) %>%
...
figsub3_3 <- figsub3_2 %>% group_by(col.Monday.date) %>% filter(research.sv!="Submitted" & research.sv!="Started") %>%  mutate(weekly_notsub=n)

figsub3_4 <- figsub3_3 %>% group_by(col.Monday.date) %>%  filter(research.sv!="Submitted" & research.sv!="Started") %>%  select(col.Monday.date,research.sv,weekly_notsub, per_week_parts)

figsub3_5 <-figsub3_4 %>% ungroup() %>% mutate(cum_notsub= cumsum(weekly_notsub),
                                 cumtotal= cumsum(per_week_parts))


figsub3_6 <- figsub3_5 %>%  mutate(percent_3= 100*(cum_notsub/cumtotal))


research.3 <- ggplot(figsub3_5, aes(x=as.Date(col.Monday.date), y=100*(cum_notsub/cumtotal))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),as.Date(Sys.Date())),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3. Weekly Cumulative Percent of Biospecimen Surveys \n Not Submitted, University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
        ...
        axis.title.y = element_text(size = 14, face = "bold",color="black"),
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) + 
  labs(caption = "Note: Biospecimen Surveys with a completion status of 'Started' and 'Not Started' are considered 'Not Submitted'.") +
  theme(plot.caption = element_text(hjust=0))
#guides(fill=guide_legend(nrow=2,byrow=TRUE))

...
recrver1 <- recrver1 %>% mutate(BiospeCollection = ifelse((recrver1$d_878865966 == 353358909| recrver1$d_167958071 == 353358909), "Any biospecimen Collections",
                                                          "No biospecimen collections")) 
...
col_num_1.1 <- col_num_1.1 %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` %in% c("HFH K-13 Research Clinic", "HFH Livonia Research Clinic", "HFH Cancer Pavilion Research Clinic", "HFH Pop-Up") ~ "Henry Ford Health",
    `Collection Location` %in% c("HWC, BC","All Saints (FW)") ~ "Baylor Scott and White",
    TRUE ~ "Other"
  ))
...
col_num_1.1_gt <- col_num_1.1 %>%
  ...
  gt::gt(groupname_column = "Site_Group") 
...
table2_status <- table2 %>% 
  ...
  tbl_cross(
    row = Site,
    column = BioFin_BaseBloodCol_v1r0,
    digits=c(0,1),
    percent = "row",
    label=list(Site="Site",
...
biospe.bld <- knitr::kable(table2_totals, 
                           ...
                           booktabs = TRUE, escape = FALSE)  %>% kable_styling(latex_options = "HOLD_position")   
...
site.sub <- site.sub1 %>%  mutate(Setting = case_when(d_650516960==534621077 ~ "Research Collections",
                                                        d_650516960==664882224 ~ "Clinical Collections",
                                                        TRUE ~ "No Collections"),
                                'Specimens Collected'= case_when((BioFin_BaseBloodCol_v1r0=="Any blood collected" & 
                                                                    BioFin_BaseUrineCol_v1r0=="Any urine collected") ~ 'Blood & Urine (All)',
                                                                 (BioFin_BaseBloodCol_v1r0=="Any blood collected" & 
                                                                    BioFin_BaseUrineCol_v1r0!="Any urine collected") ~ 'Blood Only',
                                                                 (BioFin_BaseBloodCol_v1r0!="Any blood collected" & 
                                                                    BioFin_BaseUrineCol_v1r0=="Any urine collected") ~ 'Urine Only',
                                                                 (BioFin_BaseBloodCol_v1r0!="Any blood collected" & 
                                                                    BioFin_BaseUrineCol_v1r0!="Any urine collected") ~ 'No Specimens Collected',
...
biospe$Site <- factor(biospe$d_827220437, levels= c("Baylor Scott and White Health", "HealthPartners", "Henry Ford Health","Marshfield Clinic Health System","Sanford Health", "University of Chicago Medicine","Kaiser Permanente Colorado","Kaiser Permanente Georgia","Kaiser Permanente Hawaii","Kaiser Permanente Northwest")) 
...
biospe <- biospe %>% mutate(bloodcomplete = ifelse((d_650516960==534621077 & bldcolumn == 5) | (d_650516960==664882224 & bldcolumn == 11), "Blood.Completion" ,#blood collection completion
                                          ifelse( (d_650516960==534621077 & (bldcol<5 & bldcol>0)) | (d_650516960==664882224 & (bldcol <  11 & bldcol < 0)),"Blood.Incompleted","NoBlood")),
                            Urine_column = ifelse(d_973670172_d_593843561==353358909, 1, ifelse(d_973670172_d_593843561==104430631,0,0)),
                            MW_column = ifelse(d_143615646_d_593843561==353358909,1,ifelse(d_143615646_d_593843561==104430631,0,0)),
...
biospe7= left7  %>%   filter(Site=="Henry Ford Health") 
...
clin_bldcnt_HF2_df <- as.data.frame(clin_bldcnt_HF2_cross)

clin_bldcnt_HF2 <- clin_bldcnt_HF2_df[c(2:nrow(clin_bldcnt_HF2_df)),]
...
biospe$Site <-factor(biospe$d_827220437,levels=c("Baylor Scott and White Health","HealthPartners", "Henry Ford Health",
                                                     "Marshfield Clinic Health System",
                                                     "Sanford Health", "University of Chicago Medicine","Kaiser Permanente Colorado",
                                                     "Kaiser Permanente Georgia","Kaiser Permanente Hawaii","Kaiser Permanente Northwest",
...
notcol.ls <- sapply(strsplit(notcol_tubes, "_d"),"[[",1)
   ...
   collection_long <- collection_long  %>% 
     dplyr:: mutate(deviation = case_when(tube_deviation == 353358909 ~ "Any Deviation",
                    ...
                    Collected = case_when(tubecolumn ==353358909 ~ "Collected",
                                          tubecolumn == 104430631 ~ "Not  Collected"),
...
```{r Table8, eval=TRUE,echo=FALSE,include=FALSE,results='hold'}
  ...
  research.coltube<- collection_long %>% filter(biocol_Setting == "Research" & tubecolumn == 104430631) %>%     
    ...
    research.coltube.bld.only<- collection_long %>% filter(biocol_Setting == "Research" & tubecolumn == 104430631) %>% 
    mutate(Site=droplevels(Site),
...
research__blood <- collection_long %>% filter(biocol_Setting == "Research" & tubecolumn == 353358909) %>% 
    mutate(Site=droplevels(Site),
...
clinic.coltube <- collection_long %>%  mutate(tube_types= case_when(tube_type=="SS Tube1" ~ "SS T1",
  ...
  clinic.coltube<- clinic.coltube %>% filter(biocol_Setting == "Clinical" & tubecolumn == 353358909 & tube_types!="Urine 1") %>% # & tube_type!="MW Tube1") %>% #added, need to test 
    mutate(Site=droplevels(Site),
...
##Table X: Reason not collected by tube type and by site for research collections only
  ...
  b <- notcol_tube[which(notcol_tube$tubecolumn == 104430631),]
  notcol_notes <- base::merge(a, b[,c("tubeID","d_820476880","tube_notcol")], by=c("tubeID","d_820476880"),all.x=TRUE)
notcol_notes <- notcol_notes %>% 
  mutate(notcol.notes = case_when(tubecolumn == 104430631 & tube_notcol==181769837 ~ "Other",
                                  tubecolumn == 104430631 & tube_notcol==234139565 ~ "Short Draw",
                                  tubecolumn == 104430631 & tube_notcol==681745422 ~ "Participant Refusal",
                                  tubecolumn == 104430631 & tube_notcol==745205161 ~ "Participant Attempted",
                                  tubecolumn == 104430631 & tube_notcol==889386523 ~ "Supply Unavailable",
                                  tubecolumn == 104430631 & is.na(tube_notcol) ~ "Missing Reasons"),
         Collected = case_when(tubecolumn ==353358909 ~ "Collected",
                               tubecolumn == 104430631 ~ "Not  Collected"),
                    biospecimen=ifelse(tubeID=="d_973670172", "urine",
                                  ifelse(tubeID=="d_143615646","Mouthwash","blood")),
...
knitr::kable(final_col_num_1.5[, c(1,2,8,9)], 
             col.names=c("Collection Location", "Total Clinical Collections", "In the Past 7 days",  "In the Past 30 days"), 
             caption='Table 1.4: Baseline Clinical Biospecimen Collections at Sanford Health by Clinic Location', 
             row.names=FALSE, align=c("l","c"), format.args = list(big.mark = ","), booktabs = TRUE) %>%  
...
knitr::kable(clin_bldcnt_HF2, 
  ...
  kable_styling(latex_options = "scale_down") %>%  
  add_indent(seq(1, nrow(clin_bldcnt_HF2) - 1))
...
tab8=knitr::kable(notcol_combo2_totals,
  ...
  column_spec(column = 2:ncol(notcol_combo2_totals),width = "4cm")  %>% 
  column_spec(column = 1:ncol(notcol_combo2_totals),width = "5cm") %>%  landscape()
...
tabe2_12 <- knitr::kable(research.notcolnotes_site_totals,  
...
knitr::kable(Table15_combined_totals,  
...
knitr::kable(Table16_combined_totals, 
...
tbl19a <-knitr::kable(Table17_combinedB_totals, 
...
tbl19b <- knitr::kable(Table17_combinedA_totals, 
...
knitr::kable(Table18_combined_totals, 
...
Coll_Card_StatusBF <- Coll_Card_Table %>% dplyr::select(Site, Card_Status )  %>% tbl_cross(
  ...
  missing_text="0 (0%") %>%
  modify_header() %>%
  modify_caption("Table 7.5: Baseline- Collection Card Status Among Home Mouthwash Kits Received") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
table7_2before <- knitr::kable(all_shipped_before,
                               ...
                               row.names=FALSE, align=c("l","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "HOLD_position")
add_footnote(table7_2before, "Note: Kits were returned using USPS prior to 6/17/2024", notation = "none")

```


knitr::kable(all_shipped_2,caption='Table 7.2 Baseline- Total Home Mouthwash Kit Return Rate for Packages Shipped up to 1 Week Ago',row.names=FALSE, align=c("l","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "HOLD_position")
...
table7_2after <- knitr::kable(all_shipped_after,format.args = list(big.mark = ","),
                              caption='Table 7.4 Baseline- Home Mouthwash Kit Return Rate for Packages Shipped up to 1 Week Ago (kits shipped after 6/17/2024)',
                              row.names=FALSE, align=c("l","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "HOLD_position")
add_footnote(table7_2after, "Note: Kits were returned using FedEx after 6/17/2024", notation = "none")
...
table7.4before <- knitr::kable(Table7.4before,
                               caption='Table 7.6: Baseline- Time (in Days) from Home Mouthwash Kit Shipment to BPTL Receipt \n (kits shipped before 6/17/2024)',
...
knitr::kable(Table7.5,
             caption='Table 7.8: Baseline- Time (in Days) from Clinical Collection to Home Mouthwash Kit Shipment',
             row.names=FALSE, align=c("l","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "HOLD_position")


knitr::kable(table32_pct,
             caption='Table 7.9: Baseline- Kit Conditions Among Received Home Mouthwash Kits',
...
tab7_7 <- knitr::kable(table7_7_totals ,
                       ...
                       caption="Table 7.10: Baseline- Mouthwash Biospecimen Survey Status Among Mouthwash Kits in 'Shipped' Status", 
                       row.names=FALSE,align=c("l","c","c","c","c"), booktabs = TRUE)
...
tab7_8 <- knitr::kable(table7_8_totals ,
                       ...
                       caption='Table 7.11: Baseline- Mouthwash Biospecimen Survey Status Among Received Home Mouthwash Kits', 
...
tbl_7_9 <- table7_9 %>%
  ...
  #italicize_levels() %>%
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 7.12: Baseline- Mouthwash Biospecimen Survey Completion Time Relative to Collection Time Reported on Collection Card \n(Pre-update to survey description on PWA)") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) %>%
  kable_styling(latex_options = "scale_down")

...
BLCol_by_Sex <- knitr::kable(BLCol_by_Sex_df, 
             caption='Table 1.12 Baseline Biospecimen Collection Status by Sex \n Among Verified Participants',
...
BLCol_by_Age <- knitr::kable(BLCol_by_Age_df, 
             caption="Table 1.13. Baseline Biospecimen Collection Status by Age \n Among Verified Participants",
             row.names=FALSE, align=c("l",rep("c", 5)), booktabs = TRUE) %>%
    kable_styling(latex_options = "scale_down") %>%  
...
BLCol_by_Race <- knitr::kable(BLCol_by_Race_df, 
             caption="Table 1.14. Baseline Biospecimen Collection Status by Race \n Among Verified Participants",
             row.names=FALSE, align=c("l",rep("c", 5)), booktabs = TRUE) %>%
    kable_styling(latex_options = "scale_down") %>%  
...
research.4_2 <- Section3_Figures("Sanford Health", "2022-06-20", "Figure 3.2: Baseline- Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Research Collection Visit Check In, Sanford Health") + 
  labs(caption="No data point exists for weeks when there were no biospecimen collections.")+
  theme(plot.caption = element_text(size = 8, hjust=0))
...
research.4_4 <- Section3_Figures("Baylor Scott and White Health", "2024-06-24", "Figure 3.4: Baseline- Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Research Collection Visit Check In, Baylor Scott and White Health") + 
  labs(caption="No data point exists for weeks when there were no biospecimen collections.")+
  theme(plot.caption = element_text(size = 8, hjust=0))
...
## The percentage of table type 2 is a column percentage ONLY for Received and Requested

req_rec_inv_kable2 <- function(data, group_var, group_var_name, title) { 
  
  group_var <- rlang::ensym(group_var) 
  
  # Helper to format N (%), with safe handling for 0 denominators
  ## Otherwise if a column has a total of 0, we get '0 (NaN%)' insead of '0 (0%)'
  fmt_npct <- function(n, total) {
    if (is.na(total) || total == 0) {
      return("0 (0%)")
    }
    paste0(n, " (", round(n / total * 100, 2), "%)")
  }
  ...
  req_rec_inv <- data %>% 
    ...
    summarise( Requested = sum(Requested, na.rm = TRUE), 
               Received = sum(Received, na.rm = TRUE), 
               Invited = sum(Invited, na.rm = TRUE), 
               .groups = "drop" ) %>% 
    # Ensure column order 
    select(!!group_var, Requested, Received, Invited) %>% 

  # Create N (%) columns with column percentage and add a Total Row
    
    {
      # totals across all groups, before adding the Total row
      tot_requested <- sum(.$Requested, na.rm = TRUE)
      tot_received  <- sum(.$Received,  na.rm = TRUE)
      tot_invited   <- sum(.$Invited,   na.rm = TRUE)
      ...
      bind_rows(
        ...
        mutate(
          `MW Kit Requested` = vapply(Requested, fmt_npct, character(1), total = tot_requested),
          `MW Sample Received (of total invited)` = vapply(Received, fmt_npct, character(1), total = tot_received),
          #`Total Participants Invited to Request a Kit` = vapply(Invited, fmt_npct, character(1), total = tot_invited)
          `Total Participants Invited to Request a Kit` = as.character(Invited)
        )
    } %>% 

    # Remove helper columns (prior to percentage added) and fix column order
  select(!!group_var, `MW Kit Requested`, `MW Sample Received (of total invited)`, 
         `Total Participants Invited to Request a Kit`)
...
kit_request_table <- kit_request_table %>% 
  mutate(tm_don_inv_weeks = as.numeric(difftime(as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_759651991)),
                                              as.POSIXct(ymd_hms(d_173836415_d_266600170_d_319972665_d_278539982)),units="days")),
         tm_don_inv = case_when(as.numeric(tm_don_inv_weeks) < 5 ~ "Less than 5 days",
                                  as.numeric(tm_don_inv_weeks) < 10 ~"5- <10 days",
                                  as.numeric(tm_don_inv_weeks) < 14 ~ "10- <14 days",
                                  as.numeric(tm_don_inv_weeks) < 30 ~ "14- <30 days",
                                  as.numeric(tm_don_inv_weeks) >= 30 ~ "30 days or more",
                                  is.na(d_173836415_d_266600170_d_319972665_d_759651991) ~ "Have not requested a kit"),
         tm_don_inv = factor(tm_don_inv, levels=c("Less than 5 days","5- <10 days", 
                                                  "10- <14 days", "14- <30 days", 
                                                  "30 days or more",
                                                  "Have not requested a kit")))

req_rec_inv_kable1(kit_request_table, tm_don_inv, 
                   "Time from Invitation to Kit Requested", 
                   "Table 8.4:  Baseline- Home MW Response Rates by Time (in Days) from Invitation to Kit Requested")
...
req_rec_inv_kable1(kit_request_table, tm_surv_inv, "Time from Last Survey Completion to Kit Request Eligible", 
                   "Table 8.5: Baseline- Home MW Response Rates by Time from Last Survey Completion to Invited to Request a Kit")

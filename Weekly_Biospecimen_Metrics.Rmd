...
recrvar <- bigrquery::bq_table_download(recr_var, bigint="integer64",n_max = Inf, page_size = 1000)
...
test18 <- clinic.survey %>%
  mutate(clinical.sv = case_when(d_253883960 == 231311385 ~ "Submitted",
                                 d_253883960 == 615768760 ~ "Started",
                                 d_253883960 == 972455046 ~ "Not Started"))
test18a <- test18 %>%  filter(Site=="Kaiser Permanente Colorado" |  Site=="Kaiser Permanente Georgia" | Site=="Kaiser Permanente Hawaii" | Site=="Kaiser Permanente Northwest" | Site=="Henry Ford Health System") 
...
#New Table 19- length of time for research survey completion (8/16/23 request) before and after covid
...
##New Table 20- length of time for clinical survey completion (8/16/23 request) before and after covid : start 534669573, finish 764863765
...
parts24_data <- bq_table_download(parts24_table, bigint="integer64",n_max = Inf, page_size = 1000)
...
avg <- bq_table_download(avg_table_cc, bigint="integer64",n_max = Inf, page_size = 1000)
...
compTS <- bq_table_download(timestamps_cc, bigint="integer64",n_max = Inf, page_size = 1000)
...
BL_Coll <- bq_table_download(SMMet,bigint = "integer64",n_max = Inf, page_size = 1000)

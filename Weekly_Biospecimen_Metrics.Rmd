...
recrver1$Site <-factor(recrver1$d_827220437,levels=c("Baylor Scott & White Health","HealthPartners", "Henry Ford Health System",
...
biospe_locations<-biospe %>% mutate(Site_location = case_when(d_951355211==777644826 ~ "UC-DCAM",
                                    ...
                                    Site=case_when(siteAcronym=="BSW" ~ "Baylor Scott & White Health", #guessing
...
biospe_site<-biospe %>% filter(d_650516960== 664882224) %>% 
                            mutate(Site=case_when(siteAcronym=="BSW" ~ "Baylor Scott & White Health",
                                                  siteAcronym=="HFHS" ~ "Henry Ford Health System",
                                                   siteAcronym=="HP" ~ "HealthPartners",
                                                   siteAcronym=="KPCO" ~ "Kaiser Permanente Colorado",
                                                   siteAcronym=="KPGA" ~ "Kaiser Permanente Georgia",
...
biospe1_5 <- biospe_site %>% 
  select(d_650516960, Connect_ID, Site, d_556788178)

table1_4_locs <- left_join(table1, Clin_Site_Loc, by = c("Site", "Connect_ID")) %>% 
  left_join(biospe1_5, by = c("Site", "Connect_ID"))
...
<<<<<<< HEAD
table1_4_locations <- add_Site_location(table1_4_locations, "~/GitHub/ccc_biospecimen_metrics_gcp_pipeline/Sanford_Clincal_Locations.csv")
=======
table1_4_locations <- add_Site_location(table1_4_locations, "Sanford_Clincal_Locations.csv")
>>>>>>> 654ed211d526bea6908d35431a06b015333fe01a
...
biospe$Site <- factor(biospe$d_827220437, levels= c("Baylor Scott & White Health", "HealthPartners", "Henry Ford Health System","Marshfield Clinic Health System","Sanford Health", "University of Chicago Medicine","Kaiser Permanente Colorado","Kaiser Permanente Georgia","Kaiser Permanente Hawaii","Kaiser Permanente Northwest")) 
...
biospe <- apply_labels(biospe,d_827220437 = "Site",#RcrtES_Site_v1r0
                       d_827220437 = c("Baylor Scott & White Health"=472940358, "HealthPartners"= 531629870, 
...
#17- by site
test17 <- research.survey %>%  filter(Site=="HealthPartners" |  Site=="Henry Ford Health System" | Site=="Marshfield Clinic Health System" | Site=="Sanford Health" | Site=="University of Chicago Medicine" | Site=="Baylor Scott & White Health")
...
BSL_after24 <- baseline_24_after %>%  mutate(time = as.numeric(difftime(as.POSIXct(ymd_hms(d_556788178)),as.POSIXct(ymd_hms(d_914594314)),units="days")),
                                     Site = case_when(d_827220437.x==472940358 ~ "Baylor Scott & White Health",
...
table2_col_bplt$Connect_ID <- as.numeric(table2_col_bplt$Connect_ID)
biospe$Connect_ID <- as.numeric(biospe$Connect_ID)
table54 <- left_join(table1_4_locations, biospe,  by=c("Connect_ID", "Site")) %>% left_join(table2_col_bplt, by="Connect_ID")
...
tab5=knitr::kable(bioset, col.names=c("Collection Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 1.6: Baseline Collection Setting Among Biospecimen Collections by Specimen Type', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)   
...
tab5_1=knitr::kable(bioset5_1, col.names=c("Collection Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 1.7: Baseline Collection Setting Among Biospecimen Collections by Specimen Type, Henry Ford Health', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)   
...
tab5_2=knitr::kable(bioset5_2, col.names=c("Collection Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 1.8: Baseline Collection Setting Among Biospecimen Collections by Specimen Type, HealthPartners', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)   
...
tab5_3=knitr::kable(bioset5_3, col.names=c("Collection Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 1.9: Baseline Collection Setting Among Biospecimen Collections by Specimen Type, Sanford Health', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)
...
Bld_urn_cli = knitr::kable(newtable_4, caption='Table 1.10: Baseline- Data System Status Among Clinical Biospecimen Collections by Specimen Type', row.names=FALSE,align=c("l","c","c"), booktabs = TRUE) %>%  kable_styling(latex_options = "hold_position") 
...
#verif_baseline_Af[c(1,2,7:9,3:6), ]
AF26 <- knitr::kable(verif_baseline_Af,caption = 'Table 4.7: Baseline- Time (in Days) from Verification to Collection Among Participants Verified on or After 6/01/2022 by Campaign Type',row.names=FALSE, booktabs = T, linesep = "", format.args = list(big.mark = ","), align = c("l", "c", "c","c", "c", "c", "c", "c", "c", "c", "c","c", "c","c"), col.names=c("Site",  'Number of Collections',"Min", "Median", "Mean", "SD", "Max", 'Number of Collections', "Min", "Median", "Mean", "SD", "Max","Total Baseline Collections") ) %>% kable_styling(latex_options = "scale_down") %>% 
  add_indent(seq(1, nrow(verif_baseline_Af) - 1)) %>% add_header_above(c(" " = 1, "Upcoming Appointment" = 6, "All Other Campaigns" = 6,  " "=1)) %>% landscape() 
...
add_footnote(tabl27, "Note: Time from verification to draw order placement calculated for participants verified on or after February 1, 2023. HealthPartners and Sanford Health are exluded due to the limited availability of draw order placement data.", notation="none")
...
add_footnote(tab4_9, "Note: HealthPartners and Sanford Health are exluded due to the limited availability of draw order placement data.", notation="none")

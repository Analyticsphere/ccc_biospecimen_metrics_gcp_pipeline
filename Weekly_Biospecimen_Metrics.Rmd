...
that are finalized. This report does not include information from participants who have requested data destruction. This report is limited to information from participants' first finalized specimen collection. In the event that a participant donated specimen(s) in a repeat collection, that information is not reflected in this report.
...
###### Ops no longer wants duplicate collections in this report
  ## Method of removal- take the first collection only for blood and urine. (If there's multiple collections we take the earliest)
...
tb_bq <- eval(parse(text=paste("bq_project_query(project, query=\"SELECT
token,Connect_ID,d_512820379,d_821247024, d_827220437,d_699625233, d_265193023, 
DATE(d_914594314) as d_914594314, d_949302066, 
-- datetime for these two still comes back as characters
d_822499427, d_222161762, 
state_d_667474224, state_d_934298480, state_d_684926335, state_d_849518448, 
state_d_119643471, state_d_253532712, state_d_435027713,state_d_706256705, 
d_173836415_d_266600170_d_319972665_d_661940160, d_173836415_d_266600170_d_319972665_d_687158491,
d_173836415_d_266600170_d_319972665_d_221592017,
d_173836415_d_266600170_d_541483796_d_221592017, d_173836415_d_266600170_d_641006239_d_221592017,",
                               select,
                               "FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants` 
                               Where Connect_ID is not null and d_821247024 = '197316935' and 
...
no_dups <- biospe %>%
  filter(!is.na(d_556788178)) %>% #Finalized collections will have a finalized date
  group_by(Connect_ID) %>%
  slice_min(d_556788178, with_ties = FALSE) %>% #Keep only the first (earliest) finalized biospecimen collection
  ungroup() 
...
no_dups <- no_dups %>% 
  mutate(
    Site_location = case_when(
      ...
      d_951355211 == 471260082 ~ "Waco Fishpond",
      d_951355211 == 603989030	~ "Killeen Main",
      d_951355211 == 528017143	~ "Southwest Family Medicine",
      d_951355211 == 253098609	~ "Grapevine",
      d_951355211 == 864987193	~ "Plano Heart",
      #SF
      ...
      d_951355211 == 246153539 ~ "Bemidji Clinic",
      #Missing (Not listed above)
...
recrver1 <- recrver1 %>%  mutate(Clin_Site_location=case_when(
                                  ...
                                  Site=="HealthPartners" & (d_173836415_d_266600170_d_185243482 == 9 | d_173836415_d_266600170_d_452847912 == 9) ~ "Burnsville Clinic and Specialty Center",
                                  Site=="HealthPartners" & (d_173836415_d_266600170_d_185243482 == 10 | d_173836415_d_266600170_d_452847912 == 10) ~ "Andover Clinic",
                                  Site=="HealthPartners" & (d_173836415_d_266600170_d_185243482 == 11 | d_173836415_d_266600170_d_452847912 == 11) ~ "St. Paul Clinic- Wabasha",
                                  Site=="HealthPartners" & (d_173836415_d_266600170_d_185243482 == 12 | d_173836415_d_266600170_d_452847912 == 12) ~ "Olivia Hospital and Clinic",
                                  Site=="HealthPartners" & (d_173836415_d_266600170_d_185243482 == 13 | d_173836415_d_266600170_d_452847912 == 13) ~ "Hutchinson Health Clinic",
                                  ...
                                  d_173836415_d_266600170_d_185243482 == 1011290016 | 
                                    d_173836415_d_266600170_d_452847912 == 1011290016 ~ "CANPAV Pathology OP Lab",
                                  d_173836415_d_266600170_d_185243482 == 1120190010 | 
                                    d_173836415_d_266600170_d_452847912 == 1120190010 |
                                    d_173836415_d_266600170_d_185243482 == 1120010009 | 
                                    d_173836415_d_266600170_d_452847912 == 1120010009 | 
                                    d_173836415_d_266600170_d_185243482 == 1120200013 | 
                                    d_173836415_d_266600170_d_452847912 == 1120200013 ~ "Jackson Professional Building",
                                  Site=="Henry Ford Health" & is.na(d_173836415_d_266600170_d_185243482) &
                                    is.na(d_173836415_d_266600170_d_452847912) ~ "Missing Both Location IDs",
                                  ...
                                  Site=="Kaiser Permanente Hawaii" & is.na(d_173836415_d_266600170_d_185243482) &
                                    is.na(d_173836415_d_266600170_d_452847912) ~ "Missing Both Location IDs",
...
```{r Date_Conversions, include=FALSE}

### This prevents the need from running as.Date() or ymd_hms() throughout the report for all diff_time()'s--------------------


tube_time_cols <- names(biocol.tb)[grepl("d_926457119", names(biocol.tb))]

biocol.tb <- biocol.tb %>%
    mutate(across(all_of(tube_time_cols), ymd_hms),
           d_556788178 = ymd_hms(d_556788178),
           d_678166505 = ymd_hms(d_678166505),
           d_822499427 = ymd_hms(d_822499427),
           d_222161762 = ymd_hms(d_222161762),
           d_764863765 = ymd_hms(d_764863765),
           d_534669573 = ymd_hms(d_534669573),
           d_915838974 = ymd_hms(d_915838974),
           d_331584571_d_266600170_d_840048338 = ymd_hms(d_331584571_d_266600170_d_840048338),
           d_331584571_d_266600170_d_343048998 = ymd_hms(d_331584571_d_266600170_d_343048998), 
           d_173836415_d_266600170_d_139245758 = ymd_hms(d_173836415_d_266600170_d_139245758),
           d_173836415_d_266600170_d_982213346 = ymd_hms(d_173836415_d_266600170_d_982213346),
           d_173836415_d_266600170_d_740582332 = ymd_hms(d_173836415_d_266600170_d_740582332),
           d_173836415_d_266600170_d_822274939 = ymd_hms(d_173836415_d_266600170_d_822274939),
           d_173836415_d_266600170_d_184451682 = as.Date(d_173836415_d_266600170_d_184451682),
           d_173836415_d_266600170_d_769615780 = as.Date(d_173836415_d_266600170_d_769615780))
...
table1_totals <- create_tbl_cross1(table1, "Site", "biocol.appoint", "row", "Site", " ")
...
table1_allsett <- create_tbl_cross1(coll_sett_all, "Site", "all_settings", "row", "Site", "Collection Setting")  
...
settings_breakdown <- knitr::kable(table1_allsett, 
  ...
  footnote(general = "Note: Any collections indicated under 'Home' setting reflect collections in which home mouthwash was the only sample collected; these collections are not counted in Table 1.1.", 
           general_title = "",
...
coll_sett_all <- coll_sett_all %>%  
  mutate(col.Monday.date = floor_date(d_556788178, unit = "week", week_start = 1)) %>% # week of collection
  ungroup()
...
col_num_1.1 <- col_num_1.1 %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` %in% c("Lake Hallie", "Marshfield", "MF Pop-Up", "Minocqua", "Weston", "Wisconsin Rapids", 
                                 "Rice Lake", "Colby Abbotsford", "Merrill",
                                 "Stevens Point", "Neillsville", "Missing MF location") ~ "Marshfield Clinic Health System",
    `Collection Location` %in% c("Sioux Falls Imagenetics", "Fargo South University",
                                 "Edith Sanford Breast Center (coded as Sanford Center)", 
                                  "Edith Sanford Breast Center","Fargo Amber Valley","Bemidji Clinic",
                                 "Missing SF location") ~ "Sanford Health",
    `Collection Location` %in% c("HFH K-13 Research Clinic", "HFH Livonia Research Clinic", 
                                 "HFH Cancer Pavilion Research Clinic", "HFH Pop-Up", 
                                 "HFH Detroit Northwest","In-home Collection", "HFH Jackson",
                                 "Missing HFH location") ~ "Henry Ford Health",
    `Collection Location` %in% c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving","Irving",
                                 "NTX Biorepository","BCC- Fort Worth",
                                 "North Garland", "Temple CDM", "Temple Roney", "Waco - MacArthur","Temple Westfield",
                                 "Waco Fishpond","Killeen Main", "Southwest Family Medicine","Grapevine","Plano Heart",
                                 "Missing BSWH location") ~ "Baylor Scott and White Health",
    TRUE ~ "Other"
  ))
...
col_num_1.1 <- col_num_1.1 %>%
  ...
  total_rows <- col_num_1.1 %>%
    ...
    dplyr::summarize(`Collection Location` = "Total", N = sum(N),
                     Total_Collections = sum(N), .groups = "drop")
  
  col_num_1.1 <- dplyr::bind_rows(col_num_1.1, total_rows) %>%
    dplyr::mutate(`Total Research Collections` = fmt_count_pct(N, Total_Collections, digits = 1))
  

col_num_1.1 <- col_num_1.1 %>%
  select("Collection Location","Site_Group", "Total Research Collections") %>% 
...
col_num_1.2 <- col_num_1.2 %>%
  mutate(Site_Group = case_when(
    `Collection Location` %in% c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", "K1 HFH Main",
                                 ...
                                 "Woodhaven Pathology", "CANPAV Pathology OP Lab", "Jackson Professional Building",
                                 "Missing Both Location IDs", "Missing HFH location") ~ "Henry Ford Health", 
    `Collection Location` %in% c("Elk River", "Chanhassen", "Park Nicollet Minneapolis Clinic", "Brooklyn Center", 
                                 "Clinic Stillwater", "New Richmond Clinic",
                                 "Burnsville Clinic and Specialty Center", "Andover Clinic","St. Paul Clinic- Wabasha", 
                                 "Olivia Hospital and Clinic","Hutchinson Health Clinic",
...
col_num_1.2 <- col_num_1.2 %>%
  ...
  col_num_1.2 <- dplyr::bind_rows(col_num_1.2, total_rows) %>%
    dplyr::mutate(`Total Clinical Collections` = fmt_count_pct(N, Total_Collections, 
                                                               digits = 1))


col_num_1.2 <- col_num_1.2 %>%
  select("Collection Location","Site_Group", "Total Clinical Collections") %>%
...
table1.5 <- create_tbl_cross1(coll_sett_all, 'Site', "BioFin_BaseBloodCol_v1r0", "row", 'Site', " ")


biospe.bld <- knitr::kable(table1.5, 
                           col.names=c("Site", "Any Blood Collected","No Blood Collected", "Total Collections"),
                           caption='Table 1.6: Baseline Blood Collection Status Among Biospecimen Collections',
...
BLCol_by_Sex <- knitr::kable(BLCol_by_Sex_df, 
             caption='Table 1.7 Baseline Biospecimen Collection Status by Sex \n Among Verified Participants',
...
BLCol_by_Age <- knitr::kable(BLCol_by_Age_df, 
             caption="Table 1.8. Baseline Biospecimen Collection Status by Age \n Among Verified Participants",
             row.names=FALSE, align=c("l",rep("c", 5)), booktabs = TRUE) %>%
    kable_styling(latex_options = "scale_down") %>%  
...
BLCol_by_Race <- knitr::kable(BLCol_by_Race_df, 
             caption="Table 1.9. Baseline Biospecimen Collection Status by Race \n Among Verified Participants",
             row.names=FALSE, align=c("l",rep("c", 5)), booktabs = TRUE) %>%
    kable_styling(latex_options = "scale_down") %>%  
...
```{r Mod1_Race_BioComp, eval=TRUE,echo=FALSE,include=FALSE,results='hold'}



## -------- This table requires race from Module 1 ------------------------------

######## MERGE MOD1V1 & MOD1V2 with the participants table exclusions
...
recr_M1 <- bq_project_query(project, query="SELECT token,Connect_ID, d_821247024, d_914594314,  d_827220437,d_512820379,
                            ...
                            FROM  `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants` WHERE  d_821247024='197316935' and 
...
irb_race <- m1_complete[!duplicated(m1_complete$Connect_ID),]



## Link back to main dataset 
coll_sett_all_m1 <- left_join(coll_sett_all, irb_race, by="Connect_ID")
...
#---- Combine stacked crosstabulations for Race, Sex, and Ethnicity by Collection Setting-------

## Race, Sex, and Ethnicity are their own tables, merged into one long table
### However, the columns are also independent: 
    # Research collections by race, sex, ethnicity 
    # Clinical collections by race, sex, ethnicity
    # Home collections by race, sex ethnicity (match denominator from section 7)




# Set IRB defined race, sex, and Hispanic ethicity
coll_sett_all_m1 = coll_sett_all_m1 %>%  
  mutate(irb_race= case_when(d_949302066!=231311385 ~ "Missing",
                         d_949302066==231311385 & multi_racial==1 ~ "More than One Race",
                         d_949302066==231311385 &  (D_384191091_D_384191091_D_412790539==1 | 
                                                      D_384191091_D_384191091_D_973565052==1) ~ "White",
                         ...
                         d_949302066==231311385 &  D_384191091_D_384191091_D_458435048==1 ~ "Black or African American",
                            #category instead
                         #d_949302066==231311385 &  D_384191091_D_384191091_D_706998638==1 ~ "Hispanic, Latino, or Spanish", 
                            #grouped with White as requested
                         #d_949302066==231311385 &  D_384191091_D_384191091_D_973565052==1 ~ "Middle Eastern or North African", 
                         d_949302066==231311385 &  D_384191091_D_384191091_D_586825330==1 ~ "Native Hawaiian or Other Pacific Islander",
                         TRUE  ~ "Unknown"),
         race = factor(race, levels=c("More than One Race", "American Indian/Alaska Native",
                                                "Asian", "Black or African American",
                                                "Native Hawaiian or Other Pacific Islander", "White",
                                                "Unknown", "Missing")),
         race_hisp= case_when(d_949302066!=231311385 ~ "Missing",
                              d_949302066==231311385 & D_384191091_D_384191091_D_706998638==0 ~ "No",
                              d_949302066==231311385 & D_384191091_D_384191091_D_706998638==1 ~ "Yes",
                              TRUE  ~ "Unknown"),
         race_hisp = factor(race_hisp, levels=c("Yes", "No", "Unknown", "Missing")),
         irb_race = fct_relevel(irb_race, "Module 1 Not Yet Completed", after = Inf))
...
# Count collections by Setting
coll_sett_all_m1 <- coll_sett_all_m1 %>% 
  # Need 'yes' values to be the same value (1) for the pivot longer to work
  mutate(rsch_coll = case_when(all_settings=="Research" ~ 1),
         clin_coll = case_when(all_settings=="Clinical" ~ 1),
                              # Home setting only
         hmw_coll = case_when(BL_BioSpm_MWSetting_v1r0=="Home" & 
                                # Initial/Replacement1/Replacement2 HMW Kit rec'vd
                                (d_173836415_d_266600170_d_319972665_d_221592017 == '375535639' |
                                   d_173836415_d_266600170_d_541483796_d_221592017 == '375535639' |
                                   d_173836415_d_266600170_d_641006239_d_221592017 == '375535639') ~ 1)) %>% 
  # HMW is independent of research/clinical setting so pivot longer needed
  pivot_longer(
    cols = c(
      rsch_coll, 
      clin_coll, 
...
# Specific Column Name Request
Rsch_m1 <- paste0("Research Collections N(%) (N=",
                  scales::comma(sum(coll_sett_all_m1$sett=="rsch_coll")),
                  ")")
Cl_m1 <- paste0("Clinical Collections N(%) (N=", 
                scales::comma(sum(coll_sett_all_m1$sett=="clin_coll")),
                ")")
H_m1 <- paste0("Home Collections N(%) (N=", 
               scales::comma(sum(coll_sett_all_m1$sett=="hmw_coll")),
               ")")


# Column Order
coll_sett_all_m1 <- coll_sett_all_m1 %>% 
  mutate(setting_header = case_when(sett=="rsch_coll" ~ Rsch_m1,
                                    sett=="clin_coll" ~ Cl_m1,
                                    sett=="hmw_coll" ~ H_m1),
         setting_header = factor(setting_header, levels=c(Rsch_m1, Cl_m1, H_m1)))


# Idividual crosstabulations

BioCol_by_IRB_Race <- coll_sett_all_m1 %>%  
  tbl_cross(
    row = irb_race,
    col = setting_header,
    digits=c(0,2),
    percent = "column",
    label=list(irb_race ~ "Race",
               setting_header= " "),
    missing="ifany",
    margin=NULL)


BioCol_by_Sex <- coll_sett_all_m1 %>%  
  tbl_cross(
    row = sex,
    col = setting_header,
    digits=c(0,2),
...
BioCol_by_IRB_Eth <- coll_sett_all_m1 %>%  
  tbl_cross(
    row = race_hisp,
    col = setting_header,
    digits=c(0,2),
    percent = "column",
    label=list(race_hisp ~ "Hispanic",
               setting_header= " "),
    missing="ifany",
    margin=NULL)
...
# Stakced crosstabs
stacked_IRB_demo_BioCols <- tbl_stack(list(BioCol_by_IRB_Race, BioCol_by_Sex, BioCol_by_IRB_Eth))


IRB_demo_BioCols <- stacked_IRB_demo_BioCols %>% 
  bold_labels() %>%
  modify_header() %>%
  modify_caption("Table 1.10: Baseline Biospecimen Collection Setting by Gender, Race, and Ethnicity") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) %>%
  kable_styling(latex_options = "scale_down") %>% 
  footnote("Note: Race and ethnicity information are defined based on responses to questions in Module 1; for those who have not completed Module 1 those values are set to 'Missing'. Gender is defined from the de-identified data reported by sites at time of invitation.",
          general_title = "",
         footnote_as_chunk = TRUE, 
         escape = FALSE, 
         threeparttable = TRUE)
...
```{r Sanford_Table_1.5B, eval=TRUE,echo=FALSE,include=FALSE,results='hold'}
...
add_dept_name <- function(table1_4_locations, csv_file_path) {
  site_data <- read_csv(csv_file_path, locale = locale(encoding = "Latin1"), show_col_types = FALSE)

  # Ensure columns are character and convert/clean encoding to UTF-8
  site_data <- site_data %>%
    mutate(
      DEPARTMENT_ID = as.character(DEPARTMENT_ID),
      DEPARTMENT_NAME = iconv(as.character(DEPARTMENT_NAME), from = "Latin1", to = "UTF-8", sub = ""),
      REGION = iconv(as.character(REGION), from = "Latin1", to = "UTF-8", sub = "")
    )
  

  if (!all(c("DEPARTMENT_ID", "DEPARTMENT_NAME", "REGION") %in% colnames(site_data))) {
    stop("The CSV file must contain 'DEPARTMENT_ID', 'DEPARTMENT_NAME', and 'REGION' columns.")
  }

   # Normalize and clean
  site_data <- site_data %>%
    mutate(
      DEPARTMENT_ID = as.character(DEPARTMENT_ID) %>% trimws(),
      DEPARTMENT_NAME = iconv(as.character(DEPARTMENT_NAME), from = "Latin1", to = "UTF-8", sub = ""),
      REGION = iconv(as.character(REGION), from = "Latin1", to = "UTF-8", sub = "")
    )

  # Build named lookup vectors (safe and fast, avoids parse/eval and nchar on large concatenated strings)
  loc_map <- setNames(site_data$DEPARTMENT_NAME, site_data$DEPARTMENT_ID)
  reg_map <- setNames(site_data$REGION, site_data$DEPARTMENT_ID)

  # Map DEPARTMENT_IDs in the table to locations/regions. Use both blood and urine DEPARTMENT_ID fields and coalesce.

  
  table1_4_locations <- table1_4_locations %>%
    mutate(
    #   DEPARTMENT_NAME = eval(parse(text = paste0("case_when(\n", case_when_string_location, "\n)"))),
    #   REGION = eval(parse(text = paste0("case_when(\n", case_when_string_region, "\n)")))
    # )
          .id_bld = as.character(d_173836415_d_266600170_d_185243482),
      .id_urn = as.character(d_173836415_d_266600170_d_452847912),
      DEPARTMENT_NAME = dplyr::coalesce( loc_map[.id_bld], loc_map[.id_urn] ),
      REGION = dplyr::coalesce( reg_map[.id_bld], reg_map[.id_urn] )
    ) %>%
    select(-.id_bld, -.id_urn)
  
  return(table1_4_locations)
}

# Apply the updated function to add DEPARTMENT_NAME and Region
table1_4_locations <- add_dept_name(table1_4_locations, "Sanford_Clincal_Locations.csv") 


## Some locations have two IDs from EPIC, need to be combined for tables
table1_4_locations$DEPARTMENT_NAME[table1_4_locations$DEPARTMENT_NAME=="Sioux Falls SD 32nd & Ellis Lab Old" | table1_4_locations$DEPARTMENT_NAME=="Sioux Falls SD 32nd & Ellis Lab"]="Sioux Falls SD 32nd & Ellis Lab "
table1_4_locations$DEPARTMENT_NAME[table1_4_locations$DEPARTMENT_NAME=="Sioux Falls SD 69th & Minnesota Lab A" | table1_4_locations$DEPARTMENT_NAME=="Sioux Falls SD 69th & Minnesota Lab B"]="Sioux Falls SD 69th & Minnesota Lab"
table1_4_locations$DEPARTMENT_NAME[table1_4_locations$DEPARTMENT_NAME=="Sioux Falls SD 69th & Louise Lab A" | table1_4_locations$DEPARTMENT_NAME=="Sioux Falls SD 69th & Louise Lab B"]="Sioux Falls SD 69th & Louise Lab"



## Handle those with missing location IDs
table1_4_locations$DEPARTMENT_NAME[is.na(table1_4_locations$d_173836415_d_266600170_d_185243482) &  
...
table1_4_locations$DEPARTMENT_NAME[is.na(table1_4_locations$DEPARTMENT_NAME) & 
...
table1_4_locations$REGION[table1_4_locations$DEPARTMENT_NAME=="Missing Both Location IDs" | table1_4_locations$DEPARTMENT_NAME=="Unidentified SF Location"] = "Unknown"



## Aggregate counts by DEPARTMENT_NAME and Region
count_all <- table1_4_locations %>%
  count(REGION, DEPARTMENT_NAME) %>%
...
## Only want top 10 collection locations listed per Region. 
...
SF_top10_collections2 <- top10 %>%
  dplyr::select(-REGION) %>%  
   kbl(caption = "Table 1.5 Top Baseline Clinical Collection Locations at Sanford Health by Region",
...
Bo_count <- SF_footnote$n[SF_footnote$REGION=="Bozeman"]
BH_count <- SF_footnote$n[SF_footnote$REGION=="Black Hills"]
UK_count <- SF_footnote$n[SF_footnote$REGION=="Unknown"]

footnote_text2 <- paste0("This table reflects Clinical Collections at the top 10 collection locations in each region. ",
                        "The total number of Clinical Collections in each region are as follows: Bemidji: ", Be_count,
                        "; Bismarck: ", Bi_count, "; Fargo: ", F_count, "; Sioux Falls: ", S_count, 
                         "; Black Hills: ", BH_count, "; Bozeman: ", Bo_count, 
                        "; Missing Location IDs: ", UK_count, ".")
...
spec_by_site_R <- create_tbl_cross1(table4_redone__R, 'Specimens Collected', "Site", "column", " ", " ")
...
spec_by_site_C <- create_tbl_cross1(table4_redone__C, 'Specimens Collected', "Site", "column", " ", " ")
...
tube_dev <- as.data.frame(aggregate(coll_sett_all$tubedev,
                                    ...
                                    FUN = sum)) 
# I get an error messga eunless this line is separated
tube_dev <- tube_dev %>% 
...
Deviation_Freq <- Deviation_Freq %>%  
  mutate(`Total` = Any_Deviation + No_Deviation,
         `Any Deviations` = fmt_count_pct(Any_Deviation, Total, digits = 1),
         `No Deviations` = fmt_count_pct(No_Deviation, Total, digits = 1),
         Site = factor(Site,levels = c("Baylor Scott and White Health",  "HealthPartners",  "Henry Ford Health",
                                       "Marshfield Clinic Health System", "Sanford Health",  "University of Chicago Medicine",
                                       "Kaiser Permanente Colorado", "Kaiser Permanente Georgia", "Kaiser Permanente Hawaii", "Kaiser Permanente Northwest"))) %>% 
  select("Site", "Any Deviations", "No Deviations", "Total") %>% 
   arrange(Site)
...
total_row <- Deviation_Freq %>%
  ...
  mutate(`Any Deviations` = fmt_count_pct(Any_num, Total, digits = 1),
         `No Deviations` = fmt_count_pct(No_num, Total, digits = 1)) %>%
  select(Site, `Any Deviations`, `No Deviations`, Total)
...
bloodtubetype_R <- create_tbl_cross1(research__blood, "Site", "tube_type", "none", "Site", " ")
...
bloodtubetype_C <- create_tbl_cross1(clinic.coltube, "Site", "tube_types", "none", "Site", " ")
...
research.notcol <- notcol_notes %>% filter(all_settings=="Research" & tubecol==104430631) %>%
  mutate(Site=droplevels(as.factor(Site)),
         ...
         notcol.notes = factor(notcol.notes, 
                               levels=c("Short Draw","Participant Attempted", "Other",
                                        "Participant Refusal", "Supply Unavailable",
                                        "Missing Reasons")))



## Reasons not collected by site 
reason_notcol <- create_tbl_cross1(research.notcol, "Site", "notcol.notes", "row", "Site", " ")
...
tube_type_notcolreasons <- create_tbl_cross1(research.notcol, "tube_type", "notcol.notes", "row", "Tube Type", " ")
...
discard.site <- create_tbl_cross1(discard.tb, "Site", "tube_type", "row", "Site", " ")
...
research.survey <- coll_sett_all %>% filter(all_settings == "Research") %>%
  mutate(Site=droplevels(as.factor(Site)),
         ctime = round(as.numeric(difftime(d_222161762,ymd_hms(d_822499427),units="mins")), digits=1),
         tm_biocolsuv = difftime(d_222161762,ymd_hms(d_331584571_d_266600170_d_840048338),units="hours"), #time b/w survey completion to bio collection visit check in
         ...
         time_biochk = difftime(ymd_hms(d_331584571_d_266600170_d_343048998),ymd_hms(d_331584571_d_266600170_d_840048338),units="mins"),
...
BUM_status <- create_tbl_cross1(research.survey, "d_265193023", "Site", "column", " ", " ")
...
precovid_BUMtime <- create_tbl_cross1(precovR, "checktmvisit", "Site", "column", " ", " ")
colnames(precovid_BUMtime)[ncol(precovid_BUMtime)] = "Total Research Surveys"

postcovid_BUMtime <- create_tbl_cross1(postcovR, "checktmvisit", "Site", "column", " ", " ")
...
Table21R_Before <- summarize_time(
  precovR_comp,
  group_var = Site,
  n_label   = "Number of Submitted Research Surveys",
  time_var  = ctime,
  perc_95   = TRUE
)

Table21R_After <- summarize_time(
  postcovR_comp,
  group_var = Site,
  n_label   = "Number of Submitted Research Surveys",
  time_var  = ctime,
...
research.vt.length <- research.survey %>%  mutate(time=as.numeric(time_biochk)) %>% 
  # Bio team explicitly asked to remove from table for QC issue, not updating prod data
  filter(Connect_ID!="9005924628") 

research.vt.length.table <- summarize_time(
  research.vt.length,
  group_var = Site,
  n_label   = "Number of Research Collection Visits",
  time_var  = time,
  perc_90   = TRUE,
  perc_95   = TRUE,
  perc_98   = TRUE
...
Section3_Figures <- function(site, start_date, title) {
  ...
  weekly_summary <- filtered %>%
    ...
    summarise(n = sum(!is.na(checktmvisit)),
              ...
              .groups = "drop")  
    # For weeks that have no data, manually add them and set to 0 so the trend line is not mislaeading
    # complete(Site,
    #          col.Monday.date = seq(min(col.Monday.date), max(col.Monday.date), by = "week"),
    #          fill = list(n = 0, hours3 = 0)) %>%
    # mutate(pct3 = ifelse(n == 0, 0, 100 * round(hours3 / n, 2)))
  ...
  plots3 <- ggplot(weekly_summary, aes(x = as.Date(col.Monday.date), y = pct3)) +
    ...
    scale_x_date(name = "Date",
                 #Bio Team wants first letter of the month - year
                 ## b for abbreviate, 1,1 for first letter of the month
                 #labels = function(x) paste0(substr(format(x, "%b"), 1, 1), "-", format(x, "%y")), 
                 date_labels = "%y-%m-%d", 
                 date_breaks = "4 weeks",
                 limits = c(as.Date(start_date), as.Date(currentDate - 1)), expand = c(0, 0)) +
...
# research.4_1 <- Section3_Figures("Henry Ford Health", "2022-06-20","Figure 3.1: Baseline- Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Research Collection Visit Check In, Henry Ford Health") + 
#   labs(caption="No data point exists for weeks when there were no biospecimen collections.")+
#   theme(plot.caption = element_text(size = 8, hjust=0))

research.4_2 <- Section3_Figures("Sanford Health", "2022-06-20", "Figure 3.1: Baseline- Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Research Collection Visit Check In, Sanford Health") + 
  labs(caption="No data point exists for weeks when there were no biospecimen collections.")+
  theme(plot.caption = element_text(size = 8, hjust=0))

# research.4_3 <- Section3_Figures("University of Chicago Medicine", "2022-06-20", "Figure 3.3: Baseline- Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Research Collection Visit Check In, University of Chicago Medicine") + 
#   labs(caption="No data point exists for weeks when there were no biospecimen collections.")+
#   theme(plot.caption = element_text(size = 8, hjust=0))

research.4_4 <- Section3_Figures("Baylor Scott and White Health", "2024-06-24", "Figure 3.2: Baseline- Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Research Collection Visit Check In, Baylor Scott and White Health") + 
  labs(caption="No data point exists for weeks when there were no biospecimen collections.")+
  theme(plot.caption = element_text(size = 8, hjust=0))
...
biospe__all <-  research.survey %>%  filter(d_410912345==353358909 & !is.na(d_678166505) & !is.na(Site_location) & !is.na(Site))

#BPTL null- not used in this table, but need to understand the denominator inconsistencies 
biospe__missings = biospe__all %>%  filter(is.na(d_143615646_d_926457119) & is.na(d_232343615_d_926457119) & 
...
coll_bptl_diff <- biospe__all %>%  
  ...
  mutate(Site_location = factor(Site_location,
                                levels=c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving",
                                         ...
                                         "Temple Roney", "Waco - MacArthur","Temple Westfield","Waco Fishpond","Killeen Main",
                                         "Southwest Family Medicine","Grapevine","Plano Heart",
                                         "Missing BSWH location",
                                         "HP Research Clinic", "HP Park Nicollet", "Missing HP location",
                                         "HFH K-13 Research Clinic","HFH Cancer Pavilion Research Clinic",
                                         "HFH Livonia Research Clinic", "HFH Pop-Up", "HFH Detroit Northwest",
                                         "In-home Collection", "HFH Jackson","Missing HFH location",
...
research_coll_bptl1 <- create_tbl_cross1(rsch_coll_bptl_days, "Site", "coll_days_diff", "row", "Site", " ")
...
rsch_loc_coll_bptl <- create_tbl_cross1(rsch_coll_bptl_days, "Site_location", "coll_days_diff", "row", "Site", " ")
rsch_loc_coll_bptl <- as.data.frame(rsch_loc_coll_bptl)
colnames(rsch_loc_coll_bptl)[1] <- "Collection_Location"



ctable_rsch_loc_bptl <- rsch_loc_coll_bptl %>%
  mutate(Site_Group = case_when(
    Collection_Location %in% c("UC-DCAM", "Ingalls Harvey", "River East", "South Loop", "UCM Pop-Up", 
                                 "Orland Park", "Missing UC location") ~ "University of Chicago Medicine",
    Collection_Location %in% c("HP Research Clinic",  "HP Park Nicollet", "Missing HP location") ~ "HealthPartners",
    Collection_Location %in% c("Lake Hallie", "Marshfield", "MF Pop-Up", "Minocqua", "Weston", "Wisconsin Rapids", "Rice Lake", 
                                 "Colby Abbotsford", "Merrill", "Stevens Point",
                                 "Neillsville", "Missing MF location") ~ "Marshfield Clinic Health System",
    Collection_Location %in% c("Sioux Falls Imagenetics", "Fargo South University", 
                               "Edith Sanford Breast Center (coded as Sanford Center)", 
                                  "Edith Sanford Breast Center","Fargo Amber Valley","Bemidji Clinic",
                               "Missing SF location") ~ "Sanford Health",
    Collection_Location %in% c("HFH K-13 Research Clinic", "HFH Livonia Research Clinic", 
                               "HFH Cancer Pavilion Research Clinic", "HFH Pop-Up", 
                                 "HFH Detroit Northwest", "In-home Collection","HFH Jackson",
                               "Missing HFH location") ~ "Henry Ford Health",
    Collection_Location %in% c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving","Irving",
                               "NTX Biorepository","BCC- Fort Worth", 
                                 "North Garland", "Temple CDM","Temple Roney", "Waco - MacArthur", "Temple Westfield",
                               "Waco Fishpond","Killeen Main",
                               "Southwest Family Medicine","Grapevine","Plano Heart",
                               "Missing BSWH location") ~ "Baylor Scott and White Health",
    Collection_Location %in% c("Unidentified location") ~ "Other",
    TRUE ~ " "
  )) 
...
total_row <- ctable_rsch_loc_bptl %>% filter(Collection_Location == "Total")

# Remove the Total row from the main table
rsch_coll_times <- ctable_rsch_loc_bptl %>%
  filter(Collection_Location != "Total") %>%
  arrange(Site_Group, Collection_Location)
...
rsch_coll_times52 <- knitr::kable(rsch_coll_times[, c(1:7)], 
  ...
  landscape() %>% 
  footnote(general = paste0("Note: At the time of reporting, ", dim(biospe__missings)[[1]], 
...
clinical.survey <- coll_sett_all %>% filter(all_settings == "Clinical") %>%
  mutate(clin.sv = case_when(d_253883960 == 231311385 ~ "Submitted",
         ...
         d_173836415_d_266600170_d_139245758_clc = ymd_hms(d_173836415_d_266600170_d_139245758), ##urine
         d_173836415_d_266600170_d_982213346_clc = ymd_hms(d_173836415_d_266600170_d_982213346), ##blood collected
         d_173836415_d_266600170_d_740582332_clc = ymd_hms(d_173836415_d_266600170_d_740582332), ##Autogenerated date/time stamp for Any specimen collected at RRL
         clinic.svtime =  difftime(ymd_hms(d_764863765),ymd_hms(d_534669573),units="hours"),
         ctime = round(as.numeric(difftime(ymd_hms(d_764863765),ymd_hms(d_534669573),units="mins")), digits=1)) ##survey time from start to complete

clinical.survey <- clinical.survey %>% 
  mutate(clc.coltm  = do.call(pmin, c(select(.,ends_with('clc')),na.rm=TRUE)),
         tm_biocolsuv =  difftime(ymd_hms(d_764863765),clc.coltm,units="hours")) #time between survey completion to biospecimen collection visit check in
...
BU_status <- create_tbl_cross1(clinical.survey, "clin.sv", "Site", "column", " ", " ")
...
coll_comp_BU <- create_tbl_cross1(clin_comp, "checktmvisit", "Site", "column", " ", " ")
...
ClinSrvy_Lgth_Before <- summarize_time(
  precovC_comp,
  group_var = Site,
  n_label   = "Number of Submitted \n Clinical Surveys",
  time_var  = ctime,
  perc_95   = TRUE
)

ClinSrvy_Lgth_After <- summarize_time(
  postcovC_comp,
  group_var = Site,
  n_label   = "Number of Submitted \n Clinical Surveys",
  time_var  = ctime,
...
clinical.survey <- clinical.survey %>%  
  mutate(coll_to_DB_BU = as.numeric(difftime(ymd_hms(d_915838974),
                                             pmin(ymd_hms(d_173836415_d_266600170_d_982213346), 
                                                  ymd_hms(d_173836415_d_266600170_d_139245758), na.rm=TRUE),units="hours")))


BU_coll_DB_comp <- summarize_time(
  clinical.survey %>% filter(coll_to_DB_BU >= 0),
  group_var = Site,
  n_label   = "Number of Clinical Collection Visits",
  time_var  = coll_to_DB_BU,
  std_dv    = TRUE,
  perc_95   = TRUE
)
...
clinical.survey <- clinical.survey %>%  
  mutate(trig_to_comp_BU = as.numeric(difftime(ymd_hms(d_764863765),ymd_hms(d_915838974),units="hours")))


BU_trig_comp <- summarize_time(
  clinical.survey,
  group_var = Site,
  n_label   = "Number of Clinical Collection Visits",
  time_var  = trig_to_comp_BU,
  std_dv    = TRUE,
  perc_95   = TRUE
)
...
avg_draw_order_time <- 
  clinical.survey %>% filter(Site != 'HealthPartners' &
                             Site != "Sanford Health" &
                             Site != 'University of Chicago Medicine') %>% 
  mutate(time = as.numeric(difftime(pmin(as.Date(d_173836415_d_266600170_d_982213346),
                                         as.Date(d_173836415_d_266600170_d_139245758)),
                                    as.Date(d_173836415_d_266600170_d_184451682),
                                    units = "days")))

avg_draw_order <- summarize_time(
  avg_draw_order_time,
  group_var = Site,
  n_label   = "Number of Clinical Collections",
  time_var  = time,
  perc_80   = TRUE,
  perc_90   = TRUE,
  perc_95   = TRUE
...
biospe__2 = clinical.survey %>%  filter(d_410912345==353358909 & !is.na(Clin_Site_location) & !is.na(Site)) %>% 
  mutate(Site=droplevels(as.factor(Site)))


#BPTL null- not used in this table, but used in the footnote to determine how many participants have missing data, if any
biospe__2_missings = biospe__2 %>% filter((is.na(d_173836415_d_266600170_d_822274939) & is.na(d_173836415_d_266600170_d_139245758)) |
...
#Used for this table- BPTL not null
biospe__2= biospe__2 %>%  filter((!is.na(d_232343615_d_926457119) | !is.na(d_299553921_d_926457119) | !is.na(d_376960806_d_926457119) |
                                         !is.na(d_454453939_d_926457119) | !is.na(d_505347689_d_926457119) | !is.na(d_589588440_d_926457119) |
                                         !is.na(d_652357376_d_926457119) | !is.na(d_677469051_d_926457119) | !is.na(d_683613884_d_926457119) |
                                         !is.na(d_703954371_d_926457119) |!is.na(d_838567176_d_926457119) | !is.na(d_958646668_d_926457119) |
                                         !is.na(d_973670172_d_926457119))) 
...
coll_bptl_diff2 <- biospe__2  %>% group_by(as.character(Connect_ID),Site, Clin_Site_location) %>% 
  mutate(any_collection = min(d_232343615_d_926457119, d_299553921_d_926457119, d_376960806_d_926457119, d_454453939_d_926457119, d_505347689_d_926457119,
         ...
         clin_coll_bu = min(d_173836415_d_266600170_d_982213346, d_173836415_d_266600170_d_139245758, na.rm=T),
...
crosstable.clin_coll_days <- create_tbl_cross1(clin_coll_bptl_diff3, "Site", "coll_days_diff", "row", "Site", " ")
...
clin_coll_days <- create_tbl_cross1(clin_coll_bptl_diff3, "Clin_Site_location", "coll_days_diff", "row", "Collection Location", " ")
...
clin_coll_days <- clin_coll_days %>%
  mutate(Site_Group = case_when(
    `Collection Location` %in% c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", 
                                 ...
                                 "E Jefferson Pathology", "Woodhaven Pathology", "CANPAV Pathology OP Lab", 
                                 "Jackson Professional Building", "Missing Both Location IDs", "Missing HFH location") ~ "Henry Ford Health", 
    `Collection Location` %in% c("Elk River", "Chanhassen", "Park Nicollet Minneapolis Clinic", "Brooklyn Center", 
                                 "Clinic Stillwater", "New Richmond Clinic", 
                                 "Burnsville Clinic and Specialty Center", "Andover Clinic","St. Paul Clinic- Wabasha",  
                                 "Olivia Hospital and Clinic","Hutchinson Health Clinic",
...
ctable_clin_loc_bptl <- knitr::kable(clin_coll_times[, c(1:7)], 
  ...
  kable_styling(latex_options = "scale_down", full_width = FALSE, font_size=7) %>% 
  footnote(general= paste0("At the time of reporting, ", dim(biospe__2_missings)[[1]], " of the Clinical Collections did not have collection and/or receipt date/time information available."), general_title = " ") %>% 
  landscape()
...
```{r Tm_Verif_BL_Coll, eval=TRUE,echo=FALSE,include=FALSE,results='hold'}


##--------------- Time from Verif to BL Collection -------------------------

baseline_colct <- coll_sett_all %>%  
  filter(d_331584571== 266600170 & !is.na(d_914594314) & !is.na(d_556788178)) %>% 
  mutate(time = as.numeric(difftime(as.Date(d_556788178),d_914594314,units="days")))





## Research, ON OR AFTER 8/1/22

BL_coll_rsch_8 <- baseline_colct %>%  
  filter(d_914594314>=as.Date("2022-08-01") & all_settings=="Research")


verif_BSL_colct_rsch_8 <- summarize_time(
  BL_coll_rsch_8,
  group_var = Site,
  n_label   = "Number of Research Collection Visits",
  time_var  = time,
...
verif_BSL_colct_rsch_8_total <- summarize_time(BL_coll_rsch_8, time,
                                               n_label = "Number of Research Collection Visits",
                                               std_dv    = TRUE,perc_95 = TRUE) %>%   
  add_column(Site = c("All Sites"), .before = 1)


verif_BSL_colct_rsch_8 <- rbind(verif_BSL_colct_rsch_8, verif_BSL_colct_rsch_8_total)

# for the footnote
verif_BSL_colct_rsch_8_footnote <- baseline_colct %>%  filter(d_914594314<as.Date("2022-08-01"))





## Clinical, BEFORE 2/1/23
BL_coll_clin_2Before <- baseline_colct %>%  
  filter(d_914594314<as.Date("2023-02-01") & all_settings=="Clinical")


verif_BSL_colctC_2Before <- summarize_time(
  BL_coll_clin_2Before,
  group_var = Site,
  n_label   = "Number of Clinical Collections",
  time_var  = time,
  std_dv   = TRUE,
...
verif_BSL_colctC_2Before_total <- summarize_time(BL_coll_clin_2Before, time,
                                               n_label = "Number of Clinical Collections",
                                               std_dv   = TRUE, perc_90 = TRUE, perc_95 = TRUE) %>%   
  add_column(Site = c("All Sites"), .before = 1)


verif_BSL_colctC_2Before <- rbind(verif_BSL_colctC_2Before, verif_BSL_colctC_2Before_total)




## Clinical, ON OR AFTER 2/1/23

BL_coll_clin_2After <- baseline_colct %>%  
  filter(d_914594314>=as.Date("2023-02-01")  & all_settings=="Clinical")


verif_BSL_colctC_2After <- summarize_time(
  BL_coll_clin_2After,
  group_var = Site,
  n_label   = "Number of Clinical Collections",
  time_var  = time,
...
verif_BSL_colctC_2After_total <- summarize_time(
  BL_coll_clin_2After,
  n_label   = "Number of Clinical Collections",
  time_var  = time,
  std_dv   = TRUE,
...
verif_BSL_colctC_2After <- rbind(verif_BSL_colctC_2After, verif_BSL_colctC_2After_total)




### Verification to Collection (R+C) Total 
verif_BL_RC_denom <- baseline_colct %>%  
  #Clin & Rsch became available at different times
  filter( (all_settings=="Clinical" & d_914594314>=as.Date("2023-02-01")) | 
          (all_settings=="Research" & d_914594314>=as.Date("2022-08-01")))

verif_BL_RC <- summarize_time(
    verif_BL_RC_denom,
    time_var  = time,
    std_dv   = TRUE,
    perc_90   = TRUE,
...
## Verification to Collection (R+C) by Campaign Type

verif_BL_Af <- baseline_colct %>%  
  filter(d_914594314>=as.Date("2022-06-01")) %>%  
  mutate(campaign = case_when((state_d_667474224==348281054 | state_d_667474224==324692899) ~ "Upcoming Appointment",
...
verif_baseline_Af <- verif_BL_Af %>% 
  ...
  dplyr::summarize(#'Number of Baseline Collections1'=n()[campaign=="Upcoming Appointment"],
    ...
    Pct_95_1= round(quantile(time[campaign=="Upcoming Appointment"], 0.95, na.rm = TRUE), digits=1),
    ...
    Pct_95_2= round(quantile(time[campaign=="All Other Campaigns"], 0.95, na.rm = TRUE), digits=1),
    Max2 = round(max(time[campaign=="All Other Campaigns"],na.rm = TRUE), digits=1),
    'Total Number of Baseline Collections'=n()) %>%
  ungroup() %>%  
  select(Site,  'Number of Baseline Collections1', Min1, Median1, Mean1, SD1, Pct_95_1, Max1, 
         'Number of Baseline Collections2', Min2, Median2, Mean2, SD2, Pct_95_2, Max2, 'Total Number of Baseline Collections')
...
```{r Tm_Verif_Draw_Order, eval=TRUE,echo=FALSE,include=FALSE,results='hold'}



ver_draw <- biocol.tb %>%  
  filter(!is.na(d_173836415_d_266600170_d_769615780) & d_914594314>=as.Date("2023-02-01")  & 
           !(Site %in% c('HealthPartners', 'Sanford Health', 
                         'University of Chicago Medicine',
                         'Henry Ford Health'))) %>% 
  mutate(Site = droplevels(Site))

verif_draw <- ver_draw %>%  mutate(time = as.numeric(difftime(d_173836415_d_266600170_d_769615780,d_914594314,units="days")))
...
verif_draw <-verif_draw %>% #group_by(Site) %>% filter(time>=0) %>%
  mutate(checktmvisit = case_when(as.numeric(time) < 0 ~ "Negative Time",
                                  ...
                                  as.numeric(time) <8 ~ "4-7 Days",
                                  as.numeric(time) <31 ~ "8-30 Days",
         ...
         checktmvisit = factor(checktmvisit, levels=c("Negative Time", "Within 1 Day", "1-3 Days",
                                                      "4-7 Days", "8-30 Days", "More than 30 Days", "Missing Time Stamp(s)")))

verif_draw_order <- create_tbl_cross1(verif_draw, "checktmvisit", "Site", "column", " ", " ")
...
avg_bldorder_present_time <- table1 %>% 
  filter(d_878865966 == "No blood collected" &
           !is.na(d_173836415_d_266600170_d_769615780) &
           Site != 'HealthPartners' &
           Site != 'Henry Ford Health' &
           Site != 'Sanford Health') %>% 
  mutate(time = as.numeric(difftime(currentDate,as.Date(d_173836415_d_266600170_d_769615780),
                                    units = "days")))

avg_bldorder_present <- summarize_time(
  avg_bldorder_present_time,
  group_var = Site,
  n_label   = "Number of Unfilled \n Blood Draw Orders",
  time_var  = time,
  perc_80   = TRUE,
  perc_90   = TRUE,
  perc_95   = TRUE
...
#### Update from Erin: 
# BioClin_BldOrderPlaced_v1r0 is yes OR NO it means the order had originally been placed.
# (BioClin_BldOrderPlaced_v1r0 is no the order was originally placed and later canceled.) 
# BioClin_BldOrderPlaced_v1r0= null means it has not yet been placed.

Table_bld_draw= table1 %>%  filter(grepl("^Kaiser", Site)) %>% ## excluding hybrid sites
  mutate(Site = droplevels(Site),
         bld_placed= case_when((BioClin_BldOrderPlaced_v1r0=="Yes" | 
                                  BioClin_BldOrderPlaced_v1r0=="No") ~ "Blood Draw Order Placed",
                               TRUE ~ "No Blood Draw Order Placed"),
  ...
  bold_labels() %>%
  modify_header() %>%
  modify_caption("Table 4.10: Baseline Blood Draw Order Status Among Verified Participants") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) %>%
  kable_styling(latex_options = "scale_down")
...
ver_date_diff <-table1  %>% filter(biocol.appoint=="No Baseline Collection")  %>% 
  mutate(day_past = difftime(currentDate, d_914594314, units="days"),
...
crosstable.coll_bptl1 <- create_tbl_cross1(ver_date_diff, "Site", "coll_days_diff", "row", "Site", " ")
...
\FloatBarrier
```{r KnitrTables1.5b, eval=TRUE,echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, results='asis'}

#Testing SF requested changes
add_footnote(SF_top10_collections2,footnote_text2, notation= "none")
...
tab2_8= knitr::kable(bloodtubetype_R, 
  ...
  kable_styling(latex_options = "scale_down")
add_footnote(tab2_8, "Note: Collection of ACD tube was discontinued on 03/01/2024; Collection of Heparin tube was discontinued on 03/26/2025; Streck tube collection was introduced at different time points across sites.", notation = "none")
...
devns <- knitr::kable(Deviation_Freq,
             ...
             caption='Table 2.5: Baseline- Deviation Status Among Biospecimen Tubes Collected Reported by the Site',
             row.names=FALSE, align=c("l",rep("c", 3)), booktabs = TRUE )
...
tabe2_12 <- knitr::kable(reason_notcol,  
  ...
  kable_styling(latex_options = "scale_down")
add_footnote(tabe2_12,c("Note: The 'Short Draw' is applicable to blood collection tubes only.",
                        "Note: 'Supply Unavailable' counts reflect misclassification of reason not collected due to periods of time when certain tubes were listed in the Biospecimen Dashboard but not expected to be collected at a site, either due to removal or staggered addition of a tube type to collection protocol."),
             notation="none")
...
add_footnote(tab21_ra,"Note: The COVID-19 survey questions were separated from the Biospecimen Survey on July 5, 2023. ", notation = "none") %>% 
  add_footnote("Research biospecimen surveys with a negative completion time have been excluded.", notation = "none")
...
add_footnote(tab21_cb,"Note: The COVID-19 survey questions were separated from the Biospecimen Survey on July 5, 2023.", notation = "none") %>% 
  add_footnote("Note: Clinical biospecimen surveys with a negative completion time have been excluded..", 
               notation = "none")
...
knitr::kable(verif_BSL_colct_rsch_8,
  ...
  kable_styling(latex_options = "scale_down") %>% 
  footnote(general = "August 2022 marks when research collection appointments were routinely offered to all new recruits at or near the time of verification. UChicago participant verification largely occurs at or near the time of collection.",
           footnote_as_chunk = TRUE, 
           escape = FALSE, 
           threeparttable = TRUE) %>% 
  footnote(paste0(nrow(verif_BSL_colct_rsch_8_footnote), " research collection visits occurred for participants verified before 8/1/2022."))





BSL_A <- knitr::kable(verif_BSL_colctC_2Before,
...
BSL_B <-knitr::kable(verif_BSL_colctC_2After,
...
knit_verif_BL_RC <- knitr::kable(verif_BL_RC,
              caption='Table 4.5 Baseline- Time (in Days) from Verification to Collection (Research or Clinical Collection) Among Participants ',
              row.names=FALSE,  align=c("l", rep("c", 9)),digits=1, booktabs = TRUE) %>% 
  kable_styling(latex_options = "scale_down")
add_footnote(knit_verif_BL_RC,"Note: This table includes data on participants who completed a research collection that were verified on or after 08/01/2022 and those who completed a clinical collection that were verified on or after 02/01/2023, which correspond with the dates each type of collection became available.", notation = "none")




AF26 <- knitr::kable(verif_baseline_Af_final,
                     caption = 'Table 4.6: Baseline- Time (in Days) from Verification to Collection Among Participants Verified on or After 6/01/2022 by Campaign Type',
                     row.names=FALSE, booktabs = T, linesep = "", format.args = list(big.mark = ","), 
                     align = c("l",  rep("c", 15)), 
                     col.names=c("Site",  'Number of Collections',"Min", "Median", "Mean", "SD", "Perc.95", "Max", 'Number of Collections', 
                                 "Min", "Median", "Mean", "SD", "Perc.95",  "Max","Total Baseline Collections") ) %>% 
  kable_styling(latex_options = "scale_down") %>% 
  add_indent(seq(1, nrow(verif_baseline_Af_final)-1)) %>% 
  add_header_above(c(" " = 1, "Appointment Based" = 7, "All Other Campaigns" = 7,  " "=1)) %>% landscape() 
add_footnote(AF26, "Note: Appointment Based includes “Screening appointment” and “Non-screening appointment” campaign types. All Other Campaigns include “Random”, “Demographic Group”, “Aging out of study”, “Geographic group”, “Post-Screening Selection”, “Technology adapters”, “Low-income/health professional shortage areas”, and “Other” campaign types.", notation = "none")




tabl27 <- knitr::kable(verif_draw_order,
                       caption='Table 4.7: Baseline- Time (in Days) from Verification to Draw Order Placement',
...
add_footnote(tabl27, "Time from verification to draw order placement calculated for participants verified on or after February 1, 2023. HealthPartners, University of Chicago Medicine and Sanford Health are excluded due to the limited availability of draw order placement data. Henry Ford is excluded because participants have collection type options, therefore, the placement of draw orders post-verification may be delayed.", notation="none") %>% 
  add_footnote("Negative times reflect the technological timing of the API calls to send verification status to C4CPDS, which may occur after the site has placed the order.", notation="none")


tab4_9 <- knitr::kable(avg_draw_order, 
                       caption='Table 4.8: Baseline- Time (in Days) from Draw Order Placement to Clinical Collection', 
...
tab4_10 <- knitr::kable(avg_bldorder_present, 
                        caption='Table 4.9: Baseline- Time (in Days) from Blood  Draw Order Placement to Present Date by Site \n for Participants with No Baseline Clinical Blood Collection', 
                        row.names=FALSE, align=c("l", rep("c", 11)),digits=1, booktabs = TRUE) %>% 
...
knitr::kable(crosstable.coll_bptl1, 
             ...
             caption='Table 4.11: Baseline- Time (in Days) Since Verification Among Participants with No Biospecimen Collection ', 
...
add_footnote(all_rsch_days, paste0("At the time of reporting, ", dim(biospe__missings)[[1]], 
...
clin_bptl <- knitr::kable(crosstable.clin_coll_days, 
             ...
             row.names=FALSE, align=c("l", rep("c", 6)), booktabs = TRUE) %>% 
  kable_styling(latex_options = "scale_down")
add_footnote(clin_bptl, paste0("At the time of reporting, ", dim(biospe__2_missings)[[1]], 
...
```{r Figs_Collection_Weekdays, warning=FALSE, eval=TRUE,echo=FALSE, message=FALSE}

## Collection to Receipt Times, Stratified by Appt Day of the Week

coll_bptl_diff <- coll_bptl_diff %>% 
  mutate(hrs_past = difftime(ymd_hms(any_collection), d_678166505, units="hours"),
...
coll_bptl_diff2 <- coll_bptl_diff2 %>% 
  mutate(hrs_past = difftime(ymd_hms(any_collection), ymd_hms(clin_coll_bu), units="hours"),
         hrs_past = as.numeric(hrs_past),
...
research_plot <- function(site_name, start_date, plot_title){
  
  data <- coll_bptl_diff %>% filter(Site==site_name)
  
  ggplot(data, aes(x=as.Date(d_678166505), y=hrs_past, color = Weekdays)) + 
  geom_point() + 
...
clinical_plot <- function(site_name, start_date, plot_title){
  
  data <- coll_bptl_diff2 %>% filter(Site==site_name)
  
  ggplot(data, aes(x = as.Date(clin_coll_bu), y = hrs_past, color = Weekdays)) + 
  geom_point() + 
    scale_x_date(name="Collection Date",
                 #Bio Team wants first letter of the month - year
                 ## b for abbreviate, 1,1 for first letter of the month
...
research_plot("Marshfield Clinic Health System", "2022-06-20", "Figure 5.1: Baseline Collection-To-Receipt Time (in Hours), \n Marshfield")

research_plot("Sanford Health", "2022-06-20", "Figure 5.2: Baseline Collection-To-Receipt Time (in Hours), \n Sanford Health Research Setting")
clinical_plot("Sanford Health", "2024-03-25", "Figure 5.3: Baseline- Collection-To-Receipt Time (in Hours), \n Sanford Health Clinical Setting")

clinical_plot("Kaiser Permanente Colorado", "2022-10-10", "Figure 5.4: Baseline Collection-To-Receipt Time (in Hours),  \n Kaiser Permanente Colorado")
clinical_plot("Kaiser Permanente Georgia", "2022-10-10", "Figure 5.5: Baseline Collection-To-Receipt Time (in Hours),  \n Kaiser Permanente Georgia")
clinical_plot("Kaiser Permanente Hawaii", "2022-10-10", "Figure 5.6: Baseline Collection-To-Receipt Time (in Hours),  \n Kaiser Permanente Hawaii")
clinical_plot("Kaiser Permanente Northwest", "2022-10-10", "Figure 5.7: Baseline Collection-To-Receipt Time (in Hours),  \n Kaiser Permanente Northwest")

research_plot("HealthPartners", "2022-06-20", "Figure 5.8: Baseline- Collection-To-Receipt Time (in Hours), \n HealthPartners Research Setting")
clinical_plot("HealthPartners", "2023-10-23", "Figure 5.9: Baseline- Collection-To-Receipt Time (in Hours), \n HealthPartners Clinical Setting")

research_plot("Henry Ford Health", "2022-06-20", "Figure 5.10: Baseline- Collection-To-Receipt Time (in Hours), \n Henry Ford Research Setting")
clinical_plot("Henry Ford Health", "2023-04-24", "Figure 5.11: Baseline- Collection-To-Receipt Time (in Hours), \n Henry Ford Clinical Setting")
...
Section6_plot_function2 <- function(data, start_date, title, site, settings, color_var) {
  ...
  if (site == "Henry Ford Health" && settings=="Research"){
    ...
    scale_color_manual(values=c("Wyandotte"="#B1C7D9", "Macomb"="#164C71", "West Bloomfield"="#FDBE19", "Royal Oak"="#F8D991", 
                              ...
                              "E Jefferson Pathology"="#74B0C7", "Woodhaven Pathology" = "#FBE5B5", "CANPAV Pathology OP Lab" = "#A8B7C5",
                              "Jackson Professional Building"="#9A9DA3", "Missing Both Location IDs" = "black"),
                       guide = guide_legend(nrow = 8, byrow = TRUE)) +
  ...
  theme(
    ...
    plt <- ggplot(weekly_counts, aes(x = as.Date(col.Monday.date), y = n, color = .data[[color_var]])) +
      scale_color_manual(values=c("Fargo South University"="#3C989E", "Sioux Falls Imagenetics"="#CC7D15",
                                ...
                                "Bemidji Clinic"="#164C71")) +
    guides(color = guide_legend(nrow = 4, byrow = TRUE, title = "Collection Location")) +
    labs(color = "Collection Location") 
    ...
    plt <- ggplot(weekly_counts, aes(x = as.Date(col.Monday.date), y = n, color = .data[[color_var]])) +
      scale_color_manual(values=c("NSC"="#99C0C4", "Elk River"="#565C65", "Chanhassen"="#CC7D15", "Park Nicollet Minneapolis Clinic"="#FDBE19",
                                ...
                                "Burnsville Clinic and Specialty Center"= "#797D83", "St. Paul Clinic- Wabasha"="#164C71", 
                                "NSC"="#309EBD", "Andover Clinic"="#B1C7D9", "Olivia Hospital and Clinic"="#FBE5B5",
                                "Hutchinson Health Clinic"="#B9D7E3"),
    ...
    plt <- ggplot(weekly_counts, aes(x = as.Date(col.Monday.date), y = n, color = .data[[color_var]])) +
      scale_color_manual(values=c("BCC- Fort Worth"="#B1C7D9", "BCC- HWC"="#164C71", "BCC- Worth St"="#FDBE19", 
                                ...
                                "Waco - MacArthur"="#99C0C4", "Temple Westfield"="#B9D7E3",
                                "Waco Fishpond"="#A8B7C5","Killeen Main"= "#797D83",
                                "Southwest Family Medicine"="#648EB4","Grapevine"="#51708A","Plano Heart"="#F6CC6C"),
...
```{r Section6_Figures1, eval=TRUE,echo=FALSE, warning = FALSE, message = FALSE}
...
```{r Section6_Figures2, fig.height=7, eval=TRUE,echo=FALSE, warning = FALSE, message = FALSE}
...
Section6_plot_function(coll_sett_all,start_date = "2022-06-20", site = "Sanford Health", 
                       title = "Figure 6.6 Baseline- Weekly Biospecimen Collections, \n Sanford Health")
#Figure 6.12
Section6_plot_function(coll_sett_all,start_date = "2022-06-20", site = "Sanford Health", 
                       color_by_setting = TRUE, color_var="all_settings",
                       title = "Figure 6.7 Baseline- Weekly Biospecimen Collections by Collection Setting, \n Sanford Health")
#Figure 6.13
Section6_plot_function2(coll_sett_all,start_date = "2022-06-20", 
                        title="Figure 6.8: Baseline- Weekly Research Biospecimen Collections \n by Collection Location, Sanford Health", 
...
Section6_plot_function(coll_sett_all,start_date = "2022-06-20", site = "HealthPartners", 
                       color_by_setting = TRUE, color_var="all_settings",
                       title = "Figure 6.9 Baseline- Weekly Biospecimen Collections by Collection Setting, \n HealthPartners")
#Figure 6.15
Section6_plot_function2(coll_sett_all,start_date = "2023-11-20", 
                        title="Figure 6.10: Baseline- Weekly Clinical Biospecimen Collections \n  by Clinic Location, HealthPartners", 
...
Section6_plot_function2(coll_sett_all,start_date = "2024-06-24", 
                        title="Figure 6.12: Baseline- Weekly Research Biospecimen Collections \n by Collection Location, Baylor Scott and White Health", 
...
# Baseline Home Mouthwash Collections- Standard Workflow
This section does not include data on participants who had mouthwash collected in the research setting and later had a home mouthwash kit sent to them for re-collection.
\vspace{-0.5em}
...
kitstatus <- "SELECT Connect_ID, 
  ...
  CASE
    WHEN state_d_706256705 = '536341288' OR state_d_435027713 = '536341288' THEN 'Female'
    WHEN state_d_706256705 = '654207589' OR state_d_435027713 = '654207589' THEN 'Male'
    ELSE 'Unknown'
    END AS sex,
...
DATE(d_173836415_d_266600170_d_319972665_d_661940160) as d_173836415_d_266600170_d_319972665_d_661940160, d_173836415_d_266600170_d_319972665_d_687158491, 
DATE(d_173836415_d_266600170_d_319972665_d_826941471) as d_173836415_d_266600170_d_319972665_d_826941471,
d_173836415_d_266600170_d_319972665_d_379252329, 
DATE(d_173836415_d_266600170_d_319972665_d_759651991) as d_173836415_d_266600170_d_319972665_d_759651991,

DATE(d_173836415_d_266600170_d_541483796_d_661940160) as d_173836415_d_266600170_d_541483796_d_661940160, 
DATE(d_173836415_d_266600170_d_541483796_d_759651991) as d_173836415_d_266600170_d_541483796_d_759651991, d_173836415_d_266600170_d_541483796_d_687158491, 
DATE(d_173836415_d_266600170_d_541483796_d_826941471) as d_173836415_d_266600170_d_541483796_d_826941471, d_173836415_d_266600170_d_541483796_d_379252329,

DATE(d_173836415_d_266600170_d_641006239_d_661940160) as d_173836415_d_266600170_d_641006239_d_661940160, 
DATE(d_173836415_d_266600170_d_641006239_d_759651991) as d_173836415_d_266600170_d_641006239_d_759651991, d_173836415_d_266600170_d_641006239_d_687158491, 
DATE(d_173836415_d_266600170_d_641006239_d_826941471) as d_173836415_d_266600170_d_641006239_d_826941471, d_173836415_d_266600170_d_641006239_d_379252329, 

DATE(d_784810139) as d_784810139, DATE(d_389890053) as d_389890053, DATE(d_843688458) as d_843688458, DATE(d_543379310) as d_543379310, d_100767870, 
DATE(d_517311251) as d_517311251, DATE(d_832139544) as d_832139544, DATE(d_770257102) as d_770257102, DATE(d_264644252) as d_264644252, DATE(d_199471989) as d_199471989,

d_265193023, d_547363263, 
DATE(d_173836415_d_266600170_d_448660695) as d_173836415_d_266600170_d_448660695, 
DATE(d_195145666) as d_195145666, 
d_286191859, d_123868967, d_906417725, d_747006172, d_987563196, d_167958071, d_878865966, d_684635302, d_173836415_d_266600170_d_915179629, d_914594314, d_173836415_d_266600170_d_982213346,d_173836415_d_266600170_d_139245758, d_265193023, d_253883960, d_173836415_d_266600170_d_319972665_d_638781731, d_173836415_d_266600170_d_319972665_d_759651991,
d_173836415_d_266600170_d_319972665_d_221592017, 
DATE(d_173836415_d_266600170_d_319972665_d_278539982) as d_173836415_d_266600170_d_319972665_d_278539982,
DATE(d_173836415_d_266600170_d_740582332) as d_173836415_d_266600170_d_740582332
-- d_569151507_d_371303487
FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants` 
where Connect_ID is not null and 
d_821247024 = '197316935' and d_831041022 = '104430631' and
(d_173836415_d_266600170_d_915179629 = '103209024' or d_173836415_d_266600170_d_915179629 is null)" #sometimes it explicitly home and sometimes its null. Believe this was an update that just never had backfilling applied


kitstatus_table <- bq_project_query(project, kitstatus)
kit_status_table <- bq_table_download(kitstatus_table, bigint="integer64",n_max = Inf, page_size = 10000)


kit_status_table <- kit_status_table %>%  
    mutate(Kit_Status_int = factor(Kit_Status_int,levels=c("Undeliverable","Initialized", "Address Printed",
                                                           "Assigned", "Shipped", "Received")),
           Kit_Status_r1 = factor(Kit_Status_r1,levels=c("Undeliverable","Initialized", "Address Printed",
                                                         "Assigned", "Shipped", "Received")),
           Kit_Status_r2 = factor(Kit_Status_r2,levels=c("Undeliverable","Initialized", "Address Printed", 
                                                        "Assigned", "Shipped", "Received")))



## Section 7 (HMW) must have an initial kit status response, cannot be null
kit_table <- kit_status_table %>%  filter(!is.na(d_173836415_d_266600170_d_319972665_d_221592017))
# Section 8 (request a kit) will have null initial kit status when invited 
kit_request_table <- kit_status_table %>% filter(d_173836415_d_266600170_d_319972665_d_638781731 == "353358909")
...
END AS Card_Status,
d_633640710_d_950521660, d_633640710_d_545319575, d_633640710_d_938338155, d_633640710_d_205954477, d_633640710_d_289239334,d_633640710_d_992420392, d_633640710_d_541085383,d_633640710_d_427719697, 
d_633640710_d_100618603, d_633640710_d_309189173, d_426588510, d_221592017
 ...
 where pts.d_821247024 = '197316935' and pts.d_831041022 = '104430631' and
 pts.Connect_ID is not null and d_173836415_d_266600170_d_319972665_d_221592017 = '375535639'  and 
(d_173836415_d_266600170_d_915179629 = '103209024' or d_173836415_d_266600170_d_915179629 is null) " #again, no research MW collections counted

# --((kt.Connect_ID is not null and d_426588510='663273321' and d_221592017 = '375535639') or ##confirmed these are the values, but adding this condition changes the counts...
...
status_elg7.1A <- create_tbl_cross1(kit_table, "Site", "Kit_Status_int", "row", "Site", "Kit Status")
...
status_elgB <-kit_table %>%
  filter( Kit_Status_r1!="No Status")

 
status_elg7.1B <- create_tbl_cross1(status_elgB, "Site", "Kit_Status_r1", "row", "Site", "Kit Status")
colnames(status_elg7.1B)[ncol(status_elg7.1B)] <- "Total Requested"
...
status_elg7.1C <- create_tbl_cross1(status_elgC, "Site", "Kit_Status_r2", "row", "Site", "Kit Status")
colnames(status_elg7.1C)[ncol(status_elg7.1C)] <- "Total Requested"
...
filtered_data <- kit_table %>% filter(Kit_Status_int %in% c("Received", "Shipped"))
...
summary_table2_all<- filtered_data %>%
  ...
  summarise(Shipped = sum(Kit_Status_int %in% c("Received", "Shipped")), #kits received were once shipped
            Shipped_TwoWeeks = sum(Kit_Status_int %in% c("Received", "Shipped") &
                                    d_173836415_d_266600170_d_319972665_d_661940160 < currentDate-14),
...
all_shipped_2_table <- all_shipped_2 %>%  select("Site", "Shipped", 
                                                   "Shipped_TwoWeeks", "Received_PCT")
...
fig7_1 <- ggplot(all_shipped_2_filtered, aes(x = Site)) +
  ...
  ggtitle("Figure 7.1. Baseline− Total Initial Kits Received of Total Initial Kits Shipped at Least 2 Weeks Ago, by Site")
...
Coll_Card_Status <- create_tbl_cross1(kit_assembly, "Site", "Card_Status", "row", "Site", " ")
...
Ship_BPTL_Rec <- kit_table %>%
  filter(Kit_Status_int=="Received" &
           !is.na(d_173836415_d_266600170_d_319972665_d_826941471) &
           !is.na(d_173836415_d_266600170_d_319972665_d_661940160)) %>%
  mutate(ctime = as.numeric(difftime(d_173836415_d_266600170_d_319972665_d_826941471,
                                     d_173836415_d_266600170_d_319972665_d_661940160,units="days"))) %>% 
  filter(ctime>0)
 
MW_Ship_BPTL_Rec <- summarize_time(
    Ship_BPTL_Rec,
    group_var = Site,
    n_label   = "N",
    time_var  = ctime,
...
total_row <- summarize_time(
       Ship_BPTL_Rec,
       time_var  = ctime,
       perc_90   = TRUE,
       perc_95   = TRUE
     ) %>% 
  add_column(Site = c("Total"), .before = 1)

MW_Ship_BPTL_Rec <- bind_rows(MW_Ship_BPTL_Rec, total_row)





### Shipment time to present day - in Shipped status (NOT received)
all_shipped_to_date <- kit_table %>%
  filter(Kit_Status_int=="Shipped")
 
all_shipped_to_date <-  all_shipped_to_date %>%
  mutate(ship_time = as.numeric(difftime(currentDate,
                                     d_173836415_d_266600170_d_319972665_d_661940160,units="days"))) %>% 
  mutate(ship_timeframe = case_when(is.na(d_173836415_d_266600170_d_319972665_d_661940160) ~ "Missing Time",
                                    ship_time < 7 ~ "< 1 week",
                                    ship_time < 15 ~ "1-2 weeks", 
                                    ship_time < 31 ~ "2-4 weeks", 
                                    ship_time < 91 ~ "1-3 months",
                                    ship_time < 181 ~ "3-6 months",
                                    ship_time < 366 ~ "6-12 months",
                                    ship_time>= 366 ~ "> 12 months"),
         ship_timeframe = factor(ship_timeframe, 
                                 levels=c("Missing Time", "< 1 week", "1-2 weeks",  
                                          "2-4 weeks", "1-3 months", "3-6 months", 
                                          "6-12 months", "> 12 months")))



Shipped_TD <- create_tbl_cross1(all_shipped_to_date, "ship_timeframe", "Site", "column", " ", " ")
colnames(Shipped_TD)[ncol(Shipped_TD)] = "Total Kits Not Yet Received"
...
first_col_time <- first_col_time %>% 
  ...
  mutate(ctime = as.numeric(difftime(d_173836415_d_266600170_d_319972665_d_661940160,
                                     as.Date(d_556788178),units="days"))) %>% 
  filter(ctime>0)
 
 
Clin_Coll_Int_Ship <- summarize_time(
    first_col_time,
    group_var = Site,
    n_label   = "N",
    time_var  = ctime
...
total_row <- summarize_time(
  ...
  mutate(Site = "Total", .before = 1)

Clin_Coll_Int_Ship <- bind_rows(Clin_Coll_Int_Ship, total_row)
 
 
 
## Kit Conditions

condition_cols <- c("d_633640710_d_950521660", "d_633640710_d_545319575", "d_633640710_d_938338155", 
                    "d_633640710_d_205954477", "d_633640710_d_289239334","d_633640710_d_992420392",
           "d_633640710_d_541085383","d_633640710_d_427719697", "d_633640710_d_100618603", "d_633640710_d_309189173")
...
table_32 <- kit_assembly %>% 
  mutate(kit_condition = case_when(multi_cond>1 & d_633640710_d_950521660!=353358909 ~ "Two or More Conditions Selected",
                                   ...
                                   d_633640710_d_289239334==353358909 ~ "Collection Cup Leaked - Partial Loss",
                                   d_633640710_d_309189173==353358909 ~ "Collection Cup Leaked - Total Loss",
                                   ...
                                   d_633640710_d_950521660==353358909 ~ "Package in Good Condition",
  ...
  table32_pct <- table_32 %>%
    dplyr::count(kit_condition, name = "n") %>%
    dplyr::mutate(denom = sum(n), 
                  `Total Received Kits` = fmt_count_pct(n, sum(n))) %>%
    dplyr::bind_rows(tibble::tibble(kit_condition = "Total", n = sum(.$n), 
                                    `Total Received Kits` = fmt_count_pct(sum(.$n), sum(.$n)))) %>%
    dplyr::select(Condition = kit_condition, `Total Received Kits`)
...
received_long <- kit_table %>%
  ...
  mutate(
    col.Monday.date = floor_date(received_date, unit = "week", week_start = 1)
  ) %>%
  filter(
...
shipped_long <- kit_table %>%
  ...
  mutate(
    col.Monday.date = floor_date(shipped_date, unit = "week", week_start = 1)
  ) %>%
  filter(
    d_173836415_d_266600170_d_319972665_d_379252329 == 976461859,
    col.Monday.date >= as.Date("2024-03-29"),
...
kit_set_col_2 <- kit_table %>% filter(Kit_Status_int=="Received") %>% 
  mutate(col.Monday.date = floor_date(d_173836415_d_266600170_d_319972665_d_826941471, 
...
kit_table2 <- kit_table %>%  filter(!is.na(d_173836415_d_266600170_d_319972665_d_661940160)) %>% 
  mutate(col.Monday.date = floor_date(d_173836415_d_266600170_d_319972665_d_661940160, unit = "week", week_start = 1))
...
thus_far <- round(sum(as.numeric(all_shipped_2_filtered$Received))/
                    sum(as.numeric(all_shipped_2_filtered$Shipped_TwoWeeks))*100, 
                  digits=2)
...
knitr::kable(all_shipped_2_table,
             format.args = list(big.mark = ","),
             col.names=c("Site", "Total Shipped","Shipped 2+ Weeks Ago", "Received"),
             caption='Table 7.2 Baseline- Home Mouthwash Initial Kit Return Rate for Packages Shipped at Least 2 Weeks Ago',
             row.names=FALSE, align=c("l", rep("c", 3)),
             digits=1, booktabs = TRUE) %>% 
  add_indent(seq(1, nrow(all_shipped_2_table) - 1)) %>% 
...
\FloatBarrier
```{r KnitrTables7.3,eval=TRUE,echo=FALSE,error=FALSE, message=FALSE,warning=FALSE, results='asis'}
...
\FloatBarrier
```{r KnitrTables7.4_7.6,eval=TRUE,echo=FALSE,error=FALSE, message=FALSE,warning=FALSE, results='asis'}
 
 
#all these will move down a table number
MW_Ship_BPTL_Rec_kable <- knitr::kable(MW_Ship_BPTL_Rec,
                         caption='Table 7.4: Baseline- Time (in Days) from Home Mouthwash Initial Kit Shipment to \n BPTL Receipt',
                         row.names=FALSE, align=c("l", rep("c", 8)),digits=1, booktabs = TRUE) %>% 
  add_indent(seq(1, nrow(MW_Ship_BPTL_Rec) - 1)) %>% 
  kable_styling(latex_options = "HOLD_position")
 
add_footnote(MW_Ship_BPTL_Rec_kable, "Note: Kits were returned using USPS prior to 6/17/2024.", notation = "none")



knitr::kable(Shipped_TD,
                         caption='Table 7.5- Baseline Time from Home Mouthwash Initial Kit Shipment to Present Date for Initial Kits Not Yet Received',
                         row.names=FALSE, align=c("l", rep("c", 9)),digits=1, booktabs = TRUE) %>% 
  add_indent(seq(1, nrow(Shipped_TD) - 1)) %>% 
  landscape() %>% 
  kable_styling(latex_options = "HOLD_position")
...
knitr::kable(Clin_Coll_Int_Ship,
             caption='Table 7.6: Baseline- Time (in Days) from Clinical Collection to Home Mouthwash Initial Kit Shipment',
             row.names=FALSE, align=c("l", rep("c", 6)),digits=1, booktabs = TRUE) %>% 
  add_indent(seq(1, nrow(Clin_Coll_Int_Ship) - 1)) %>% 
  kable_styling(latex_options = "HOLD_position")
 
 
knitr::kable(table32_pct,
             caption='Table 7.7: Baseline- Kit Conditions Among Received Home Mouthwash Initial Kits',
...
table7_8_totals <- create_tbl_cross1(table7_8, "Site", "MWSrvy_status", "row", "Site", " ")
colnames(table7_8_totals)[ncol(table7_8_totals)] <- "Total Participants with Received Kits"

knitr::kable(table7_8_totals,
             caption="Table 7.8: Baseline- Mouthwash Biospecimen Survey Status Among Any Received Home Mouthwash Kits",
...
generate_mw_table <- function(comparison) {
  kit_table %>% 
    filter(d_547363263 == '231311385' & !is.na(d_195145666) & 
             (Kit_Status_r2 == "Received" | Kit_Status_r1 == "Received" | Kit_Status_int == "Received") &
             eval(parse(text = paste0("d_173836415_d_266600170_d_319972665_d_661940160 ", 
                                      comparison, " as.Date('2024-05-22')")))) %>%
    mutate(MW_completion = difftime(d_173836415_d_266600170_d_448660695, 
                                    d_195145666, units = "days"),
           MW_comp_diff = case_when(as.numeric(MW_completion) <= -4 ~ "4+ Days before Collection",
           ...
           MW_comp_diff = factor(MW_comp_diff,levels = c("4+ Days before Collection", "3 Days before Collection",
...
table7_9_totals <- create_tbl_cross1(table7_9, "Site", "MW_comp_diff", "row", "Site", " ")

tbl_7_9 <- knitr::kable(table7_9_totals,
                        caption = "Table 7.9: Baseline- Mouthwash Biospecimen Survey Completion Time Relative to Collection Time Reported on \n Collection Card of the First Received Mouthwash Kit ",
                        row.names = FALSE, align = c("l", rep("c", 11)), booktabs = TRUE) %>%
  add_indent(seq(1, nrow(table7_9_totals) - 1)) %>%
  kable_styling(latex_options = "scale_down") %>%
  landscape()

add_footnote(tbl_7_9, "Note: In cases where the collection card is not returned with the mouthwash sample or the collection date recorded by the participant is invalid, the FedEx drop off or pick up date and time are used as proxy collection date and time.", notation = "none")
...
## Cut off at 5/1/2025
table7_10 <- table7_10 %>%  filter(as.Date(d_173836415_d_266600170_d_319972665_d_661940160) <= as.Date('2025-05-01'))

table7_10_totals <- create_tbl_cross1(table7_10, "Site", "MW_comp_diff", "row", "Site", " ")

tbl_7_10 <- knitr::kable(table7_10_totals,
                         caption = "Table 7.10: Baseline- Mouthwash Biospecimen Survey Completion Time Relative to Collection Time Reported on Collection Card \n(Post-update to survey description on PWA)",
                         row.names = FALSE, align = c("l", rep("c", 11)), booktabs = TRUE) %>%
  add_indent(seq(1, nrow(table7_10_totals) - 1)) %>%
  kable_styling(latex_options = "scale_down") %>%
  landscape()

add_footnote(tbl_7_10, "Note: In cases where the collection card is not returned with the mouthwash sample or the collection date recorded by the participant is invalid, the FedEx drop off or pick up date and time are used as proxy collection date and time.", notation = "none")
...
survey_cc_int <- kit_table %>%  
  filter(!is.na(d_195145666) & !is.na(d_173836415_d_266600170_d_448660695) & as.Date(d_195145666) > "2025-05-01" & d_547363263 == '231311385') %>% 
  mutate(ctime = round(as.numeric(difftime(d_195145666,
                                           d_173836415_d_266600170_d_448660695,units="days")), 
                       digits=0),
         MW_comp_card = case_when(as.numeric(ctime) <= -4 ~ "4+ Days before Collection",
                                  as.numeric(ctime) == -3 ~ "3 Days before Collection",
                                  as.numeric(ctime) == -2 ~ "2 Days before Collection",
                                  as.numeric(ctime) == -1 ~ "1 Day before Collection",
                                  as.numeric(ctime) == 0 ~ "Same Day as Collection",
                                  as.numeric(ctime) == 1 ~ "1 Day after Collection",
                                  as.numeric(ctime) == 2 ~ "2 Days after Collection",
                                  as.numeric(ctime) == 3 ~ "3 Days after Collection",
                                  as.numeric(ctime) >= 4 ~ "4+ Days after Collection"),
           MW_comp_card = factor(MW_comp_card,levels = c("4+ Days before Collection", "3 Days before Collection",
                                                         "2 Days before Collection", "1 Day before Collection", "Same Day as Collection",
                                                         "1 Day after Collection", "2 Days after Collection", "3 Days after Collection",
                                                         "4+ Days after Collection")))   

Table7.10a <- create_tbl_cross1(survey_cc_int, "Site", "MW_comp_card", "row", "Site", " ")  

 
Table7_10a <- knitr::kable(Table7.10a,
                           caption='Table 7.11 Baseline- Mouthwash Biospecimen Survey Completion Time (in Days) Relative to Collection Time \n Reported on Collection Card for Initial Kit (Post update to  survey introduction)',
                           row.names=FALSE, align=c("l",rep("c", 7)),digits=1, booktabs = TRUE) %>%
  add_indent(seq(1, nrow(Table7.10a) - 1)) %>%
  kable_styling(latex_options = "scale_down") %>%
  landscape()
add_footnote(Table7_10a, "Note: In cases where the collection card is not returned with the mouthwash sample or the collection date recorded by the participant is invalid, the FedEx drop off or pick up date and time are used as proxy collection date and time.", notation = "none")
...
# Baseline Home Mouthwash Collections- Request a Kit Workflow

This section only includes data on participants who were invited to request a home mouthwash kit. Notifications were sent to these participants and home MW kits were requested via MyConnect or by the participant contacting the CSC. The Request a Kit workflow only applies to initial kits.

\FloatBarrier
```{r RequestKits_Functions, include=FALSE}

# -------   Functions for Section 8 ------------------



## The percentages for Requested and Received is out of Invited
req_rec_inv_kable1 <- function(data, group_var, group_var_name, title) { 
  
  group_var <- rlang::ensym(group_var) 
  
  req_rec_inv <- data %>% 
    mutate( Requested = !is.na(d_173836415_d_266600170_d_319972665_d_759651991), 
            Received = Kit_Status_int == "Received",
            Invited = d_173836415_d_266600170_d_319972665_d_638781731 == "353358909") %>% 
    # Crosstab 
    group_by(!!group_var, .drop = FALSE) %>% 
    summarise( Requested = sum(Requested, na.rm = TRUE), 
               Received = sum(Received, na.rm = TRUE), 
               Invited = sum(Invited, na.rm = TRUE), .groups = "drop" ) %>% 
    # Ensure column order 
    select(!!group_var, Requested, Received, Invited) %>% 
  ...
  # Create the pcts of the attrition columns
  mutate(
    # Of those that were invited how many requested a kit
    `MW Kit Requested` = fmt_count_pct(Requested, Invited),
    # Of those that were invited, how many have we received
    `MW Sample Received (of total invited)` = fmt_count_pct(Received, Invited)) %>%

    # Remove helper columns (prior to percentage added) and fix column order
  select(!!group_var, `MW Kit Requested`, `MW Sample Received (of total invited)`, Invited)
  
  colnames(req_rec_inv)[1] <- group_var_name
  colnames(req_rec_inv)[4] <- "Total Participants Invited to Request a Kit"

  knitr::kable(req_rec_inv, 
               caption = title, 
               row.names = FALSE,
               align=c("l", rep("c", 3)),
               digits=1, booktabs = TRUE) %>% 
    add_indent(seq(1, nrow(req_rec_inv) - 1)) %>%
    kable_styling(latex_options ="scale_down") 
}
...
```{r RequestKits_1,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}


req_rec_inv_kable1(kit_request_table, Site, "Site", 
                   "Table 8.1: Baseline- Home Mouthwash Kits Requested Among Participants Invited to Request a Kit")
...
\FloatBarrier
```{r RequestKits2,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

## Need to get subgroup of only those who requested a kit
 req_kit <- kit_request_table %>% 
   filter(!is.na(d_173836415_d_266600170_d_319972665_d_759651991))
   
 
  
req_kit_table2 <- create_tbl_cross1(req_kit, "Site", "Kit_Status_int", "row", "Site", "Kit Status")
colnames(req_kit_table2)[ncol(req_kit_table2)] <- "Total Kits Requested"

knitr::kable(req_kit_table2,
             caption="Table 8.2: Baseline- Home Mouthwash Initial Kit Status Among Participants who Requested a Kit",
             row.names=FALSE, align=c("l", rep("c", 7)),booktabs = TRUE) %>%
  add_indent(seq(1, nrow(req_kit_table2) - 1)) %>%
  kable_styling(latex_options = "scale_down")
...
```{r RequestKits3,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

kit_request_table <- kit_request_table %>% 
  mutate(tm_don_inv_years = as.numeric(difftime(d_173836415_d_266600170_d_319972665_d_278539982,
                                        d_173836415_d_266600170_d_740582332,units="days")),
#Specifically requested to have labels as years with these exact counts of days
         tm_don_inv = case_when(as.numeric(tm_don_inv_years) < 731 ~ "Less than 2 years",
                                  as.numeric(tm_don_inv_years) < 912 ~"2- <2.5 years",
                                  as.numeric(tm_don_inv_years) < 1095 ~ "2.5- <3 years",
                                  as.numeric(tm_don_inv_years) < 1277 ~ "3- <3.5 years",
                                  as.numeric(tm_don_inv_years) < 1460 ~ "3.5- <4 years",
                                  as.numeric(tm_don_inv_years) >= 1460 ~ "4 years or more",
                                  TRUE ~ "Missing Time Stamp(s)"),
         tm_don_inv = factor(tm_don_inv, levels=c("Less than 2 years","2- <2.5 years",
                                                  "2.5- <3 years", "3- <3.5 years",
                                                  "3.5- <4 years",
                                                  "4 years or more",
                                                  "Missing Time Stamp(s)")))

req_rec_inv_kable1(kit_request_table, tm_don_inv, "Time from Clinical Biospecimen Donation to Invitation", 
                   "Table 8.3: Baseline- Home MW Response Rates by Time from Clinical Biospecimen Donation to Invited to Request a Kit")

```



```{r RequestKits4,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

kit_tm_don_inv <- kit_request_table %>% 
  filter(!is.na(d_173836415_d_266600170_d_319972665_d_759651991)) %>% 
  mutate(tm_don_inv_diff = as.numeric(difftime(d_173836415_d_266600170_d_319972665_d_759651991,
                                               d_173836415_d_266600170_d_319972665_d_278539982,
                                               units="days")),
         tm_don_inv = case_when(as.numeric(tm_don_inv_diff) < 5 ~ "Less than 5 days",
                                  as.numeric(tm_don_inv_diff) < 10 ~"5- <10 days",
                                  as.numeric(tm_don_inv_diff) < 14 ~ "10- <14 days",
                                  as.numeric(tm_don_inv_diff) < 30 ~ "14- <30 days",
                                  as.numeric(tm_don_inv_diff) >= 30 ~ "30 days or more"),
         tm_don_inv = factor(tm_don_inv, levels=c("Less than 5 days","5- <10 days", 
                                                  "10- <14 days", "14- <30 days", 
                                                  "30 days or more"))) %>% 
  
  # Want all levels included, even if they are zero
  count(tm_don_inv, name = "MW Kit Requested") %>%
  complete(tm_don_inv, fill = list(`MW Kit Requested` = 0)) %>%
  mutate(denom = sum(`MW Kit Requested`)) %>%
  bind_rows(dplyr::summarise(., tm_don_inv = "Total",
                                      `MW Kit Requested` = sum(`MW Kit Requested`),
                                      denom = sum(`MW Kit Requested`))) %>%
    dplyr::rename(`Time from Invitation to Kit Requested` = tm_don_inv) %>%
    mutate(`MW Kit Requested` = fmt_count_pct(`MW Kit Requested`, denom, digits = 1)) %>%
    select(`Time from Invitation to Kit Requested`, `MW Kit Requested`)


  knitr::kable(kit_tm_don_inv, 
               caption = "Table 8.4:  Baseline- Home MW Response Rates by Time (in Days) from Invitation to Kit Requested", 
               row.names = FALSE,  
               align=c("l", "c"),
               digits=1, booktabs = TRUE) %>% 
    add_indent(seq(1, nrow(kit_tm_don_inv) - 1)) %>%
    kable_styling(latex_options ="scale_down") %>% 
    footnote(general="Note: Reminders are sent on Days 5, 14 and 30 following initial invitation.",
           general_title = "",
           footnote_as_chunk = TRUE, 
           escape = FALSE, 
           threeparttable = TRUE)
...
```{r RequestKits5,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

kit_tm_inv_recvd <- kit_request_table %>% 
  filter(Kit_Status_int=="Received") %>% 
  mutate(tm_inv_recvd_weeks = as.numeric(difftime(d_173836415_d_266600170_d_319972665_d_826941471,
                                              d_173836415_d_266600170_d_319972665_d_278539982,units="weeks")),
         tm_inv_recvd = case_when(as.numeric(tm_inv_recvd_weeks) < 1 ~ "Less than 1 week",
                                  as.numeric(tm_inv_recvd_weeks) < 2 ~"1-<2 weeks",
                                  as.numeric(tm_inv_recvd_weeks) < 3 ~ "2-<3 weeks",
                                  as.numeric(tm_inv_recvd_weeks) < 4 ~ "3-<4 weeks",
                                  as.numeric(tm_inv_recvd_weeks) >= 4 ~ "4 weeks+"),
         tm_inv_recvd = factor(tm_inv_recvd, levels=c("Less than 1 week","1-<2 weeks", 
                                                      "2-<3 weeks", "3-<4 weeks", "4 weeks+"))) %>% 
  
  ## Want all levels included, even if they are zero
  count(tm_inv_recvd, name = "MW Sample Received") %>%
  complete(tm_inv_recvd, fill = list(`MW Sample Received` = 0)) %>%
  mutate(denom = sum(`MW Sample Received`)) %>%
  bind_rows(summarise(., tm_inv_recvd = "Total",
                                      `MW Sample Received` = sum(`MW Sample Received`),
                                      denom = sum(`MW Sample Received`))) %>%
   dplyr::rename(`Time from Invitation to MW Sample Received` = tm_inv_recvd) %>%
   mutate(`MW Sample Received` = fmt_count_pct(`MW Sample Received`, denom, digits = 1)) %>%
   select(`Time from Invitation to MW Sample Received`, `MW Sample Received`)


  knitr::kable(kit_tm_inv_recvd, 
               caption = "Table 8.5: Baseline-Home MW Return Rates by Time (in Weeks) from Invitation to MW Sample Received", 
               row.names = FALSE,  
               align=c("l", "c"),
               digits=1, booktabs = TRUE) %>% 
    add_indent(seq(1, nrow(kit_tm_inv_recvd) - 1)) %>%
    kable_styling(latex_options ="scale_down") 
...
```{r RequestKits6,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

kit_request_table <- kit_request_table %>% 
  mutate(last_survey = pmax(d_784810139, #COVID completion time
                            d_389890053, #CSH completion time
                            d_843688458, #PROMIS completion time
                            d_543379310, #ROI completion time
                            d_517311251, #BOH completion time
                            d_832139544, #MRE completion time
                            d_770257102, #SAS completion time
                            d_264644252, #LAW completion time
                            d_199471989, na.rm=T), #CES completion time
         tm_surv_daymonths = as.numeric(difftime(d_173836415_d_266600170_d_319972665_d_278539982,
                                          last_survey,units="days")),
         #Specifically requested to have labels as months with these exact counts of days
         tm_surv_inv = case_when(as.numeric(tm_surv_daymonths) < 182 ~ "Less than 6 months",
                                  as.numeric(tm_surv_daymonths) < 364 ~"6- <12 months",
                                  as.numeric(tm_surv_daymonths) < 546 ~ "12- <18 months",
                                  as.numeric(tm_surv_daymonths) < 729 ~ "18- <24 months",
                                  as.numeric(tm_surv_daymonths) < 911 ~ "24- <30 months",
                                  as.numeric(tm_surv_daymonths) < 1094 ~ "30- <36 months",
                                  as.numeric(tm_surv_daymonths) < 1276 ~ "36- <42 months",
                                  as.numeric(tm_surv_daymonths) < 1459 ~ "42- <48 months",
                                  as.numeric(tm_surv_daymonths) >= 1460 ~ "48 months or more",
                                 # didn't complete any surveys but was invited
                                 is.na(last_survey) ~ "No Surveys Completed",
                                  TRUE ~ "Missing Time Stamp(s)"),
         tm_surv_inv = factor(tm_surv_inv, levels=c("Less than 6 months","6- <12 months", 
                                                  "12- <18 months", "18- <24 months","24- <30 months", 
                                                  "30- <36 months", "36- <42 months", "42- <48 months", 
                                                  "48 months or more", "No Surveys Completed",
                                                  "Missing Time Stamp(s)")))

req_rec_inv_kable1(kit_request_table, tm_surv_inv, "Time from Last Survey Completion to Kit Request Eligible", 
                   "Table 8.6: Baseline- Home MW Response Rates by Time from Last Survey Completion to Invited to Request a Kit")

```



```{r RequestKits7,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

# Row order, and ensures all leves included
kit_request_table$completion <- factor(kit_request_table$completion, 
                                       levels = c('Completed Survey and Sample(s)','Completed Survey, No Sample(s)', 
                                                  'Completed Sample(s), No Survey', 'Completed Neither'))


req_rec_inv_kable1(kit_request_table, completion, "Completion", 
                   "Table 8.7: Baseline- Home MW Response Rates by Baseline Data Completeness Among Participants Invited to Request a Kit")

```


```{r RequestKits8,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

# Row order, and ensures all leves included
kit_request_table$sex <- factor(kit_request_table$sex, 
                                       levels = c("Female", "Male", "Unknown"))


req_by_sex <- req_rec_inv_kable1(kit_request_table, sex, "Sex", 
                   "Table 8.8: Baseline- Home MW Response Rates by Sex Among Participants Invited to Request a Kit")
...
```{r RequestKits9,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

# Row order, and ensures all leves included
kit_request_table$age <- factor(kit_request_table$age, 
                                       levels = c("30-34", "35-39", "40-45", "46-50", "51-55", "56-60",
                                                  "61-65", "66-70","Unknown"))


req_by_age <- req_rec_inv_kable1(kit_request_table, age, "Age", 
                   "Table 8.9: Baseline- Home MW Response Rates by Age Among Participants Invited to Request a Kit")
...
\FloatBarrier
```{r RequestKits10, echo=FALSE, message=FALSE,warning=FALSE, results='hold'}


up_hist_query <- bq_project_query(project, 
query= "WITH address_update_flags AS (

  -- Join first so we have access to participant date bounds, then collapse to one row per participant
  ---- Needed because the user_profile_history table has >1 row per participant 
  
  SELECT
    p.Connect_ID,
    MAX(
      CASE
        WHEN TIMESTAMP(up.d_371303487) > TIMESTAMP(p.d_173836415_d_266600170_d_319972665_d_278539982)
         AND TIMESTAMP(up.d_371303487) < TIMESTAMP(p.d_173836415_d_266600170_d_319972665_d_759651991)
        THEN 1 ELSE 0
      END
    ) AS has_address_update
  FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants` p
  LEFT JOIN `nih-nci-dceg-connect-prod-6d04.FlatConnect.user_profile_history` up
    ON p.Connect_ID = up.Connect_ID
  WHERE p.d_173836415_d_266600170_d_319972665_d_638781731 = '353358909'
    AND p.d_173836415_d_266600170_d_319972665_d_759651991 IS NOT NULL
  GROUP BY p.Connect_ID  -- One row per participant
),


----------- Generate the row counts ------------------------------------------
status_counts AS (
  SELECT
    CASE
      WHEN has_address_update = 1 THEN 'Address Update'
      ELSE 'No Address Update'
    END AS Address_Update_Status,
    COUNT(*) AS n
  FROM address_update_flags
  GROUP BY 1
),

----------- This forces both rows to appear even if one has count of 0 -------
all_status AS (
  SELECT 'Address Update' AS Address_Update_Status
  UNION ALL
  SELECT 'No Address Update'
)

SELECT
  a.Address_Update_Status,
  COALESCE(s.n, 0) AS N
FROM all_status a
LEFT JOIN status_counts s
  USING (Address_Update_Status)
ORDER BY a.Address_Update_Status;")

up_hist <- bq_table_download(up_hist_query, bigint="integer64",
                             n_max = Inf, page_size = 1000)


  up_hist_table <- up_hist %>%
    mutate(denom = sum(N)) %>%
    bind_rows(summarise(., Address_Update_Status = "Total",
                        N = sum(N), 
                        denom = sum(N))) %>%
    mutate(`Number of Participants` = fmt_count_pct(N, denom, digits = 2)) %>%
    select(Address_Update_Status, `Number of Participants`)

  knitr::kable(up_hist_table, 
               caption = "Table 8.10: Baseline- Participants With Address Update After Being Invited to Request a Kit, \n Before MW Kit Request Submission", 
               row.names = FALSE,  
               align=c("l", "c"),
               digits=1, booktabs = TRUE) %>% 
    add_indent(seq(1, nrow(up_hist_table) - 1)) %>%
    kable_styling(latex_options ="scale_down") 
...
```{r RequestKits11,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}

## Counts of Kits Shipped, Kits Shipped at Least 2 Weeks Ago, and Kits Received, by Site
 
req_shipped <- kit_request_table %>% filter(Kit_Status_int %in% c("Received", "Shipped"))
 
summary_req_ship<- req_shipped %>%
  group_by(Site) %>%
  summarise(Shipped = sum(Kit_Status_int %in% c("Received", "Shipped")), #kits received were once shipped
            Shipped_TwoWeeks = sum(Kit_Status_int %in% c("Received", "Shipped") &
...
total_shipped <- sum(summary_req_ship$Shipped)
total_received <- sum(summary_req_ship$Received)
total_ship_previous_week <- sum(summary_req_ship$Shipped_TwoWeeks)
...
# Set column names
colnames(total_df) <- colnames(summary_req_ship)

 
# Append the "Total" row to summary_req_ship
req_shipped_total <- rbind(summary_req_ship, total_df)


# Add percentages to received column (out of shipped two weeks ago)
req_shipped_total$Received_PCT <- paste0(req_shipped_total$Received, " (",
                                      round(as.numeric(req_shipped_total$Received)/as.numeric(req_shipped_total$Shipped_TwoWeeks)*100, digits=1),
                                     "%)")

req_shipped_total <- req_shipped_total %>%  select("Site", "Shipped", 
                                                   "Shipped_TwoWeeks", "Received_PCT")

knitr::kable(req_shipped_total,
             format.args = list(big.mark = ","),
             col.names=c("Site", "Total Shipped","Shipped 2+ Weeks Ago", "Received"),
             caption='Table 8.11: Home Mouthwash Kit Return Rate for Packages Shipped at Least 2 Weeks Ago to Participants Invited to Request a Kit',
             row.names=FALSE, align=c("l", rep("c", 3)),
             digits=1, booktabs = TRUE) %>% 
  add_indent(seq(1, nrow(req_shipped_total) - 1)) %>% 
  kable_styling(latex_options = "HOLD_position")
...
```{r RequestKits12,echo=FALSE, message=FALSE,warning=FALSE,results='hold'}


# Counts
req_rec_lags <- kit_request_table %>%
  mutate(
    # Kit requested at least 2 weeks ago
    Requested_2Weeks = as.Date(d_173836415_d_266600170_d_319972665_d_759651991) <= (currentDate - 14),
    #All Received
    Received = Kit_Status_int == "Received",
    # Invited at least 3 weeks ago
    Invited_3Weeks = as.Date(d_173836415_d_266600170_d_319972665_d_278539982) <= (currentDate - 21)) %>%
  
  summarise(Requested_2Weeks = sum(Requested_2Weeks, na.rm = TRUE),
            Received         = sum(Received, na.rm = TRUE),
            Invited_3Weeks   = sum(Invited_3Weeks, na.rm = TRUE))

# Add Percentages -- Both out of the invited column
req_rec_lags <- req_rec_lags %>%
  mutate(Requested_2Weeks = paste0(Requested_2Weeks,
                                   " (", 
                                   round(Requested_2Weeks/Invited_3Weeks*100, 2), 
                                   "%)"),
         Received= paste0(Received,
                          " (", 
                          round(Received/Invited_3Weeks*100, 2), 
                          "%)"))



knitr::kable(req_rec_lags,
             format.args = list(big.mark = ","),
             col.names=c("MW Kits Requested as of 2 Weeks Ago", "MW Samples Received to Date (of total invited as of 3 weeks ago)", "Total Participants Invited to Request a Kit as of 3 Weeks Ago"),
             caption='Table 8.12: Baseline- Home Mouthwash Kits Requested Among Participants Invited to Request a Kit (with time lags)',
             row.names=FALSE, align=c("l", rep("c", 2)),
             digits=1, booktabs = TRUE) %>% 
  column_spec(c(1:3), width = "5cm")  %>%  # make column 2's long title wrap into 2 lines
  kable_styling(latex_options = c("scale_down","HOLD_position")) %>% 
  footnote(general = "Note: It is possible for a MW to be received from a participant who requested a kit less than 3 weeks ago, so this could be an over-estimate.", 
           general_title = "",
         footnote_as_chunk = TRUE, 
         escape = FALSE, 
         threeparttable = TRUE) 

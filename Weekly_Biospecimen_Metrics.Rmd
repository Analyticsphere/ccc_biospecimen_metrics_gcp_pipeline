...
## Reused summarize function, optional additions
summarize_time <- function(data, group_var, n_label, time_var,
                           std_dv  = FALSE,
                           perc_80 = FALSE,
                           perc_90 = FALSE,
                           perc_95 = FALSE,
  ...
  data %>%
    ...
    dplyr::summarize(
      !!n_label := n(),
      Min    = round(min({{ time_var }}, na.rm = TRUE), digits),
      Q1     = round(quantile({{ time_var }}, 0.25, na.rm = TRUE), digits),
      Median = round(median({{ time_var }}, na.rm = TRUE), digits),
      Mean   = round(mean({{ time_var }}, na.rm = TRUE), digits),
      .SD    = if (std_dv)  round(sd({{ time_var }}, na.rm = TRUE), digits)               else NULL,
      Q3     = round(quantile({{ time_var }}, 0.75, na.rm = TRUE), digits),
      .P80   = if (perc_80) round(quantile({{ time_var }}, 0.80, na.rm = TRUE), digits)   else NULL,
      .P90   = if (perc_90) round(quantile({{ time_var }}, 0.90, na.rm = TRUE), digits)   else NULL,
      .P95   = if (perc_95) round(quantile({{ time_var }}, 0.95, na.rm = TRUE), digits)   else NULL,
      .P98   = if (perc_98) round(quantile({{ time_var }}, 0.98, na.rm = TRUE), digits)   else NULL,
      .P99   = if (perc_99) round(quantile({{ time_var }}, 0.99, na.rm = TRUE), digits)   else NULL,
      Max    = round(max({{ time_var }}, na.rm = TRUE), digits),
      .groups = "drop"
    ) %>%
    rename(any_of(c(
      "SD"      = ".SD",
...
IRB_demo_BioCols <- stacked_IRB_demo_BioCols %>% 
  ...
  kable_styling(latex_options = "scale_down") %>% 
  footnote("Note: Ethnicity information for participants who have not completed Module 1 is Missing. Race is defined as the de-identified race data reported by sites at time of invitation.",
          general_title = "",
         footnote_as_chunk = TRUE, 
         escape = FALSE, 
         threeparttable = TRUE)
...
avg_draw_order <- summarize_time(
  clinical.survey %>% filter(
    Site != 'HealthPartners' &
    Site != "Sanford Health" &
    Site != 'University of Chicago Medicine'
  ) %>% mutate(time = as.numeric(difftime(
    pmin(as.Date(d_173836415_d_266600170_d_982213346),
...
```{r Collection_BU_BUM_Sub, include=FALSE, echo=FALSE, warning=FALSE}

# One varibale is renamed to Not Started, Started, Submitted; the other is not; need to fix later
all_coll_to_sub <- coll_sett_all %>%  
  filter(d_265193023 == 'Submitted' | d_253883960=='231311385') %>% 
  mutate(collection = if_else(d_265193023 == 'Submitted', d_678166505, d_556788178),
         surv_sub = if_else(d_265193023 == 'Submitted', d_222161762, d_764863765),
         coll_to_sub = as.numeric(round(difftime(as.Date(surv_sub), as.Date(collection),
                                                 units="days"), digits=0)),
         time = case_when(as.numeric(coll_to_sub) <=1 ~"Within 1 Day",
                          as.numeric(coll_to_sub) <= 3 ~ "2-3 Days",
                          as.numeric(coll_to_sub) <=5 ~ "4-5 Days",
                          as.numeric(coll_to_sub) <=14 ~ "6-14 Days",
                          as.numeric(coll_to_sub) <=30 ~ "15-30 Days",
                          as.numeric(coll_to_sub) <=60 ~ "31-60 Days",
                          as.numeric(coll_to_sub) <=90 ~ "61-90 Days",
                          as.numeric(coll_to_sub) <=120 ~ "91-120 Days",
                          as.numeric(coll_to_sub) <=180 ~ "121-180 Days",
                          as.numeric(coll_to_sub) >180 ~ "More than 180 Days",
                          TRUE ~ "Missing Time Stamp(s)"),
         #time = droplevels(time), # don't include catgories with a count of 0
         time = factor(time, levels=c("Within 1 Day", "2-3 Days","4-5 Days","6-14 Days","15-30 Days","31-60 Days",
                                      "61-90 Days","91-120 Days","121-180 Days", "More than 180 Days", 
                                      "Missing Time Stamp(s)"))) %>% 
  group_by(time) %>%  tally()

### Add percentage -- separating to a new df so the total row can be mereged in afterwards. % will throw off sum function
all_coll_to_sub_with_pct <- all_coll_to_sub %>% 
  mutate(n_pct = paste0(all_coll_to_sub$n, " (",
                        round(all_coll_to_sub$n/sum(all_coll_to_sub$n)*100, digits=1), "%)"))

### Add total row
total_row <- c("Total", paste0(as.numeric(sum(all_coll_to_sub$n)), " (100%)"))

total_df <- data.frame(t(total_row), stringsAsFactors = FALSE) #set to df to bind
 
colnames(total_df) <- colnames(all_coll_to_sub_with_pct[, c(1,3)])

all_coll_to_sub_with_pct <- rbind(all_coll_to_sub_with_pct[, c(1,3)], total_df)
...
avg_bldorder_present <- summarize_time(
  table1 %>% filter(
    d_878865966 == "No blood collected" &
    ...
    currentDate,
    as.Date(d_173836415_d_266600170_d_769615780),
    units = "days"
  ))),
  group_var = Site,
...
CTS <-knitr::kable(all_coll_to_sub_with_pct,
                   caption='Table 3.3: Time (in Days) From Specimen Collection to Biospecimen Survey Completion',
                   col.names = c("Time", "N(%)"),
                   format.args = list(big.mark = ","),
                   row.names=FALSE,  align=c("l", "c"),digits=1, booktabs = TRUE)  %>% 
  add_indent(seq(1, nrow(all_coll_to_sub_with_pct) - 1))  %>%
  kable_styling(latex_options = "scale_down")
add_footnote(CTS,"Note: Collection times reflected in this table for collections that occurred in the clinical collection setting are an estimate, based on when samples were finalized in the Biospecimen Dashboard, actual collections would have occurred on average 1 day prior to the Connect team receiving the samples.", notation = "none")
...
tbl19b <- knitr::kable(postcovid_BUMtime, 
                       caption='Table 3.5: Baseline Biospecimen Survey Completion Time from Research Collection Visit Check In \n (Post- COVID-19 Questions Removal)',
                       row.names=FALSE,align=c("l",rep("c", 6)), booktabs = TRUE)  %>% 
  kable_styling(latex_options = "scale_down")
...
knitr::kable(coll_comp_BU, 
             caption='Table 3.6: Baseline Clinical Biospecimen Survey Complete Time from Collection Time', 
             row.names=FALSE,align=c("l", rep("c", 8)), booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")

knitr::kable(BU_coll_DB_comp,
             caption='Table 3.6.a Time (in Hours) From Clinical Biospecimen Collection to Biospecimen Dashboard Entry',
             row.names=FALSE, align=c("l", rep("c", 9)),digits=1, booktabs = TRUE) %>%
  kable_styling(latex_options = "scale_down")


knitr::kable(BU_trig_comp,
             caption='Table 3.6.b Baseline Clinical Biospecimen Survey Complete Time (in Hours) From Survey Becoming Available',
             row.names=FALSE, align=c("l", rep("c", 9)),digits=1, booktabs = TRUE) %>%
  kable_styling(latex_options = "scale_down")
...
tab21_ra=knitr::kable(Table21R_After,
                      caption='Table 3.8: Baseline Biospecimen Survey Completion Length (in Minutes) Among Submitted \n Research Surveys (Post-COVID-19 Questions Removal)',
                      row.names=FALSE, align=c("l", rep("c", 11)),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
tab21_ca=knitr::kable(ClinSrvy_Lgth_After,
                      caption='Table 3.10: Baseline Biospecimen Survey Completion Length (in Minutes) Among Submitted Clinical Surveys \n (Post-COVID-19 Questions Removal)',
                      row.names=FALSE, align=c("l", rep("c", 10)),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
\newpage
\FloatBarrier
# Baseline Collection-To-Receipt Times and Package Conditions
\vspace{-0.5em}
\FloatBarrier
...
status_elg7.1A <- create_tbl_cross(kit_table, "Site", "Kit_Status_int", "row", "Site", "Kit Status")
...
all_shipped_to_date <- kit_table %>%
  filter(Kit_Status_int=="Shipped" & !is.na(d_173836415_d_266600170_d_319972665_d_661940160))
 
all_shipped_to_date <-  all_shipped_to_date %>%
  mutate(ctime = as.numeric(difftime(currentDate,
                                     d_173836415_d_266600170_d_319972665_d_661940160,units="days"))) %>% 
  filter(ctime>0)
 
Shipped_TD <- summarize_time(
    all_shipped_to_date,
    group_var = Site,
    n_label   = "N",
    time_var  = ctime,
...
Shipped_TD <- bind_rows(Shipped_TD, total_row)
...
total_row <- summarize_time(
  ...
  mutate(Site = case_when(is.na(Site) ~ "Total"))
...
pending_TD <- knitr::kable(Shipped_TD,
                         caption='Table 7.5- Baseline Time (in Days) from Home Mouthwash Initial Kit Shipment to Present Date for Initial Kits Not Yet Received',
                         row.names=FALSE, align=c("l", rep("c", 8)),digits=1, booktabs = TRUE) %>% 
...
Table7.10a <- create_tbl_cross(survey_cc_int, "Site", "MW_comp_card", "row", "Site", " ")  
...
req_kit_table2 <- create_tbl_cross(req_kit, "Site", "Kit_Status_int", "row", "Site", "Kit Status")

...
biospe_locations<-biospe %>% mutate(Site_location = case_when(d_951355211==777644826 ~ "UC-DCAM",
                                    ...
                                    Site=case_when(siteAcronym=="BSW" ~ "Baylor Scott & White Health",
...
col_num_1.2 <- col_num_1.2 %>%
  mutate(Site_Group = case_when(
    `Collection Location` %in% c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", "K1 HFH Main","Sterling Heights","Columbus", 
                                 "Livonia","Taylor", "Ford Road", "NCO","Brownstown") ~ "Henry Ford Health System", 
...
##UC
home_mw_counts_HP <-sum(as.numeric(site_home_mw$f0_[site_home_mw$d_827220437==809703864]))
...
col_set5 <- list()
for (i in 1:3){
  recrver1_5_4 <- recrver1 %>%  filter(Site=="University of Chicago Medicine") #& BL_BioSpm_BloodSetting_v1r0=="Clinical" & BL_BioSpm_UrineSetting_v1r0=="Clinical" & BL_BioSpm_MWSetting_v1r0=="Clinical"
tmp <-eval(parse(text=paste("pct_tb(recrver1_5_4$",setting[i],",recrver1_5_4$",bio[i],")",sep="")))
#tmp <- tmp %>%
tmp <- tmp %>% select(contains("n_Any")) %>%  mutate(Setting = trimws(rownames(tmp)))
col_set5[[i]] <- tmp
}
biosettings5_4 <- col_set5 %>% reduce(full_join,by="Setting")
biosettings5_4[4,c(1:4)] <- c("0", "Home",  "0", as.numeric(home_mw_counts_HP))
biosettings5_4[1,4] <- "0"
biosettings5_4 <- biosettings5_4[order(biosettings5_4$Setting),c(2,1,3,4)]
colnames(biosettings5_4)[2:4] <- gsub("n_","",colnames(biosettings5_4)[2:4])

biosettings5_4[4,4] <- as.numeric(biosettings5_4[1,4]) + as.numeric(biosettings5_4[2,4]) + as.numeric(biosettings5_4[3,4])
bioset5_4 <- biosettings5_4

a5<- round(as.numeric(as.character(bioset5_4[1,2]))/ as.numeric(as.character(bioset5_4[4,2])), digits=3)*100
b5 <- round(as.numeric(as.character(bioset5_4[1,3]))/ as.numeric(as.character(bioset5_4[4,3])), digits=3)*100
c5 <- round(as.numeric(as.character(bioset5_4[1,4]))/ as.numeric(as.character(bioset5_4[4,4])), digits=3)*100
d5 <- round(as.numeric(as.character(bioset5_4[2,2]))/ as.numeric(as.character(bioset5_4[4,2])), digits=3) *100
e5<- round(as.numeric(as.character(bioset5_4[2,3]))/ as.numeric(as.character(bioset5_4[4,3])), digits=3)*100
f5 <- round(as.numeric(as.character(bioset5_4[2,4]))/ as.numeric(as.character(bioset5_4[4,4])), digits=3)*100
g5 <- round(as.numeric(as.character(bioset5_4[3,2]))/ as.numeric(as.character(bioset5_4[4,2])), digits=3)*100
h5 <- round(as.numeric(as.character(bioset5_4[3,3]))/ as.numeric(as.character(bioset5_4[4,3])), digits=3)*100
i5 <- round(as.numeric(as.character(bioset5_4[3,4]))/ as.numeric(as.character(bioset5_4[4,4])), digits=3)*100


bioset5_4[1,2] <- paste0(bioset5_4[1,2], " (", a5, "%)")
bioset5_4[1,3]  <- paste0(bioset5_4[1,3], " (", b5, "%)")
bioset5_4[1,4]  <- paste0(bioset5_4[1,4], " (", c5, "%)")
bioset5_4[2,2] <- paste0(bioset5_4[2,2], " (", d5, "%)")
bioset5_4[2,3]  <- paste0(bioset5_4[2,3], " (", e5, "%)")
bioset5_4[2,4]  <- paste0(bioset5_4[2,4], " (", f5, "%)")
bioset5_4[3,2] <- paste0(bioset5_4[3,2], " (", g5, "%)")
bioset5_4[3,3] <- paste0(bioset5_4[3,3], " (", h5, "%)")
bioset5_4[3,4]  <- paste0(bioset5_4[3,4], " (", i5, "%)")

bioset5_4[4,2] <- paste0(bioset5_4[4,2] , " (100%)")
bioset5_4[4,3] <- paste0(bioset5_4[4,3], " (100%)")
bioset5_4[4,4] <- paste0(bioset5_4[4,4], " (100%)")
...
biospe7= left7  %>%  
  ...
  mutate(HF_Sites= case_when(d_173836415_d_266600170_d_185243482==1050010095 ~ "Wyandotte",
                             ...
                             d_173836415_d_266600170_d_185243482==1050020026 ~ "Brownstown",
                             d_173836415_d_266600170_d_185243482==1010200009 ~ "Ford Road"))
                             

biospe7 <- biospe7 %>% filter(!is.na(HF_Sites)) #to avoid errors when new sites come in without a heads up
#Make list of HF Sites consistent 
biospe7$HF_Sites <- factor(biospe7$HF_Sites,levels=c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", "Main - K1","Sterling Heights","Columbus", 
                                 "Livonia","Taylor", "Ford Road", "NCO","Brownstown")) 
...
biospe_adhoc2$Site_location <- factor(biospe_adhoc2$Site_location,levels=c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", "K1 HFH Main","Sterling Heights",
                                                                           "Columbus",  "Livonia","Taylor", "Ford Road", "NCO", "Brownstown", #"NSC", 
...
research.vt_plotUC + guides(color = guide_legend(title = "Collection Location")) 
 ...
 clinical.vt_plUC_locations <- clinic.survey %>% filter(Site=="University of Chicago Medicine") %>%
  # filter(d_173836415_d_266600170_d_185243482==1 | d_173836415_d_266600170_d_452847912==1 | 
  #          d_173836415_d_266600170_d_185243482==2 | d_173836415_d_266600170_d_452847912==2 | 
  #          d_173836415_d_266600170_d_185243482==3 | d_173836415_d_266600170_d_452847912==3 |
  #          d_173836415_d_266600170_d_185243482==4 | d_173836415_d_266600170_d_452847912==4 |
  #          d_173836415_d_266600170_d_185243482==5 | d_173836415_d_266600170_d_452847912==5) %>%

...
#                            caption='Table 1.5: Baseline Blood Collection Status Among Biospecimen Collections', row.names=FALSE,align=c("l","c","c","c"), 
...
final_col_num_1.5 <- final_col_num_1.5 %>% filter(Count_7days>0 | Count_30days>0)
final_col_num_1.5 <- final_col_num_1.5 %>% arrange(Region, desc(Count_30days))

# Keep the Region column initially for arranging and grouping
final_col_num_1.5 <- final_col_num_1.5[, c("Region", "Collection Location", "In the Past 7 Days", "In the Past 30 Days")]


SF_collections <- final_col_num_1.5 %>%
  ...
  #row_spec(1:nrow(final_col_num_1.5), extra_latex_after = "\\hline") %>%
  pack_rows(index = table(final_col_num_1.5$Region))  # Group by Region
...
## Used in footnote
Bi_count <- sum(top10$`Top Collection Counts`[top10$Region=="Bismarck"])
F_count <- sum(top10$`Top Collection Counts`[top10$Region=="Fargo"])
S_count <- sum(top10$`Top Collection Counts`[top10$Region=="Sioux Falls"])
Be_count <- sum(top10$`Top Collection Counts`[top10$Region=="Bemidji"])
...
#     digits = c(0,1),
#     percent = "row",
#     label = list(Site_location = "Clinical Location",
#                  bld_complt = " "),
#     missing = "ifany",
#     missing_text = "0 ( 0 %)",
#     margin_text = "Total"
...
# research__blood$Site <-factor(research__blood$Site,levels=c("Baylor Scott and White Health","HealthPartners", "Henry Ford Health",
#                                                             "Marshfield Clinic Health System",
#                                                             "Sanford Health", "University of Chicago Medicine"))
# 
...
"; Bismarack: ", Bi_count, "; Fargo: ", F_count, "; Sioux Falls: ", S_count, "."), notation = "none")
...
knitr::kable(Deviation_Freq[,c(7,5,6,4)],

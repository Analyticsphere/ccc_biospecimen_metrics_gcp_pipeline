...
site_results["Henry Ford Health System"]
...
biospe$Site <-factor(biospe$d_827220437,levels=c("Baylor Scott & White Health","HealthPartners", "Henry Ford Health System",
                                                     "Marshfield Clinic Health System",
                                                     "Sanford Health", "University of Chicago Medicine","Kaiser Permanente Colorado",
                                                     "Kaiser Permanente Georgia","Kaiser Permanente Hawaii","Kaiser Permanente Northwest",
                                                     "National Cancer Institute","Other"))
...
Tb.research.coltube$Site <-factor(Tb.research.coltube$Site,levels=c("Baylor Scott & White Health","HealthPartners", "Henry Ford Health System",
                                                     "Marshfield Clinic Health System",
...
research__blood$Site <-factor(research__blood$Site,levels=c("Baylor Scott & White Health","HealthPartners", "Henry Ford Health System",
                                                     "Marshfield Clinic Health System",
                                                     "Sanford Health", "University of Chicago Medicine"))
...
biospe_adhoc= biospe %>%  filter(d_650516960==534621077 & d_410912345==353358909 & !is.na(d_678166505) & 
                     ...
                     mutate(Site_location = case_when(d_951355211==777644826 ~ "UC-DCAM",
                            ...
                            Site = case_when(d_827220437==472940358 ~ "Baylor Scott & White Health",
                                             ...
                                             TRUE ~ "Remove")) %>%  filter(Site!="Remove")
...
biospe_adhoc$Site<- factor(biospe_adhoc$Site, levels=c("Baylor Scott & White Health","HealthPartners","Henry Ford Health","Marshfield","Sanford Health","University of Chicago"))
...
crosstable.coll_bptl28a$Site <- list("Baylor Scott & White Health","HealthPartners","Henry Ford Health","Marshfield","Sanford Health","University of Chicago", "Total")
...
#crosstable.coll_bptl28a <- crosstable.coll_bptl28a %>%  select("Site","n_pct_1_Day", "n_pct_2_Day","n_pct_3_Day","n_pct_4_Day","n_pct_4_plus_days", "Total_Pct")

crosstable.coll_bptl28a <- crosstable.coll_bptl28a[,c(13:19)]
# crosstable.coll_bptl28a[7,] <- list('Total', sum(as.numeric(gsub(",", "", str_extract(crosstable.coll_bptl28a[,2], "\\b\\d{1,4}(,\\d{3})*(?=\\s*\\()")))),
...
# 
# crosstable.coll_bptl28a[7,2] <- paste(crosstable.coll_bptl28a[7,2], "(",round(100*as.numeric(crosstable.coll_bptl28a[7,2])/as.numeric(crosstable.coll_bptl28a[7,7]), digits=1), "%)")
# crosstable.coll_bptl28a[7,3] <- paste(crosstable.coll_bptl28a[7,3], "(",round(100*as.numeric(crosstable.coll_bptl28a[7,3])/as.numeric(crosstable.coll_bptl28a[7,7]), digits=1), "%)")
# crosstable.coll_bptl28a[7,4] <- paste(crosstable.coll_bptl28a[7,4], "(",round(100*as.numeric(crosstable.coll_bptl28a[7,4])/as.numeric(crosstable.coll_bptl28a[7,7]), digits=1), "%)")
# crosstable.coll_bptl28a[7,5] <- paste(crosstable.coll_bptl28a[7,5], "(",round(100*as.numeric(crosstable.coll_bptl28a[7,5])/as.numeric(crosstable.coll_bptl28a[7,7]), digits=1), "%)")
# crosstable.coll_bptl28a[7,6] <- paste(crosstable.coll_bptl28a[7,6], "(",round(100*as.numeric(crosstable.coll_bptl28a[7,6])/as.numeric(crosstable.coll_bptl28a[7,7]), digits=1), "%)")
# crosstable.coll_bptl28a[7,7] <- paste(crosstable.coll_bptl28a[7,7], " (100%)")
...
biospe_adhoc2= table5.2 %>%  filter(d_650516960==664882224 & d_410912345==353358909  & #clinical and finalized collections
                     ...
                     mutate(Site_location = case_when(#d_173836415_d_266600170_d_185243482==1 | d_173836415_d_266600170_d_452847912==1 ~ "NSC",
                      ...
                      d_827220437=="809703864" & (d_173836415_d_266600170_d_185243482==5 | d_173836415_d_266600170_d_452847912==5) ~ "Dearborn Station",
                       ...
                       d_827220437==657167265 ~ "Sanford Health",
                       TRUE ~ "Remove"),
                       Site = case_when(#d_827220437==472940358 ~ "Baylor Scott & White Health", #not yet clinical
...
```{r AdHoc2, eval=TRUE,echo=FALSE, warning=FALSE, include=FALSE}
Livonia_ad_hoc <- visit_wk_site.hybrid %>% filter(as.Date(col.Monday.date)>="2023-12-11" & Site_location=="HFH Livonia Research Clinic")

 Livonia_adhoc <- Livonia_ad_hoc[,c(5,6)]

sorted_adhoc <- Livonia_adhoc %>%
  arrange(col.Monday.date)
...
LivoniaAH_totals <- as.data.frame(sorted_adhoc_with_total)
LivoniaAH <- LivoniaAH_totals[-1, ]

knitr::kable(LivoniaAH, caption='Ad Hoc HFH Livonia Research Clinic Visit Count Per Week', row.names=FALSE,align=c("l","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "HOLD_position")
```

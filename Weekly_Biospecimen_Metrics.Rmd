...
recrver1 <- recrver1 %>% mutate(BiospeCollection = ifelse((recrver1$d_878865966 == 353358909| recrver1$d_167958071 == 353358909), "Any biospecimen Collections","No biospecimen collections")) 
...
col_num_1.1 <- col_num_1.1 %>%
  mutate(Site_Group = case_when(
    ...
    `Collection Location` %in% c("HP Research Clinic",  "HP Park Nicollet") ~ "HealthPartners",
    ...
    `Collection Location` %in% c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving","NTX Biorepository","BCC- Fort Worth") ~ "Baylor Scott and White",
    TRUE ~ "Other"
  ))
...
#for footnote:
footnote_count <- sum(table1_2_locations$biocol.appoint=="Number of Clinical Collections" & table1_2_locations$Site=="Sanford Health")
...
SF_collections <- knitr::kable(final_col_num_1.5[, c(1,8,9)],  #final_col_num_1.5[, c(1,2,8,9)], 
             #col.names=c("Collection Location", "Total Clinical Collections", "In the Past 7 days",  "In the Past 30 days"),
             col.names=c("Collection Location", "In the Past 7 days",  "In the Past 30 days"),
             caption='Table 1.4: Baseline Clinical Biospecimen Collections in the Past 30 Days at Sanford Health by Clinic Location', 
             row.names=FALSE, align=c("l","c"), format.args = list(big.mark = ","), booktabs = TRUE) %>%  
  column_spec(3:4, width = "4cm")

add_footnote(SF_collections, 
             paste0("This table reflects Clincal Collections in the past 30 days; the total number of Clinical Collections at Sanford Health is ", footnote_count),
             notation = "none")
```
\FloatBarrier
...
summary_table_after2<- filtered_data %>%
  ...
  summarise(Shipped = sum(Kit_Status %in% c("Received", "Shipped") & as.Date(d_173836415_d_266600170_d_319972665_d_661940160) > "2024-08-28"),
            Shipped_OneWeek = sum(Kit_Status %in% c("Received", "Shipped") & as.Date(d_173836415_d_266600170_d_319972665_d_661940160) > "2024-08-28" & 
                                    as.Date(d_173836415_d_266600170_d_319972665_d_661940160) < (as.Date(currentDate) -7)),
...
total_ship_previous_week2 <- sum(summary_table_after2$Shipped_OneWeek)
...
for(i in 1:nrow(all_shipped_after2)){
  all_shipped_after2[i,"Received"] <- paste0(all_shipped_after2[i,"Received"], " (", round(as.numeric(all_shipped_after2[i,"Received"])/as.numeric(all_shipped_after2[i,"Shipped_OneWeek"])*100, digits = 1), "%)")
...
table7_2after2 <- knitr::kable(all_shipped_after2,format.args = list(big.mark = ","),
                              caption='Table 7.5 Baseline- Home Mouthwash Kit Return Rate for Packages Shipped up to 1 Week Ago (kits shipped after 8/28/2024)',
                              row.names=FALSE, align=c("l","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "HOLD_position")

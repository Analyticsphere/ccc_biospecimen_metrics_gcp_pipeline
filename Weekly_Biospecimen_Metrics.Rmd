...
```{r Sanford_Table_1.5, eval=TRUE,echo=FALSE,include=FALSE,results='hold'}
...
add_Site_location <- function(table1_4_locations, csv_file_path) {
  ...
  site_data <- site_data %>%
    mutate(
      ID = as.character(ID),
      Site_location = iconv(as.character(Site_location), from = "Latin1", to = "UTF-8", sub = ""),
      Region = iconv(as.character(Region), from = "Latin1", to = "UTF-8", sub = "")
    )
  

  if (!all(c("ID", "Site_location", "Region") %in% colnames(site_data))) {
  ...
  site_data <- site_data %>%
    mutate(
      ID = as.character(ID) %>% trimws(),
      Site_location = iconv(as.character(Site_location), from = "Latin1", to = "UTF-8", sub = ""),
      Region = iconv(as.character(Region), from = "Latin1", to = "UTF-8", sub = "")
    )

  # Build named lookup vectors (safe and fast, avoids parse/eval and nchar on large concatenated strings)
  loc_map <- setNames(site_data$Site_location, site_data$ID)
  reg_map <- setNames(site_data$Region, site_data$ID)

  # Map IDs in the table to locations/regions. Use both blood and urine ID fields and coalesce.
  ...
  table1_4_locations <- table1_4_locations %>%
    ...
    # )
      ...
      Site_location = dplyr::coalesce( loc_map[.id_bld], loc_map[.id_urn] ),
...
query= "WITH address_update_flags AS (
  ...
  SELECT
    ...
    MAX(
      CASE
        WHEN DATE(up.ts_user_profile_updated) > DATE(p.d_173836415_d_266600170_d_319972665_d_278539982)
         AND DATE(up.ts_user_profile_updated) < DATE(p.d_173836415_d_266600170_d_319972665_d_759651991)
        THEN 1 ELSE 0
      END

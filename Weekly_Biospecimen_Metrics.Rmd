...
col_num <- table(table1_2_locs$Clin_Site_location, table1_2_locs$biocol.appoint)
...
col_num_1.2 <- col_num_1.2 %>%
  mutate(Site_Group = case_when(
    `Collection Location` %in% c("Macomb", "West Bloomfield", "Wyandotte","Fairlane (Dearborn)", "Plymouth", "Royal Oak", "Troy", "K1 HFH Main","Sterling Heights","Columbus", 
                                 "Livonia","Taylor", "Ford Road", "NCO","Brownstown", "Missing HFH location") ~ "Henry Ford Health", 
    `Collection Location` %in% c("Elk River", "Chanhassen", "Missing HP location") ~ "HealthPartners", #"NSC", 
    `Collection Location` %in% c("South Loop","Orland Park","Kenwood","River East","Dearborn Station", "Ingalls", "Missing UC location") ~ "University of Chicago Medicine",
    TRUE ~ "Other"
  ))
...
table1_4_locations$Site_location[is.na(table1_4_locations$Site_location) & 
                                   is.na(table1_4_locations$d_173836415_d_266600170_d_185243482) & is.na(table1_4_locations$d_173836415_d_266600170_d_452847912)]="Missing Both Location IDs"
...
coll_bptl_diff <- biospe_adhoc_all %>%  
  ...
  mutate(Site_location = factor(Site_location,
                                levels=c("BCC- HWC","FW All Saints","BCC- Plano","BCC- Worth St","BCC- Irving",
                                         ...
                                         "HFH Livonia Research Clinic", "HFH Pop-Up", "Missing HFH location",
...
#   gt::gt(groupname_col = "Site_Group") %>% 
#   tab_header('Table 5.2: Baseline Collection-To-Receipt Time (in Days) Among Research Collections by Collection Location') %>% 
#   tab_footnote(footnote= paste0("There are ", dim(biospe_adhoc_missings)[[1]], " participants with missing collection receipt times."),
#                locations = gt::cells_title(groups = "title")) %>% 
#   gt::tab_style(style = list(gt::cell_text(weight = "bold")),
#                 locations = gt::cells_row_groups()) %>% 
#   gt::tab_style(style = list(gt::cell_text(align = "left")), # Align title to the left
#                 locations = gt::cells_title(groups = "title")) %>%  # Target the title
#   gt::cols_width(everything() ~ gt::px(100)) %>%  # Narrow column widths if needed  
#   gt::tab_options(#table.width = "60%", # Adjust the width as needed
...
crosstable.coll_bptl28 <- ctable.coll_bptl28  %>%
  ...
  gt::cols_width(everything() ~ gt::px(80),
                  `Collection Location` ~ gt::px(120)) %>%  # Narrow column widths if needed  
  gt::tab_options(table.width = "90%", # Adjust the width as needed
...
#                                  "Livonia","Taylor", "Ford Road", "NCO","Brownstown", "Missing HFH Location ID","Elk River", "Chanhassen", "Missing HP location", "Kaiser Permanente Colorado","Kaiser Permanente Georgia","Oahu Clinics", "Non-Oahu Clinics", "Kaiser Permanente Northwest","Sanford Health", "South Loop","Orland Park","Kenwood","River East","Dearborn Station", "Ingalls", "Missing UC location")) 
...
add_footnote(settings_breakdown, "Note: Any collections indicated under 'Home' setting reflect collections in which home mouthwash was the only sample collected. . Home setting is assumed to be 0 (0%) when not listed.", notation = "none")
...
\FloatBarrier
```{r Coll_Time_Spread,echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,results='asis'}
Hrs_bptl_rsch
...
clinic_locations <- c("South Loop", "Orland Park", "Kenwood", "River East", "Dearborn Station", "Ingalls")
...
<!-- #  -->
<!-- # ###### For some reason, every time this report is run all of the plots are saving as a pdf under a newly created folder "Weekly_Biospecimen_Metrics_files". Need that deleted. -->
<!-- #  -->
<!-- # # Define the folder path -->
<!-- # folder_path <- "C:/Users/dowlingk2/Documents/GitHub/ccc_biospecimen_metrics_gcp_pipeline/Weekly_Biospecimen_Metrics_files" -->
<!-- #  -->
...
<!-- ! Unable to load picture or PDF file 'Weekly_Biospecimen_Metrics_files/figure-latex/Figure1-1.pdf'. -->
<!-- <to be read again>  -->
<!--                    } -->
<!-- l.628 ...Metrics_files/figure-latex/Figure1-1.pdf} -->

<!-- Error: LaTeX failed to compile Weekly_Biospecimen_Metrics.tex. See https://yihui.org/tinytex/r/#debugging for debugging tips. See Weekly_Biospecimen_Metrics.log for more info. -->
<!-- Execution halted -->

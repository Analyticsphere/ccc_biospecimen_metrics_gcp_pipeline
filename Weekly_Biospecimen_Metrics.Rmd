...
col_num_1.1$`Number of Research Collections` <- paste0(col_num_1.1$`N`, " (", col_num_1.1$Percentage,"%)")
...
col_num_1.1_gt <- col_num_1.1_gt %>%  summary_rows(
  groups = TRUE,
  columns = vars("Number of Research Collections"),
  fns = (Sum = ~sum(.)),
) %>% tab_header("Table 1.1 Baseline Research Biospecimen Collections by Collection Location")
...
table1.2 <- biocol.tb %>% filter(d_410912345== 353358909) %>%
  mutate(#appointment = ifelse((d_410912345!= 353358909 | is.na(d_410912345)), 104430631, 353358909),
    biocol.appoint = case_when(BiospeCollection == "Any biospecimen Collections" ~ "Baseline Collection", # & d_410912345== 353358909
                               #BiospeCollection == "No biospecimen collections" & (d_410912345!= 353358909 | is.na(d_410912345)) ~ "No Baseline Collection"
                               TRUE ~ "No Baseline Collection"))
...
col_num_1.2$`Number of Clinical Collections` <- paste0(col_num_1.2$`N`, " (", col_num_1.2$Percentage,"%)")
...
col_num_1.2_gt <- col_num_1.2_gt %>%  summary_rows(
  groups = TRUE,
  columns = vars("Number of Clinical Collections"),
  fns = (Sum = ~sum(.)),
) %>% tab_header("Table 1.2 Baseline Clinical Biospecimen Collections by Clinic Location")
...
coll_completenessR <- coll_completenessR %>%
  ...
  #italicize_levels() %>% 
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 5a. Baseline Research Collection Completeness by Site") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
coll_completenessC <- coll_completenessC%>%
  ...
  #italicize_levels() %>% 
  modify_header(stat_0 = "Total") %>%
  modify_caption("Table 5b. Baseline Clinical Collection Completeness by Site") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
```
...
# As of 3/1/24 ACD tubes are dropped; fully collected is 4 tubes not 5 tubes 
#Those before 3/1/24 would be fully collected at 5 tubes, on or after 3/1/24 fully collected is 4 tubes
#Streck will eventually replace ACD, but for now that's only the case at HP
#HP still needs 5 since Streck is implemented there

#So if the site is HP or collection before 3/1/24, then full collection is 5 tubes, else its 4 tubes
...
#complete blood is now 4 tubes not 5, ACD tube removed and STRECK not implemented yet 
...
colnames(newtable_4) <- c(md("Data System"), md("Blood"), md("Urine"))
...
clinic.coltube<- collection_long %>% filter(biocol_Setting == "Clinical" & tubecol == 353358909) %>% # & tube_type!="MW Tube1") %>% #added, need to test 
  mutate(Site=droplevels(Site),
...
notcol_combo2 <- array(pct_n(clinic.ctable$t,clinic.ctable$prop.col),dim=c(6,13))     
...
#total <- sum(count)
#notcol_combo2$Total.Notcol <- paste0(apply(count[c(1:6),],1,sum), "( ", round(100*apply(count,1,sum)/total,2), " %)")
count$Total.NotCol <- apply(count,1,sum)
total <- sum(count)

notcol_combo2$Site <- rownames(count)

notcol_combo2[7,c(1:14)] <- as.character(apply(count,2,sum))
notcol_combo2[7,14] <- NA
notcol_combo2$Site[is.na(notcol_combo2$Site)] <- replace_na("Total") 
notcol_combo2 <- notcol_combo2[, c(14, 1:12)]

notcol_combo2

#notcol_combo2 <- notcol_combo2[, c(15, 1:12)] #added, need to test 
...
research.notcolnotes_site <- research.notcol %>% select(notcol.notes,Site) %>% 
  tbl_cross(  col = notcol.notes,
              ...
              missing_text="0 ( 0 %)")%>%
  modify_header() %>%
  modify_caption("Table 12a. Reason Not Collected for Tubes Not Collected at Research Collection Visits by Site") %>% 
...
research.notcolnotes_tube <- research.notcol %>% select(notcol.notes, tube_type) %>%
  tbl_cross(  col = notcol.notes,
              ...
              missing_text="0 ( 0 %)")%>%
  modify_header() %>%
  modify_caption("Table 12b. Reason Not Collected for Tubes Not Collected at Research Collection Visits by Tube Type") %>%
  as_kable_extra() %>% kable_styling(latex_options = "scale_down") 
...
Table15_combined <- test17 %>% dplyr::select(Site, research.sv )  %>% tbl_cross(
  ...
  #italicize_levels() %>%
  modify_header() %>%
  modify_caption("Table 14. Baseline Research Biospecimen Survey Status by Site") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
Table16_combined <- test18a %>% dplyr::select(Site, clinical.sv )  %>% tbl_cross(
  ...
  #italicize_levels() %>%
  modify_header() %>%
  modify_caption("Table 15. Baseline Clinical Biospecimen Survey Status by Site") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
```
...
Table17_combinedB <- test17_b %>% dplyr::select(Site, checktmvisit )  %>% tbl_cross(
  ...
  #italicize_levels() %>% 
  modify_header() %>%
  modify_caption("Table 16a. Baseline Research Biospecimen Survey Completion Time from Visit Check-In Time (Pre-COVID-19 Questions Removal)") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
Table17_combinedA <- test17_a %>% dplyr::select(Site, checktmvisit )  %>% tbl_cross(
  ...
  #italicize_levels() %>% 
  modify_header() %>%
  modify_caption("Table 16b. Baseline Research Biospecimen Survey Completion Time from Visit Check-In Time (Post-COVID-19 Questions Removal)") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
Table18_combined <- test18a_B %>% dplyr::select(Site, checktmvisit )  %>% tbl_cross(
  ...
  #italicize_levels() %>% 
  modify_header() %>%
  modify_caption("Table 17. Baseline Clinical Biospecimen Survey Complete Time from Collection Time") %>% 
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
clc_both_vt.site.CO<- clc.both.wk_site.CO  %>% ggplot( aes(x=as_date(clc.both.wk_site.CO$Monday.date.start), y=n)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2023-01-16"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") + 
  
  ggtitle(label="Figure 21. Weekly Biospecimen Collections \n at Kaiser Permanente Colorado") +
...
clc_both_vt.site.GA<- clc.both.wk_site.GA  %>% ggplot( aes(x=as_date(clc.both.wk_site.GA$Monday.date.start), y=n)) + 
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2023-01-16"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") + 
  ggtitle(label="Figure 22. Weekly Biospecimen Collections \n at Kaiser Permanente Georgia") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
clc_both_vt.site.HI<- clc.both.wk_site.HI %>% ggplot( aes(x=as_date(clc.both.wk_site.HI$Monday.date.start), y=n)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2023-01-16"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") + 
  ggtitle(label="Figure 23. Weekly Biospecimen Collections \n at Kaiser Permanente Hawaii") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
clc_both_vt.site.NW<- clc.both.wk_site.NW  %>% ggplot( aes(x=as_date(clc.both.wk_site.NW$Monday.date.start), y=n)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2023-01-16"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") + 
  ggtitle(label="Figure 24. Weekly Biospecimen Collections \n at Kaiser Permanente Northwest") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
Table_bld_draw <- blood_placed %>% dplyr::select(Site, bld_placed)  %>% tbl_cross(
  ...
  #italicize_levels() %>% 
  modify_header() %>%
  modify_caption("Table 28. Clinical Blood Draw Order Status Among Verified Participants") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
research.4_1 <- ggplot(visit_wk_site4__1, aes(x=as.Date(col.Monday.date), y=100*(hours3/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 4.1 Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Visit Check-In, Henry Ford Health System") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
...
research.4_2 <- ggplot(visit_wk_site4__2, aes(x=as.Date(col.Monday.date), y=100*(hours3/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 4.2  Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Visit Check-In, Sanford Health") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
research.4_3 <- ggplot(visit_wk_site4__3, aes(x=as.Date(col.Monday.date), y=100*(hours3/n))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 4.3 Weekly Percent of Biospecimen Surveys Submitted within 3 Hours of \n Visit Check-In, University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
\FloatBarrier
```{r KnitrTables2_5, eval=TRUE,echo=FALSE, ,results='asis'}
biospe.bld %>% kable_styling(latex_options = "scale_down")




tab5=knitr::kable(bioset, col.names=c("Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 3. Baseline Biospecimen Collection by Collection Setting', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)   
...
tab5_1=knitr::kable(bioset5_1, col.names=c("Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 3.1 Baseline Biospecimen Collection by Collection Setting at Henry Ford Health System', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)   
...
tab5_2=knitr::kable(bioset5_2, col.names=c("Setting","Any Blood Collected", "Urine Collected", "Mouthwash Collected"), caption='Table 3.2 Baseline Biospecimen Collection by Collection Setting at HealthPartners', row.names=FALSE, align=c("l","c","c","c"), format.args = list(big.mark = ","), booktabs = TRUE)   
...
Bld_urn_cli = knitr::kable(newtable_4, caption='Table 4. Data System Status Among Baseline Clinical Collections by Specimen Type', row.names=FALSE,align=c("l","c","c"), booktabs = TRUE) %>%  kable_styling(latex_options = "hold_position") 
...
coll_completenessC %>% kable_styling(latex_options = "scale_down")
...
research.1_1 <- ggplot() +
  ...
  ggtitle(label = "Figure 1. Weekly Percent of Incomplete Blood Draws, \n University of Chicago Medicine") +
  theme(panel.background = element_blank(),
...
final_fig2_table <- help2_1_fig2_inc %>%  group_by(col.Monday.date) %>% dplyr::summarize( incomplete=sum(incomp_blood),
                                                                                          partial=sum(partial_blood), 
                                                                                          none_blood=sum(no_blood), all_collections=sum(all_blood), 
                                                                                          All_Incomplete=100*(incomplete/all_collections),
                                                                                          Partial__Blood=100*(partial/all_collections), 
                                                                                          No__Blood=100*(none_blood/all_collections))
...
research.1_2 <- ggplot() + #, color=UC_Sites) + #,fill=Site, color=Site)) +
  ...
  ggtitle(label="Figure 2. Weekly Percent of Incomplete Blood Draws, \n Henry Ford Health System") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position="bottom",
...
knitr::kable(bldcol_research_HF2, col.names=c("Collection Location","All Blood Tubes Collected","Partial Blood Tubes Collected","Total"), caption='Table 7.3 Blood Completeness among Henry Ford Baseline Clinical Collections by Clinic Location', row.names=FALSE, align=c("l","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") #"No Blood Tubes Collected",


knitr::kable(notcol_combo[, c(8,1:7)] ,col.names=c("Site", "SS Tube1", "SS Tube 2", "Heparin Tube1", "EDTA Tube1", "ACD Tube1","STRECK Tube1", "Total Not Collected"), caption='Table 8. Blood Tubes Not Collected Among Partial Blood Collections at Research Collection Visits', row.names=FALSE,align=c("l","c","c","c","c","c","c","c"), booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down") %>% landscape()  
...
tab10_2= tube_completeness %>% kable_styling(latex_options = "scale_down")
add_footnote(tab10_2, "Note: The expected number of blood collection tubes varies by site and is as follows: KPHI-9; KPGA-7; KPNW-9; KPCO-8, HFH-5, HP-5", notation = "none") 


tab8=knitr::kable(notcol_combo2,caption='Table 10. Blood Tubes Collected by Tube Type at Clinical Collections', row.names=FALSE,align=c("l","c","c","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>%  kable_styling(latex_options = "scale_down", full_width = FALSE)   %>% column_spec(column = 2:ncol(notcol_combo2),width = "4cm")  %>% column_spec(column = 1:ncol(notcol_combo2),width = "5cm") %>%  landscape()
add_footnote(tab8, "Note: Expected number of collection tubes for each Kaiser Permanente site is as follows: (1)KPCO: 4 SST, 2 Heparin, 2 EDTA. (2)KPGA: 4 SST, 1 Heparin, 2 EDTA. (3)KPHI: 4 SST, 2 Heparin, 3 EDTA. (4)KPNW: 5 SST, 2 Heparin, 2 EDTA. (5)HFH: 2 SST, 1 Heparin, 1 EDTA, 1 ACD. (6) HP: 2 SST, 1 Heparin, 1 EDTA, 1 STRECK.", notation = "none") 


knitr::kable(Deviation_Freq[,c(7,5,6,4)],col.names=c("Site","Any Deviations","No Deviations", "Total Tubes Collected"), caption='Table 11. Frequency of Tubes with Any Deviation by Site',row.names=FALSE, align=c("l","c","c","c"), booktabs = TRUE )
```


```{r KnitrTables12_14,echo=FALSE,error=FALSE,message=FALSE,results='asis'}


research.notcolnotes_site
#research.notcolnotes_tube

add_footnote(research.notcolnotes_tube, "Note: 'Short Draw' reason applicable to blood collection tubes only.", notation="none")


knitr::kable(discard.site, caption='Table 13. Number of Tubes Discarded by Type and by Site', row.names=FALSE,  col.names=c("Site",  "SS Tube1", "SS Tube2", "SS Tube5", "Heparin Tube1", "Heparin Tube2", "EDTA Tubes", "ACD Tubes", "Urine Tubes", "Total Tubes"), align=c("l","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")
...
Table15_combined %>% kable_styling(latex_options = "scale_down")
...
research.3 <- ggplot(figsub3_5, aes(x=as.Date(col.Monday.date), y=100*(cum_sub/cumtotal))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3. Cumulative Percent of Biospecimen Surveys Submitted, \n University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
research.3.1 <- ggplot(figsub3_5, aes(x=as.Date(col.Monday.date), y=100*(weekly_sub/per_week_parts))) + #, color=Site)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 3.1 Weekly Percent of Biospecimen Surveys Submitted, \n University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        #legend.position="right",
...
Table16_combined  %>% kable_styling(latex_options = "scale_down")


tbl19a <- Table17_combinedB  %>% kable_styling(latex_options = "scale_down")
...
tbl19b <- Table17_combinedA  %>% kable_styling(latex_options = "scale_down")
...
tab21_rb=knitr::kable(Table21R_Before,caption='Table 18a. Research Biospecimen Survey Completion Length (in Minutes) by Site (Pre-COVID-19 Questions Removal)',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
tab21_ra=knitr::kable(Table21R_After,caption='Table 18b. Research Biospecimen Survey Completion Length (in Minutes) by Site (Post-COVID-19 Questions Removal)',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
tab21_cb=knitr::kable(ClinSrvy_Lgth_Before,caption='Table 19a. Clinical Biospecimen Survey Completion Length (in Minutes) by Site (Pre-COVID-19 Questions Removal)',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
tab21_ca=knitr::kable(ClinSrvy_Lgth_After,caption='Table 19b. Clinical Biospecimen Survey Completion Length (in Minutes) by Site (Post-COVID-19 Questions Removal)',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down")
...
tab23=knitr::kable(research.vt.length.table,caption='Table 20. Research Biospecimen Collection Visit Length (in Minutes) by Site',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down") #%>% landscape()
...
uc_col_time <- knitr::kable(research.vt.length.UC.table,caption='Table 20a. Research Biospecimen Collection Visit Length (in minutes) for Chicago Collection Locations',row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down") #%>% landscape()
add_footnote(uc_col_time,"Note: Times at the 95th percentile and above are likely the result of participants not having been checked out at the end of their visit and do not reflect actual visit lengths.", notation = "none") 
...
UC_Time <- ggplot(UC_Time_plot, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=Site, color=Site)) +
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="# Minutes",limits=c(0,300)) +     #limits=c(0,100*round(max(UC_Time_plot$n/100,1)))) +          #, limits=c(0,30), breaks=seq(0,30,5)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-1)),expand=c(0,0)) +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 5. 'Weekly Research Biospecimen Collection Average Visit Length (in minutes) \n for Chicago Collection Locations'") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        legend.position="bottom",
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +
  guides(fill=guide_legend(nrow=2,byrow=TRUE))+
  labs(caption = "Note: Times greater than 300 minutes (5 hours) excluded as are likely the result of participants not having been checked out at the end of their visit and do not reflect actual visit lengths.") +
  theme(plot.caption = element_text(hjust=0))
...
tabbA <- knitr::kable(verif_BSL_colctC,caption='Table 21a. Time (in Days) from Verification to Research Baseline Collection for Participants Verified Before 08/01/2022',row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")
...
tabbB <- knitr::kable(verif_BSL_colctD,caption='Table 21b. Time (in Days) from Verification to Research Baseline Collection for Participants Verified on or After 08/01/2022',row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")
...
BSL_A <- knitr::kable(verif_BSL_colctA,caption='Table 22a. Time (in Days) from Verification to Baseline Clinical  Collection for Participants Verified Before 02/01/2023',row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)  %>%  kable_styling(latex_options = "scale_down")
add_footnote(BSL_A,"Note: Baseline Clinical Collections were launched in February 2023.", notation = "none")

BSL_B <-knitr::kable(verif_BSL_colctB,caption='Table 22b. Time (in Days) from Verification to Baseline Clinical  Collection for Participants Verified on or After 02/01/2023',row.names=FALSE,  align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)  %>%  kable_styling(latex_options = "scale_down")
add_footnote(BSL_B,"Note: Baseline Clinical Collections were launched in February 2023.", notation = "none")




AF26 <- knitr::kable(verif_baseline_Af[c(1,2,7:9,3:6), ],caption = 'Table 23. Time (in Days) from Verification to Clinical Baseline Collection for \n Participants Verified After 06/01/2022, by Campaign Type',row.names=FALSE, booktabs = T, linesep = "", format.args = list(big.mark = ","), align = c("l", "c", "c","c", "c", "c", "c", "c", "c", "c", "c","c", "c","c"), col.names=c("Site",  'Number of Baseline Collections',"Min", "Median", "Mean", "SD", "Max", 'Number of Baseline Collections', "Min", "Median", "Mean", "SD", "Max","Total Number of Baseline Collections") ) %>% kable_styling(latex_options = "scale_down") %>% add_indent(c(1:9)) %>% add_header_above(c(" " = 1, "Upcoming Appointment" = 6, "All Other Campaigns" = 6,  " "=1)) %>% landscape() 
...
tabl27 <- knitr::kable(verif_draw_order,caption='Table 24. Time (in Days) from Verification to Draw Order Placement',row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)  %>% kable_styling(latex_options = "scale_down")

add_footnote(tabl27, "Note: Time from verification to draw order placement calculated for participants verified on or after February 1, 2023.", notation="none")


knitr::kable(avg_draw_order, caption='Table 25. Time (in Days) from Draw Order Placement to Clinical Collection by Site', row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down") #%>% landscape()


knitr::kable(avg_bldorder_present, caption='Table 26. Time (in Days) from Draw Order Placement to Present Date by Site', row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c","c","c","c","c"),digits=1, booktabs = TRUE)%>% kable_styling(latex_options = "scale_down") 

coll_bplt_knitr <- knitr::kable(crosstable.coll_bptl1, col.names=c("Site", "< 30 Days", "30 to <60 Days","60 to <90 Days", "90 to <180 Days", "180 to <270 Days", "270 Days or More", "Total"), caption='Table 27. Time (In Days) Since Verification Among Participants \n with No Baseline Biospecimen Collection by Site', row.names=FALSE, align=c("l","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
coll_bplt_knitr

Table_bld_draw 


knitr::kable(crosstable.coll_bptl28a, col.names=c("Site", "1 Day", "2 Days", "3 Days", "4 Day", ">4 Days", "Total"), caption='Table 29.a Research Biospecimen Collection to Receipt at BPTL (in Days) by Site', row.names=FALSE, align=c("l","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")

coll_bplt_knitr <- knitr::kable(crosstable.coll_bptl28, col.names=c("Collection Location", "1 Day", "2 Days", "3 Days", "4 Day", ">4 Days", "Total"), caption='Table 29.b Research Biospecimen Collection to Receipt at BPTL (in Days) by Collection Location', row.names=FALSE, align=c("l","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
knitr::kable(crosstable.coll_bptl29a, col.names=c("Site", "1 Day", "2 Days", "3 Days", "4 Days", ">4 Days", "Total"), caption='Table 30.a Clinical Biospecimen Collection to Receipt at BPTL (in Days) by Clinic Location', row.names=FALSE, align=c("l","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 


coll_bplt_knitr2 <- knitr::kable(crosstable.coll_bptl_2, col.names=c("Collection Location", "1 Day", "2 Days", "3 Days", "4 Days", ">4 Days", "Total"), caption='Table 30.b Clinical Biospecimen Collection to Receipt at BPTL (in Days) by Clinic Location', row.names=FALSE, align=c("l","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
coll_bplt_knitr3 <- knitr::kable(Hrs_bptl_col, col.names=c("Collection Location", 'Min', 'Q1', 'Median', 'Mean', 'Q3','Perc.90','Perc.98'), caption='Table 31. Time (in Hours) from Research Biospecimen Collection to Receipt at BPTL by Collection Location', row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
coll_bplt_knitr4 <- knitr::kable(Hrs_bptl_col4, col.names=c("Collection Location", 'Min', 'Q1', 'Median', 'Mean', 'Q3','Perc.90','Perc.98'), caption='Table 32. Time (in Hours) from Clinical Biospecimen Collection to Receipt at BPTL by Clinic Location', row.names=FALSE, align=c("l","c","c","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") 
...
combined_plotHF1 <- ggplot(scatCHF, aes(x = as.Date(coldate), y = hrs_past,  color = factor(weekdays(as.Date(coldate))))) + #shape = Group,
  ...
  theme(panel.background = element_blank(),
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +
  theme(plot.caption = element_text(hjust=0), legend.title = element_text(size = 8.5))  +
  ggtitle(label="Figure 14.1 Hours from Collection to BPTL Receipt, \n Henry Ford Clinical Setting")
combined_plotHF1
...
combined_plotHF2 <- ggplot(scatRHF, aes(x = as.Date(coldate), y = hrs_past, color = factor(weekdays(as.Date(coldate))))) + #shape = Group,
  ...
  theme(panel.background = element_blank(),
        ...
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) +
  theme(plot.caption = element_text(hjust=0), legend.title = element_text(size = 8.5))  +
  ggtitle(label="Figure 14.2 Hours from Collection to BPTL Receipt, \n Henry Ford Research Setting")
combined_plotHF2
...
research.vt1 <- ggplot(visit_wk_site1, aes(x=as.Date(col.Monday.date), y=n)) + #,fill=Site, color=Site)) +
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-1)),expand=c(0,0)) +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 15. Weekly Biospecimen Collections, \n All Sites") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        legend.position="bottom",
...
```{r Figures16_18, eval=TRUE,echo=FALSE, message=FALSE, warning=FALSE}
...
research.vt_plotH <- ggplot(visit_wk_site.hybridHF, aes(x=as.Date(col.Monday.date), y=n, color=Setting )) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  ...
  #scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 16. Weekly Biospecimen Collections \n at Henry Ford") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
research.vt_plotHF <- ggplot(edited_fig3_1, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  ...
  scale_y_continuous(name="# Number Collections") +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  scale_color_manual(values=c("HFH K-13 Research Clinic"="#164C71", "HFH Cancer Pavilion Research Clinic"="#FDBE19", "HFH Livonia Research Clinic"="#3C989E")) +
  #scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 16.1 Weekly Research Biospecimen Collections \n at Henry Ford by Collection Location") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        ...
        axis.title.y = element_text(size = 14,color="black"),   #, face = "bold"
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold"))  
research.vt_plotHF + guides(color = guide_legend(title = "Site Locations")) 
...
research.vt_plotH_Clin <- ggplot(edited_fig3_1_Clin_tally, aes(x=as.Date(col.Monday.date), y=n, color=HF_Sites )) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="# Number Collections") +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2023-05-22"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  #scale_fill_brewer(palette="Dark2") +
  ...
  ggtitle(label="Figure 16.2 Weekly Clinical Biospecimen Collections \n at Henry Ford by Clinic Location") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        ...
        axis.title.y = element_text(size = 14,color="black"),   #, face = "bold"
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) #guides(fill=guide_legend(nrow=2,byrow=TRUE))
research.vt_plotH_Clin + guides(color = guide_legend(title = "Site Locations")) 
...
research.vt_plotUC <- ggplot(research.vt_plUC, aes(x=as.Date(col.Monday.date), y=total_collections)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  ...
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 17. Weekly Biospecimen Collections at \n University of Chicago Medicine") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
research.vt_plUC= visit_wk_site_edited %>%  filter( Site=="University of Chicago Medicine")
research.vt_plotUC1 <- ggplot(research.vt_plUC, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="# Number Collections") +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-3)),expand=c(0,0))  +
  #scale_fill_brewer(palette="Dark2") +
  scale_color_manual(values=c("Ingalls Harvey"="#B1C7D9", "Orland Park"="#2973A5", "River East"="#FDBE19", 
                              "South Loop"="#565C65", "UC-DCAM"="#3C989E", "UCM Pop−Up"="#CC7D15")) +
  ggtitle(label="Figure 17.1 Weekly Biospecimen Collections at \n University of Chicago Medicine Locations") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(text="Site Location"),
        legend.position = "bottom",
        ...
        axis.title.y = element_text(size = 14,color="black"),   #, face = "bold"
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) #guides(fill=guide_legend(nrow=2,byrow=TRUE))
research.vt_plotUC1 + guides(color = guide_legend(title = "Site Locations")) 
...
research.vt_plotMF <- ggplot(research.vt_plMF, aes(x=as.Date(col.Monday.date), y=total_collections)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-3)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 18. Weekly Biospecimen Collections at \n Marshfield Clinic Health System") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
research.vt_plotMF <- ggplot(research.vt_plMF, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  ...
  ggtitle(label="Figure 18.1 Weekly Biospecimen Collections at \n Marshfield Clinic Health System Locations") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(text="Site Location"),
        legend.position = "bottom",
        ...
        axis.title.y = element_text(size = 14,color="black"),   #, face = "bold"
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) #guides(fill=guide_legend(nrow=2,byrow=TRUE))
research.vt_plotMF + guides(color = guide_legend(title = "Site Locations")) 
...
research.vt_plotSH <- ggplot(research.vt_plSH, aes(x=as.Date(col.Monday.date), y=n)) + #,fill=site, color=site)) +   #research.vt <-    ,fill=Site, color=Site
  ...
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2022-06-20"),(as.Date(Sys.Date())-3)),expand=c(0,0))  +
  scale_fill_brewer(palette="Dark2") +
  ggtitle(label="Figure 19. Weekly Biospecimen Collections \n at Sanford Health") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
research.vt_plotSH <- ggplot(research.vt_plSH, aes(x=as.Date(col.Monday.date), y=n, color=Site_location)) + 
  ...
  ggtitle(label="Figure 19.1 Weekly Biospecimen Collections \n at Sanford Health Locations") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
        ...
        axis.title.y = element_text(size = 14,color="black"),   #, face = "bold"
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) #guides(fill=guide_legend(nrow=2,byrow=TRUE))
research.vt_plotSH + guides(color = guide_legend(title = "Site Locations")) 
...
research.vt_plotHP <- ggplot(research.vt_plHP, aes(x=as.Date(col.Monday.date), y=n, color=Setting)) + 
  ...
  #scale_fill_brewer(palette="Dark2") +
  scale_color_manual(values=c("Clinical"="#164C71", "Research"="#FDBE19")) +
  ggtitle(label="Figure 20. Weekly Biospecimen Collections \n at HealthPartners") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
...
research.vt_plotHP_locs <- ggplot(edited_fig11_1_Clin_tally, aes(x=as.Date(col.Monday.date), y=n, color=HP_cl_locs)) +  
  geom_line() + geom_point(width = 0.2) +
  scale_y_continuous(name="# Number Collections", breaks = seq(min(edited_fig11_1_Clin_tally$n), max(edited_fig11_1_Clin_tally$n), by = 1)) + #, limits=c(0,30), breaks=seq(0,30,5)) +
  scale_x_date(name="Date",date_labels = "%y-%m-%d", date_breaks = "2 weeks", limits=c( as.Date("2023-11-20"),(as.Date(Sys.Date())-1)),expand=c(0,0))  +
  #scale_fill_brewer(palette="Dark2") +
  scale_color_manual(values=c("NSC"="#99C0C4", "Elk River"="#565C65", "Chanhassen"="#CC7D15")) +
  ggtitle(label="Figure 20.1 Weekly Clinical Biospecimen Collections \n at HealthPartners by Clinic Location") +
  #theme_ipsum() +
  theme(panel.background = element_blank(),
        #legend.title = element_text(size=7),
        legend.position = "bottom",
        ...
        axis.title.y = element_text(size = 14,color="black"),   #, face = "bold"
        plot.title = element_text(hjust = 0.5,size = 12, face = "bold")) #guides(fill=guide_legend(nrow=2,byrow=TRUE))
research.vt_plotHP_locs + guides(color = guide_legend(title = "Site Locations")) 
...
recrbio_1 <- recrbio_1[, c("Connect_ID","RcrtES_Site_v1r0","BioFin_BaseBloodCol_v1r0","BioFin_BaseUrineCol_v1r0","BioFin_BaseMouthCol_v1r0","BioSpm_BloodSettingBL_v1r0", "BioSpm_UrineSettingBL_v1r0","BioSpm_MWSettingBL_v1r0","BioChk_CompleteBL_v1r0","BioChk_TimeBL_v1r0","BioFin_CheckOutTmBL_v1r0","BioFin_ResearchBldTmBL_v1r0", "BioFin_ResearchUrnTmBL_v1r0","BioFin_BMTimeBL_v1r0","BioClin_SiteBldLocBL_v1r0","BioClin_SiteUrLocatBL_v1r0","BioClin_SntBloodAccIDBL_v1r0","BioClin_SntUrineAccIDBL_v1r0", "BioClin_PolyBloodIDBL_v1r0","BioClin_PolyUrineIDBL_v1r0","BioClin_SiteBloodCollBL_v1r0","BioClin_ClinBloodTmBL_v1r0","BioClin_SiteUrineCollBL_v1r0", "BioClin_ClinicalUrnTmBL_v1r0","BioClin_SiteBloodRRLBL_v1r0","BioClin_SiteBldRRLDtBL_v1r0","BioClin_SiteUrineRRLBL_v1r0","BioClin_SiteUrnRRLDtBL_v1r0", "BioClin_BldOrUrnPlcdBL_v1r0","BioClin_BldUrnPlcdTmBL_v1r0","BioClin_BldOrderPlcdBL_v1r0","BioClin_BldOrdPlacdDtBL_v1r0","BioClin_UrnOrdPlacedBL_v1r0", "BioClin_UrnOrdPlcdDtBL_v1r0","BioClin_DBBloodRRLBL_v1r0","BioClin_DBBloodRRLDtBL_v1r0","BioClin_DBUrineRRLBL_v1r0",
                           "BioClin_DBUrineRRLDtBL_v1r0","BioClin_AnySpecRRLBL_v1r0", "BioClin_AnySpecRRLTmBL_v1r0","BioClin_AnyBldUrnRecBL_v1r0","RcrtSI_RecruitType_v1r0","RcrtUP_Submitted_v1r0","RcrtV_Verification_v1r0","RcrtV_VerificationTm_V1R0", "SrvBLM_ResSrvCompl_v1r0","SrvBLM_TmComplete_v1r0","SrvBLM_TmStart_v1r0","SrvBlU_BaseComplete_v1r0","SrvBlU_TmComplete_v1r0","SrvBlU_TmStart_v1r0",
                           "BioClin_BldAndUrnRef_v1r0", "HdRef_Baseblood_v1r0","HdRef_DateBaseblood_v1r0","date","token")]

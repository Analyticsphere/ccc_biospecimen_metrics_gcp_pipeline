...
#SEX
simple_funct(BUM_tib,D_522008539, "SEX: Biological Sex at Birth")
...
#EATDRINKBEFORE
simple_funct(BUM_tib,D_867307558, "Table 3: When Last Food or Beverage Consumed")
...
MW_time1 <- avg_coll %>% group_by(Site, MW) %>%  
  summarize(n=n(),
            percentage=100*n()/nrow(.)) %>%  
  select(Site,  MW, n, percentage) 
...
BUM_sig <- BUM_tib %>%  
  #v1 and v2 were answered in this survey
  filter(!is.na(D_299417266_V2) | !is.na(D_299417266)) %>%  
...
BUM_SLEEP <- summarize_time(
    BUM_sig,
    time_var = time,
    n_label = "Number of Participants",
    std_dv = TRUE,
    digits = 1
  )

bm_wake <- knitr::kable(BUM_SLEEP,
                        caption='Table 5: Hours Participant Fell Asleep Before Sample Donation',
                        digits=1, booktabs = TRUE) %>% 
  kable_styling(latex_options = "scale_down") #%>% landscape()
add_footnote(bm_wake,"Note: Those who did not answer this question are removed from the Number of Participants in order to calculate difference in existing times.", 
             notation = "none") #and greater than 500 minutes 
...
bm_wake <- knitr::kable(BUM_WAKE,
                        caption='Table 6: Hours Awakened Before Sample Donation',
                        digits=1, booktabs = TRUE)%>% 
...
simple_funct(BUM_tib,D_487532606_D_619765650, "TYLENOL: Medication Taken, if so, Last Time Taken",
factor_levels = c("Yes, in the past day", "Yes, in the past two days", "Yes, in the past week", "Yes, in the past month", "No") )

simple_funct(BUM_tib,D_487532606_D_520755310, "NSAIDS: Medication Taken, if so, Last Time Taken",
factor_levels = c("Yes, in the past day", "Yes, in the past two days", "Yes, in the past week", "Yes, in the past month", "No") )

simple_funct(BUM_tib,D_487532606_D_839329467, "Acid Reducers: Medication Taken, if so, Last Time Taken",
factor_levels = c("Yes, in the past day", "Yes, in the past two days", "Yes, in the past week", "Yes, in the past month", "No") )
...
#MENSTPRD
one_logic_funct(BUM_tib,D_380603392, D_522008539,'536341288',
                "MENSTPRD: Had Menstrual Period in the Last 12 Months",
                   factor_levels = c("Yes", "No"))


#MENST60
one_logic_funct(BUM_tib,D_112151599, D_380603392, "353358909","MENST60: Had Menstrual Period in the Last 60 Days",
                   factor_levels = c("Yes", "No")) 
...
BUM_TimeM <- BUM_tibM  %>% dplyr::group_by(Mdate) %>% 
  dplyr::summarize(n=n()) %>% 
  dplyr::ungroup() %>%  
  dplyr::select(Mdate, n)
...
#PREGNANT
one_logic_funct(BUM_tib,D_518916981, D_522008539,'536341288',"PREGNANT: Currently Pregnant",
                   factor_levels = c("Yes", "No"))


#PREG3MON
one_logic_funct(BUM_tib,D_234714655, D_518916981, "104430631", "Table 13: Been Pregnant in the Last 3 Months",
                   factor_levels = c("Yes", "No")) 

#BRSTFD
one_logic_funct(BUM_tib,D_798452445, D_522008539,'536341288',"Table 14: Currently Breastfeeding",
                   factor_levels = c("Yes", "No"))

#BRSTFD3MON
one_logic_funct(BUM_tib,D_563539159, D_798452445, "104430631", "Table 15: Breastfed in the Last 3 Months",
                   factor_levels = c("Yes", "No")) 

#CONTRACEPT
one_logic_funct(BUM_tib,D_875535246, D_518916981, "104430631", "Table 16: Used Hormonal Contraceptives in the Last Month",
                   factor_levels = c("Yes", "No")) 

#HORMONE
one_logic_funct(BUM_tib,D_130311122, D_518916981, "104430631", "Table 17: Used Prescription Hormone Therapy in the Last Month",
                   factor_levels = c("Yes", "No")) 
...
#ORALHLTH
simple_funct(BUM_tib,D_350251057, "Table 117: Overall Health Ranking",
                 factor_levels = c("Excellent", "Very Good", "Good", "Fair", "Poor"))



#MWBEFORE
simple_funct(BUM_tib,D_877878167, "Table 19: Brushed Teeth Within the Hour Before Sample Donation")


#RINSEBEFORE
simple_funct(BUM_tib,D_800703566, "Table 20: Rinsed Mouth Within the Hour Before Sample Donation")


#GUMBEFORE
simple_funct(BUM_tib,D_294886836, "Table 21: Chewed Gum Within the Hour Before Sample Donation")


#TOBACCOBEFORE
simple_funct(BUM_tib,D_642044281, "Table 22: Smoke, Vape or Chew Products Within the Hour Before Sample Donation")
```
...
simple_funct(hy_prod, products, "Table 122: Oral Health Products Used in the Last Month")




#BRUSH2
one_logic_funct(BUM_tib,D_498984275, D_479143504_D_203919683, "1", "Table 123: Frequency Using a Toothbrush in the Last Month",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )



#FLOSS2
one_logic_funct(BUM_tib,D_205713835, D_479143504_D_807884576, "1", "Table 124: Frequency Using Floss in the Last Month",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )


#WTRPICK2
one_logic_funct(BUM_tib,D_353467497, D_479143504_D_165596977, "1", "Table 125: Frequency Using a Water Based Flosser or Pick in the Last Month",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )



#TONGUE2
one_logic_funct(BUM_tib,D_429994023, D_479143504_D_578402172, "1", "Table 126: Frequency Using a Tongue Scrubber in the Last Month",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )



#WHITE2
one_logic_funct(BUM_tib,D_499977481, D_479143504_D_184513726, "1", "Table 127: Frequency Using Teeth Whitener in the Last Month",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )
...
simple_funct(which_mwuse_long, MW_Type, "Table 29: Mouthwash Products Used in the Last Month") %>% 
  create_SATA_footnote()
...
#MWALC
one_logic_funct(BUM_tib,D_349659426, D_406270109_D_950773275, "1", "Table 129: Frequency Using Alcohol Based Mouthwash",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )


#MWALCFREE
one_logic_funct(BUM_tib,D_766370065, D_406270109_D_762727133, "1", "Table 130: Frequency Using Alcohol Free Mouthwash",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )



#MWCHLOR
one_logic_funct(BUM_tib,D_520416570, D_406270109_D_877842367, "1", "Table 131: Frequency Using Chlorhexidine Mouthwash",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )


#MWFLOURIDE
one_logic_funct(BUM_tib,D_921972241, D_406270109_D_886771318, "1", "Table 132: Frequency Using Flouride Mouthwash",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )


#MWPEROX
one_logic_funct(BUM_tib,D_526973271, D_406270109_D_404389800, "1", "Table 133: Frequency Using Peroxide Mouthwash",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )


#MWCETYL
one_logic_funct(BUM_tib,D_460873842, D_406270109_D_463302301, "1", "Table 134: Frequency Using Cetylpyridinium Cloride Mouthwash",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )


#MWSENSITIVE
one_logic_funct(BUM_tib,D_430060900, D_406270109_D_259744087, "1", "Table 135: Frequency Using Mouthwash for Sensitive Teeth",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )


#MWDRY
one_logic_funct(BUM_tib,D_800752981, D_406270109_D_523660949, "1", "Table 136: Frequency Using Mouthwash for Dry Mouth",
factor_levels = c("Never","Less than Once Per Week", "1-2 Times per Week", "3-5 Times per Week", "Once per Day", "Two or more times per day") )
...
simple_funct(dt_PERMTTHLOST1, products, "Table 38 (V1): Lost Permanent Adult Teeth, not Including your Wisdom Teeth")
...
simple_funct(dt_PERMTTHLOST2, products2, "Table 38 (V2): Lost Permanent Adult Teeth, not Including your Wisdom Teeth")
...
simple_funct(TEETHLOSTAI, answer, "Table 40: Number of Teeth Lost from Accident or Injury")
...
simple_funct(TEETHLOSTD, answer, "Table 41: Number of Teeth Lost from Tooth Decay or Disease")
...
simple_funct(TEETHLOSTOTH, answer, "Table 42: Number of Teeth Lost from Other Incidents")
...
simple_funct(dnt, products, "Table 43: Lost Permanent Adult Teeth, not Including your Wisdom Teeth")



#DENTALCELAN
simple_funct(BUM_tib,D_339570897, "Table 143: Last Professional Dental Cleaning",
factor_levels = c("In the last month", "In the last 1-6 months", "In the past 6-12 months","Over a year, in the Last Two Years", "More than 2 years","Unknown") )


#CAVITY
simple_funct(BUM_tib,D_983043203, "Table 45: Had Cavities in Adult Teeth",
factor_levels = c("Yes", "No", "Unknown"))


#GUMDISEASE
simple_funct(BUM_tib,D_736028153, "Table 46: Diagnosed with Gum Disease",
factor_levels = c("Yes", "No", "Unknown"))


#GUMTX
simple_funct(BUM_tib,D_792134396, "Table 47: Treated for Gum Disease",
factor_levels = c("Yes", "No", "Unknown"))


#ANTIBIO
simple_funct(BUM_tib,D_736393021, "Table 48: Taken Antibiotics in the Last 2 Months",
factor_levels = c("Yes", "No", "Unknown"))


#ANTIBIOTIME
one_logic_funct(BUM_tib,D_318641324, D_736393021, "353358909", "Table 148: Date Antiobiotic Last Taken",
factor_levels = c("Within the Last 24 Hours", "In the Past 2-7 Days", "In the last 2-4 weeks", "More than 4 Weeks Ago")  )
